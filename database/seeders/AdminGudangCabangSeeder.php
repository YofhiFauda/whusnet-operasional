...
class AdminGudangCabangSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        if (! $role) {
            $this->command?->error('Role pop_admin tidak ditemukan. Pastikan RoleSeeder sudah dijalankan.');
        ...
        for ($i = 1; $i <= 3; $i++) {
            $user = User::updateOrCreate(
                ['email' => "admingudangcabang{$i}@whusnet.com"],
                [
                    'name' => "Admin Gudang Cabang {$i}",
                    'phone' => '08160000000'.str_pad((string) $i, 2, '0', STR_PAD_LEFT),
        ...
        $this->command?->info('✅ 3 Akun Admin Gudang Cabang berhasil dibuat!');
    }
}
