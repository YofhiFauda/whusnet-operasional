...
| ADHOC-40 | **Rancangan API Eksternal** — (1) **webhook pemasangan** outbound (`installation.started` + `installation.completed`, nempel di event `InstallationStarted`/`InstallationCompleted` yang sudah ada, HMAC, rantai fallback SN & ODP, `idempotency_key` untuk pemasangan revisi); (2) **portal pelanggan** — merinci kontrak QR §6.6 yang sudah dikonfirmasi pemilik produk (`login_id` `{prefix_pop}-{customer_code}`, `customer_portal_accounts` + `customer_portal_tokens` terpisah, klaim via PIN, kwitansi lewat `ReceiptPresenter` dipangkas, status tiket lewat `Ticket::resolveStatus()`, `invoice.updated` dari `Invoice::recalculateFromPayments()`). Repo belum punya API layer sama sekali — `routes/api.php`, API Resource, rate limiter, CORS, JSON exception handler semuanya nol. **Nol dependensi baru.** Dokumen: [`docs/api/`](api/README.md). Direview 2026-08-18, 15 temuan ditambal (rev. 2) | **Rancangan** — 2026-08-18 |
