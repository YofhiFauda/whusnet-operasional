...
| ADHOC-92 | **Saldo Pelanggan — bayar di muka + auto-pakai ke tagihan bulanan** — analisa & rancangan dulu, **di luar sprint aktif, permintaan eksplisit user (2026-09-21), belum diimplementasi**. Kasus: pelanggan paket 150k, prorate 100k, bayar 550k → AWAL tetap 100k, 450k masuk Saldo, lalu otomatis dipakai bayar BULANAN begitu terbit (tgl 1); saldo kurang dari tagihan tetap dipakai → `sebagian`/cicilan. **Separuh fitur sudah ada (ADHOC-38)**: ledger `customer_balance_mutations`, overpay → credit, pakai saldo manual. Gap yang dirancang: auto-pakai di `GenerateMonthlyInvoicesCommand` (dalam transaksi yang sama), `PaymentMethod::SALDO` (satu payment per tagihan), kolom `source` di ledger, `payments.balance_used_amount` agar laporan kas tak menghitung saldo sebagai uang fisik, reject payment yang memakai saldo mengembalikan saldo, kartu Saldo di Detail Pelanggan, RBAC `customer_balance`. **Risiko utama:** credit lama langsung terpakai di generator pertama setelah deploy → wajib dry-run `billing:apply-balance` dulu. Sudah disetujui user (2026-09-21): FIFO tagihan terlama dulu, kolom `balance_used_amount`. Asumsi rancangan: hanya BULANAN; saldo yang masuk setelah tagihan terbit (kasus pinggiran, bukan studi kasus utama) dipakai lewat command manual. Kerjakan setelah ADHOC-90 di-commit. Dokumen: `docs/plan/billing/analisa-rancangan-saldo-pelanggan.md` | **Terbuka** — dianalisa 2026-09-21, implementasi belum dimulai |
...
| ADHOC-84 | **Alokasi Pembayaran (disederhanakan 2026-09-19) + FIFO Kolektor + peringatan piutang admin** — rancangan awal (enum `PaymentAllocation`, kolom `allocation_type`, guard penolak) **dibatalkan** setelah dicek ke kode: `PaymentService` sudah memisah cicilan/lebih bayar otomatis & menjumlahkan tunai+saldo. Lingkup final: hapus dropdown "Alokasi" (dekorasi, cuma ditempel ke `note`), konfirmasi modal untuk lebih bayar, peringatan non-blokir di admin kalau ada piutang lebih lama, FIFO (periode tertua dulu) sebagai isian awal HANYA di kolektor (admin tanpa FIFO). Tanpa migrasi/kolom baru. **Di luar sprint aktif**, rancangan selesai, belum diimplementasi. Dokumen: `docs/plan/billing/analisa-skema-alokasi-pembayaran-dan-saldo.md` | **Terbuka** — dianalisa 2026-09-18, implementasi belum dimulai |
...
| ADHOC-68 | **Skema Upgrade/Downgrade Paket Internet (prorate + deposit)** — analisa & rancangan dulu, **di luar sprint aktif**, belum diimplementasi. Rancangan: `docs/plan/billing/upgrade-downgrade/analisa-upgrade-downgrade-paket.md` | **Terbuka** — dianalisa 2026-09-15, implementasi belum dimulai |
| ADHOC-69 | **Skema Putus Langganan (denda otomatis TANPA prorate, terbit sbg Tagihan Manual/Lainnya "Denda Putus Langganan" — keputusan user 2026-09-19, bagian denda bergantung ADHOC-70; master alasan, kolom pendaftar)** — analisa & rancangan dulu, **di luar sprint aktif**, belum diimplementasi. Rancangan: `docs/plan/billing/analisa-rancangan-putus-langganan.md` | **Terbuka** — dianalisa 2026-09-15, implementasi belum dimulai |
...
| ADHOC-70 | **Tagihan Manual (Detail Pelanggan + Halaman Tagihan, halaman bukan modal, invoice+payment sekaligus)** — ditulis ulang 2026-09-19 murni dari studi kasus user (3 jenis pendapatan tetap: Perbaikan/Lainnya/Pindah Lokasi + deskripsi + nominal + Cash/TF); **tidak** memakai rancangan lama ADHOC-60 (master kategori, `invoice_items`, `INSIDENTAL`), analisa & rancangan dulu, **di luar sprint aktif**, belum diimplementasi. Rancangan: `docs/plan/billing/analisa-rancangan-tagihan-manual.md` | **Terbuka** — dianalisa 2026-09-15, implementasi belum dimulai |
