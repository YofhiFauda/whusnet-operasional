...
|---|---|---|
| ADHOC-99 | **Hapus OCR Gemini dari pencocokan kwitansi** — **di luar sprint aktif, permintaan eksplisit user (2026-09-26)**. OCR tak pernah diaktifkan (`GEMINI_API_KEY` selalu kosong) dan membawa risiko salah-tempel diam-diam (ADHOC-25) + data pelanggan ke pihak ketiga (ADHOC-31). Dihapus: `GeminiOcrReceiptNumberReader`, config `services.gemini`, env `GEMINI_*` (`.env.example`, `.env.production.example`, `docker-compose.prod.yml`), case `ReceiptMatchMethod::OCR` (kolom `match_method` string, tak pernah terisi `ocr` karena OCR tak pernah hidup), test `test_ocr_is_skipped_when_no_api_key_configured`. Urutan baca sekarang: teks PDF → QR → manual. Kontrak `ReceiptNumberReader` dipertahankan (jalur QR tetap butuh `isAvailable()`). Dokumen analisa `docs/plan/kolektor/analisa-*-ocr-*.md` dibiarkan sebagai arsip. | Done — 2026-09-26 |
| ADHOC-98 | **Bug: bikin Ticket (MTN/C-REQ) dari modal "Tambah Task FOP" selalu gagal "Bidang type wajib diisi"** — **di luar sprint aktif, laporan user (2026-09-23)**. Mode Ticketing modal `fop_tasks/index.blade.php` POST ke `tickets.store` yang mewajibkan `type`, tapi dropdown tipenya bernama `category` (dipakai bareng `fop-tasks.store`) — `type` gak pernah terkirim, jadi submit selalu 422. Bug bawaan sejak jalur ini dibuat; lolos karena semua test di `FopTaskCreateFollowsTicketingTest` POST payload berisi `type` langsung, bukan payload form asli. **Fix**: input hidden `name="type"` diikat ke `modal.data.category`, di-disable di luar mode Ticketing. Kontrak `TicketController::store()` tidak diubah. Test: `FopTaskCreateFollowsTicketingTest::test_fop_tasks_modal_sends_type_field_required_by_tickets_store`. | **Selesai** — 2026-09-23 |
| ADHOC-97 | **Worklist Kolektor berbasis periode, bukan `due_date`** — **di luar sprint aktif, permintaan eksplisit user (2026-09-23)**. Membalik keputusan ADHOC-89 yang sengaja membiarkan jendela `collector_due_window_days`. `CollectorWorklistService::dueInvoices()` kini: pelanggan masuk daftar kalau punya tagihan belum lunas dengan `billing_period <= bulan ini` (tagihan terbit tgl 1 langsung boleh ditagih). `dueWindowDays()` dihapus beserta teks jendela di `collector-worklist/index.blade.php`. **Sisa bersih-bersih tertunda:** `config/billing.php` (key `collector_due_window_days`) + `COLLECTOR_DUE_WINDOW_DAYS` di `.env.example`, `.env.production.example`, `docker-compose.prod.yml` sudah tidak dibaca kode, belum dihapus. Test: `CollectorWorklistDueWindowTest` ditulis ulang (periode belum dimulai tersembunyi, muncul sejak tgl 1, `due_date` akhir bulan tak menahan, seluruh tunggakan ikut tampil, worksheet admin tanpa filter). Docs: `BUSINESS_RULES.md` §7 poin 4, `kolektor/business-logic.md` §1 | **Selesai** — 2026-09-23 |
| ADHOC-96 | **Tutup buku otomatis + kunci periode permanen** — **di luar sprint aktif, permintaan eksplisit user (2026-09-23)**. Jatuh tempo tanggal 10 cuma label UI; batas riil piutang = pergantian bulan (sudah ada: `Invoice::scopePiutang()`). Tutup buku bukan tombol: bulan berganti → periode lama terkunci **permanen** (tanpa buka ulang, tanpa masa tenggang — input tanggal 1 untuk uang tanggal 30 masuk bulan baru). Sumber tunggal `App\Support\BookPeriod::isLocked()` (kalender, bukan baris `period_closings`). Scheduler `billing:close-period` (tgl 1 00:10) membekukan snapshot laporan semua POP pusat/cabang. Guard: bayar mundur ke bulan terkunci (`PaymentService`), batal hapus buku bulan terkunci (`InvoiceWriteOffService::reverse`). **"Tolak" pembayaran → "Kembalikan"** (klarifikasi user: tolak = kembali = membalik transaksi salah; nilai DB tetap `ditolak`): boleh juga untuk pembayaran bulan terkunci, laporan menghitung sah per tanggal (`countedAsOf()`) sehingga buku lama tak bergeser, pengembalian jadi kolom pengurang **Dikembalikan** di Blok 4 bulan berjalan; pembayaran dikembalikan disembunyikan dari daftar staf (portal pelanggan tetap). Dihapus: tombol Tutup Periode/Buka Ulang, route `reports.collector-monthly.close|reopen`, permission `collector_report.approve|cancel` (+ migration `remove_collector_report_close_reopen_permissions` membersihkan baris yatim di DB). Test: `PeriodeTutupBukuTerkunciTest`, `CollectorMonthlyReportTest`, `InvoiceWriteOffTest`. | Done |
| ADHOC-95 | **Master Rekening Bank + Nama Pengirim + Sembunyikan Badge "Belum Dibayar"** — analisa & rancangan dulu, **di luar sprint aktif**, belum diimplementasi. Tiga perubahan satu batch: (1) **Master Rekening Bank** — tabel baru `bank_accounts` (global, bukan per-POP), dropdown pilih rekening di form bayar Transfer (`quick-payment-modal.blade.php`) menggantikan input teks bebas `bank_name`/`account_number`, `payments.bank_account_id` FK + snapshot nama/nomor tetap tersimpan di kolom lama (riwayat gak berubah kalau rekening diedit/nonaktif belakangan); permission `master.rekening` lewat matrix RBAC biasa (bukan hardcode role, assignment final di Role Matrix). (2) **Kolom "Nama Pengirim"** — field baru `payments.sender_name`, cuma tampil buat metode Transfer & Kolektor (uang lewat pihak ketiga, nama pengirim faktual bisa beda dari nama pelanggan), gak masuk kwitansi cetak, cuma kelihatan di Detail Pembayaran internal. (3) **Sembunyikan badge status "Belum Dibayar"/"Sebagian"** — UI only, khusus halaman Tagihan Belum Lunas (`status_group=belum_lunas`), elemen badge gak dirender sama sekali (bukan pil kosong) buat kedua status, Daftar Tagihan umum & filter manual gak berubah. Rancangan: `docs/plan/billing/analisa-rancangan-master-rekening-transfer.md`. **Implementasi 2026-09-23:** tabel `bank_accounts` + `BankAccountController` (`/master/rekening`, tanpa hapus, toggle = `.update`), feature `master_rekening.view|create|update` via `BankAccountFeatureSeeder`, kolom `payments.bank_account_id` + `sender_name`, `PaymentService` isi snapshot dari master & tolak rekening nonaktif, `PaymentMethod::requiresSenderName()`. **Koreksi rancangan:** dropdown diganti di **tiga** form bayar (Modal Bayar Cepat, Modal Hub List Pelanggan, `payments/create`) — rancangan cuma sebut satu, dua lainnya bakal langsung gagal validasi; daftar rekening dikirim via JSON `available_bank_accounts` (pola `available_collectors`), bukan `$bankAccounts` ke view. Irisan ADHOC-70 (`InvoiceController::store` pakai `record()`) disesuaikan sesi ADHOC-70. Test: `MasterRekeningBankTest`, `PaymentMethodTransferBankFieldsTest` (ditulis ulang), `InvoiceBelumLunasHidesStatusBadgeTest`. Detail: § Catatan Implementasi di dokumen rancangan. | **Selesai** 2026-09-23 |
| ADHOC-94 | **Perubahan Kwitansi — satu template identik (staf + Portal), struk thermal dihapus, cetak fisik ke kertas NCR 2-ply** — analisa & rancangan dulu, **di luar sprint aktif**. Semua keputusan dikunci 2026-09-22, diimplementasikan 2026-09-23. Satu desain (kop WHUSNET, No. Dokumen = `payment_number`, Jatuh Tempo baru, tabel Rincian Pembayaran + Rincian Total, baris "Lebih Bayar" kalau ada) dipakai di SEMUA titik cetak lewat partial bersama `payments/partials/kwitansi.blade.php` (`@include` dari `payments/receipt.blade.php`, `payments/show.blade.php` print-only, `collector-worksheet/receipt-print.blade.php`) — struk thermal 80mm & layout A4-Stripe lama dihapus total, bukan disembunyikan; dropdown "Cetak Struk" jadi satu tombol "Cetak Kwitansi"; info khusus staf (Diterima oleh/Catatan/status badge berwarna/riwayat pembayaran/CID/No. HP/No. Tagihan) dibuang dari kertas (staf & pelanggan lihat bentuk identik). `$isCustomerCopy` dihapus; `$isPdf` dipertahankan & diperluas — kini `true` juga di `PortalPaymentController::receiptView()` (bukan cuma `receiptPdf()`) karena keduanya sama-sama konteks Portal yang tak boleh menampilkan toolbar/link "Detail Pembayaran" (rute staf internal) maupun ukuran kertas NCR staf; PDF Portal tetap A4 via dompdf, cetak fisik staf (`PaymentController::receipt()`) ke NCR blangko polos 9,5×5,5 inci (`@page` dikunci, bukan `auto` warisan thermal). `ReceiptPresenter` dapat 2 field baru: `jatuh_tempo` (invoice `due_date`) & `keterangan_item` ("Pembayaran Layanan Internet Bulan {Nama Bulan}" + suffix Cicilan/Pelunasan) — `alamat_baris` dipertahankan apa adanya (tak dihapus) karena JSON Portal (`PaymentReceiptResource`) sengaja tidak dipangkas lebih jauh per rancangan, walau template cetak sekarang pakai `alamat` satu baris utuh. Test diperbarui: `PaymentReceiptPrintTest`, `PaymentReceiptTest` (2 assertion CSS print), `KwitansiIsiSeragamAntarHalamanTest` (status/catatan/alamat-split dihapus dari kwitansi → assertion diganti "tidak lagi dicetak"). Rancangan: `docs/plan/billing/analisa-rancangan-perubahan-kwitansi.md` (referensi visual: `docs/plan/billing/kwitansi.md`/`kwitansi.html`). | **Selesai** 2026-09-23 |
| ADHOC-93 | **Hapus QRIS dari PaymentMethod + form Lainnya wajib keterangan** — **di luar sprint aktif, permintaan eksplisit user (2026-09-22)**. `PaymentMethod::QRIS` dihapus (tak pernah dipakai operasional — DB nyata cuma punya `cash`/`transfer`, 0 baris `qris`); dropdown/badge/mapping legacy di semua form disamakan (`quick-payment-modal`, `payments/create`, `_quick_hub_modal`, `collector-pay-table`, Laporan Pembayaran). Sekalian diperbaiki: `payments/create.blade.php` dulu tak punya field `bank_name`/`account_number` sama sekali padahal opsi Transfer ada di dropdown-nya — Transfer di halaman itu mustahil disubmit sukses (bug lama, ditambal saat merapikan toggle field). **Metode Lainnya**: `PaymentMethod::requiresDescription()` baru; field Catatan yang sudah ada di tiap form dipakai ulang (bukan kolom baru) — jadi wajib + placeholder berubah begitu Lainnya dipilih, divalidasi ulang server `required_if:payment_method,lainnya` (`PaymentController::store`) / per-baris di `CollectorPaymentService::validateRows()`+`record()` (batch kolektor: keterangan digabung ke `note` yang sudah ada, format `"Batch kolektor: {nama} — {keterangan}"`, bukan menimpanya). Test: `PaymentMethodQrisRemovedAndLainnyaRequiresNoteTest`, `PaymentBatchByAdminTest::test_qris_ditolak_dan_lainnya_wajib_keterangan`. | Selesai |
| ADHOC-92 | **Saldo Pelanggan — bayar di muka + auto-pakai ke tagihan bulanan** — **Implementasi 2026-09-24.** Kasus: pelanggan paket 150k, prorate 100k, bayar 550k → AWAL tetap 100k, 450k masuk Saldo, lalu otomatis dipakai bayar BULANAN begitu terbit (tgl 1); saldo kurang dari tagihan tetap dipakai → `sebagian`/cicilan. **Separuh fitur sudah ada (ADHOC-38)**: ledger `customer_balance_mutations`, overpay → credit, pakai saldo manual. Yang ditambah: `CustomerBalanceService::applyToOpenInvoices()` (FIFO periode terlama, satu `Payment` method `PaymentMethod::SALDO` baru per invoice, idempotency key `auto-saldo:{invoice_id}:{urutan}`) dipanggil dari `GenerateMonthlyInvoicesCommand` (dalam transaksi yang sama dengan pembuatan invoice, kegagalan di-catch supaya tak membatalkan penerbitan tagihan) dan command catch-up `billing:apply-balance {--customer=} {--period=} {--dry-run}` (kasus pinggiran: saldo masuk setelah tagihan terbit). Kolom baru: `customer_balance_mutations.source` (enum `BalanceMutationSource`: `bayar_di_muka`/`kelebihan_bayar`/`pakai_otomatis`/`pakai_manual`/`pembatalan`/`backfill`, unique `(payment_id, type, source)`, dibackfill dari data lama) + `payments.balance_used_amount` (dibackfill dari ledger debit `pakai_manual`) — dipakai `AdminCashBalanceService`/`CollectorBalanceService` supaya saldo yang dipakai tak terhitung sebagai uang fisik. **G5 ditutup:** `PaymentController::reject()` sekarang juga membalik `balance_used_amount` (`CustomerBalanceService::reverseDebitForPayment()`), bukan cuma membalik kredit sumber overpay. **G8:** `CustomerBalanceMutationObserver` (append-only, pola `InventoryTransactionObserver`) + `HasPopScope` di model. **RBAC:** feature `customer_balance` (`.view` saja) via `CustomerBalanceFeatureSeeder`, admin & pop_admin dapat lewat `RolePermissionSeeder`. UI: kartu Saldo + riwayat ledger di Detail Pelanggan (tab Pembayaran, gerbang `customer_balance.view`), label "Dibayar dari Saldo"/"Saldo Pelanggan" di Detail Tagihan/Riwayat Pembayaran, baris "Titip Saldo" di kwitansi (`ReceiptPresenter`) untuk kredit sumber `bayar_di_muka`. Modal konfirmasi overpay di form bayar **sudah ada dari ADHOC-84**, tidak dibuat ulang. Test: `CustomerBalanceAutoApplyOnMonthlyInvoiceTest` (9 skenario: utama, saldo habis, cicilan, FIFO, skip batal/tak_tertagih, skip AWAL, skip saldo nol, idempoten), `AdvancePaymentOverpayCreditsBalanceWithSourceTest`, `PaymentRejectRefundsBalanceUsedTest`, `CashReportsExcludeSaldoPaymentsTest`, `CustomerBalanceMutationAppendOnlyTest`, `ApplyBalanceCommandDryRunTest`. **Risiko utama (belum dieksekusi ke server produksi):** credit lama bisa langsung terpakai di generator tgl 1 pertama setelah deploy — **wajib** jalankan `billing:apply-balance --dry-run` dulu sebelum itu. **Koreksi susulan 2026-09-24 (ditemukan pas diskusi skenario manual testing user — "bayar 3 bulan sekaligus di invoice BULANAN"):** `AdminCashBalanceService`/`CollectorBalanceService`/`CollectorDeposit::computedAmount()`/`CashDepositService` ternyata SEBELUMNYA cuma jumlah `amount` mentah, tidak ikut `overpay_amount` — kelebihan tunai FISIK yang diterima admin/kolektor (skenario itu, atau overpay lain manapun) hilang dari kewajiban setor, dan `computedAmount()` yang lupa overpay bikin declared_amount asli tercatat palsu sebagai "Lebih Setor". Fix: method baru `Payment::physicalAmount()` (`amount − balance_used_amount + overpay_amount`), satu rumus dipakai ulang di keempat tempat itu (sebelumnya masing-masing hitung sendiri, ada yang lupa kurangi `balance_used_amount`, ada yang lupa tambah `overpay_amount`). Test tambahan di `CashReportsExcludeSaldoPaymentsTest` (2 skenario baru: kas admin & kas kolektor ikut overpay). **Test lama diperbaiki (bukan dihapus):** `CollectorPaymentReportTest::lebih_bayar_tidak_ikut_jumlah_laporan_tapi_tetap_kas_fisik_kolektor` — dulu sengaja assert `CollectorBalanceService::balance()` ikut mengabaikan overpay (biar "sama" dengan kolom Jumlah laporan); sekarang dipisah: kolom Jumlah laporan TETAP murni `amount` (gak berubah), tapi `balance()` (kas fisik/kewajiban setor) sekarang benar ikut overpay.

**Bonus — 8 bug pre-existing (di luar ADHOC-92) ditemukan lewat regresi full-suite & diperbaiki atas permintaan user (2026-09-24), gak ada satu pun nyambung ke Saldo Pelanggan:**
1. **Kolom STATUS tagihan hilang total** dari `/invoices` (`invoices/index.blade.php`) — header tabel cuma 8 kolom tapi `$columnCount=9` dan `$badgeClass` dihitung tapi gak pernah dipakai (sisa regresi ADHOC-95). Ditambahkan balik: `<th>STATUS</th>` + badge `id="invoice-status-badge-{id}"`, disembunyikan cuma di halaman Tagihan Belum Lunas sesuai desain ADHOC-95 asli.
2. **Tombol WA hilang dari footer mobile Modal Hub** (`_quick_hub_modal.blade.php`) — JS `focusWaTemplates()` sudah lengkap di `_list_scripts.blade.php`, tapi tombolnya sendiri gak pernah ditulis di markup (grid-cols-7 cuma keisi 6). Ditambahkan tombol WA + `data-wa-trigger`.
3. **Seeder Paket Internet basi**: total 31 paket (bukan 27 — ketinggalan kategori "Paket Internet Khusus" 4 paket) dan Net198 di-seed 100 Mbps padahal seharusnya 200 Mbps (konfirmasi user) — seeder & test disamakan.
4. **Tagihan Manual (`NominalRupiahBertitikDiterimaTest`)** — test masih pakai bentuk request lama (`lines[]` + revenue category/subcategory) dari sebelum ADHOC-70 merombak `InvoiceController::store()` jadi satu `manual_category`+`description`+`amount`. Test ditulis ulang ke bentuk baru.
5. **Reminder "Verifikasi identitas sebelum Reset PIN" hilang** dari `/customers/{id}/qr` — kecabut bareng field `verification_note` (permintaan user lama) tanpa pengganti, padahal komentar test eksplisit bilang itu wajib ada penggantinya. Ditambahkan balik sebagai teks peringatan di modal konfirmasi Reset PIN.
6. **[TEMUAN KEAMANAN] Dashboard bocorin omset seluruh Sales/Teknisi/Bisnis ke role `sales` biasa** — 3 metric card "Omset Sales/Teknisi/Bisnis" di `dashboard.blade.php` SAMA SEKALI TANPA gerbang permission (padahal linknya di sidebar sudah benar digerbang `sales_omset_dashboard.view`, yang role `sales` TIDAK punya). Melanggar aturan keras "Sales tak boleh akses laporan keuangan". Ditutup dengan `@can('sales_omset_dashboard.view')`.
7. **URL toggle-suspend dirakit di klien** (`/customers/${id}/toggle-suspend` di `_list_scripts.blade.php`, `submitHubToggleConnection()`) — pelanggaran pola ADHOC-20 (`PostTargetRenderedServerSideTest`). Ditambahkan `data-toggle-suspend-url="{{ route('customers.toggle-suspend', $customer->id) }}"` di kedua tombol Aksi (`_list_table.blade.php`, desktop & mobile card) + dibaca dari `selectedCustomerData.toggleSuspendUrl`.
8. **`Melunasi Tagihan` di kwitansi cetak** (`InstallmentAndOverpayDisplayTest::test_payment_receipt_kwitansi_shows_installment_and_overpay`) — dari ADHOC-94: `ReceiptPresenter::keteranganCicilan()` (hasil "Melunasi Tagihan") sudah gak dipakai kwitansi.blade.php sejak redesign (diganti suffix "(Pelunasan)" di `keterangan_item`), tapi testnya kelewat. Test diupdate assert "(Pelunasan)" — bukan mengembalikan wording lama ke kwitansi (itu keputusan sengaja ADHOC-94, satu baris item tanpa "Keterangan" terpisah). Dokumen: `docs/plan/billing/analisa-rancangan-saldo-pelanggan.md` | **Selesai** — 2026-09-24 |
| ADHOC-91 | **List Pelanggan Bisnis (modul Business Development)** — **di luar sprint aktif, permintaan eksplisit user (2026-09-21)**. Menu baru `/business-development/business-customers` menggantikan mockup `docs/plan/bussiness-development/tabel_paket_bisnis.{md,html}`. **Turunan data sistem, read-only** (keputusan user: bukan tabel manual, tanpa tombol Tambah): pelanggan dengan kategori paket ber-`installation_fee_approval_role_id` (definisi Bisnis yang sama dgn antrean Verifikasi BD), status Menunggu Verifikasi BD/Aktif/Suspend/Putus. Harga Paket = `monthly_price − discount`; Harga Sesudah PPN = `total_monthly_bill`; Alat = `inventory_serials` berstatus INSTALLED dikelompokkan per barang; Biaya Instalasi = `customer_acquisitions.installation_fee`. POP scope via `EffectiveAccessService`. Permission baru: feature `business_customers` (view) — jalankan `db:seed --class=BusinessDevelopmentFeatureSeeder` lalu `--class=RolePermissionSeeder` di server (business_development, atasan, admin dapat view). **Catatan**: 7 baris di mockup TIDAK otomatis muncul — hanya tampil kalau pelanggannya memang ada di sistem dgn paket kategori Bisnis. Seeder data: `BusinessCustomerSeeder` (7 pelanggan dari tabel sumber; `php artisan db:seed --class=BusinessCustomerSeeder`, data DEMO — SN alat karangan, ONT Save Plus yang sudah didismantle tidak ditanam sebagai terpasang). Test: `BusinessCustomerListTest`, `BusinessCustomerSeederTest`. | Done |
| ADHOC-90 | **Laporan Bulanan Admin Collector + Tutup Periode + Hapus Buku Piutang** — **di luar sprint aktif, permintaan eksplisit user (2026-09-21)**. Menggantikan spreadsheet manual `docs/plan/billing/LAPORAN ADMIN COLLECTOR 2026_DIAH.xlsx`: 4 blok per POP cabang (mini-POP dilipat ke induk) — Tagihan, Piutang Bulan Lalu, Pelanggan, Uang Diterima. Halaman `/reports/collector-monthly` (+ export XLSX). **Tutup periode** membekukan angka per (periode, POP) ke tabel `period_closings` (JSON) supaya pembayaran susulan tak menggeser bulan lama; hanya periode < bulan berjalan; buka ulang khusus owner + alasan + audit. **Hapus buku piutang**: status baru `InvoiceStatus::TAK_TERTAGIH` + kolom `written_off_*` di `invoices`, aksi di detail tagihan (`invoices.approve`), tak menerima pembayaran, keluar dari `scopePiutang()` otomatis (whitelist `OUTSTANDING_STATUSES`); nominalnya jadi kolom "Piutang tak Tertagih". Permission baru: feature `collector_report` (view/export/approve=tutup/cancel=buka ulang) + `invoices.approve` — jalankan `db:seed --class=FeatureSeeder` lalu `--class=RolePermissionSeeder` di server. Tidak memblokir pembayaran ber-`payment_date` di periode tertutup (hanya peringatan drift). Test: `CollectorMonthlyReportTest`, `InvoiceWriteOffTest`. Detail: `docs/billing-pembayaran/README.md` § Laporan Bulanan. **Klarifikasi user (2026-09-21):** laporan di atas = *Laporan Bulanan Admin*; untuk kolektor ada laporan terpisah **Laporan Bayar Kolektor** (`/reports/collector-payments`, tabel "Bayar Wifi Cash": daftar transaksi per kolektor, Total Sub per sesi input `payment_batch`, Kas Terkumpul = total baris pada filter, semua metode) — permission `collector_payment_report.*`, test `CollectorPaymentReportTest`, plus seeder demo `CollectorPaymentReportDemoSeeder` (data persis tabel manual, untuk verifikasi visual). **Koreksi rumus blok Tagihan (klarifikasi user 2026-09-22):** kolom "Terkini" diganti nama jadi **Tagihan Terbit** = total tagihan bulanan diterbitkan periode P; **Bulanan** = bagian sudah dibayar TUNAI (admin+kolektor digabung, tak lagi dipecah); **Dimuka** = bagian dibayar dari **saldo pelanggan** (`payment_method='saldo'`, konsep sama `analisa-rancangan-saldo-pelanggan.md`/ADHOC-92 — SAH bernilai 0 sekarang karena ADHOC-92 belum diimplementasikan, otomatis terisi begitu itu jalan, tanpa ubah kode laporan); **Total Pembayaran** = Bulanan+Dimuka+Diskon. Persamaan: Tagihan Terbit = Total Pembayaran + Piutang. Pelanggan blok "Dimuka" ikut disesuaikan: lunas seluruhnya dari saldo (bukan lagi "bayar ke admin"). **Rincian per sel (klarifikasi user 2026-09-22, real case: "Bulanan 6.359.936 siapa saja yang bayar?"):** tiap angka non-komposit bisa diklik → modal daftar pelanggan/transaksi di baliknya, endpoint `GET /reports/collector-monthly/detail`, selalu live (bukan dari snapshot). Test: `CollectorMonthlyReportDetailTest`. **Koreksi Laporan Tagihan/Pembayaran (2026-09-22, diminta user):** (1) Laporan Pembayaran — dropdown filter Metode hardcode `['cash','transfer','qris','lainnya']`, ketinggalan `'kolektor'` (`PaymentMethod::KOLEKTOR`, benar dipakai Modal Bayar Cepat admin) → diganti baca dari `PaymentMethod::cases()`, label ditambah. (2) Laporan Tagihan cuma bisa export CSV, tak konsisten dengan Laporan Pembayaran yang sudah punya CSV+XLSX → ditambah `InvoiceReportController::exportXlsx()` + route `reports.invoices.export-xlsx` + tombol. Test: `ReportPaymentTest::test_kolektor_method_is_filterable_and_page_renders_without_error`, `ReportInvoiceTest::test_owner_can_export_xlsx` + `test_export_xlsx_requires_permission_and_validates_pop_scope`. | Selesai (rumus Tagihan dikoreksi + rincian per sel ditambah, user 2026-09-22; kolom Dimuka menunggu ADHOC-92) |
| ADHOC-89 | **Piutang ditentukan pergantian bulan, bukan `due_date`** — **di luar sprint aktif, permintaan eksplisit user (2026-09-21)**. Aturan admin: `due_date` tanggal 10 hanya formalitas/label UI tanpa sanksi; batas riil = akhir bulan `billing_period`; tagihan belum lunas dari periode lalu jadi piutang tepat saat bulan berganti (tutup kas admin SELALU tanggal 1, jadi batas tidak perlu dikonfigurasi). Satu sumber di kode: `Invoice::scopePiutang()` / `isPiutang()`, dipakai dashboard (kartu+tabel "Piutang Tagihan"), list pelanggan (badge+ringkasan "Piutang"), tabel bayar kolektor. Worklist Kolektor (`collector_due_window_days`) sengaja TIDAK diubah. Hub pelanggan (`CustomerController::paymentInfo`, "Total Piutang") ikut scope piutang. Test: `InvoicePiutangBatasBulanTest` (model) + `PiutangDitampilkanBerdasarkanPeriodeTest` (dashboard, list, hub; sudah diuji mutasi — gagal kalau logika `due_date` lama dikembalikan). **Keputusan user 2026-09-21: "tunggakan" = piutang** (periode sebelum bulan berjalan; tagihan bulan ini yang belum dibayar = "belum dibayar", bukan tunggakan). Angka ikut scope piutang: dashboard "Total Tunggakan" (`total_unpaid_amount`, kini tanpa filter periode — saldo posisi), `invoices.index` "Total Nunggak — Awal/Bulanan", Laporan Tagihan (kartu Total Tunggakan + toggle `show_tunggakan` + ekspor CSV; kolom tabel diganti "Sisa Tagihan"). **Worksheet/Worklist Kolektor SENGAJA tetap menjumlah semua tagihan belum lunas** (kolektor menagih bulan berjalan juga) — hanya label diganti "Belum Dibayar". Docs: `BUSINESS_RULES.md` §7 poin 4, `billing-pembayaran/database-schema.md`, `kolektor/business-logic.md` §1 | **Selesai** — 2026-09-21 |
| ADHOC-88 | **Riwayat Pengambilan Alat + Terima Modem dari Pelanggan + petunjuk SN legacy** — **di luar sprint aktif, permintaan eksplisit user**, lanjutan ADHOC-86 (dijawab dari pertanyaan user 2026-09-21: log teknisi harus terpisah, riwayat tidak boleh hilang saat Langganan Lagi, data legacy hanya SN & kotor, modem legacy tanpa harga, modem diantar pelanggan tanpa task). **Data lama (DB lokal):** 1.957 detail teknis, 1.684 SN terisi, 273 kosong, 93 nilai SN muncul di >1 pelanggan (kemungkinan besar satu modem berpindah antar pelanggan), 426 catatan memuat merek (label berantakan: `ZTE F609`, `router GPON`, `1`, …) → backfill massal tetap TIDAK dilakukan. **Implementasi:** (1) tabel baru `device_retrieval_logs` (satu baris per SN: pelanggan, teknisi/petugas pengambil, penerima gudang, gudang tujuan, kondisi, nilai, foto, waktu; model `DeviceRetrievalLog`, enum `DeviceRetrievalSource`) — ditulis oleh `pickupSerialFromCustomer()`/`confirmReturnedSerial()`/`receiveSerialFromCustomerAtWarehouse()`; (2) halaman **Riwayat Pengambilan Alat** `/warehouse/retrievals` (filter teknisi/status/sumber/periode/cari, scope POP gudang tujuan, `warehouse.view`) + menu sidebar; (3) kartu riwayat di tab Perangkat Detail Pelanggan; (4) **Terima modem dari pelanggan** `/warehouse/returns/from-customer` (cari → pilih pelanggan putus → form, satu langkah tanpa transit, `warehouse_reassign.create`; ditolak kalau ada task Ambil Alat berjalan; banyak SN atomik); (5) **nilai taksiran opsional** di Terima Retur & walk-in, kosong = Rp 0, disimpan di `unit_price_snapshot` ledger; (6) `LegacyDeviceHintService` — petunjuk merek data lama, dipetakan ke master barang hanya kalau ≥5 karakter dan tepat satu barang cocok (`ZTE F609` ya; `ZTE`, `1`, `router GPON`, `ZTE F6` tidak), dipakai sbg tebakan awal di form DEAC/walk-in & petunjuk di Terima Retur; (7) **Langganan Lagi** (`CustomerController::reactivate()`, area ADHOC-85) kini mengosongkan `device_retrieved_at` — riwayat tetap utuh di log. **Temuan uji manual user 2026-09-21:** (a) tombol Ambil Alat menolak pelanggan tanpa baris `customer_devices` ("Data alat pelanggan tidak ditemukan.") — di data dev hanya 229 dari 1.957 pelanggan punya baris itu (dibuat backfill hanya untuk yang sudah putus), jadi ~1.700 pelanggan legacy aktif akan mentok saat diputus. Diperbaiki: dibuatkan placeholder `CustomerDevice::LEGACY_DEVICE_TYPE` ('Data Migrasi Legacy', pola sama import/backfill); (b) List Putus Langganan kini menampilkan badge **"Sedang Diproses"** (tombol Ambil Alat ikut disembunyikan selama status ini dan saat Sudah Diambil; muncul lagi kalau task dibatalkan) untuk pelanggan yang task Ambil Alat-nya sudah dibuat tapi belum selesai/dibatalkan — definisi "belum selesai" sama dengan penjaga tombol, satu kueri per halaman, tiga keadaan: Sudah Diambil > Sedang Diproses > Belum Diambil. (c) **Detail Laporan task DEAC** (`tasks/show`): sebelumnya jatuh ke blok "Laporan Pekerjaan Teknisi" milik Maintenance (kendala, material, foto OPM/speedtest) sehingga laporan pengambilan alat tidak tampil. Kini partial `tasks/partials/device-retrieval-report` (dipasang di rantai `@elseif` yang sama dgn Survey/Pemasangan): hasil, modem yang dibawa per SN (transit / sudah diterima gudang + penerima + kondisi), kelengkapan, foto kondisi, catatan/alasan, pelapor. Instruksi statis "Aset ISP yang Wajib Ditarik" hanya tampil sebelum laporan masuk; (d) **tab "Return dari Pelanggan"** di Barang di Tangan Teknisi (`/warehouse/custody`): SN berstatus `RETURNED` (transit) per teknisi — filter teknisi/gudang/cari (SN, barang, nama pelanggan) & scope POP gudang tujuan, tombol Terima → Terima Retur (`warehouse_reassign.create`); hilang dari tab begitu gudang menerima. Beda dari tab Perangkat Serial Number (barang yang DIBAWA ke lapangan, `ISSUED`). Butuh `npm run build` (class Tailwind `teal-*`); (e) **Riwayat Task FOP** (`/fop-tasks/history/{id}`, `fop_tasks/history_detail`): kategori Ambil Modem sebelumnya jatuh ke `@else` "Tipe task ini tidak punya laporan lapangan terstruktur" — tanpa laporan dan tanpa foto. Kini cabang khusus: hasil, pelapor, modem per SN (transit / diterima gudang + kondisi), kelengkapan, catatan/alasan, dan **foto kondisi alat terlampir** (thumbnail + tautan; kalau filenya hilang di disk tampil pesan, bukan gambar rusak). Eager-load `task.deviceRetrieval` & `task.completedBy` di `FopTaskController::showHistory()`. **Verifikasi MySQL (2026-09-21):** test dijalankan di MySQL 8.0.46 pada database sementara — 38 lulus, 7 gagal `SAVEPOINT transN does not exist`; penyebabnya artefak harness (bukan produk): `WorkflowTransitionPermissionSeeder` memakai `truncate()` yang di MySQL melakukan implicit commit atas transaksi pembungkus test (juga membuat tiap test ~90 dtk). Jalur penolakan service diverifikasi ulang langsung di MySQL dalam transaksi nyata (skrip di luar PHPUnit): semua benar (exception tepat, atomik, tak ada data tersisa, level transaksi utuh). Catatan: lookup SN di MySQL tidak membedakan huruf besar-kecil (collation `utf8mb4_0900_ai_ci`). **Koreksi ADHOC-86:** 3 view memakai `customer->name` (kolom sebenarnya `full_name`) sehingga nama pelanggan tampil kosong — diperbaiki + test regresi. Test: `DeviceRetrievalHistoryAndWalkInTest` (29 kasus) + regresi DEAC/Inventory. **Belum dikerjakan:** export Excel riwayat, laporan agregat per teknisi, peringatan "belum dinilai" di Laporan Bulanan, notifikasi modem transit terlalu lama. **Deploy:** `php artisan migrate`, `php artisan db:seed --class=ItemSeeder`, `npm run build`. Rancangan: `docs/plan/warehouse/analisa-riwayat-dan-terima-modem-dari-pelanggan.md` | Done — 2026-09-21 |
| ADHOC-87 | **Request Deaktivasi + Cuti Berlangganan — membebaskan tagihan langganan per periode** — analisa & rancangan dulu, **di luar sprint aktif**, belum diimplementasi. Studi kasus user 2026-09-21: pelanggan tak bayar & tak pakai internet, admin harus bisa menghilangkan tagihan bulan yang tak dipakai (sudah terbit maupun belum), Agustus tetap piutang. Temuan: **tidak ada jalur yang menetapkan invoice `batal`** (hanya import legacy; semua pembaca sudah benar terhadap `batal`); `invoices` tanpa SoftDeletes sehingga "hapus" = `batal`; generator `--period` bisa **menerbitkan ulang** invoice batal, dan periode belum terbit butuh penanda → tabel baru `customer_billing_waivers`; Portal `/me/invoices` tidak mengecualikan `batal` (Portal sudah dicek: tak perlu ubah kode Portal, cukup API menyaring `batal` bila `status` kosong). Rancangan: satu kemampuan, dua pintu ("Request Putus Langganan" — hanya label, tanpa persetujuan — dgn dropdown riwayat tagihan + catatan, & Cuti), guard: hanya `bulanan` tanpa pembayaran, jendela mundur 1 bulan, permission terpisah. Irisan dgn ADHOC-69 (tanpa prorate) & ADHOC-70. Pertanyaan terbuka sudah dijawab user 2026-09-21. Dokumen: `docs/plan/billing/analisa-rancangan-request-deaktivasi-bebas-tagihan-periode.md` | **Terbuka** — dianalisa 2026-09-21, implementasi belum dimulai |
| ADHOC-86 | **Ambil Modem (DEAC) → Gudang, termasuk SN legacy** — **di luar sprint aktif, permintaan eksplisit user**, paralel Sprint 8.10. Masalah: `TaskService::complete()` mengembalikan SN `INSTALLED` langsung ke `AVAILABLE` tanpa konfirmasi gudang lewat form Maintenance generik (wajib foto OPM/speedtest, tanpa input SN); SN modem legacy (hanya di `customer_technical_details.router_or_ont_serial`, tanpa nama barang) tidak pernah ada di `inventory_serials` sehingga diabaikan diam-diam sementara `device_retrieved_at` tetap terisi. Keputusan user 2026-09-19: (1) teknisi input SN fisik di form DEAC khusus + sistem auto-adopsi SN legacy (item dipilih teknisi / placeholder "Modem Legacy", dikoreksi gudang); (2) status transit — `SerialStatus::RETURNED` (case mati, dipakai ulang) sampai halaman Gudang "Terima Retur" mengonfirmasi; (3) form DEAC khusus. BAP tidak dipilih. Keputusan tambahan: DEAC tanpa SN boleh selesai dengan alasan wajib dan `device_retrieved_at` tetap kosong; permission Terima Retur reuse `warehouse_reassign.create`. **Implementasi:** tabel baru `task_device_retrievals` (model `TaskDeviceRetrieval`, enum `DeviceRetrievalOutcome`); `TaskDeviceRetrievalController` + view `tasks.device-retrieval-report` (form Maintenance dialihkan untuk DEAC); `InventoryReassignService::pickupSerialFromCustomer()`/`confirmReturnedSerial()` (`SerialStatus::RETURNED` dipakai sebagai transit, tanpa case enum baru); `WarehouseReturnReceiveController` + halaman `/warehouse/returns` + menu sidebar "Terima Retur"; item placeholder `MODEM-LEGACY` di `ItemSeeder`; guard `TaskService::complete()` (DEAC tidak bisa selesai tanpa laporan); reject FOP mengosongkan `device_retrieved_at`. **Item `MODEM-LEGACY` di-rename jadi `MODEM-PELANGGAN-LAMA` (2026-09-22)** — `ItemSeeder` rename-in-place (`code` lama → baru sebelum `updateOrCreate`, idempoten, ID tidak berubah), sudah diterapkan ke DB dev. **Penyimpangan dari rancangan:** reject FOP TIDAK membalik SN `RETURNED` (ledger append-only; kirim ulang laporan idempoten). `returnInstalledSerialFromCustomer()` dipertahankan (fixture 11 test ADHOC-80), tetapi tidak lagi dipanggil `TaskService`. Test: `DeviceRetrievalDeacToWarehouseTest` (17 kasus); `CustomerRetrieveDeviceCreatesFopTaskTest` disesuaikan. Tidak bentrok dengan ADHOC-69 (regresi badge/tombol Ambil Alat §3.2 terjaga). **Butir yang tadinya belum dikerjakan ditangani di ADHOC-88:** laporan per teknisi (halaman Riwayat Pengambilan Alat), reset `device_retrieved_at` saat Langganan Lagi. **Sengaja tidak dilakukan:** backfill massal SN legacy (data kotor). **Celah yang diketahui:** notifikasi ke gudang saat modem transit; reject FOP setelah gudang menerima membuat laporan tak bisa dikirim ulang; aksi "pending" FOP tidak mengosongkan `device_retrieved_at`. **Deploy:** `php artisan migrate`, `php artisan db:seed --class=ItemSeeder`, `npm run build` (lihat `docs/RUNBOOK_COMMANDS.md`). Rancangan: `docs/plan/warehouse/analisa-ambil-modem-deac-ke-gudang.md` | Done — 2026-09-19 |
| ADHOC-85 | **Terminasi & "Langganan Lagi" lewat state machine + pemulihan akses portal** — terminasi & reactivate dulu `update()` status langsung (tanpa guard status, `old_values` audit di-hardcode `active`, tidak masuk `customer_status_logs`; enum bilang `terminated` final padahal tombol Langganan Lagi ada). **Fix**: keduanya lewat `CustomerWorkflowService::transition()`, enum `TERMINATED => [ACTIVE]`, terminasi dari status selain `active`/`suspended` ditolak, audit `customers`/`terminate` dipertahankan (dibaca List Putus & import legacy) dengan `old_values` status asli. `terminated_at` **sengaja tidak dikosongkan** saat Langganan Lagi (dipakai churn `DashboardController::growthStats`). Akun portal `disabled` → `pending_claim` + QR/PIN baru lewat `CustomerObserver` → `PortalAuthService::restoreAfterReactivation()`. **Celah keamanan ditemukan & ditutup**: `PortalAuthService::login()` tidak membaca status akun, pelanggan terminated bisa login ulang dengan password lama. Test baru: `CustomerTerminationRejectsInvalidStatusTest`, `CustomerReactivateWritesStatusLogAndRestoresPortalTest`, `PortalDisabledAccountCannotLoginTest` (terverifikasi uji mutasi). Efek samping tercatat: pelanggan Langganan Lagi dapat notifikasi aktivasi ulang & kartu QR baru perlu dicetak. Irisan dengan ADHOC-69 dijelaskan di dokumen. Dokumen: `docs/plan/billing/rancangan-terminate-reactivate-state-machine.md` | Done — 2026-09-19 |
| ADHOC-84 | **Alokasi Pembayaran (disederhanakan 2026-09-19) + FIFO Kolektor + peringatan piutang admin** — rancangan awal (enum `PaymentAllocation`, kolom `allocation_type`, guard penolak) **dibatalkan** setelah dicek ke kode: `PaymentService` sudah memisah cicilan/lebih bayar otomatis & menjumlahkan tunai+saldo. **Dikerjakan 2026-09-23 (sebagian):** (1) dropdown "Alokasi" dihapus dari `quick-payment-modal.blade.php` (select `#qp-allocation` + penempelan ke `note` dihilangkan, `note` kembali murni catatan kasir); (2) konfirmasi `<x-ui.modal>` untuk lebih bayar di modal & `payments/create.blade.php` (gerbang `preventDefault()` + `requestSubmit()` setelah dikonfirmasi, server tetap tidak menolak overpay); (3) peringatan piutang non-blokir — `Invoice::olderUnpaidInvoices()` (satu sumber, dipakai `PaymentController::create()` server-render & `InvoiceController::show()` JSON `older_unpaid_invoices` buat modal); (4) klasifikasi majemuk `Payment::classification(): array` (pola sama `installmentContext()`, enum `PaymentPeriodType` dapat case baru `CICILAN`) ditampilkan di **4 dari 5** permukaan §8: Laporan Pembayaran (kolom + filter `classification`, SQL-nya SENGAJA pakai `whereHas`/subquery korelasi, bukan `join`, karena `applyUserScope()` sudah nulis `where('pop_id', …)` tanpa prefix tabel — `join` ke `invoices` bikin itu ambigu), Laporan Tagihan (badge Piutang/Bulanan Berjalan dari `Invoice::isPiutang()`), Laporan Bayar Kolektor (ganti badge tunggal `periodType()` lama), Audit Internal (batch-load `Payment::with('invoice')` per halaman, dilewati kalau payment-nya sudah tak ada — bukan crash), dan Laporan Bulanan Admin modal detail (cuma kolom Bulanan/Dimuka blok Tagihan, sesuai cakupan `docs`§8.2 — bukan kolom Piutang Bulan Lalu/Uang Diterima yang basisnya agregat invoice, bukan payment tunggal). Test baru: `PaymentAllocationClassificationTest` (12 kasus). **FIFO Kolektor (§4.4) — selesai 2026-09-23, keputusan user:** aturan lama "kelebihan bayar kolektor dikembalikan tunai, tidak jadi kredit" **DIBALIK** — user eksplisit: admin & kolektor SEKARANG SAMA soal kelebihan bayar, yang beda cuma siapa yang membayar. `CollectorPaymentService::record()`/`validateRows()` diubah pola sama `PaymentService::record()` jalur admin: auto-split `amount`/`overpay_amount` per baris (bukan lagi tolak nominal > sisa), overpay dikredit `CustomerBalanceService::credit()`. Isian awal FIFO (isi 1 baris = 1 invoice tetap, TIDAK ada payment lintas-invoice baru) murni kemudahan KLIEN — `cbApplyFifo()` di `collector-pay-script.blade.php`: toolbar "Total Diterima" muncul otomatis per pelanggan yang punya 2+ tagihan tertunggak di halaman yang sama, distribusi ke baris tertua dulu (urutan DOM yang sudah dikelompokkan per pelanggan), baris TERAKHIR menyerap sisa apa adanya (termasuk kalau melebihi sisa tagihannya sendiri) — nilainya tetap bisa diedit manual. Partial ini dipakai BERSAMA oleh Worksheet Admin (atas nama kolektor) & Worklist Kolektor (`docs/plan/kolektor/analisa-alur-kolektor-2.0.md` §9) — sengaja tidak dibedakan, konsisten dgn keputusan user. 2 test lama yang tadinya assert 422 untuk overpay (`CollectorSelfPaymentTest`, `CollectorBatchNotificationTest`, `PaymentBatchByAdminTest`) diperbaiki (§2.6) — bukan dilonggarkan, tapi diganti jadi assert perilaku baru/pakai baris tidak valid yang beda (metode Lainnya tanpa keterangan, invoice sudah lunas). Test baru: `CollectorFifoOverpayTest` (3 skenario — 200k Sept+Okt sebagian, 500k keduanya lunas+saldo 200k, batch tetap atomik+idempoten dgn overpay). Dokumen: `docs/plan/billing/analisa-skema-alokasi-pembayaran-dan-saldo.md` | **Selesai** — 2026-09-23 (§4.1–§4.4 & §8 4/5 permukaan) |
| ADHOC-83 | **Bug nyata: `unit_price_snapshot` roll disimpan mentah per-ROLL, padahal qty ledger selalu METER — nilai kekali `meter_per_roll` di seluruh kalkulasi hilir** — laporan user 2026-09-18 (kasus nyata: Dropcore 4Core, 12 roll @1.000m/roll, Rp 777.000/roll — sebelumnya kehitung sbg 12.000 meter × Rp 777.000 = 1000x lipat). **Fix**: `InventoryReceiveService::receiveRoll()` sekarang konversi `unitPrice ÷ meter_per_roll` SEBELUM disimpan ke `InventoryRoll.unit_price_snapshot` & `InventoryTransaction.unit_price_snapshot` — satu-satunya titik masuk roll ke ledger, semua kalkulasi hilir (custody/`TaskMaterial`, nilai rugi Adjustment, Kelola Stok badge harga, Laporan Bulanan `nilai`, Invoice) otomatis benar tanpa disentuh. Didokumentasikan di docblock `InventoryRoll`. **Invoice** (`WarehouseTransferController::invoice()`) sekarang tampil satuan **ROLL** buat item tracking_type ROLL (Qty=jumlah roll, Harga Satuan=harga per roll = harga-per-meter × meter_per_roll) — BUKAN meter kayak tracking internal (keputusan user: staf beli/nilai per roll, cuma pencatatan gudang/pemakaian yang meter). Surat Jalan **TIDAK diubah** — tetap 1 baris per roll_code + panjang meter-nya (informasi fisik verifikasi, bukan nilai uang). 2 test lama (`WarehouseReceiveTest`) yang assert nilai LAMA (bug) diperbaiki ke nilai BENAR. Test baru: `WarehouseTransferInvoiceSuratJalanTest` (+2 kasus — roll ke-simpan per-meter, Invoice tampil 12 roll/Rp 777.000/Rp 9.324.000 total, bukan meter). **Belum dikerjakan (perlu konfirmasi user)**: (1) data roll yang SUDAH kepencet di DB produksi sebelum fix ini butuh backfill/koreksi manual — ledger append-only, gak boleh di-`UPDATE` diam-diam; (2) Laporan Bulanan (`WarehouseReportController::export()`, ADHOC-79) qty roll-nya JUGA masih tampil meter mentah di kolom Barang Masuk/Stok Awal/Stok Akhir/Rusak/Hilang — nilai (Rp) bakal otomatis benar dari fix ini, tapi TAMPILAN qty-nya perlu konversi roll juga kalau user mau konsisten sama Invoice. | Done (fix inti) — 2026-09-18, 2 follow-up nunggu konfirmasi |
| ADHOC-82 | **Konfirmasi Terima Transfer disederhanakan + hint Issue "transfer belum dikonfirmasi"** — dua perbaikan terpisah dari laporan user (2026-09-18). (1) **Konfirmasi Terima** (`warehouse.transfers.show`, form ke `warehouse.transfers.receive`): checkbox per-SN/roll + input qty per baris (kontrol anti-manipulasi 2026-09-04, `kontrol-anti-manipulasi.md` §4) **dihapus** — di kiriman ratusan/ribuan item jadi beban, bukan kontrol yang kepake (keputusan eksplisit user). Diganti 1 tombol "Konfirmasi & Terima Semua Barang" + **modal warning** (`<x-ui.modal>`, bukan `confirm()` browser) yang nyebut jumlah item + nama Pusat + peringatan "tidak dapat dibatalkan" sebelum submit. `WarehouseTransferController::receive()` gak lagi baca body request sama sekali — server sendiri yang bangun `confirmed_serial_numbers`/`confirmed_roll_codes`/`confirmed_quantities` dari SEMUA baris dispatch (anggap 100% cocok), `InventoryTransferService::receiveTransfer()` TIDAK diubah. Konsekuensi: **partial-receive dari form ini sekarang mustahil** — selisih fisik lewat jalur Adjustment/opname terpisah, dicatat di amandemen `kontrol-anti-manipulasi.md` §4. (2) **Hint transfer pending** (`warehouse.issues.create`): sebelumnya kalau staf transfer Pusat→Cabang lalu buka Issue sebelum konfirmasi terima, Cabang tampil "Stok kosong" polos tanpa alasan (stok Cabang baru nambah SETELAH fase Konfirmasi Terima, bukan pas dispatch). `WarehouseIssueController::availableStock()` sekarang balikin `pending_transfer_count` (hitung `InventoryTransfer` `in_transit` ke pop itu), Alpine di `warehouse.issues.create` tampilin banner kuning + link ke `/warehouse/transfers/pending` kalau stok kosong TAPI ada transfer pending. Test baru: `WarehouseTransferReceiveSimplifiedTest` (5 kasus — receive tanpa body tetap full-received untuk quantity & serialized, halaman show gak ada checkbox lagi, `pending_transfer_count` 1 saat in_transit lalu 0 setelah dikonfirmasi). Regresi disapu: `Warehouse` (140 test) — nol regresi. | Done — 2026-09-18 |
| ADHOC-81 | **Invoice & Surat Jalan Transfer Gudang Pusat→Cabang** — dua PDF A4 (dompdf) dicetak dari `InventoryTransfer` yang sudah ada, 1:1 per transfer, TANPA migration baru (harga pakai `inventory_transactions.unit_price_snapshot` yang sudah ada, PJ Cabang pakai `pops.pic_name` yang sudah ada). **Invoice** (berharga): kolom Qty/Kode Barang-SN/Nama/Harga Satuan/Subtotal/Total, TTD Kepala Gudang murni nama+jabatan hardcode di `config/warehouse.php` + garis kosong (nama resmi beda dari nama akun sistemnya, keputusan eksplisit user) — **TIDAK ada gambar TTD**, diisi tangan di kertas sama seperti Surat Jalan (koreksi user 2026-09-17: jangan ada mekanisme upload/gambar TTD sama sekali). Permission root TERPISAH `warehouse_transfer_invoice.view` (feature baru `WarehouseFeatureSeeder`, cuma admin/owner — pop_admin cabang TIDAK dapat). **Surat Jalan** (tanpa harga): `unit_price_snapshot` SENGAJA gak ikut di-query sama sekali (bukan cuma disembunyikan di Blade), TTD Admin Gudang Pusat + PJ Cabang murni nama+garis kosong diisi tangan di kertas — TIDAK ADA TTD yang masuk sistem sama sekali (keputusan eksplisit, gak ada opsi capture digital). Reuse permission `warehouse_transfer.view` yang sudah ada (pop_admin cabang tetap bisa cetak punya cabangnya). Endpoint baru `WarehouseTransferController::invoice()`/`suratJalan()`, pola dompdf sama `PortalPaymentController::receipt()` (`setPaper('a4')`, toggle `?download=1`). Rancangan: `docs/plan/warehouse/rancangan-invoice-surat-jalan-transfer.md`. Test baru: `WarehouseTransferInvoiceSuratJalanTest` (6 kasus — owner akses keduanya, admin pusat vs pop_admin cabang beda akses Invoice, pop_admin cabang tetap bisa Surat Jalan, assert `unit_price_snapshot` gak ke-select + string harga gak ada di HTML Surat Jalan). **Redesign susulan 2026-09-17**: dua Blade dirombak ngikutin template resmi perusahaan (`docs/plan/warehouse/laporan/Surat_Jalan_Transfer_Gudang_WHUSNET.{docx,md,png}`, `INVOICE.{docx,md,png}`) — banner WHUSNET, blok Pengirim/Penerima berisi alamat+telp+PJ POP asli (`pops.address/village/district/city/pic_phone`, bukan hardcode), tabel Diskon/Pajak (selalu Rp 0, warehouse gak py konsep itu, dipertahankan demi kesetiaan template), dan **No. Surat Jalan** — field TERPISAH dari `reference_number` transfer, format `SJ/WHUS/{tahun}/{bulan}/{id transfer:03d}` diturunkan deterministik (`WarehouseTransferController::suratJalanNumber()`), TANPA counter baru tersimpan. **Koreksi kedua 2026-09-17**: kolom Kode Barang/SN dihapus dari tabel Invoice (redundan sama Surat Jalan yang udah py daftar per-SN) — jadi `Qty | Nama Barang | Harga Satuan | Diskon | Jumlah`, baris digabung per item+harga di controller (bukan lagi 1 baris per transaksi/SN). Test tambahan: dua SN item sama digabung jadi satu baris qty=2 (`WarehouseTransferInvoiceSuratJalanTest`, 7 kasus total). **Update KOP SURAT Resmi 2026-09-19**: Header kedua dokumen diperbarui menggunakan KOP SURAT resmi (`docs/plan/warehouse/laporan/KOP SURAT .docx`) — logo Connexa, nama resmi PT CONNEXA DIGITAL NETWORK, alamat Pucangombo, Tegalombo, Pacitan, kontak info@connexa.net.id │ 082240003434, serta garis divider hitam 3pt di kedua Blade (`invoice.blade.php` & `surat-jalan.blade.php`), 9 test passed. | Done — 2026-09-19 |
| ADHOC-68 | **Skema Upgrade/Downgrade Paket Internet (prorate + deposit)** — `CustomerPackageService::change()` ditulis ulang total: rumus tunggal `total = prorate_paket_lama + prorate_paket_baru`, n-segmen kalau ganti paket berkali-kali dalam satu periode (tabel baru `customer_package_changes`, audit + basis hitung), upgrade diblok kalau ada tunggakan periode sebelumnya (downgrade tetap boleh), deposit downgrade lewat `CustomerBalanceService::creditWithoutPayment()` (payment_id null, method baru). Invoice `belum ada` (belum lewat tanggal generate) tetap tanpa prorate seperti perilaku lama. Rancangan: `docs/plan/billing/upgrade-downgrade/analisa-upgrade-downgrade-paket.md`. Test: `CustomerPackageChangeTest` (13 skenario: upgrade/downgrade unpaid/lunas, tunggakan blokir upgrade/lolos downgrade, 2x ganti paket 3 segmen, edge hari pertama, invoice AWAL). **Belum disentuh**: breakdown `invoice_items` untuk invoice yang di-prorate ulang (subtotal/total berubah tapi baris rincian lama tidak di-rebuild — §5 rancangan, potensi PR susulan kalau laporan butuh rincian per segmen). | **Selesai** — diimplementasikan 2026-09-23, `vendor/bin/pint` bersih |
| ADHOC-69 | **Skema Putus Langganan (denda otomatis TANPA prorate, terbit sbg Tagihan Manual/Lainnya "Denda Putus Langganan" — keputusan user 2026-09-19, bagian denda bergantung ADHOC-70; master alasan, kolom pendaftar)** — **Implementasi (2026-09-23):** `CustomerTerminationService` baru (dipindah dari `CustomerTerminationController` yang cuma update status) — masa langganan dihitung dari `customer_services.activation_date` s.d. tanggal submit; **<=1 tahun** (inklusif, `activation_date` NULL diperlakukan sama) → `penalty_amount` wajib diisi manual (0 sah), invoice denda (`InvoiceType::MANUAL`/`ManualInvoiceCategory::LAINNYA`/sub `"Denda Putus Langganan"`, `belum_dibayar` tanpa Payment) cuma terbit kalau >0; **>1 tahun** → field denda tidak tampil di form & `penalty_amount` klien diabaikan sepenuhnya di server (guard anti tamper). Tanpa prorate — invoice Bulanan/tunggakan lama tidak disentuh. Master baru `customer_termination_reasons` (`Master\CustomerTerminationReasonController`, pola `TicketIssueCategoryController` + hapus permanen diblok `restrictOnDelete()` kalau masih dipakai pelanggan) + kolom `customers.termination_reason_id`/`termination_note`. Permission baru `termination_reasons.view/create/update/delete` (feature RBAC dinamis, terpisah dari `customers.deactivate` — admin dapat penuh, pop_admin cuma `.view`). List Putus (`RendersCustomerList`) baca alasan dari relasi `terminationReason` (bukan lagi AuditLog di memori), tambah kolom Sales (`salesUser`)/Teknisi Survei (`latestSurvey.technician`), filter+sort `termination_reason_id` di level query. Form Putus Langganan (`customers/show.blade.php`) jadi dropdown alasan + field denda kondisional + catatan opsional. 3 test lama disesuaikan field `reason`→`termination_reason_id` (`CustomerTerminationTest`, `CustomerTerminationRbacTest`, `CustomerTerminationRejectsInvalidStatusTest`) + 3 lagi ditemukan lewat regresi full-suite (`CustomerLifecycleNotificationTest`, `CustomerListStatusTimestampOrderingTest`, `CustomerReactivateWritesStatusLogAndRestoresPortalTest`). Test baru: `CustomerTerminationPenaltyTest` (7 kasus), `CustomerTerminationReasonMasterTest` (5 kasus), `CustomerTerminatedListReasonSalesSurveyTest` (3 kasus). Rancangan: `docs/plan/billing/analisa-rancangan-putus-langganan.md`. **Belum dikerjakan (di luar scope)**: kolom "Total Piutang" di List Putus (disebut opsional/"boleh" di rancangan §3.3, bukan wajib). | **Selesai** — 2026-09-23 |
| ADHOC-74 | **Riwayat Mutasi Gudang: 1 kartu per dokumen, bukan 1 baris per barang** — Input/Transfer/Serah Terima yang mencakup banyak jenis barang (mis. 30 modem + 20 roll kabel) sebelumnya nampil sebagai puluhan baris ledger terpisah di `/warehouse/history`. Dikelompokkan jadi SATU baris per `reference_number` (Transfer tetap pecah per arah leg Dikirim/Diterima, dikonfirmasi bug lama 2026-09-03 sengaja dipertahankan), klik masuk ke halaman detail yang **sudah ada** (`warehouse.receive.show`/`warehouse.issues.show`/`warehouse.transfers.show`, tidak diubah — sudah menampilkan rincian per barang). Grouping di PHP (`WarehouseHistoryController::groupByDocument()`), paginasi manual `LengthAwarePaginator` atas Collection (pola sama `WarehouseStockController`). Tipe tanpa halaman detail per-dokumen (return/adjustment/stock_opname/transfer_custody/install) tetap tampil 1 baris per baris seperti sebelumnya. Test baru: `WarehouseHistoryGroupedByDocumentTest` (2 kasus) | Done — 2026-09-16 |
| ADHOC-73 | **Verifikasi Registrasi oleh Admin/CS** — gerbang baru: pelanggan hasil Registrasi (non-Skip-Survey) berhenti di status `registered` TANPA Task/FopTask Survey sampai disetujui di antrean baru `/customer-registration-verifications` (`CustomerRegistrationVerificationController`, permission `customer_registration_verification.view/approve/reject`, default digrant role `helpdesk`, murni RBAC — bisa dipindah lewat Role Matrix). Approve → Task+FopTask Survey kebentuk (logic dipindah dari `CustomerController::store()`) + status pindah `waiting_survey`. Tolak → `WorkflowTransition::REJECTED`, alasan wajib (textarea bebas). Skip Survey dikecualikan (tetap langsung `waiting_installation` seperti sebelumnya, tidak tersentuh). Rancangan: `docs/plan/pendaftaran-pelanggan/analisa-verifikasi-registrasi.md`. Test baru: `CustomerRegistrationVerificationTest` (7 kasus: gak-langsung-Task, approve, reject, reject-wajib-alasan, skip-survey-tetap-jalan, RBAC 403, POP scope). 4 test lama disesuaikan ke status `registered` yang baru (`CustomerCreateTest`, `CustomerActionRedirectPermissionGapTest`, `CustomerRegistrationTest`, `CustomerRegistrationSkipSurveyTest`) | Done — 2026-09-15 |
| ADHOC-80 | **Kondisi Fisik Barang SERIALIZED (Baru/Bekas-Kondisi Baik/Bekas-Rusak)** — **di luar sprint aktif, permintaan eksplisit user**, paralel Sprint 8.10. Implementasi `docs/plan/warehouse/analisa-gap-kondisi-barang.md` (rancangan sudah "siap dieksekusi" sejak 2026-09-16). Gap: `returnInstalledSerialFromCustomer()` (retrieval DEAC/Ambil-Alat) nyetel SN balik ke `status=AVAILABLE` — SAMA PERSIS status SN baru dari `receiveSerialized()`, jadi modem bekas-copotan (mungkin sudah soak, belum tentu "rusak" secara formal) bisa ke-Issue lagi ke teknisi lain tanpa jejak/gate inspeksi apa pun. Fix: enum baru `App\Enums\ItemCondition` (`new`/`used_good`/`used_damaged`, axis independen dari `SerialStatus`) + 3 kolom baru `inventory_serials` (`condition` default `new`, `condition_checked_at`/`condition_checked_by` nullable). `receiveSerialized()`/`receiveSerializedAuto()` set `condition=new` eksplisit; `returnInstalledSerialFromCustomer()` set `condition=used_good` + checked null (asumsi awal, BELUM dicek). Gate baru `InventorySerial::isClearedForIssue()` (SN `new` selalu lolos; SN bekas cuma lolos kalau `condition_checked_at` udah keisi) ditegakkan di `InventoryIssueService::issueSerialized()` (Service, bukan cuma UI) + dropdown "Stok Tersedia" (`WarehouseIssueController::availableStock()`) exclude SN yang belum lolos gate. Aksi baru "Sudah Dicek" (`InventoryReassignService::markSerialConditionChecked()` — TANPA tulis ledger, murni metadata inspeksi, permission REUSE `warehouse_reassign.create`) — inline toggle di halaman Lacak Barang (`WarehouseTraceabilityController::checkCondition()`, pola-3 CLAUDE.md karena udah di halaman Detail SN spesifik). Kolom baru `inventory_transactions.resulting_status` (general, SERIALIZED & ROLL) disnapshot eksplisit di `adjustSerialStatus()`/`adjustRollStatus()` — dulu status hasil adjustment (LOST/DAMAGED/SCRAPPED/QUARANTINE) cuma bisa ditebak dari status SN/Roll SAAT INI, gak akurat historis kalau di-adjust lagi belakangan. Riwayat Mutasi (`WarehouseHistoryController`) dapat 2 filter baru: **Kondisi** (baca `serial.condition`/`condition_checked_at`, 4 opsi niru badge UI) dan **Alasan** (baca `resulting_status`, aktif cuma pas `type=adjustment` — opsi "Opname" SENGAJA tidak dimasukkan, itu type ledger terpisah `STOCK_OPNAME` yang udah punya filter Tipe sendiri, taruh di sini cuma bikin dobel/salah kaprah, koreksi dari rancangan awal doc). Badge kondisi ditambah di 4 titik (poin 6 riset dokumen): Lacak Barang (detail penuh + tombol "Sudah Dicek"), Kelola Stok (modal quick-look SN), Custody (list SN dipegang teknisi), Scan Barang (hasil lookup). Scope SENGAJA cuma SERIALIZED — Roll kabel & `customer_devices.device_retrieved_at` eksplisit di-skip (keputusan user 2026-09-16, tercatat di dokumen rancangan). Test baru: `InventorySerialConditionTrackingTest` (13 kasus — receive set kondisi baru, return set bekas-belum-dicek, gate Issue nolak SN belum dicek, "Sudah Dicek" melepas gate + jejak actor, tolak input kondisi `new`/SN yang masih `new`, `resulting_status` ke-snapshot, endpoint HTTP + validasi + redirect, badge+tombol tampil di Traceability, filter Kondisi & Alasan di Riwayat Mutasi). Regresi disapu: filter `Warehouse` (116 test) + `Inventory\|Installation\|TaskService\|CustomerInstallation` (131 test) — nol regresi. | Done — 2026-09-17 |
| ADHOC-79 | **Laporan Gudang diperluas jadi "Laporan Akhir Bulan"** (layout kolom final persis `test1.html`, ronde ke-6: grup Identitas/Barang Masuk/Stok Awal/Stok Terpakai/Stok Akhir/Barang Rusak/Hilang, 23 kolom, header 2-baris berwarna+merge). Ditemukan+ditutup bug `receivedInPeriod()` (Barang Masuk kepotong ADJUSTMENT gara-gara filter `type` belum dipasang). — halaman existing `warehouse.reports.index` (bukan halaman baru): kolom Nilai Rugi Rusak+Hilang (qty × last-cost RECEIVE) di tab Kerugian + KPI + Download Excel **di-plot per POP** (satu sheet per gudang + sheet custody teknisi tanpa atribusi POP) + **Stok Awal & Stok Akhir historis per-tanggal** (2-slot Lama/Baru buat QUANTITY, satu harga buat SERIALIZED/ROLL — service baru `WarehouseStockAsOfService`, replay ledger, bukan baca `InventoryBalance` terkini). Sekalian fix bug klasifikasi lama (`buildAdjustmentSummary()` grouping pakai `reason` mentah, seharusnya `resulting_status` buat SERIALIZED/ROLL). Kolom HUTANG TETAP belum ada — gak ada konsep utang-distributor di sistem. Analisa+keputusan final (4 ronde klarifikasi): `docs/plan/warehouse/analisa-nilai-rugi-rusak-laporan-bulanan.md`. Test: `WarehouseReportLossValueTest` (9 kasus, hijau); full suite Warehouse 135 test hijau | **Selesai** — 2026-09-17, cuma HUTANG yang masih terbuka (butuh rancangan konsep utang baru, di luar lingkup) |
| ADHOC-70 | **Tagihan Manual (Detail Pelanggan + Halaman Tagihan, halaman bukan modal)** — ditulis ulang 2026-09-19 murni dari studi kasus user (3 jenis pendapatan tetap: Perbaikan/Lainnya/Pindah Lokasi + deskripsi + nominal); **tidak** memakai rancangan lama ADHOC-60 (master kategori, `invoice_items`, `INSIDENTAL`). **Implementasi (2026-09-23):** `InvoiceType::MANUAL` baru (`manual_category`/`manual_subtype_name`/`description` kolom baru di `invoices`, enum `ManualInvoiceCategory`) di luar `SUBSCRIPTION_TYPES` — boleh terbit bareng tagihan Bulanan periode sama. Halaman baru `/invoices/create` (`InvoiceController::create()`/`store()`, route statis sebelum `/invoices/{invoice}`) — dua pintu masuk: Detail Pelanggan (`?customer_id=…`, terkunci) & Halaman Tagihan polos (cari CID/Nama, pola `where` sama `index()`). Modal lama `#manual-invoice-modal` + `CustomerController::storeManualInvoice()` + route `customers.invoices.manual` **dihapus bersih** sesuai rancangan §4 butir 6. **Revisi 2026-09-23 (koreksi user):** rancangan awal §3.4 (Invoice + Payment sekaligus dalam satu submit, field Metode Bayar + Nominal Dibayar) **dibatalkan** — metode bayar itu ranah Pembayaran, bukan Tagihan. Form sekarang **cuma menerbitkan Invoice** (`belum_dibayar`), tanpa Payment; pembayarannya dicatat belakangan lewat jalur biasa (List Tagihan → Bayar/Bayar Cicil, `PaymentController::store()`/`PaymentService::record()` yang sudah ada, tidak diubah). Field `payment_method`/`bank_account_id`/`sender_name`/`paid_amount` dicabut dari form & controller. Test: `InvoiceCreateTest` ditulis ulang total (7 kasus — customer terkunci tanpa Payment, pencarian CID/Nama, Lainnya wajib sub-nama, koeksistensi dgn Bulanan, nominal wajib+normalisasi ribuan, POP scope, route lama sudah hilang). **Belum dikerjakan (di luar scope)**: tagihan Reaktivasi kehilangan satu-satunya jalur penerbitan setelah modal lama dihapus (dicatat di dokumen rancangan §5 butir 1, follow-up terpisah); breakdown per jenis pendapatan di Laporan Tagihan/Pembayaran. Rancangan: `docs/plan/billing/analisa-rancangan-tagihan-manual.md` | **Selesai** — 2026-09-23 |
| ADHOC-75 | **2-Slot Harga (Lama/Baru) untuk barang QUANTITY + hapus `TrackingType::BATCH`** — Kelola Stok Gudang cuma nampilin 1 harga per barang QUANTITY, padahal 1 barang bisa diterima 2x dengan harga beda. Dicek pembukuan real (`docs/plan/warehouse/laporan/LAPORAN ADMIN GUDANG PER AGST 26.xlsx`): admin gudang sudah lama pakai pola persis 2 slot harga (Lama/Baru) per barang. `BATCH` enum dihapus (0 item pernah pakai). `InventoryReceiveService::receiveQuantity()` sekarang auto-tentuin `lot_no` (`resolveQuantityLot()`) — lot pertama tetap sentinel `''` (kompatibel data lama), lot kedua digenerate sistem (`{kode}-{tanggal}-{urut}`) begitu harga beda dari lot aktif, ke-3 harga sekaligus **ditolak** (guard eksplisit, belum pernah kejadian di data real). `InventoryIssueService`/`InventoryTransferService` cuma relaksasi validasi lot (gak ada lagi gerbang BATCH), infrastruktur pilih-lot per baris (panel "Stok Tersedia") ternyata sudah generic dari awal — gak perlu controller baru. Ketauan & dibenerin sekalian bug laten: `resolveLastCost()` di Issue/Transfer Service salah baca harga lot Lama begitu ada 2 lot aktif (`null` = skip filter, bukan filter ke lot `''`) — sekarang selalu difilter. Rancangan: `docs/plan/warehouse/analisa-2-slot-harga-quantity.md`. Susulan (laporan user 2026-09-16, "pathcore 2 lot kebentuk tapi Kelola Stok gak nunjukin harga"): `WarehouseStockController::index()` sekarang hitung `$lastPriceByKey` (last-cost RECEIVE per lot) + `$multiLotPopItemKeys` (barang yang BENERAN lagi py 2 lot aktif di gudang yang sama), Kelola Stok nampilin label "Harga Lama"/"Harga Baru" + nominal Rp per baris — TAPI label cuma muncul kalau emang ada 2 lot (barang 1-lot tampil harga polos tanpa label "Lama", biar gak nyesatkan). Dead branch `@elseif($tracking==='batch')` di view ikut dibuang. Test: `receive_quantity_harga_beda_otomatis_pecah_jadi_2_lot` (`WarehouseReceiveTest`), `kelola_stok_nampilin_label_dan_harga_per_lot_barang_quantity` + `kelola_stok_barang_quantity_satu_lot_tampil_harga_tanpa_label_lama` (`WarehouseStockPageTest`) + ~20 test lama disesuaikan (hapus arg `lotNo`, `tracking_type=batch`→`quantity`). Full suite 2038 passed (12 gagal pra-eksisting, tidak terkait — InternetPackageSeeder/invoices.store/PaymentReceipt/QR, dicek gak nyentuh Inventory). | Done — 2026-09-16 |
| ADHOC-72 | **FAB (Formulir Akan Berlangganan Bisnis) wajib untuk paket kategori Bisnis saat Registrasi** — `DocumentType::FAB` baru, disimpan lewat `CustomerDocument` (bukan restricted_packages — itu whitelist paket per role, gak ada hubungan dengan kategori Bisnis/Rumahan). Ketauan sekalian bug lama: upload Foto Rumah Registrasi cuma dirender di dalam blok `@can('customers.registration.skip_survey')`, jadi actor tanpa permission itu (default cuma role `sales`) sama sekali gak punya cara upload walau aturannya opsional bukan gak-ada — dipindah ke luar blok itu, sekarang selalu tampil & opsional untuk semua role/semua kategori paket, wajib cuma kalau Skip Survey aktif (tidak berubah). Foto Rumah Laporan Survey tetap wajib mutlak (tidak diubah, sudah sesuai) | Done — 2026-09-15 |
| ADHOC-74 | **Kabel per-Roll Tracking (TrackingType::ROLL)** — **di luar sprint aktif, permintaan eksplisit user**, paralel Sprint 8.10. Kabel gudang gak lagi cuma QUANTITY/BATCH biasa — tiap roll fisik dapat identitas unik digenerate sistem (`{item.code}-{YYYYMMDD}-{6 digit}`, counter per-bulan per-prefix, pola sama `generateReferenceNumber()`), tercetak sebagai label QR (satuan/batch, browser-print, model dari `customers/qr/print.blade.php`), discan lewat Scan Barang existing, dilacak lewat Asset Traceability, dan dilacak sisa meternya sampai habis. Tabel baru `inventory_rolls` (mirror `inventory_serials`, numerik bukan biner — `length_total`/`length_remaining`), enum baru `RollStatus` (11 case, beda vocabulary dari `SerialStatus` — roll gak pernah INSTALLED atomik, ada IN_USE/DEPLETED buat potongan sebagian), `TrackingType::ROLL` baru (4 case sekarang). Vendor dicatat per-roll (bukan kolom `items`), konversi Roll→Meter (`items.meter_per_roll`) fixed di master, di-snapshot ke `length_total` saat generate. Alur pemakaian sama SN modem: Issue 1 roll utuh ke custody teknisi → teknisi pilih roll + input meter di Laporan Pemasangan (opsional, draft pointer `selected_inventory_roll_id`/`roll_meters_used` di `customer_installations`, potong beneran di `storeSpeedtest()`)/Maintenance (opsional, one-shot langsung potong) → `InventoryService::consumeFromRoll()` decrement `length_remaining` + tulis `TaskMaterial` (TIDAK menulis ledger `inventory_transactions` — sama pola `consumeFromCustody()`). Semua service Inventory dapat cabang ROLL: `InventoryReceiveService::receiveRoll()` (generate N roll dari `roll_count`+vendor+harga), `InventoryIssueService::issueRoll()`, `InventoryTransferService::dispatchRoll()`+confirm (krusial — tanpa ini roll gak bisa pindah Pusat→Cabang buat diissue), `InventoryReassignService::returnRollToWarehouse()` (BEDA dari SN: roll balik BAWA SISA METERNYA, bukan "utuh")+`transferRollToTechnician()`, `InventoryAdjustmentService::adjustRollStatus()` (Lapor Rusak/Hilang/Scrap, guard evidence wajib tetap berlaku). RBAC: TIDAK ada permission baru — ROLL cuma tracking_type baru lewat controller/permission yang SAMA (`warehouse_transfer.*`/`warehouse_issue.*`/dst). Controller baru `WarehouseRollController` (2 route print, permission REUSE `warehouse_transfer.view`) + `RollLabelQrRenderer` (pola sama `CustomerQrCodeRenderer`/`ReceiptQrRenderer`). **Susulan (2026-09-15, sama hari, gap ditutup)**: UI Reassign/Adjustment roll yang tadinya dicatat belum diwiring — SEKARANG selesai. `WarehouseReassignController::createRoll()`/`storeRoll()` (return bawa sisa meter / alih custody) + `WarehouseAdjustmentController::createRoll()`/`storeRoll()` (Lapor Rusak/Hilang/Scrap/Karantina, guard evidence tetap berlaku), view `warehouse/reassign/roll.blade.php` + `warehouse/adjustments/roll.blade.php` (mirror `serial.blade.php`), rute baru `warehouse.reassign.roll.*`/`warehouse.adjustments.roll.*` (permission REUSE, gak ada permission baru), kolom "Aksi Kelola" (dropdown ⋮ sama pola tab Serial) ditambah di tab Roll Kabel halaman Custody, dan `WarehouseScanController::resolveRollActions()` diperluas penuh (Transfer/Issue/Reassign/Adjust/Traceability — sebelumnya cuma link Traceability). Test baru: `WarehouseReassignAdjustmentRollControllerTest` (6 kasus HTTP). Test baru (12 file total, seluruhnya hijau): `InventoryReceiveRollTest`(digabung ke `WarehouseReceiveTest`), `WarehouseRollPrintTest`, `WarehouseScanRollLookupTest`, `WarehouseTraceabilityRollTest`, `InventoryIssueRollTest`, `InventoryServiceConsumeFromRollTest`, `InstallationRollConsumptionTest`, `TaskMaintenanceRollConsumptionTest`, `InventoryTransferRollTest`, `InventoryReassignRollTest`, `InventoryAdjustmentRollTest`, `WarehouseReassignAdjustmentRollControllerTest`. Regresi disapu: `WarehouseScanLookupTest`, `WarehouseScanBatchModeTest`, `WarehouseCustodyAndTraceabilityTest`, `WarehouseTransferAndIssueTest`, `InstallationSerialDropdownAuthorityTest`, `TaskMaintenanceModemInstallTest`, `InstallationMaterialCustodyReconcileTest`, `InstallationActivatedWebhookTest`, `InstallationSpeedtestActivationGateTest`, `WarehouseAdjustmentEvidenceUploadTest` — nol regresi. | Done — 2026-09-15, gap UI Reassign/Adjustment roll ditutup sama hari |
| ADHOC-76 | **Fix bug: Master Barang ROLL ke-render "QUANTITY" di daftar + Issue/Transfer roll gak bisa disubmit dari browser sama sekali** — laporan user langsung ("sudah dipilih roll namun pada list masih terdeteksi quantity"). Root cause: `master/items/index.blade.php` badge switch cuma py cabang `serialized`/`batch`, `roll` ketiban default else → tampil QUANTITY (fix: tambah cabang `roll` amber "ROLL KABEL"). **Investigasi lanjut nemu bug kelas sama yang lebih parah**: `warehouse/transfers/create.blade.php` & `warehouse/issues/create.blade.php` py Alpine manager SENDIRI-SENDIRI (`warehouseTransferManager`/`warehouseIssueManager`), BUKAN pakai `<x-inventory-line-rows>` yang diupdate ADHOC-74 — jadi kedua form itu LITERAL gak pernah punya field `roll_codes` sama sekali sejak fitur ROLL dibangun. Staf pilih barang tracking_type=roll di kedua halaman itu bakal jatuh ke Mode "Quantity & Batch" (minta qty, bukan roll_codes) — submit ke `WarehouseTransferController`/`WarehouseIssueController` PASTI gagal validasi backend (`InvalidArgumentException` "daftar Roll ID wajib diisi"). Backend (`InventoryTransferService::dispatchRoll()`/`InventoryIssueService::issueRoll()`, ADHOC-74 fase 5&7) SENDIRI udah benar & full-tested — gap-nya murni di 2 file Blade ini yang gak ikut ke-update waktu component shared diubah. Diperbaiki: cabang `x-if="row.tracking_type === 'roll'"` baru di kedua file (textarea `roll_codes`, badge amber "ROLL KABEL", `rollCount()` helper, `isDetailsComplete`/Mode-2-exclude disesuaikan) + chip picker "Roll ID" di panel Stok Siap Kirim/Stok di Cabang Ini (`$dispatch('pick-roll', ...)`+`onPickRoll()`, backend `availableStock()` di kedua controller UDAH balikin `rolls[]` sejak ADHOC-74 tapi gak pernah dirender). Test baru/diperluas: `MasterItemRollFieldsTest` (+1 kasus badge list), `WarehouseTransferAndIssueTest` (+guard `assertSee('roll_codes')` di 2 test render-page, biar regresi kelas ini ketauan tanpa perlu laporan user lagi — Blade Alpine template gak kebaca test HTTP biasa yang POST langsung array, makanya bug ini lolos dari `InventoryTransferRollTest`/`InventoryIssueRollTest` yang test Service langsung). Regresi disapu: full filter `Warehouse\|Item\|Master` (156 test) — nol regresi (1 gagal pra-existing gak nyambung). | Done — 2026-09-16 |
| ADHOC-77 | **3 fix roll kabel lanjutan (laporan user 2026-09-16)**: (1) **Barcode 1D, bukan QR** — koreksi eksplisit user, `RollLabelQrRenderer` (Endroid) diganti total jadi `RollLabelBarcodeRenderer` (Code128, `picqer/php-barcode-generator` — dependency baru, dikonfirmasi user via AskUserQuestion sebelum install, alternatif JsBarcode-CDN ditolak karena beda pola dari semua QR generator server-side existing). Layout `warehouse/rolls/print.blade.php` diubah dari horizontal (QR-kotak di samping) jadi vertikal (barcode-wide di atas → roll_code teks → nama barang di bawah), sesuai spek persis user. (2) **Unit "1.000 roll" seharusnya "1.000 meter"** — root cause GANDA: (a) semua tempat yang nampilin qty baris roll (`receive/show.blade.php`, `transfers/show.blade.php` 3 titik, `stock/index.blade.php`) masih baca `$line->item->unit` mentah (nilai bebas admin ketik) alih-alih hardcode `'meter'` (padahal `length_total`/`length_remaining` SELALU meter by design — `issues/show.blade.php` udah bener duluan, jadi referensinya), (b) placeholder Master Barang ("meter / pcs / roll") NYARANIN admin ngetik "roll" sebagai satuan — item produksi nyata `DC-4C` kesimpen `unit="Roll"` gara2 ini. Fix: field Satuan di `master/items/create.blade.php`+`edit.blade.php` di-disable+dikunci ke "meter" via `<template x-if>` hidden input begitu `tracking_type=roll` (bukan `x-show`, biar gak dobel `name=unit` bentrok pas submit), `ItemController::validateItem()` maksa `$validated['unit']='meter'` server-side (gak percaya client), data existing `DC-4C` diperbaiki langsung via tinker. (3) **Kabel yang udah diterima ke Gudang Pusat gak kerekam di Kelola Stok** — sama kelas bug yang dulu kejadian ke SERIALIZED (2026-09-07, lihat komentar existing di `WarehouseStockController`): `receiveRoll()` cuma nulis `inventory_rolls`, gak pernah nyentuh `inventory_balances`, dan controller Kelola Stok cuma baca balance asli + agregat sintetis SERIALIZED — ROLL kelewat total. Fix: `$rollBalances` agregat sintetis baru (pola sama `$serializedBalances`, `qty=SUM(length_remaining)` dalam meter, cuma roll status AVAILABLE/RECEIVED yang MASIH di gudang — bukan yang udah diissue ke teknisi), badge "ROLL KABEL" + modal AJAX `warehouse.stock.rolls` (endpoint baru, pola sama `serials()`) nampilin daftar roll_code+sisa meter per gudang+barang. `transfers/show.blade.php` (halaman konfirmasi terima) TERNYATA juga belum punya cabang roll SAMA SEKALI (ketauan pas nutup fix #2) — checkbox `confirmed_roll_codes[]` + Detail column + scan-kamera auto-confirm buat roll ditambahkan, tanpa ini Transfer roll gak akan PERNAH bisa dikonfirmasi diterima dari UI (servis `receiveTransfer()` udah dukung param-nya dari ADHOC-74, cuma HTML form-nya yang kelewat). Test baru: `WarehouseStockRollTest` (3 kasus — muncul di Kelola Stok, gak ikut kehitung kalau udah diissue, AJAX endpoint), `MasterItemRollFieldsTest` (+1 kasus unit-lock server-side anti-bypass). Regresi disapu: full filter `Warehouse\|Item\|Master` (160 test) + full suite — nol regresi. **Susulan (sama hari)**: label cetak roll dirapikan — baris meter/vendor dicabut dari sticker (spek awal user cuma barcode+kode+nama barang). Modal "SERIAL NUMBER"/"ROLL KABEL" di Kelola Stok (badge trigger sendiri udah bener) diperluas dari chip-list polos jadi TABEL detail (model `receive/show.blade.php` "Rincian Barang & Serial Number" — kolom Kode, Harga Beli, Diterima; Roll tambah Sisa/Total Meter + Vendor) — tapi sumber datanya STOK LIVE di gudang itu (`WarehouseStockController::serials()`/`rolls()`), BUKAN snapshot 1 transaksi Receive kayak halaman RCV-. Harga per SN diambil dari baris RECEIVE ledger pertama SN itu (gak disimpan di `inventory_serials` sendiri). | Done — 2026-09-16 |
| ADHOC-78 | **Halaman khusus Konfirmasi Barang Transfer** — permintaan eksplisit user ("Pusat dan Cabang mudah untuk konfirmasinya"), paralel Sprint 8.10. Sebelumnya transfer `in_transit` cuma bisa ketemu lewat cari manual (Riwayat Mutasi/Kelola Stok), gak ada satu titik ringkas nunjukin "apa yang harus saya konfirmasi sekarang". Halaman baru `warehouse.transfers.pending` (`WarehouseTransferController::pending()`, permission REUSE `warehouse_transfer.view` — gak ada permission baru) nampilin 2 daftar: **Perlu Dikonfirmasi Di Sini** (transfer `in_transit` yang `to_pop_id` masuk scope POP user — tombol "Tinjau & Konfirmasi" ke `warehouse.transfers.show` existing, tempat aksi konfirmasi beneran terjadi, pola View-only/link-ke-halaman-mutasi sesuai aturan 3-pola CLAUDE.md) dan **Terkirim, Menunggu Cabang Tujuan** (transfer yang `from_pop_id` masuk scope tapi `to_pop_id` di luar scope — read-only, cuma status "Dalam Perjalanan"). Route statis `/warehouse/transfers/pending` ditaruh SEBELUM `/warehouse/transfers/{transfer}` (urutan static-first, `routes/web.php`). Nav tab "Konfirmasi Transfer" ditambah di ketiga lokasi header Gudang (`<x-warehouse.header>` desktop nav, mobile dropdown grid, mobile carousel), gated `$canViewTransfer`. Test baru: `WarehouseTransferPendingTest` (5 kasus — actionable ke-render, pop_admin cabang tujuan lihat actionable, pop_admin cabang pengirim lihat "menunggu cabang lain" bukan tombol konfirmasi, pop_admin di luar scope gak lihat sama sekali, user tanpa permission 403). Regresi disapu: filter `Warehouse\|Transfer` (114 test) — nol regresi. | Done — 2026-09-16 |
| ADHOC-71 | **Edit Pelanggan dirombak jadi 7-step (skin terbaru, samain Registrasi/Laporan Survey/Pemasangan) + nambah step Parameter Teknis terstruktur (customer_devices/customer_technical_details, penulis kedua selain CustomerInstallationController) + fix npwp & jenis_kontrak yang dari awal gak pernah divalidasi (Registrasi & Edit) + fix bug `total_monthly_bill` ikut memasukkan `other_fee` (harusnya murni, ketemu dari data CID C1X4ARQ000004)** — lihat detail di `docs/data-pelanggan/README.md` §Perubahan Terbaru 2026-09-14, `docs/pendaftaran-pelanggan/README.md` §Perubahan Terbaru 2026-09-14, `docs/billing-pembayaran/perbandingan-tagihan-awal-vs-bulanan-legacy.md` §6.3, `docs/customer-lifecycle/business-logic.md` §6 | Done — 2026-09-14 |
...
| ADHOC-25 | **OCR kwitansi: hasil tebakan tak boleh jadi keputusan** — lubang dorman yang hidup saat `GEMINI_API_KEY` diisi (lihat detail di bawah) | **Ditutup** — OCR Gemini dihapus (ADHOC-99, 2026-09-26) |
...
| ADHOC-43 | **Skip Survey saat Registrasi** — Sales (atau role lain yang diberi permission) bisa lewat tahap survey lapangan asal input data survey lengkap (ODP terdekat, estimasi kabel, tingkat kesulitan, foto rumah, foto ODP) + titik koordinat langsung di form registrasi. Permission baru `customers.registration.skip_survey` (`ActionCode::SKIP_SURVEY`, feature `customers.registration`) — default cuma role Sales di `RolePermissionSeeder`, digerbangi juga di `CustomerRegistrationRequest::authorize()` (403 kalau `skip_survey=1` tanpa permission, bukan validasi field survey yang membingungkan). Customer lahir langsung `status=waiting_acc` (skip `waiting_survey`/`survey_in_progress`/`surveyed` sepenuhnya) — `WorkflowTransition::REGISTERED` nambah edge ke `WAITING_ACC`. `CustomerSurvey` dibuat otomatis (`survey_status=completed`, `technician_id`=Sales, `survey_note` ber-tag "Skip Survey"). **Sengaja TIDAK ada Task/FopTask SURVEY yang lahir** — beda dari alur normal yang selalu auto-create keduanya. Alur pasca-`waiting_acc` (ACC Admin → `waiting_installation` → provisioning FopTask PEMASANGAN) sama persis dengan pelanggan survey normal, tidak disentuh. Test: `CustomerRegistrationSkipSurveyTest` (4 kasus: sukses, validasi wajib, 403 tanpa permission, regresi alur normal). **User demo:** `SalesSeeder` (baru, dipanggil dari `DatabaseSeeder`) — `sales@whusnet.com`/`password`, role `sales`, scope `all_pop`, permission `customers.registration.skip_survey` otomatis lewat `RolePermissionSeeder`. Test `SalesSeederSkipSurveyScenarioTest` (2 kasus) ngiket seeder ke skenario penuh — kalau seeder lupa assign role/scope/permission yang bener, ini yang gagal duluan. **Catatan operasional:** di DB yang sudah ada duluan (bukan `migrate:fresh --seed`), permission baru gak otomatis muncul — wajib re-run `FeatureSeeder`+`ActionSeeder`+`RolePermissionSeeder` lalu `EffectiveAccessService::clearCache($user)` buat user yang udah pernah login (permission di-cache). Diverifikasi manual di dev DB (docker `whusnet-app`) — `hasPermission()` sales balik `NO` sebelum re-seed, `YES` sesudahnya. **Dua bug UI ditemukan & diperbaiki pasca-implementasi:** (1) panel `skip-survey-fields` dirender `class="grid ... hidden"` — utility `hidden` dan `grid` sama-sama `display`, specificity sama, `hidden` kalah cascade → checkbox dicentang tapi panel gak pernah nongol. Fix: pindah ke inline `style="display:none"` (pola sama `surveys/report.blade.php`, sudah didokumentasikan sebelumnya tapi kelewat di sini). (2) `AppServiceProvider::boot()` sebelumnya `Gate::define()` satu-satu dari `Permission::all()` SEKALI saat app boot — permission baru yang di-seed pasca-boot (kasus nyata: test yang seeding di `setUp()`, juga DB lama yang belum di-`migrate:fresh`) gak pernah punya Gate ability terdaftar, jadi `@can()` salah walau `hasPermission()` langsung sudah benar. Diganti `Gate::before()` — delegasi semua ability ke `hasPermission()` langsung, gak perlu pre-registrasi, sekalian hapus query `Permission::all()` tiap boot. Ability berbasis nama manusia (`permission->name`) yang lama didefinisikan dilepas — diverifikasi grep gak dipakai di satu pun `@can()`. Ditambah **Tanggal Request Pemasangan** (opsional, sama seperti Lapor Survey teknisi) ke panel Skip Survey. Test tambahan: `test_skip_survey_panel_uses_inline_style_not_conflicting_hidden_class`, `test_skip_survey_rejects_past_requested_installation_date`, assert `requested_installation_date` di test sukses. **`Gate::before()` global sempat bikin regresi nyata saat full-suite run** (bukan cuma hipotesis) — `FopTaskCancelCascadeAuthTest::test_owner_wildcard_cannot_bypass_survey_invariant` gagal (expect 403, dapat 200): `Gate::before()` versi awal cek SEMUA ability tanpa syarat, jadi ability Policy kata-tunggal-tanpa-titik (`TaskPolicy::cancel`/`cancelViaFopTask`) ikut ke-short-circuit SEBELUM `TaskPolicy::before()` sempat jalan — wildcard `*` Owner nembus invarian SRV/PSB yang sengaja dikecualikan Policy itu (`in_array($ability, ['cancel','cancelViaFopTask'])`). Fix: `Gate::before()` cuma aktif kalau `$ability` mengandung `.` (bentuk kode permission `feature.action`, format tetap — lihat CLAUDE.md § RBAC) — ability Policy selalu kata tunggal, otomatis lolos gak disentuh. Ketauan justru karena full-suite dijalankan lengkap (bukan cuma file terkait), bukan dari baca kode. Regresi RBAC/Policy disapu penuh setelah fix (`RolePermissionMatrixTest`, `TicketingRbacTest`, `TaskFopActionsTest`, `TaskOwn*Test`, `FopTaskCancelCascadeAuthTest`, dll — 76 test) + full suite (`php artisan test`) — hijau semua kecuali kegagalan `Permission denied` `storage/framework/testing/disks/local` yang pre-existing (izin folder docker, gak terkait perubahan ini). | Done — 2026-08-21 |
| ADHOC-44 | **Papan /fop-tasks & kartu Task Teknisi gak update realtime kalau perubahan status dipicu dari halaman LAIN** (bukan dari halaman itu sendiri) — user minta SPA-like, nol polling/refresh manual. Root cause: `TaskStarted`/`TaskCompleted` (`App\Events`, dipicu `TaskService::start()`/`complete()` — dipakai SEMUA tipe task: Survey/Pemasangan/Maintenance) cuma disiarkan ke channel `fop.{pop_id}` (Dashboard FOP doang). Papan `/fop-tasks` (channel `fop-tasks.{pop_id}`) cuma dengar `.FopTaskUpdated` — event itu cuma di-dispatch dari `FopTaskController` sendiri (edit lewat papan), gak pernah ke-trigger kalau teknisi start/lapor dari halaman Task-nya sendiri. Kartu Task Teknisi (`teknisi.{user_id}`) cuma dengar `TaskScheduled` — gak dengar status berubah kalau dipicu ANGGOTA TIM LAIN (task 1-3 orang, siapa pun boleh start/lapor — bukan cuma yang start, lihat diskusi sebelumnya). Fix: `TaskStarted`/`TaskCompleted::broadcastOn()` sekarang nyiarin ke TIGA channel sekaligus — `fop.{pop_id}` (gak berubah), `fop-tasks.{pop_id}` (baru), + `teknisi.{user_id}` per anggota tim (baru, pola sama persis `TaskScheduled`). Payload tambah `fop_task_id` (resolve `FopTask::where('task_id',...)`, null kalau gak ada FopTask terhubung) — dipakai papan nentuin baris mana yang di-refresh (beda dari `Task.id`). Frontend: `fop_tasks/index.blade.php` & `tasks/own.blade.php` nambah `.listen('TaskStarted'/'TaskCompleted', ...)` di subscription channel yang SUDAH ADA, reuse helper DOM-patch yang sudah ada (`refreshFopTaskRow()`/`refreshTaskCard()` — fetch partial HTML, ganti elemen, TANPA reload/polling). **Dashboard FOP sudah realtime sebelumnya** (listen `TaskStarted`/`TaskCompleted`/`Survey*`/`Installation*` di `fop.{pop_id}`) — gak disentuh, cuma dua target lain yang punya gap. Test: `TaskBroadcastingTest` +2 kasus (`test_task_start_broadcasts_to_fop_tasks_board_and_team_members`, `test_task_completed_payload_resolves_linked_fop_task_id`) — 7 test total, semua existing tetap hijau (channel `fop.{pop_id}` masih index `[0]` di array, urutan dipertahankan). | Done — 2026-08-21 |
| ADHOC-45 | **Worksheet Admin, Setoran Kas, Worklist Kolektor — SPA-like penuh** (lanjutan ADHOC-44, modul kas kolektor). Temuan: Worksheet Admin & Worklist Kolektor SEBENARNYA sudah punya infra realtime (`partials/collector-realtime.blade.php`, dengar `CollectorDepositUpdated`/`CollectorActivityUpdated`) — tapi **sengaja** cuma nongolin toast + bar "Muat ulang" manual, alasan didokumentasikan di kode lama: halaman ini menghitung UANG FISIK, angka yang berubah diam-diam di tengah hitungan admin dianggap berbahaya. **User diberi tahu risiko ini secara eksplisit lalu memilih override penuh** ("auto-update SEMUA angka, termasuk pas form kebuka") — bukan keputusan yang diambil sepihak. Setoran Kas (`cash-deposits`) malah belum punya broadcast apa pun sama sekali, dibangun dari nol. **Perubahan:** (1) `partials/collector-realtime.blade.php` — bar+"Muat ulang" manual DICABUT, diganti auto-patch: tiap event masuk, `fetch(window.location.href)` ulang, ambil elemen ber-`id="live-content"` dari hasilnya, `replaceWith()` elemen yang sama di halaman sekarang + `Alpine.initTree()` (pola identik `refreshFopTaskRow`/`refreshTaskCard` ADHOC-44) — TANPA syarat "skip kalau modal/form lagi kebuka" (sesuai pilihan user; kalau admin lagi ngetik nominal di form yang ada di dalam `#live-content` pas event lain masuk, ketikannya bisa ketiban data fresh — keputusan sadar, didokumentasikan di komentar file). Toast tetap tampil sebagai kabar, cuma gak lagi wajib diklik buat lihat data baru. (2) Wrapper `id="live-content"` ditambahkan ke div konten utama `collector-worksheet/index.blade.php`, `show.blade.php`, `collector-worklist/index.blade.php`, `cash-deposits/index.blade.php` (4 halaman). (3) Event baru `App\Events\CashDepositUpdated` (`ShouldBroadcastNow`, pola sama `CollectorDepositUpdated`) — channel `cash-deposits` (global, gerbang `cash_deposit.view` di `routes/channels.php`, gak per-POP karena pemeriksanya Owner/atasan yang sudah bypass scope) + `App.Models.User.{depositor_id}` (kanal generik yang sudah ada). Dispatch di 3 titik `CashDepositService` (`submit()`→`diajukan`, `verify()`→`diverifikasi`, `writeOff()`→`ditutup_selisih`), dibungkus `safelyNotify()` yang sudah ada (gagal kirim kabar TIDAK boleh menggagalkan transaksi uang). (4) `CollectorWorksheetController::activityChannels()` nambah kanal pribadi viewer sendiri (`App.Models.User.{id}`) — biar admin dengar kabar setoran kasnya SENDIRI ke Owner tanpa reload Worksheet. Test baru: `CashDepositBroadcastingTest` (3 kasus: submit/verify/writeOff nyiarin ke channel yang benar), `CollectorWorksheetTest` +1 kasus (kanal pribadi + `id="live-content"` ke-render). Regresi disapu: `CashDepositVerificationTest`, `CollectorWorklist*Test`, `CollectorWorksheetTest`, `TaskBroadcastingTest`, `FopTaskCancelCascadeAuthTest` — 58 test, semua hijau. | Done — 2026-08-21 |
| ADHOC-46 | **QR Pelanggan Fase 1 — Fondasi** (rancangan: [`docs/plan/qr-code/rancangan-qr-pelanggan-final.md`](plan/qr-code/rancangan-qr-pelanggan-final.md), di luar sprint aktif — dikerjakan atas permintaan eksplisit user). Fondasi HMAC + dispatcher + Fungsi B (ticketing prefill) — PIN/halaman tagihan (Fase 2) dan absen teknisi (Fase 3) BELUM disentuh. `config/qr.php` (`QR_HMAC_SECRET` wajib di production, guard di `AppServiceProvider::boot()`) + `Base32Helper` (RFC 4648, tanpa dependency). Migration `customer_qr_tokens` (token 128-bit acak + `signed_pop_id`/`signed_customer_code` dibekukan saat terbit, TANPA kolom PIN — menyusul Fase 2) + `qr_scan_logs` (semua scan tercatat termasuk gagal). `CustomerQrTokenService` (`issue`/`revoke`/`signature`/`verify`/`resolve`/`recordScan`, HMAC-SHA256 `pop_id|customer_code|token`, dukung rotasi `previous_secrets`). `CustomerQrTokenObserver::creating()` menegakkan invariant "maks 1 token aktif/pelanggan" dari SEMUA jalur (bukan cuma Service). `CustomerObserver` ditambah hook: pelanggan `terminated` atau `pop_id` berubah (re-homing) → token QR ikut tercabut otomatis (notifikasi admin POP masih TODO, kanal belum diputuskan). Dispatcher publik `GET /q1/{code}` (regex base32 `[A-Z2-7]{26}\.[A-Z2-7]{10}`, throttle `qr-public` 60/menit/IP) — 4 kegagalan (`token_not_found`/`bad_signature`/`token_revoked`/`pop_mismatch`) SEMUA 404 identik, detail cuma masuk log; tamu dapat placeholder tagihan (BUKAN 404 — QR-nya valid, cuma halaman Fase 2 belum ada); staf ber-`tickets.create` diarahkan 2-hop (`/q1/{code}` → `/q1/{code}/tiket`, hop kedua di belakang middleware `auth` ASLI, defense-in-depth) ke `tickets.create?customer_id=` yang prefill+lock field pelanggan (`TicketController::resolvePrefillCustomer()` reuse `customerPayload()` yang sama persis hasil lookup manual — Alpine `pick()` yang sudah ada dipakai apa adanya, **nol perubahan** ke `TicketService::create()`/alur sync Ticket↔FopTask↔Task). Halaman staf `customers/{customer}/qr` (lihat/terbitkan/cabut) + `/qr/cetak` (stiker QR+nama+REQ ID+POP, TANPA PIN/signature teks — `endroid/qr-code` versi existing `^5.1` dipakai apa adanya, SVG via `CustomerQrCodeRenderer` pola sama `ReceiptQrRenderer`; sempat ke-upgrade gak sengaja ke v6 saat `composer require`, di-revert). 6 permission baru lewat `PermissionGeneratorService` (`customers.qr.view/create/cancel/print`, `tasks.qr_attendance.create` & `qr_scan_logs.view` diseed sekarang meski enforcement-nya baru Fase 2/3) — `QrFeatureSeeder`, digrant sesuai tabel §5 dokumen (admin penuh; pop_admin view+print; helpdesk/fop view; fop+teknisi `qr_attendance.create`; owner/atasan/admin `qr_scan_logs.view`). Test: `tests/Feature/QrCode/CustomerQrTokenServiceTest.php` (7 kasus — signature deterministik & berubah per bahan, dua pelanggan beda POP `customer_code` sama → signature beda, issue idempoten, tolak `customer_code`/`pop_id` kosong, revoke idempoten, invariant satu-token-aktif dari jalur non-HTTP/tinker, verify terima secret lama saat rotasi) + `tests/Feature/QrCode/QrScanDispatchTest.php` (9 kasus — semua status kegagalan + body 404 identik, format invalid ditolak di level route tanpa query DB, `pop_mismatch` tetap kedeteksi walau `pop_id` diubah lewat query builder BYPASS Observer, out_of_scope 403, alur ticketing 2-hop penuh sampai `assertViewHas('prefillCustomer', ...)`, tamu dapat placeholder bukan 404, scan sukses menaikkan `scan_count`) — 17 test, 44 assertion, hijau. Regresi disapu: `TicketingTest`/`TicketingRbacTest`/`TicketCidDisplayTest` (74 test), `PermissionGeneratorServiceTest`, `PostTargetRenderedServerSideTest`, `PortalCustomerObserverDisablesPortalOnTerminatedTest` — semua hijau.
**Perbaikan pasca-review (sesi sama, sebelum ditutup):** (1) `tests/Feature/QrCode/QrStaffPageSmokeTest.php` (3 kasus baru) — `CustomerQrController` (`show`/`issue`/`print`/`revoke`) sebelumnya NOL test HTTP sungguhan (cuma level Service/dispatcher publik), termasuk render SVG QR asli lewat `endroid/qr-code`; ketiganya hijau di percobaan pertama, gak ada bug ketemu tapi sebelumnya buta total. (2) **Race condition ditutup** — `CustomerQrTokenService::issue()`/`revoke()` sebelumnya cek-lalu-insert/update tanpa lock: dua `issue()` bersamaan buat pelanggan sama (TOCTOU) bisa lolos dua-duanya sebelum salah satu commit, invariant "1 token aktif" bisa jebol di skenario konkuren (`CustomerQrTokenObserver` tetap correct tapi query-nya sendiri gak locked). Fix: `issue()` dibungkus `DB::transaction()` + `Customer::whereKey()->lockForUpdate()` (kunci baris PELANGGAN, bukan token — belum ada baris token buat dikunci saat pelanggan belum pernah dapat QR), menyerialkan semua `issue()` pelanggan yang sama; `revoke()` dibungkus lock serupa pada baris token-nya sendiri biar dobel-klik "Cabut" gak saling timpa `revoke_reason`. Pola sama persis `TicketService::close()`/`cancel()` yang sudah ada di repo — bukan pola baru. Race sungguhan (multi-proses) gak ditest — repo ini juga gak punya presedennya buat `lockForUpdate()` manapun (dicek: grep `race`/`concurrent`/`bersamaan` ke test Ticket = nol hasil), diverifikasi lewat code review + 20 test existing tetap hijau setelah perubahan. — 20 test total, 55 assertion, hijau. | Done (Fase 1) — 2026-08-26, **Fase 2 (PIN + halaman tagihan) & Fase 3 (absen) belum dikerjakan** |
| ADHOC-47 | **QR Pelanggan Fase 2 — PIN + Halaman Tagihan Publik** (lanjutan ADHOC-46, rancangan §6.1/§6.5/§10 Fase 2, di luar sprint aktif — permintaan eksplisit user). Migration TERPISAH `add_pin_columns_to_customer_qr_tokens_table` (pin_hash bcrypt, pin_issued_at/by, pin_version, pin_must_change, pin_first_used_at, pin_expires_at 90 hari, pin_failed_attempts/pin_locked_until — lockout DB, bukan cache). `CustomerQrTokenService` +5 method: `issuePin()` (random_int CSPRNG, tolak 000000/123456/6-digit-sama/berurutan via `isWeakPin()` — cek "tanggal lahir" di dokumen SENGAJA dilewati, `customers` gak punya kolom tanggal lahir sama sekali), `verifyPin()` (urutan persis §6.5.4: expired→locked→invalid/success, lockout 5x/15menit), `changePin()` (tolak sama-dengan-lama + pola lemah), `verifyLegacyPhoneSuffix()` (jalur 4-digit HP buat pelanggan belum punya PIN). **Penerbitan OTOMATIS** di `CustomerWorkflowService::transition()` saat masuk `waiting_installation` — `issue()` token idempoten via `wasRecentlyCreated`, PIN CUMA diterbitkan kalau token BENAR-BENAR baru (instalasi diulang tidak menerbitkan PIN kedua yang mematikan kartu lama); gagal (customer_code/pop_id kosong) di-catch & di-log, TIDAK menggagalkan transisi workflow inti. `QrBillingController` (publik, TANPA auth) — two-step disclosure: `show()` gerbang PIN/4-digit-HP (masking nama via `FormatHelper::maskName()`, jumlah bintang TETAP biar panjang kata asli gak bocor) kalau sesi belum verified, rincian tagihan penuh (invoice list + rekening+salin+WA admin POP dari `pops.pic_phone`, TANPA gateway/QRIS — itu Fase 4 yang ditahan) kalau sudah; `verify()` (limiter TERPISAH `qr-billing-verify`, `perHour(5)` per IP+kode — penyederhanaan sadar dari "5/15menit lalu blokir 1 jam" dua-tahap doc, didokumentasikan sebagai simplifikasi) memindahkan sesi via flag boolean/timestamp SCOPED PER TOKEN, PIN ITU SENDIRI TIDAK PERNAH masuk session. **Wajib ganti PIN cetak di pemakaian pertama** (§6.5.5b) — `pin_must_change=true` menahan di halaman `ganti-pin` (session flag `pending_change` terpisah dari flag "verified penuh", gak bisa dilompati langsung) sebelum tagihan kebuka. `QrScanController` guest branch diubah dari placeholder Fase 1 (view inline) jadi redirect ke `qr.billing` sungguhan — `resources/views/qr/billing-placeholder.blade.php` DIHAPUS (dead code). Halaman staf `CustomerQrController::issuePin()` — SENGAJA BUKAN pola PRG redirect (nol perubahan; dokumen eksplisit larang PIN masuk session/flash/cache/log), PIN dirender LANGSUNG di body respons POST + header `Cache-Control: no-store`/`X-Robots-Tag: noindex`, satu-satunya momen PIN plaintext pernah ada di luar `pin_hash`. Numpang permission `customers.qr.create` yang sama (dokumen gak mendefinisikan permission terpisah buat PIN). Test: `CustomerQrPinServiceTest` (9 kasus — hash bukan plaintext, reset PIN gak ubah token/signature, verify sukses reset percobaan, 5 gagal kunci & bertahan walau `Cache::flush()`, PIN kedaluwarsa ditolak walau PIN benar, fallback 4-digit HP, changePin tolak sama-lama & pola lemah, changePin sukses matikan wajib-ganti & gak ikut kedaluwarsa) + `QrBillingPageTest` (6 kasus HTTP — gerbang 4-digit HP end-to-end, wajib-ganti-PIN sebelum lihat tagihan, halaman ganti-PIN gak bisa dilompati langsung, lockout 5x bertahan ke percobaan ke-6 walau PIN benar, kode invalid 404, invoice pelanggan muncul di detail) + `CustomerWorkflowQrAutoIssueTest` (2 kasus — waiting_installation nerbitkan token+PIN otomatis, transisi berulang TIDAK nerbitkan token/PIN kedua) + `QrStaffPageSmokeTest` +2 kasus (terbitkan-PIN HTTP sungguhan dengan verifikasi PIN yang ke-render di body beneran cocok sama hash yang tersimpan — bukan cuma dipercaya kode; 404 kalau belum ada token aktif) — **17 test baru, 56 assertion, hijau**, total modul QR jadi 39 test/123 assertion. Regresi disapu: `CustomerInstallationTest`/`CustomerRegistrationSkipSurveyTest`/`SalesSeederSkipSurveyScenarioTest`/`SurveyInstallationQueueScopeTest`/`TaskFopActionsTest`/`FopDashboardProcessToTimTest` (32 test, jalur-jalur lain yang juga transisi ke `waiting_installation`) — semua hijau, hook auto-issue gak mengganggu alur existing.
**Perbaikan pasca-review (sesi sama, sebelum ditutup):** (1) **PIN plaintext bocor ke session `_old_input` ditutup** — `$request->validate()` Laravel, KALAU gagal di validasi FORMAT (bukan business-logic `verifyPin()`), default-nya nge-flash SEMUA input ke session lewat `withInput()`; Laravel cuma mengecualikan `password`/`password_confirmation`/`current_password` dari daftar itu secara bawaan. Field `pin`/`hp_last4`/`new_pin`/`new_pin_confirmation` TIDAK ada di daftar itu — bug nyata, dibuktikan: matiin fix-nya sementara → test merah (PIN masuk `_old_input`), pasang lagi → hijau. Fix: `bootstrap/app.php` `$exceptions->dontFlash([...])` didaftarkan GLOBAL (bukan per-controller, biar gak lupa kepasang lagi kalau field serupa nambah nanti). Test baru: `QrBillingPageTest::pin_format_salah_tidak_pernah_masuk_old_input_session`. (2) **Race condition kelas sama seperti ADHOC-46 ketemu lagi di kode PIN baru** — `verifyPin()` cek-lalu-increment `pin_failed_attempts` tanpa lock: skrip brute-force paralel bisa bikin satu increment ke-skip (TOCTOU), lockout 5x bisa ditembus lebih dari 5 tebakan efektif. Fix: `issuePin()`/`verifyPin()`/`changePin()` ketiganya dibungkus `DB::transaction()` + `CustomerQrToken::whereKey()->lockForUpdate()`, pola identik `issue()`/`revoke()` (ADHOC-46). Ditemukan lewat self-review proaktif (bukan diminta ulang) setelah user tanya soal race condition di sesi sebelumnya — pola yang sama harus dicek ulang tiap kali nulis kode serupa, bukan cuma ditambal sekali. — 40 test total modul QR (termasuk 1 test regresi baru), 124 assertion, hijau. Full suite disapu ulang: 1586 passed (2 gagal `PaymentReceiptPrintTest` — pre-existing, DIKONFIRMASI tidak terkait: nol file yang disentuh task ini menyerempet receipt, string yang dicari test itu sudah gak ada di kode manapun sejak refactor ADHOC-29).
**Koreksi kesalahan logic (laporan user 2026-08-26, bukan self-review):** (1) **QR + PIN sekarang diterbitkan BARENG dalam satu aksi** — versi sebelumnya salah paham §6.5.3 ("dipicu ... bukan aksi admin lepas"): staf harus klik "Terbitkan Token QR" LALU klik terpisah "Terbitkan PIN", dua langkah manual padahal dokumen jelas bilang satu paket. Fix: `CustomerQrController::issue()` sekarang menerbitkan token (idempoten) DAN auto-menerbitkan PIN kalau `pin_hash` masih kosong, satu tombol "Terbitkan QR + PIN" — aman diklik berulang karena PIN CUMA terbit sekali (gak pernah menimpa yang sudah ada). (2) **Reset PIN sekarang wajib verifikasi eksplisit** — sebelumnya "Terbitkan Ulang PIN" satu klik langsung nimpa PIN aktif tanpa konfirmasi apa pun; risiko nyata: klik gak sengaja mematikan PIN yang sudah dipegang pelanggan (§6.5.5 sendiri mensyaratkan admin verifikasi identitas 2 faktor SEBELUM reset). Fix: aksi reset dipecah jadi `reissuePin()` TERPISAH, permission dinaikkan ke `customers.qr.cancel` (setara mencabut token, bukan `.create`), route `/qr/pin/reset`, WAJIB field `verification_note` (dicatat ke `AuditLog` — bukan PIN-nya) + dialog konfirmasi eksplisit (pola sama "Cabut Token"). (3) **Login ID portal ditampilkan** — sebelumnya nol jejak `login_id` (§6.6.2: "dicetak di kartu pelanggan bersama PIN") di seluruh alur QR/PIN, padahal itu kredensial yang pelanggan butuh buat klaim akun portal belakangan. Fix: `Customer::getPortalLoginIdAttribute()` accessor baru (`{pop.registration_prefix}-{customer_code}`, null-safe kalau POP belum punya prefix — sama syarat yang sudah dicek `CustomersBackfillPortalLoginId`), ditampilkan di kartu PIN (`pin-issued.blade.php`) dan halaman status QR (`show.blade.php`). Test disapu ulang & ditambah: `QrStaffPageSmokeTest` — 2 kasus lama diganti (jalur PIN gabungan + jalur reset terpisah), +3 kasus baru (klik ulang gak menimpa PIN, reset wajib verification_note + audit log, Login ID muncul di kartu) — 43 test modul QR total, 139 assertion, hijau.
**`PortalAuthController::claim()` diaktifkan (keputusan eksplisit user, sesi sama)** — sebelumnya stub 501 dari sesi Portal API terdahulu ("menunggu modul QR/PIN"), sekarang modul itu sudah jalan. `PortalAuthService::claim()` (method baru, `lockForUpdate()` pada baris `customer_portal_accounts` — pola sama `CustomerQrTokenService::issue()`): login_id → cari akun `pending_claim` → PIN diverifikasi lewat `CustomerQrTokenService::verifyPin()` YANG SAMA dipakai gerbang QR publik (lockout 5x/15menit ikut berlaku, gak ada jalur bypass kedua, sesuai §6.6.5) → sukses: `password_hash` diganti ke pilihan pelanggan sendiri, status `pending_claim`→`active`, `claimed_at` diisi, token API diterbitkan. Outcome 4 jenis dipetakan ke HTTP berbeda: `success`→200, `already_claimed`→409 ("gunakan Lupa Password"), `locked`→423 (lockout PIN, BUKAN lockout password akun), lainnya→401 generik (login_id gak ada/PIN salah/belum ada token QR AKTIF — SEMUA pesan sama, gak dibedakan, pola IDOR-safe yang sama dengan `login()`). `PortalClaimRequest` baru — validasi `pin` (`digits:6`) + `new_password` (`min:10` + `StrongPortalPassword` reuse rule yang sama `PortalUpdatePasswordRequest` pakai). Field `pin`/`new_password` ikut masuk daftar `dontFlash` global (`bootstrap/app.php`) yang sudah ada — meski endpoint JSON API secara struktural gak pernah flash session (`invalidJson()` Laravel, beda jalur dari `invalid()` yang dipakai `QrBillingController`), ditambahkan untuk konsistensi kalau field ini kepakai ulang di endpoint web nanti. Docs disinkronkan: `docs/api/api-portal-pelanggan/business-logic.md` + `rencana-implementasi.md` — status stub 501 dihapus, kontrak endpoint asli ditulis ulang. Test: `PortalClaimStubReturns501Test` DIHAPUS (mengetes perilaku yang sekarang sengaja diganti), diganti `PortalClaimTest` (6 kasus — klaim sukses+password ke-hash+account jadi active, klaim ulang setelah aktif 409, PIN salah 401 pesan identik dengan login_id gak ada, pelanggan belum punya token QR 401, password lemah 422, PIN terkunci 5x 423 dengan limiter portal dilonggarkan di test biar bukan numpang limiter API yg angkanya kebetulan sama) — hijau. Regresi disapu: seluruh `tests/Feature/Api/CustomerPortal/` (unaffected, `login()`/`refresh()`/`logout()` tidak disentuh) + `tests/Feature/QrCode/` — 122 test gabungan, 428 assertion, hijau.
**Reset PIN dirombak lagi (koreksi kedua user, sesi sama)** — dialog "isi verification_note" yang baru ditambal ternyata dianggap gak perlu ("hilangkan saya"), dan pola "dialog → buka tab baru berisi halaman penuh" buat lihat PIN dianggap ambigu/bikin bingung. Diganti total: `verification_note` DICABUT dari `reissuePin()` (gerbangnya sekarang murni modal pratinjau sisi klien, bukan field wajib server-side). `CustomerQrController::issue()`/`reissuePin()` sekarang JSON response (bukan HTML/redirect) — dikonsumsi `fetch()` dari `show.blade.php` yang sekarang Alpine-driven (`x-data="qrPinPage()"`), buka `<x-ui.modal>` (komponen modal reusable yang sudah ada di repo, transisi bawaan) nampilin QR+nama+Login ID+PIN dengan animasi reveal (`x-transition`), tombol Cetak (`window.print()` di-scope ke `#pin-print-card` doang lewat `@media print`) + Selesai — TANPA reload/tab baru kecuali token itu BARU pertama kali kebentuk (kasus itu doang, reload dibutuhkan karena seluruh blok QR/dl belum pernah dirender PHP-nya). `resources/views/customers/qr/pin-issued.blade.php` (halaman penuh lama) DIHAPUS, gak dipakai lagi sama sekali.
**Riwayat PIN ditambahkan** (diminta user: "Status PIN juga menampilkan riwayat") — BUKAN kolom/tabel baru, dibaca dari `AuditLog` yang sekarang ditulis di `CustomerQrTokenService::issuePin()` sendiri (dipindah dari controller — supaya SEMUA jalur pemicu PIN, termasuk hook otomatis `CustomerWorkflowService` saat WAITING_INSTALLATION, `$actor` null, ikut tercatat, bukan cuma jalur staf manual). `CustomerQrController::pinHistory()` — query `AuditLog` per token, action `pin_issued`/`pin_reissued`, ikut di payload JSON (biar modal+bagian "Status PIN" update tanpa request kedua) DAN di GET `show()` biasa (biar riwayat tetap muncul walau di-reload, bukan cuma hidup di state Alpine sesi itu).
**Belum diputuskan — pertanyaan terbuka ke user:** apakah "Cetak Stiker" (`print.blade.php`, reprint kapan pun) SEHARUSNYA juga tampilkan Login ID+PIN — user pilih "gabung jadi satu cetakan" tapi belum jawab sub-pertanyaan teknisnya: PIN disimpan HASH (bcrypt), begitu direspons sekali pas terbit PIN plaintext-nya HILANG dari sistem selamanya, jadi "Cetak Stiker" yang reprintable kapan pun secara struktural TIDAK BISA nampilin PIN kecuali penyimpanan PIN diubah jadi reversible (encrypt, bukan hash) — regresi keamanan lebih besar dari yang sudah dijelaskan ke user (bukan cuma "sticker kefoto", tapi siapa pun akses DB bisa baca PIN semua pelanggan). `print.blade.php` BELUM diubah menunggu jawaban ini.
Test disapu ulang: `QrStaffPageSmokeTest` — assertion HTML-scraping (`preg_match` regex nyari `<div class="pin">`) diganti baca payload JSON langsung (`$response->json('pin')`), 2 kasus lama diganti (tanpa `verification_note`, cek riwayat 2 baris terbaru-dulu) + 1 kasus baru (riwayat tetap muncul setelah reload biasa, bukan cuma di JSON) — 44 test modul QR total, 145 assertion, hijau. Full suite disapu ulang lagi setelahnya.
**Cetak Stiker digabung dengan Login ID (keputusan final user, ditegaskan ulang)** — `customers.qr.print` (`print.blade.php`) sekarang satu layout QR+nama+REQ ID+POP+**Login ID Portal**, bukan dua layout terpisah. PIN TETAP TIDAK ikut di halaman ini — bukan lagi didasarkan cuma "§6.5.1 dua faktor harus dua media", tapi keterbatasan TEKNIS: `pin_hash` disimpan bcrypt, begitu direspons sekali (modal reveal) plaintext-nya hilang dari sistem selamanya, gak ada cara reprint-nya tanpa ubah penyimpanan jadi reversible (encrypt) — itu keputusan keamanan terpisah, TIDAK dieksekusi di sini, cuma didokumentasikan sebagai batasan. `docs/plan/qr-code/rancangan-qr-pelanggan-final.md` §7.1 & §7.6 diberi catatan REVISI 2026-08-26 (rancangan "dua layout" lama diberi strikethrough + dijelaskan kenapa diganti, bukan dihapus diam-diam — jejak keputusan tetap kebaca). Test `QrStaffPageSmokeTest` +1 assertion (Login ID muncul di respons cetak) — 44 test modul QR, 146 assertion, hijau.
**PIN reversible DITOLAK, alasan konkret dikasih ke user (bukan cuma prinsip)** — user minta "PIN juga diberikan" di halaman reprint; dijelasin dengan skenario sisi-sisi ("pelanggan telepon kartu ilang") buat 2 opsi: (A) tetap hash + reset (sudah jalan) vs (B) reversible/encrypt (PIN sama muncul lagi kapan aja, tapi kebocoran DB = SEMUA PIN pelanggan kebaca sekaligus, bukan cuma yang direset). User pilih (A) eksplisit.
**Ketauan alur Reset PIN eksisting cacat waktu "ngerjain Opsi A dengan benar"** — 2 celah nyata: (1) **field `verification_note` dicabut TANPA pengganti apa pun** — §6.5.5 tetap mensyaratkan admin verifikasi identitas pelanggan (NIK/alamat/pembayaran terakhir) SEBELUM reset, tapi safeguard itu hilang total waktu field-nya dicabut, gak ada yang gantiin. Fix: teks pengingat statis di atas tombol Reset PIN (BUKAN form field lagi, sesuai permintaan user — cuma mengingatkan, bukan memaksa lewat validasi server). (2) **Docblock kode BOHONG soal perilaku sendiri** — komentar bilang "modal pratinjau, PIN ditampilkan dulu baru staf menegaskan", padahal `resetPin()` yang sebenarnya langsung fetch+commit ke server begitu tombol diklik, TANPA konfirmasi apa pun sebelumnya (PIN lama mati SEKETIKA). Ini persis kekhawatiran awal user ("klik gak sengaja jadi masalah") yang MASIH belum ditutup meski sempat diklaim beres. Fix: `confirm()` native browser dipasang di `@click` tombol Reset PIN SEBELUM `resetPin()` dipanggil sama sekali — satu klik "Yakin?" tanpa form, PIN baru cuma di-generate setelah dikonfirmasi. Docblock modal diperbaiki biar sesuai kode sungguhan. Ditemukan lewat verifikasi ulang proaktif atas permintaan eksplisit user ("kerjakan dengan benar dan logis, jangan hanya selesai") — bukan laporan bug baru dari user, tapi audit ulang kode yang sudah "selesai" sebelumnya. Test baru: `QrStaffPageSmokeTest::halaman_show_mengingatkan_verifikasi_identitas_sebelum_reset_pin` — 45 test modul QR total, 148 assertion, hijau.
**BUG NYATA DI DATA PRODUKSI-LIKE — ketemu lewat verifikasi HTTP MANUAL terhadap dev DB sungguhan (MySQL+curl), bukan cuma PHPUnit** — atas permintaan eksplisit user ("jangan hanya berdasarkan test hijau ... cek beneran"). Login real (session asli), issue token+PIN via HTTP nyata, reset PIN via HTTP nyata, verifikasi hasil LANGSUNG ke tabel MySQL (bukan lewat app) — semua kepindahan Reset PIN korek. TAPI nemu 1 hal PHPUnit gak pernah nangkep: **`customer_portal_accounts.login_id` basi buat 99/100 baris di DB dev.** Sebabnya: formula `login_id` direvisi sesi sebelumnya (`registration_prefix`→`cid_prefix`, `keputusan.md` §3 poin 1) SETELAH baris-baris itu dibuat — baris lama masih nyimpen format lama (`RQ-RQ000003`), padahal kartu yang staf cetak SEKARANG selalu tampilin `Customer::portal_login_id` (accessor, live, formula terkini — `C00RQ000003`). Efeknya nyata & dibuktikan lewat curl: `/auth/claim` pakai login_id yang BENERAN tercetak di kartu → 401 GAGAL; pakai login_id lama yang udah gak pernah dicetak lagi → 200 SUKSES. Ini kebalik dari yang seharusnya. PHPUnit gak pernah nemuin ini karena SEMUA test bikin baris baru pakai accessor terkini — skenario "baris lama, formula udah beda" gak pernah ke-exercise walau ratusan test hijau.
**Fix:** `customers:backfill-portal-login-id` (command lama) dapat flag `--resync` baru — update `login_id` cuma buat baris `status=pending_claim` (belum ada kredensial nyata yang bergantung ke nilai lama, aman ditimpa) ke formula terkini; baris `status=active` (SUDAH pernah diklaim, mungkin pelanggan udah tau & pernah login pakai nilai lama) SENGAJA TIDAK disentuh — resync itu bakal jadi lockout, bukan perbaikan. Dijalankan sekali terhadap DB dev sungguhan: 97 akun `pending_claim` di-resync, 0 tersisa basi (dicek ulang). 2 akun `active` yang juga basi (termasuk 1 yang beneran punya `last_login_at` — pelanggan asli pernah pakai) sengaja DIBIARKAN. Diverifikasi ulang end-to-end via curl setelah fix: login_id yang tercetak SEKARANG untuk pelanggan lain yang kena bug ini → `/auth/claim` sukses (200 + token asli). Semua artefak verifikasi manual (user admin sementara, token/PIN/akun portal percobaan) dibersihkan dari DB dev setelahnya.
Test baru: `CustomersBackfillPortalLoginIdCommandTest` +3 kasus (resync memperbarui baris basi, TIDAK menyentuh baris `active`, `--dry-run` gak nulis apa pun) + `PortalClaimTest` +1 kasus (reproduksi bug persis — login_id basi berhasil klaim, login_id yang BENERAN tercetak malah gagal) — 128 test gabungan (QrCode + CustomerPortal), 448 assertion, hijau. | Done (Fase 2 + aktivasi claim() + UX reset PIN dirombak 3x + Cetak Stiker digabung Login ID + bug login_id basi di data real ditemukan & ditambal) — 2026-08-26, **Fase 3 (absen) belum dikerjakan** |

**PIN dibalik jadi reversible (keputusan (B) sebelumnya ditolak, sekarang perintah eksplisit user membalik lagi)** — `pin_hash` pindah dari bcrypt (buta permanen) ke `Crypt::encryptString()` (AES-256, `APP_KEY`), method baru `CustomerQrTokenService::revealPin()`. `/qr/cetak` (`print.blade.php`) sekarang nampilin PIN juga, bukan cuma QR+nama+POP+Login ID — bisa dibuka ulang kapan pun, gak lagi sekali-tampil doang di modal. Trade-off didokumentasikan eksplisit di docblock `issuePin()`/`print.blade.php`: siapa pun akses DB+`APP_KEY` bisa baca PIN semua pelanggan. Baris `pin_hash` lama (dibuat sebelum perubahan ini, masih format bcrypt) `revealPin()` balikin null — halaman cetak nunjukin "PIN belum tersedia, reset dari halaman QR Pelanggan" buat baris itu, bukan error. Test: `CustomerQrPinServiceTest` (2 kasus diganti — assert `Crypt::decryptString()` bukan `Hash::check()`) + `QrStaffPageSmokeTest` +2 kasus baru (PIN sama persis di dua kali buka halaman cetak, baris bcrypt lama gak bikin 500) — semua hijau. Label "Fase 2 — SELESAI SEBAGIAN" di `docs/api/api-portal-pelanggan/rencana-implementasi.md` diperbaiki ke "SELESAI" — isinya sendiri sudah full functional sejak `claim()` aktif, cuma judulnya ketinggalan diupdate.

| ADHOC-49 | **Gerbang tagihan QR dicabut, scan QR pelanggan SEKARANG ke Portal** (keputusan eksplisit user, membalik arsitektur §6.1 dokumen QR). `QrScanController` cabang tamu dulu redirect ke `QrBillingController` (gerbang tagihan internal, dua-step disclosure PIN/4-digit-HP) — SEKARANG `redirect()->away("{PORTAL_BASE_URL}/klaim?code={code}")`, `QrBillingController` + 3 view `resources/views/qr/billing/*` DIHAPUS total (bukan dianggurin). Endpoint API baru `GET /api/customer-portal/qr/resolve?code=` (`PortalAuthService::resolveQr()` + `PortalAuthController::resolveQr()`) — resolve token QR yang SAMA (`CustomerQrTokenService::resolve()`) jadi `{login_id, account_status}`, TANPA PIN sama sekali (PIN tetap cuma diverifikasi di `/auth/claim`), dipanggil Portal buat pre-fill halaman klaim. Guard eksplisit di `QrScanController`: `PORTAL_BASE_URL` kosong → 500 gamblang ("belum dikonfigurasi"), BUKAN 404 diam-diam yang keliatan seperti QR-nya sendiri rusak. `config/qr.php` — `bank_account` (dead config, cuma dipakai gerbang lama) dihapus, `portal_base_url` baru; limiter `qr-billing-verify` (`AppServiceProvider`) dicabut, gak ada controller yang makai lagi. Test: `PortalQrResolveTest` (8 kasus baru — pending_claim/active/akun-belum-ada semua balikin login_id, PIN gak pernah di payload, 4 kegagalan resolve semua 404 identik, tanpa client secret 401) + `QrScanDispatchTest` (test tamu diganti dari assert redirect `qr.billing` jadi assert redirect Portal URL beneran, +1 kasus baru guard 500 kalau `PORTAL_BASE_URL` kosong) — `QrBillingPageTest` (6 kasus lama) DIHAPUS, fiturnya udah gak ada buat ditest. Diverifikasi REAL lewat HTTP terhadap dev server: `curl /q1/{code}` → 302 ke `http://localhost:3001/klaim?code=...` (bukan simulasi test doang), `GET /qr/resolve` real → `{"login_id":"C00RQ000004","account_status":"pending_claim"}` cocok data asli pelanggan #2, guard 500 dicoba beneran (kosongin `PORTAL_BASE_URL` di `.env` dev, `config:clear`, curl ulang → 500, lalu dipulihkan). Docs disinkronkan: `business-logic.md` (§QR baru), `frontend-nextjs-rancangan.md` (halaman `/klaim` baru dengan 3 cabang alur berdasar `account_status`, `/aktivasi` diperjelas jadi jalur sekunder buat pelanggan yang gak lewat scan), `rancangan-qr-pelanggan-final.md` §6.1 diberi catatan REVISI (dicabut, isi lama dipertahankan sebagai jejak keputusan bukan kontrak aktif). **Sisi Next.js (Portal, `d:\whusnet\portal-pelanggan`, di luar repo ini) DISELESAIKAN sesi yang sama**: `ClaimForm.tsx` diekstrak jadi komponen reusable (dulu inline di `/aktivasi`), halaman baru `/klaim` (Server Component, resolve `code` server-side lewat `callLaravel()` LANGSUNG — bukan lewat Route Handler sendiri, pola optimasi sama `(portal)/layout.tsx`) dengan 3 cabang (`pending_claim`→form ter-prefill, `active`→pesan+link `/login`, gagal→error generik), `proxy.ts` diupdate biar `/klaim` gak ke-redirect ke `/login`. Diverifikasi end-to-end REAL lewat rantai penuh: `curl /q1/{code}` (Laravel) → redirect → halaman `/klaim` (Portal, container Docker `portal-pelanggan-web-1` yang join network `whusnet-operasional_app_network` via `whusnet-nginx`, BUKAN lewat tunnel publik) → form ter-render dengan `login_id` asli. Ketemu 2 masalah infra nyata pas verifikasi (bukan dari kode QR): (a) container `app`+`nginx` OOM-crash (SIGKILL, exit 127) di tengah kerja, di-restart; (b) `.env.local` Portal sempat nunjuk tunnel Cloudflare yang udah mati, diperbaiki ke jalur internal Docker yang bener. | Done penuh (Laravel + Portal) — 2026-08-27 |
| ADHOC-50 | **Scan QR Internal (staf) — halaman baru, gantikan asumsi app scanner/kamera pihak ketiga** (keputusan eksplisit user, lanjutan diskusi ADHOC-49). Akar masalah: cabang STAF `QrScanController::dispatch()` (shortcut bikin tiket, dulu juga customer detail) ditentukan `auth()->check()` — COOKIE SESI. App scanner/kamera pihak ketiga BUKA LINK DI WEBVIEW/BROWSER TERPISAH yang gak bawa cookie sesi Operasional, jadi staf yang scan lewat app luar SALAH KEBACA sebagai tamu. Fix: halaman baru `/scan-qr` (permission `qr_scan.view`, gerbang di dalam app — staf WAJIB login duluan buat sampai ke situ), kamera dibaca LANGSUNG di halaman (JS `resources/js/qr-scan.js`, entry Vite TERPISAH dari `app.js` biar library decode gak kekirim ke semua halaman), begitu QR pelanggan valid kebaca (divalidasi pola `/q1/{token}.{sig}` + origin sama — QR toko/produk lain ditolak) browser navigasi LANGSUNG ke URL itu DI TAB YANG SAMA (`window.location.href`) — cookie sesi ikut otomatis, NOL logic dispatch/resolve diduplikasi di sini. **Optimasi kecepatan** (diminta eksplisit): `BarcodeDetector` API native browser dicoba dulu (hardware-accelerated, Chrome/Edge/Android WebView), `jsqr` (npm, entry Vite terpisah) cuma FALLBACK buat browser yang gak dukung (Safari lama); frame di-downscale ke maks 480px sebelum decode jsQR (O(pixel), gambar kecil = jauh lebih cepat); loop dibatasi ~10 scan/detik (`requestAnimationFrame` + time-check) bukan tiap frame kamera 60fps — cukup kerasa instan tapi gak bakar CPU/baterai; kamera+loop STOP LANGSUNG begitu satu QR valid kebaca, juga di-`pagehide` (jangan nyala di background). RBAC: feature root baru `qr_scan` (action `view` doang, bukan CRUD), permission `qr_scan.view` di-grant ke admin/helpdesk/fop/teknisi (role yang beneran interaksi sama QR pelanggan). Link masuk: dropdown "Profil Saya" (`layouts/app.blade.php`), gerbang `@can('qr_scan.view')`. Test baru: `QrInAppScanPageTest` (7 kasus — 3 role ber-permission bisa buka, role tanpa permission 403, tamu redirect login, link MUNCUL di dropdown buat yang punya permission DAN TIDAK MUNCUL buat yang gak punya — pola regresi sama ADHOC-46 "tombol ada tapi gak ada jalan masuk"). Diverifikasi REAL lewat HTTP: login helpdesk asli → `/scan-qr` 200 + video element + bundle JS ter-build (`qr-scan-yRms8Te5.js`, terpisah dari `app-*.js`) kekirim beneran; dropdown dashboard ada link `href="https://localhost/scan-qr"`; login sales asli → `/scan-qr` 403. `npm run build` (`docker compose run --rm assets`) sukses, bundle `qr-scan.js` 132KB gzip 48KB — TERPISAH dari `app.js` (202KB), gak numpang ke semua halaman. **Belum dikerjakan** (di luar scope sesi ini, disinggung tapi ditahan): mindahin fitur "kirim tiket dari QR" (`tickets.create` shortcut) & "absen teknisi via QR" (Fase 3, belum ada sama sekali) supaya masuk lewat halaman ini juga, bukan lagi lewat `QrScanController::dispatch()` cabang staf yang lama — itu perubahan alur terpisah, perlu dibahas lagi kalau mau dieksekusi. | Done — 2026-08-27, **migrasi ticketing/absen ke jalur ini belum dikerjakan** |
| ADHOC-51 | **Redesign Halaman Profil Pengguna (`/profile`)** — Tampilan profesional modern dengan dukungan penuh responsive mobile view, hero identity banner, integrasi detail RBAC & cakupan wilayah POP, riwayat aktivitas akun terkini, toggle show/hide password interaktif, live password strength meter & match indicator, copy clipboard, serta dark/light theme support. | Done — 2026-09-01 |
| ADHOC-52 | **Unifikasi QR staf/kolektor → Portal Pelanggan (Fase 1 — Operasional)** (lanjutan ADHOC-50/51, rancangan `docs/plan/qr-code/analisa-unifikasi-qr-staff-portal.md`). Cabang staf-ticketing & kolektor `QrScanController::dispatch()` yang tadinya redirect internal (`qr.ticket.create`/`collector-worklist.index`) SEKARANG redirect ke Portal (app terpisah), bawa `StaffPortalToken` one-shot (TTL 15 menit, purpose-scoped `tickets`/`kolektor`, dikonsumsi cuma setelah aksi PENULISAN sukses). Permission BARU `tickets.qr.create`/`kolektor.qr.pay` — TERPISAH dari `tickets.create`/`kolektor.pay` dashboard (channel beda risiko). Endpoint API baru `POST /api/customer-portal/tickets` (middleware `portal_staff_token:tickets`, delegasi penuh ke `TicketService::create()`) dengan dedup guard baru (`enforceDuplicateGuard` param, opt-in — dashboard TIDAK berubah perilakunya) yang menolak 409 `DuplicateTicketException` kalau pelanggan masih punya tiket terbuka & belum `confirmed_duplicate`. Route `qr.ticket.create` internal DIPERTAHANKAN (keputusan eksplisit user) buat non-QR, cuma sudah bukan tujuan dispatch QR lagi. Test baru: `PortalStaffTicketStoreTest` (8 kasus), `QrScanDispatchTest`/`QrScanCollectorPaymentTest` diupdate ke perilaku baru. Fase 2a (2026-08-29, sesi lanjutan): endpoint `GET /api/customer-portal/kolektor/worklist/{code}` + `POST .../payments` (`PortalStaffKolektorController`) — `payments()` REUSE PENUH trait `RecordsCollectorBatch` (sama persis dipakai `CollectorPaymentController` dashboard), `worklist()` reuse `CollectorWorklistService::dueInvoices()` tersaring ke 1 pelanggan. Token dikonsumsi cuma kalau batch beneran diproses sukses (bukan idempotency replay, bukan 422). Test baru: `PortalStaffKolektorTest` (8 kasus). Fase 2b (2026-08-29, sesi lanjutan, repo `portal-pelanggan`): halaman `/staff/tickets` (`StaffTicketForm` — form + tampilan 409 duplikat + tombol "Tetap Buat Baru") dan `/staff/kolektor` (worklist di-resolve server-side + `StaffKolektorPaymentForm` — checklist invoice + nominal manual per baris). Route Handler proxy `api/staff/tickets`, `api/staff/kolektor/payments` — pola BFF sama endpoint pelanggan (`X-Portal-Client` di server, TIDAK PERNAH ke browser), bedanya `staff_token` dikirim per-request lewat body (BUKAN `iron-session` cookie — token itu one-shot punya staf, bukan sesi pelanggan). `ApiResult` (`laravel-client.ts`) ditambah field `raw` biar body error non-standar (409 `existing_ticket_number`) bisa diteruskan tanpa `as` yang nebak-nebak. Route group `(staff)` baru, layout kartu tunggal (pola sama `(auth)`). Tipe TS baru di `portal-api.ts`. Diverifikasi `tsc --noEmit` + `eslint` bersih (full `next build` gagal di sesi ini karena EPERM UNC path — batasan environment lokal, bukan kode; belum diverifikasi lewat browser asli). `docs/api/api-portal-pelanggan/business-logic.md` dapat §5 "Staf/Kolektor (QR)" (kontrak 3 endpoint), larangan #9 `frontend-nextjs-rancangan.md` dikoreksi (sudah gak berlaku). Bug ditemukan lewat uji coba user pasca-Fase 2, ditambal sesi yang sama: (1) `proxy.ts` Portal nolak `/staff/*` ke `/login` (whitelist gak diupdate pas Fase 2b) — fix tambah pengecualian `pathname.startsWith('/staff/')`. (2) Akun full-access yang eligible `tickets.qr.create` DAN `kolektor.qr.pay` sekaligus buat pelanggan yang sama selalu kelempar ke kolektor diam-diam (urutan `if`) — fix: `QrScanController::dispatch()` hitung eligibility dua-duanya, kalau ambigu redirect ke halaman baru `qr.scan.choose` (pilih eksplisit, internal Operasional), `chooseConfirm()` ulang semua cek dari nol. (3) Setelah #2 ditambal, owner tetap gak pernah ke chooser — cabang kolektor ternyata masih mensyaratkan `hasRole('kolektor')`. Sempat diubah jadi permission-only, tapi **DIBALIKIN** setelah ketemu `hasRole('kolektor')` juga dipakai 8+ titik lain di modul Kolektor 2.0 (termasuk endpoint TULIS `PortalStaffKolektorController::payments()`) sebagai gerbang identitas buat pembukuan kolektor (saldo/setoran/worklist) — BUKAN pengganti RBAC, lapis beda dari permission. Permission-only cuma di `QrScanController` doang bikin owner ke-route ke halaman kolektor lalu 403 pas submit. Final: `resolveEligibility()` balik ke `hasRole('kolektor') && hasPermission(...)`, konsisten sama sisa modul — owner memang gak pernah eligible kolektor lewat QR, itu benar & disengaja. File baru `resources/views/qr/scan-choose.blade.php`, route `GET/POST /scan-qr/choose/{code}`, test `QrScanAmbiguousPermissionTest` (7 kasus). Feature/permission baru (`tickets.qr`, `kolektor.qr`) sempat cuma ke-generate di test DB, lupa di-seed ke DB dev nyata — udah dijalanin manual (`db:seed --class=QrFeatureSeeder` + `RolePermissionSeeder`). (4) Dokumentasi Scramble endpoint staf salah: summary kosong/berantakan (docblock method gak ada judul pendek) + security scheme salah subjek (nunjuk token pelanggan, padahal staf token beda total) — fix docblock + scheme `StaffPortalToken` baru di `ScrambleServiceProvider`. (5) Kolektor dikasih `tickets.qr.create` (keputusan eksplisit user — kolektor ketemu pelanggan langsung di lapangan, sering dapet komplain pas nagih): scan pelanggan di worklist → chooser "Tagih Pembayaran"/"Lapor Komplain"; scan pelanggan DI LUAR worklist → tetap bisa lapor tiket (gak 403 lagi, ticketing emang gak dibatasi kepemilikan pelanggan). Detail: `docs/plan/qr-code/analisa-unifikasi-qr-staff-portal.md` §8-10. | Done (Fase 1 + 2a + 2b + 5 bug/perbaikan tambal) — 2026-08-29, **verifikasi `next build`/browser asli belum jalan (batasan environment sesi ini)** |
| ADHOC-51 | **Kolektor scan QR pelanggan → langsung ke pencatatan pembayaran, tersaring ke worklist-nya sendiri** (keputusan eksplisit user, lanjutan ADHOC-50). Cabang baru `QrScanController::dispatch()` — kalau pemindai `hasRole('kolektor') && hasPermission('kolektor.pay')`: cek pelanggan itu ADA di worklist-nya (query SAMA persis `CollectorWorklistService::dueInvoices($user,$user)` yang dipakai `CollectorWorklistController::index()`, tinggal tambah filter `customer.id`) → redirect `collector-worklist.index?search={customer_code}`, REUSE fitur pencarian yang SUDAH ADA (bukan modal/auto-centang/auto-isi nominal baru — kolektor tetap input manual jumlah yang diterima, scan cuma bantu navigasi cepat ke baris yang bener). Pelanggan DI LUAR worklist-nya (beda kolektor, atau `collector_id` kosong) → **403 tegas** ("Pelanggan ini bukan tanggung jawab Anda"), TIDAK jatuh ke cabang lain manapun (ticketing/detail) — pilihan eksplisit user lewat `AskUserQuestion`, konsisten sama invarian yang sudah didokumentasikan di `CollectorWorklistController`: "tak bisa menyentuh pelanggan di luar collector_id-nya sendiri". RBAC: `qr_scan.view` ditambah ke role `kolektor` (kelewat pas ADHOC-50, cuma admin/helpdesk/fop/teknisi yang dikasih). Test baru: `QrScanCollectorPaymentTest` (4 kasus — scan pelanggan sendiri redirect ke worklist tersaring, scan pelanggan kolektor LAIN 403, scan pelanggan tanpa `collector_id` 403, staf NON-kolektor yang scan pelanggan yang KEBETULAN punya `collector_id` tetap jalan alur biasa/ticketing — cabang kolektor cuma aktif kalau PEMINDAI-nya kolektor). Diverifikasi REAL lewat HTTP: kolektor asli scan QR pelanggan miliknya sendiri → 302 ke `/collector-worklist?search=RQ997001` (bukan simulasi test doang); kolektor sama scan pelanggan `collector_id=null` → 403 beneran. 75 test modul QR total hijau (1 gagal pre-existing gak terkait, sudah ditandai sebelumnya). | Done — 2026-08-27 |
| ADHOC-53 | **Worksheet Helpdesk — Search free-text/CID otomatis, POP manual, No. HP Pelapor, Tiket Batch Parent/Child** (di luar sprint aktif, permintaan eksplisit user). (1) Search Customer Data tetap lookup CID/nama/HP seperti biasa, tapi kalau Kategori Issue yang dipilih ber-checklist `is_batch` DAN gak match pelanggan manapun, teks yang diketik dipakai sebagai label tiket (numpang kolom snapshot `customer_name`, bukan kolom baru) — bukan error. (2) Dropdown POP manual (`Pop::forUser()`) muncul begitu mode label-bebas aktif, gantiin POP yang biasanya ikut dari pelanggan terpilih. (3) Input baru "No. HP Pelapor" (`tickets.reporter_phone`, nullable, opsional) — diisi tampil di List, kosong fallback HP pelanggan (`Ticket::contactPhone()`). (4) Kategori Master Issue dapat checklist `is_batch` (mis. "ODP LOS") — tiket ber-kategori ini support Parent/Child: `tickets.customer_id` jadi nullable, child (pelanggan terdampak) dicatat di tabel BARU `ticket_batch_members` (`TicketBatchMember`) — **BUKAN** Ticket/FopTask sendiri (satu ODP rusak = satu perbaikan lapangan lewat FopTask di tiket PARENT, bukan N task identik per pelanggan). Tombol "Tambah" di List Task (cuma tiket batch) expand/collapse baris child + modal "Tambah Pelanggan Terdampak" (`TicketService::addBatchMember()`, endpoint `tickets.batch-members.store`) — modal SENGAJA menyimpang dari pola default repo (halaman create buat mutasi data majemuk), override eksplisit user, ditutup pakai submit AJAX (validasi gagal balik JSON, gak numpang `back()->withErrors()` yang bisa nutup modal + List kosong tanpa pesan, ADHOC-20) — URL endpoint tetap dirender server-side per tiket (`batch_members_store_url`), klien gak merakit path dari `ticket.id`. Migration: `add_is_batch_to_ticket_issue_categories_table`, `add_batch_and_reporter_fields_to_tickets_table` (`customer_id` nullable + `reporter_phone`), `create_ticket_batch_members_table`. Test baru: `tests/Feature/TicketBatchTest.php` (9 kasus) + `PostTargetRenderedServerSideTest` +1 kasus guard URL. Detail: `docs/ticketing/business-logic.md` §18. Regresi disapu: `TicketingTest`, `TicketingRbacTest`, `TicketCidDisplayTest`, `PostTargetRenderedServerSideTest` (82 test) — hijau; full suite 1863 passed (12 gagal pre-existing tidak terkait — `ReportPaymentTest`/`PaymentReceiptPrintTest`/`CustomerToggleSuspendTest`/dll, sudah ada sebelum task ini disentuh).
**Perbaikan pasca-review (sesi lanjutan, laporan user):** (1) Helper "Pelanggan tidak ditemukan..." dicabut dari Search Customer Data. (2) **Bug akar ketemu: `is_batch` gak pernah bisa diaktifkan** — CRUD Master Issue (`Master\TicketIssueCategoryController` + view create/edit) gak pernah dikasih field `is_batch` sama sekali waktu kolomnya ditambahkan, jadi dropdown POP manual & mode label-bebas gak pernah nongol di real usage (bukan bug Alpine — emang gak ada kategori `is_batch=true` di DB manapun). Fix: checkbox di create/edit + `validateCategory()` normalisasi `$request->boolean('is_batch')` (checkbox unchecked → gak ngirim field, wajib dinormalisasi eksplisit biar uncheck beneran reset ke false) + badge "Batch" di index listing. Test baru: `TicketIssueCategoryCRUDTest::test_is_batch_checkbox_can_be_toggled_on_and_off` — 59 test disapu ulang (`TicketIssueCategoryCRUDTest`+`TicketBatchTest`+`TicketingTest`), hijau. | Done — 2026-09-09 |
| ADHOC-48 | **Tab Riwayat Ticketing Pelanggan: regime tampilan ngikut `handler` tiket** — permintaan eksplisit user, tiket helpdesk/NOC yang sudah turun ke Ticketing FOP wajib tercatat sebagai "Ticket FOP" (bukan lagi "Ticket Helpdesk/NOC"), tiket yang selesai/dibatalkan di Helpdesk/NOC (tidak pernah ke FOP) tetap "Ticket Helpdesk/NOC". Sebelumnya tab ini SAMA SEKALI tidak menampilkan model `Ticket` — cuma `Task` (teknisi) dan `FopTask` (field task legacy), jadi tiket yang selesai murni di Helpdesk/NOC tanpa pernah eskalasi TIDAK PERNAH muncul di riwayat pelanggan sama sekali. Fix: `CustomerController::show()` +query `$customerTickets` (`$customer->tickets()->with(['fopTask.technicians', ...])`), satu baris per tiket — `handler=FOP` dirender pakai data `fopTask` (nomor TFOP, badge "TICKET FOP", link ke `fop-tasks.history.show`), `handler=HELPDESK/NOC` dirender pakai data tiket sendiri (nomor TKT, badge "TICKET {HANDLER}", link ke `tickets.show`) — reuse `Ticket::statusLabel()`/`statusBadgeClasses()` yang sudah ada, bukan logic baru. `$customerFopTasks` (seksi "FOP Field Task" legacy) ditambah `whereDoesntHave('ticket')` supaya FopTask hasil eskalasi tiket TIDAK dobel tampil di dua seksi sekaligus. Test baru: `CustomerTicketingHistoryRegimeTest` (3 kasus — tiket selesai di Helpdesk tampil sebagai "TICKET HELPDESK", tiket dieskalasi ke FOP tampil sebagai "TICKET FOP" pakai nomor TFOP dan TIDAK dobel di seksi legacy, FopTask non-tiket tetap tampil seperti sebelumnya) — hijau. Regresi disapu: `TicketCancellationTest`, `FopTaskHistoryFollowsTicketDetailTest`, + seluruh test yang menyentuh route `customers.show` (77 test) — semua hijau. | Done — 2026-08-27 |
...
| ADHOC-54 | **Modul Gudang/Inventory & Asset Traceability** — **di luar sprint aktif, dikerjakan atas permintaan eksplisit user**, PARALEL dengan Sprint 8.10 (gak mengganti `S8.10-T003`). Gudang Pusat (`pops.type=pusat`) → Anak Gudang per Cabang (`pops.type=cabang`, BUKAN mini_pop) → custody Teknisi → Pelanggan, ledger append-only sebagai sumber kebenaran (bukan angka stok). Klasifikasi barang: `tracking_type` (SERIALIZED/QUANTITY/BATCH) + `equipment_class` dua-level + `ownership_mode` (installable/company_asset). Fase 0-8 + ronde penutup selesai — detail di bawah. | **Fase 1 selesai** — 2026-09-02 |
| ADHOC-55 | **Fase 2 Gudang — Adaptasi 7 Komponen WMS Klasik ke Konteks ISP** (lanjutan ADHOC-54, atas pertanyaan eksplisit user: apakah modul Gudang sudah menyediakan komponen WMS klasik — Inbound/PO+putaway, Inventory Control/opname+damaged+adjustment, Outbound/picking+shipping manifest, Cross-docking, Master Data/barang+supplier+lokasi, Storage/FIFO, Laporan&Dashboard). Tiap poin ditinjau ADOPT/ADAPT/SKIP dgn alasan bisnis eksplisit — bukan ditolak/diadopsi mentah. Prioritas 1 (TINGGI, **selesai** — lihat detail di bawah): bukti fisik (`evidence_file_path`) wajib di `InventoryAdjustmentService` buat klaim LOST/DAMAGED/SCRAPPED + kategori `reason` terarah di custody, + `recordStockOpname()`/Stock Opname terpisah dari Penyesuaian Stok biasa, + kolom "opname terakhir" di Management Stock. Prioritas 2 (SEDANG, **selesai** — lihat detail di bawah): `WarehouseReportController` — laporan agregat movement (RECEIVE/TRANSFER/ISSUE per gudang) & kerugian (per kategori) per periode, permission baru `warehouse_report.view` (realtime tetap SKIP, keputusan sadar Fase 1). Prioritas 3-5 (RENDAH, belum dikerjakan): `reference_no` dokumen inbound, checklist print serah-terima, badge umur lot. SKIP total (dikonfirmasi user): cross-docking, supplier master, lokasi/bin/rak — masing-masing dgn opsi "kalau skala naik" dicatat, bukan dibangun sekarang. Rancangan penuh: [`docs/plan/warehouse/fase-2-adaptasi-wms.md`](plan/warehouse/fase-2-adaptasi-wms.md). | **Prioritas 1 & 2 selesai** — 2026-09-03, P3-P5 belum dikerjakan |
| ADHOC-56 | **Redesign UI/UX Ekosistem Gudang & Inventory** — Header terpadu (`<x-warehouse.header />`), Dashboard elevated KPI + Restock urgency, Management Stok responsif, Custody tabbed segmented switcher (Serial vs Material), Asset Traceability hero & connected timeline, repeat rows modern `<x-inventory-line-rows />`, serta perbaikan strict lazy loading guard (`Item::getEffectiveEquipmentClassAttribute()`, `fromTechnician`/`toTechnician` eager loading). | Done — 2026-09-03 |
| ADHOC-57 | **Ambang Stok Rendah + Permintaan Stok Cabang→Pusat** — jawaban 2 pertanyaan eksplisit user: (1) "kalau Jetis (POP cabang) habis stok dan Pusat gak sadar, gimana?" — ketauan `minimum_stock` gak punya form pengisian SAMA SEKALI (kolomnya ada, cuma dibaca `isLowStock()`, badge Stok Rendah praktis mati); (2) diminta bikin jalur cabang PROAKTIF kasih sinyal ke Pusat, bukan Pusat nunggu notice sendiri. Dua perbaikan: `WarehouseStockController::createThreshold()`/`storeThreshold()` (form isi minimum/maximum per gudang+barang+lot), dan modul baru **Permintaan Stok** (`stock_requests`/`stock_request_items`, `StockRequestService`, `WarehouseStockRequestController`, permission `warehouse_stock_request.*`) — cabang ajukan, Pusat lihat SEMUA permintaan pending lintas cabang di satu antrean tanpa perlu diberitahu manual, fulfill/tolak/batalkan. SENGAJA bukan sumber pergerakan stok kedua — Fulfilled cuma nandain tiket selesai, Transfer sungguhan (ledger) tetap dibikin terpisah lewat alur existing. Detail di bawah. | Done — 2026-09-03 |
| ADHOC-58 | **Widget "Pemakaian Alat Gudang" di FOP Dashboard** — di luar sprint aktif, atas permintaan eksplisit user ("alat itu harus berasal dari Gudang dan dipakai di lapangan dari Laporan Teknisi"). Investigasi awal: linkage Gudang↔Task material SUDAH ada sejak ADHOC-54 (`InventoryService::consumeFromCustody()`/`installSerial()`, dipanggil dari `CustomerInstallationController`/`TaskMaintenanceController`) — gap-nya cuma papan FOP gak nampilin angkanya sama sekali. `FopDashboardController::index()` nambah 3 angka (cache 30 detik, pola sama `$stats`): **Keluar** (`SUM(inventory_transactions.qty)` type ISSUE hari ini, scope `from_pop_id`), **Terpakai** (`SUM(task_materials.qty)` kind=terpakai hari ini, scope lewat `fop_tasks.pop_id`), **Sisa di Tangan Teknisi** (`SUM(technician_custody.qty_remaining)` aktif — snapshot posisi sekarang, bukan kejadian hari ini). Tidak ada tabel/kolom baru. Test: `FopDashboardGudangStatsTest` (2 test — angka benar utk akses ALL_POP, POP lain ter-exclude utk scope SELECTED_POP). Susulan (permintaan user hari yang sama): chart **"Tren Pemakaian Alat (Top 5)"** di `/fop/analytics` (`FopAnalyticsController`) sumbernya diganti — sebelumnya `task_work_tools` (alat kerja checklist bawa-pulang, COUNT baris), sekarang `task_materials` kind=terpakai (barang gudang, SUM qty — lebih berarti karena punya satuan meter/pcs). `buildToolTrendChartRows()` digeneralisasi jadi `buildTrendChartRows($trend, $ranking, $labelKey)` dipakai dua chart (alat kerja & barang gudang) via param `$labelKey` beda (`tool_name` vs `item_name`). Susulan kedua (2026-09-11, dikonfirmasi user): chart 1 **"Ranking Alat Kerja Terpakai"** JUGA diganti jadi **"Ranking Barang Gudang Terpakai"** (`buildMaterialUsageRanking()`, GROUP BY `item_name` SUM qty — pola sama trend). View: id SVG `toolChart`→`materialRankingChart`, var `$toolRanking`→`$materialRanking` (dipakai render); `$toolRanking`/`buildToolUsageRanking()` TETAP ADA di controller — masih dipakai KPI card "Pemakaian Alat" (`$toolTotal`/`toolTypesUsed`/`toolTypesActive`) & `periodComparison`, yang SENGAJA tidak disentuh (di luar permintaan user). Investigasi live DB (2026-09-11, atas kecurigaan user "yang masuk malah alat kerja"): dicek — Master Barang (`items`, 41 baris) bersih, nol alat kerja; alat kerja (10 baris: Tang Krimping, Fusion Splicer, OTDR, dst) hidup terpisah permanen di `work_tools`, gak pernah nyampur. `inventory_transactions`/`technician_custody`/`task_materials` masih kosong di DB produksi (modul Gudang belum dipakai lapangan) — widget/chart ADHOC-58 makanya nol datanya, bukan bug pemisahan. Susulan ketiga (2026-09-12, scope dibatasi user ke `/fop/analytics` doang): section baru **"Modem/ONT Terpasang — Instalasi vs Maintenance"** — `buildModemUsageRanking()` query TERPISAH dari ranking barang gudang biasa, karena modem `tracking_type=SERIALIZED` gak pernah lewat `task_materials` (`consumeFromCustody()` menolak item serialized eksplisit) — dicatat `installSerial()` ke `inventory_transactions` type INSTALL. Group by `items.name` (join `item_categories.code=modem_ont`) × `fop_tasks.category` (PSB=instalasi, MTN=maintenance). **Temuan penting**: `installSerial()` SAAT INI cuma dipanggil dari jalur Instalasi (`CustomerInstallationController::storeSpeedtest()`) — `TaskMaintenanceController` belum py jalur ganti-modem yang menulis SN, jadi kolom "maintenance" akan selalu 0 sampai fitur itu dibangun (di luar scope, TIDAK dibangun di sini; view kasih catatan penjelas kalau maintenance=0). Test: `test_modem_terpasang_dipecah_instalasi_vs_maintenance`. Susulan keempat (2026-09-12, sama hari): gap "maintenance=0" di atas LANGSUNG ditutup atas permintaan user — `TaskMaintenanceController` sekarang punya dropdown SN modem/perangkat aktif, pola SAMA `CustomerInstallationController::eligibleSerialsForTeam()`/`installSerial()` (ADHOC-54), tapi **OPSIONAL** (beda dari Pemasangan yang wajib) — `selected_inventory_serial_id` nullable, gak ada draft pointer terpisah (Maintenance one-shot, `installSerial()` langsung dipanggil di `store()` kalau SN dipilih, di-skip total kalau kosong). View `tasks/maintenance-report.blade.php` nambah Section 2b. Test baru: `TaskMaintenanceModemInstallTest` (3 test — gak pilih SN tetap tersimpan tanpa INSTALL, pilih SN dari custody sendiri menjalankan `installSerial()`, SN di luar custody ditolak validasi). | Done — 2026-09-12 |
| ADHOC-60 | **Urutan Papan `/fop-tasks` dibalik: Tanggal Kerja dulu, Prioritas jadi tie-breaker** — di luar sprint aktif, laporan user ("Task FOP acak-acakan, rulesnya jelas berdasarkan tanggal dahulu baru prioritas") lalu dikonfirmasi eksplisit membalik rancangan lama (Task 8, `flowchart.md §8`/`user-flow.md` — prioritas dulu baru `created_at`, `task_date` gak pernah dipakai sorting sama sekali). `FopTaskController::index()`: bucket `client_request_date` "belum waktunya" tetap paling depan (gak berubah), lalu `task_date` ASC (NULL disink ke bawah, defensif — kolomnya nullable di skema tapi praktiknya selalu keisi), prioritas cuma tie-breaker kalau `task_date` PERSIS SAMA, `created_at` ASC tie-breaker terakhir (arah SERAGAM semua kategori — asimetri lama Survey/PSB ASC vs tipe lain DESC dicabut). Test lama (`FopTaskSortingTest`) yang cuma assert priority-first/category-asimetri diganti jadi assert tanggal-menang-atas-prioritas + tie-breaker eksplisit (`test_earlier_task_date_beats_higher_priority`, `test_priority_breaks_ties_within_same_task_date`, `test_created_at_breaks_ties_within_same_task_date_and_priority`) — versi lama kebetulan tetap hijau di bawah urutan baru (data test-nya gak membedakan dua rezim), jadi diganti biar benar-benar mengunci rezim baru, bukan lolos kebetulan. Docs disinkronkan: `flowchart.md §8` (rewrite), `user-flow.md`, `analisa-auto-team.md` (catatan lama Task 8 dipertahankan sebagai jejak keputusan + pointer ke rancangan baru). Regresi disapu: `FopTaskSortingTest`, `FopTasksTest`, `FopTaskClientRequestDateTest`, `FopTaskStatusSyncTest`, `FopTaskCancelCascadeAuthTest`, `FopDashboardProcessToTimTest`, `TaskFopActionsTest`, `TicketBatchTest` — 75 test hijau. | Done — 2026-09-10 |
| ADHOC-62 | **Ganti Paket Internet Pelanggan Aktif (Paket A → Paket B)** — di luar sprint aktif, dikerjakan atas permintaan eksplisit user (sebelumnya belum ada sama sekali, cuma disebut di dokumen rencana). Permission baru `customers.detail.packages.change` (`ActionCode::CHANGE`, sengaja terpisah dari `.update` yang dipakai form edit pelanggan umum — default cuma ke-grant lewat wildcard `owner`/`admin`, role lain ditambah manual via Role Matrix). `CustomerPackageService::change()` cuma menimpa kolom terkait paket (`internet_package_id`, snapshot nama/kecepatan, `monthly_price`, `total_monthly_bill`) — diskon/PPN/biaya lain TIDAK ikut direset, dan `customers.internet_package_id` (FK legacy) disinkron di titik yang sama. Efek "baru berlaku periode berikutnya" TANPA kolom pending tambahan: `GenerateMonthlyInvoicesCommand` cuma baca `monthly_price` ULANG saat generate tagihan BULANAN periode baru — tagihan periode berjalan yang sudah terbit (snapshot di baris invoice-nya sendiri) tidak pernah disentuh ulang. `CustomerPackageController::update()` (route `PUT /customers/{customer}/package`, `customers.package.update`) — pola aksi #3 CLAUDE.md: inline toggle Alpine di tab "Paket & Layanan" halaman Detail Pelanggan (bukan modal/halaman baru, karena sudah di halaman 1 record). Test: `CustomerPackageChangeTest` (4 test — ganti berhasil + audit field yang tak berubah, tolak paket sama, 403 tanpa permission `.change`, tagihan bulan berjalan yang sudah terbit tetap utuh). **Susulan (permintaan user hari yang sama, 2 iterasi):** aksi yang sama ditambah ke **Customer Quick Hub** (`_quick_hub_modal.blade.php`, dipakai di `customers.index`). Iterasi 1 (link kecil "Ganti" ketimpa di antara baris teks Paket Internet) dan iterasi 2 (tombol di header kartu "Ringkasan Tagihan") dikomplain user — kelewat, susah ketemu. Ditaruh final di **Footer Quick Hub**, posisi sama dengan "Edit Master Data": desktop (`>=lg`) tombol "Upgrade Paket" sejajar Detail Full/Edit Master Data; mobile/tablet ikon "Paket" ditambah ke grid footer (6→7 kolom) sejajar ikon Edit. Klik → tutup Quick Hub, buka **modal terpisah** (`_package_change_modal.blade.php`, pola sama `_network_assignment_modal.blade.php` — modal ke-2 disisipkan sejajar Modal 1 lewat `@include`) lewat `triggerPackageChangeFromHub()` → `closeActionsModal()` + `openPackageChangeModal()` di `_list_scripts.blade.php` (juga didaftarkan ke `anyModalOpen()`/Escape handler, sama seperti modal Atur Jaringan). Data paket/harga aktif diambil dari `selectedCustomerData` yang sama (dari `data-*` baris tabel), TIDAK fetch ulang. URL submit form (`data-package-update-url`) & id paket aktif (`data-current-package-id`) dirender server-side per baris di `_list_table.blade.php` (pola sama `data-network-update-url` — ADHOC-20 langkah 3, `PostTargetRenderedServerSideTest`), JS cuma pindahin ke `form.action`/`select.value`, tidak pernah merakit path. | Done — 2026-09-11 |
| ADHOC-64 | **Modul Customer Acquisition (dipakai tim Busdev) — "Pelanggan Aktif < 30 Hari Diverifikasi"** — di luar sprint aktif, permintaan eksplisit user (lampiran contoh tabel sales, diklarifikasi lewat tanya-jawab: cuma monitoring, bukan hitung komisi otomatis; kolom "Harga Dikurangi PPN" diisi manual Busdev; kolom "OLT" di gambar acuan ternyata maksudnya POP/Cabang, bukan device OLT teknis). Nama internal SENGAJA `CustomerAcquisition`/`customer_acquisitions` (tabel/model/controller/permission), bukan literal "Busdev" — itu nama tim pemakai, bukan istilah domain (dipilih user dari beberapa opsi). Baris `customer_acquisitions` (unique `customer_id` — sekali seumur hidup per pelanggan) dibuat OTOMATIS oleh `CustomerObserver::updated()` persis saat status pertama kali jadi `WorkflowTransition::ACTIVE`, dikelompokkan per `periode` (YYYY-MM, bulan verifikasi); reactivation pasca-suspended TIDAK bikin baris baru (`firstOrCreate`). "Reset tanggal 1" SENGAJA tanpa job/cron — `CustomerAcquisitionController::index()` default query `periode` bulan berjalan (otomatis kosong lagi begitu bulan berganti), bulan lalu tetap bisa dibuka read-only lewat filter (`CustomerAcquisition::isEditable()` = `periode === bulan berjalan`, dicek ulang server-side di `update()`). Kolom tabel ikut gambar acuan: No, Nama, POP/Cabang (`customers.pop->name` — dikoreksi dari `olt_code` setelah user klarifikasi), Alamat (`clean_address`), Tanggal Aktivasi (`customer_services.activation_date`), Biaya Langganan (`total_monthly_bill`), Harga Dikurangi PPN (manual, `nullable decimal`). Permission baru `customer_acquisitions.view`/`.update` (feature root `customer_acquisitions`, TANPA create/delete — baris otomatis) — `owner` via `*`, `admin` dapat `.* `, `atasan` cuma `.view` (monitoring), `sales` dapat `.view`+`.update` (isi kolom manual). Wajib POP scope (`EffectiveAccessService`) di index & update. Route `/customer-acquisitions`, menu sidebar tetap berlabel "Busdev" (istilah yang dikenal user sehari-hari). Test: `CustomerAcquisitionModuleTest` (7 test — baris kebentuk sekali saat ACTIVE, tidak dobel saat reaktivasi, status lain tidak memicu, index cuma tampilkan periode diminta, update jalan di periode berjalan, update ditolak 403 di periode lampau, user ber-scope POP gak bisa lihat/ubah baris di luar scope-nya). Regresi disapu: `CustomerWorkflowServiceTest` tetap hijau, full suite sebelumnya 414 passed (4 gagal pra-existing tak tersentuh task ini: `CustomerHubModalFooterResponsiveTest`, `PaymentReceiptPrintTest`, `PostTargetRenderedServerSideTest`). | Done — 2026-09-11 |
| ADHOC-63 | **Kategori Paket Internet jadi Master (`package_categories`)** — sebelumnya hardcode di `InternetPackage::CATEGORIES` (const array), diminta user eksplisit. Tabel baru `package_categories` (`name` unik, `is_active`, `sort_order`) ditanam 4 kategori bawaan lewat migrasi (pola sama `item_categories`/`revenue_categories`) — TANPA kolom `code`: beda dengan dua master itu, `internet_packages.category` tidak punya kontrak code, kolomnya menyimpan `name` apa adanya, jadi tidak perlu migrasi ulang kolom `category`. Penambahan kategori baru **modal**, bukan halaman/CRUD terpisah (skala kecil, cuma 1 field) — `PackageCategoryController::store()` (route `POST /master/paket/categories`, permission `packages.create\|packages.update`) dipanggil via `fetch` JSON dari `_category_modal.blade.php` yang di-`@include` di `master/paket/create.blade.php` & `edit.blade.php`; sengaja BUKAN form POST biasa supaya gagal validasi tidak kena masalah `back()->withErrors()` balik ke referer yang mengosongkan form paket yang sedang diisi (ADHOC-20 lesson). Kategori baru langsung disisipkan & terpilih di `<select>#category` lewat `CustomEvent('package-category-created')`, tanpa reload. Validasi paket (`InternetPackageController::validatePackage()`) pindah dari `Rule::in(array_keys(CATEGORIES))` ke `Rule::exists('package_categories','name')`; dropdown filter index & create pakai kategori aktif (`PackageCategory::options()`), form edit menambahkan kategori paket itu sendiri ke daftar kalau kebetulan sudah dinonaktifkan admin (supaya select tidak diam-diam mencopot kategori lama). Test: `PackageCategoryQuickCreateTest` (5 test — 4 kategori bawaan tertanam migrasi, kategori baru langsung bisa dipakai bikin paket, nama wajib & unik, kategori karangan ditolak validasi paket, endpoint butuh permission). | Done — 2026-09-11 |
| ADHOC-61 | **`GET /api/tickets/undefined/detail` 404 — klik Tiket Aktif di NOC Dashboard** (bug pre-existing, ditemukan lewat browser console log user). Root cause: `noc/dashboard.blade.php` dispatch event `open-ticket-drawer` dengan payload `{ ticketId: ... }`, padahal `tickets/partials/detail-drawer.blade.php` (di-`@include` di halaman yang sama) dengerin `$event.detail.id` (key `id`). Drawer selalu buka dengan id `undefined` — bukan salah baris, SEMUA klik di widget itu kena. Fix: satu baris, samakan key jadi `id`. Test baru: `TicketDetailDrawerTest::test_noc_dashboard_ticket_row_dispatches_id_key_matching_drawer_listener`. Regresi: `TicketDetailDrawerTest`, `NocDashboardTest`, `NocWorksheetTest` — 41 test hijau. | Done — 2026-09-10 |
| ADHOC-59 | **Dashboard Analitik FOP** — **di luar sprint aktif, dikerjakan atas permintaan eksplisit user** (setelah Gudang selesai). Halaman baru `/fop/analitik` (`FopAnalyticsController`, permission sendiri `fop_analytics.view`) — agregat lintas periode, terpisah dari `/fop` (operasional harian): (1) ranking + tren alat kerja terpakai (`task_work_tools` GROUP BY `tool_name` snapshot), (2) ranking kecamatan pemasangan/maintenance(proksi komplain)/dibatalkan(proksi gagal), (3) leaderboard beban tugas (`TaskTeam` per assignment) & solving terbanyak (`completed_by`, badge peringatan data historis sebelum 2026-08-07/commit `f6a2f77`), (4) backlog Task draft/terjadwal/pending (reuse `Task::slaDeadline()`/`isOverSla()`), (5) top 20 durasi pengerjaan terlama + baseline rata-rata (reuse `Task::actualDurationMinutes()`). Chart SVG manual (pola `warehouse/reports/index.blade.php`, gak ada lib chart JS di project). Rancangan: [`docs/plan/analisa-dashboard-analitik-fop.md`](plan/analisa-dashboard-analitik-fop.md). **Susulan setelah data dummy diisi (`FopAnalyticsDummySeeder`) dan dashboard direview ulang** — 5 gap diperbaiki: (1) filter dropdown POP sebelumnya diam-diam gak narrow backlog/solving leaderboard/durasi terlama, sekarang konsisten ke semua 8 section; (2) tren pemakaian alat (`toolTrend`) dihitung tapi gak pernah dirender, sekarang chart top-5 alat; (3) ranking wilayah/leaderboard `limit(20)` mentah tanpa indikasi, sekarang kasih caption "top 20 dari N total"; (4) KPI "N/M jenis alat aktif dipakai" dari spek awal ditambahkan; (5) komparasi periode sebelumnya (delta % naik/turun) buat 3 KPI periode-scoped, plus fix KPI "Task Dibatalkan" yang sempat ikut kepotong top-20 pas fix #3, dan grant `fop_analytics.view` ke role `fop` sendiri (audiens utama, sebelumnya kelewat). Test: `FopAnalyticsDashboardTest` (16 test). | Done — 2026-09-09 |
| ADHOC-65 | **3 fitur Business Development (Skema 1-3)** — di luar sprint aktif, permintaan eksplisit user. Role baru `business_development` (`roles.is_package_restricted` flag baru: Sales & Teknisi ditandai). **Skema 1 — Restriksi Paket per Role**: tabel `restricted_packages` (daftar GLOBAL, satu daftar dipakai bareng semua role restricted — keputusan user), `InternetPackage::scopeAvailableFor(User)` jadi SATU-SATUNYA titik query dropdown paket di form Customer (create/edit) — fail-open (tampilkan semua paket aktif) selama `restricted_packages` masih kosong sama sekali, biar Sales gak terkunci total sebelum Busdev sempat mengisi daftarnya. Validasi submit (`CustomerRegistrationRequest`, `CustomerController::update()`) menolak paket di luar daftar buat role restricted (bukan cuma sembunyikan di dropdown — cegah bypass request langsung), dengan fail-open yang sama. Halaman kelola `/business-development/package-restrictions` (checkbox paket, permission `package_restrictions.*`). **Skema 2 — Dashboard Omset Sales**: `/business-development/sales-omset` (`SalesOmsetDashboardController`), REUSE formula `CustomerAcquisition::getHargaDikurangiPpnAttribute()` (Biaya Langganan × (1−11%)) — **tidak bikin formula baru** — diagregasi per `customers.sales_user_id`, wajib POP scope (`EffectiveAccessService`). Klik nama Sales → modal breakdown per pelanggan (data di-embed, bukan endpoint AJAX terpisah — pola "view-only → modal" CLAUDE.md). **Skema 3 — FK Sales/Agent/Referral**: `customers.sales_user_id` (FK `users`, role sales), `agent_id` (FK tabel baru `agents` — master mitra, BUKAN akun login, dikelola Busdev), `referral_customer_id` (FK self `customers`) — kolom lama (`sales_code`/`agent_code`/`referral_customer_code` varchar) **tidak dihapus**, tetap tampil sebagai fallback data legacy di halaman Detail. Sales yang login **dipaksa** `sales_user_id = auth()->id()` di server (submission spoof diabaikan); `agent_id` cuma berlaku dari actor ber-permission `agents.view` (Busdev/admin/owner) — field disembunyikan dari role lain. Form registrasi (`customers/create.blade.php`) yang SEBELUMNYA sama sekali tidak punya section referral (cuma ada di edit) sekarang menyertakannya — akar masalah "data komisi hilang" yang dilaporkan user. Autocomplete Referral (`GET /customers/search-referral`, vanilla fetch + Alpine, cari CID/nama pelanggan existing) dipakai di create & edit. Migration baru: `agents`, `restricted_packages`, `add_is_package_restricted_to_roles_table`, `add_sales_agent_referral_fk_to_customers_table`. RBAC: `BusinessDevelopmentFeatureSeeder` (features `agents`/`package_restrictions`/`sales_omset_dashboard`), permission di-grant ke `business_development` + `admin`. Test: `PackageRestrictionTest` (5), `CustomerReferralFkTest` (5), `SalesOmsetDashboardTest` (3) — 13 test baru, hijau. Regresi disapu: `CustomerRegistrationSkipSurveyTest`, `CustomerAcquisitionModuleTest`, `PermissionGeneratorServiceTest`, `RolePermissionMatrixTest` — hijau (fail-open di atas ditemukan justru dari regresi `CustomerRegistrationSkipSurveyTest` merah sebelum fix). Full suite disapu: 1864 passed, 1 regresi asli ditemukan & ditutup (`RoleTest::test_role_seeder_inserts_expected_roles` — hardcode `10` role, sekarang `11`), 10 sisanya dikonfirmasi pre-existing (file dirty sebelum sesi ini: WA template Quick Hub, `InternetPackageSeeder` data, route `customers.invoices.manual`, struk pembayaran, `PostTargetRenderedServerSideTest`/`_list_scripts.blade.php`, QR reset-PIN UI — nol overlap sama file yang disentuh task ini).
**Susulan (2 gap yang diakui ke user, dikerjakan atas permintaan "kerjakan bro"):** (1) **Toggle `is_package_restricted` di UI Role Management** — sebelumnya cuma bisa diset lewat seeder/tinker. `RolePermissionController::store()`/`updateRole()` (`resources/views/roles/index.blade.php`) sekarang punya checkbox "Batasi pilihan paket internet" di modal Tambah/Ubah Role, BOLEH diubah walau role-nya `is_system` (Sales/Teknisi `is_system=true`, justru dua role itu yang paling perlu ditoggle — beda dari field `code` yang tetap terkunci buat role sistem). (2) **Permission `package_restrictions.view`/`agents.view`/`sales_omset_dashboard.view` di-grant ke Atasan** (view-only/monitoring, konsisten pola `cash_deposit`/`warehouse` — eksekusi `.update`/`.create` tetap Busdev/admin). Test: `RolePackageRestrictedToggleTest` (3 kasus — create role dengan flag, lepas flag dari role sistem existing, tambah flag ke role yang sebelumnya tidak restricted). Regresi disapu: `RoleTest`, `PackageRestrictionTest`, `CustomerReferralFkTest`, `SalesOmsetDashboardTest`, filter `RoleManagement`/`RolePermission` — 35 test hijau.
**Koreksi kedua (laporan user, sesi sama)** — dropdown/autofill "ID Sales" (`CustomerController::create()`/`edit()`/`store()`/`update()`, `customers/create.blade.php`, `customers/edit.blade.php`) sebelumnya HARDCODE `role.code === 'sales'`, gak konsisten dengan Skema 1 yang sudah dinamis per-role. Diganti jadi berbasis `roles.is_package_restricted` (SATU sumber kebenaran dipakai dua fungsi: role yang paketnya dibatasi otomatis JUGA jadi "role penjual" yang muncul di dropdown ID Sales + ke-agregasi Dashboard Omset) — kalau nanti Teknisi diberi akses registrasi pelanggan, otomatis ikut tanpa ubah kode lagi. Dashboard Omset Sales (`SalesOmsetDashboardController`) sendiri TIDAK perlu diubah — sudah dari awal agregasi murni dari kolom `sales_user_id`, gak pernah filter berdasarkan role. Test baru: `CustomerReferralFkTest::test_non_sales_role_marked_package_restricted_also_autofills_and_appears_in_dropdown` (role selain kode 'sales' yang ditandai restricted tetap autofill + muncul di dropdown). Regresi disapu ulang — 25 test hijau.
**Koreksi ketiga (laporan user, sesi sama)** — dua hal: (1) menu sidebar "Role & Permission" TERNYATA di bawah submenu **"Pengguna & RBAC"**, bukan "Administrasi/Pengaturan" seperti yang sempat disebut salah ke user (jawaban lisan doang, gak ada kode yang berubah). (2) `resources/views/roles/matrix.blade.php` — `agents`/`package_restrictions`/`sales_omset_dashboard` (dan `customer_acquisitions` lama, ternyata sama-sama belum pernah dikategorikan sejak ADHOC-64) kebuang ke bucket fallback "Modul Tambahan Lainnya" tanpa label ramah, karena `$functionalCategories`/`$featureMeta`/`$permissionDescMap` di file itu di-hardcode manual per fitur (bukan auto dari DB) dan keempatnya belum pernah didaftarkan. Fix: kategori baru `group_busdev` ("Business Development") + entri `$featureMeta`/`$permissionDescMap` buat keempat fitur. Test baru: `RoleMatrixBusinessDevelopmentCategoryTest` (halaman matrix render + kategori & 3 nama fitur baru muncul). Regresi disapu — 26 test hijau.
**Susulan keempat** — user tanya soal daftar paket per-role (Teknisi 4 paket beda dari Sales 8 paket): DIKONFIRMASI TETAP satu daftar global (keputusan awal tidak berubah, cuma diklarifikasi ulang). Diminta terpisah: halaman **"Pelanggan Baru Diverifikasi Admin"** (`customer-acquisitions.index`) ditambah kolom **"Diinput Oleh"** (`customer.salesUser.name` + label role-nya, fallback `sales_code` legacy kalau FK belum keisi) + 2 filter baru **Role Penginput** & **Nama Penginput** — KHUSUS role ber-`is_package_restricted` (disebut user "role dengan pembatasan paket tadi"), reuse `customers.sales_user_id` (Skema 3), BUKAN kolom baru. Ini yang menutup gap "Busdev mau tahu jumlah pelanggan diinput satu Teknisi" dari sesi sebelumnya — begitu Teknisi (role restricted) diberi akses registrasi pelanggan nanti, otomatis muncul di filter Role tanpa ubah kode. `CustomerAcquisitionController::index()` dapat 2 query param (`role_id`, `sales_user_id`), keduanya `whereHas` ke `customer`/`customer.salesUser`, tetap di dalam POP scope yang sudah ada. Test baru: `CustomerAcquisitionModuleTest::test_filter_by_role_and_name_narrows_records_to_matching_sales_user` + `::test_diinput_oleh_column_shows_sales_user_name` — 8 test modul ini total, hijau. Regresi disapu: `SalesOmsetDashboardTest`, `PackageRestrictionTest`, `CustomerReferralFkTest`, `RoleMatrixBusinessDevelopmentCategoryTest` — 15 test hijau.
**Data seeder demo** — `BusinessDevelopmentSeeder` baru (dipanggil `DatabaseSeeder` SETELAH `TechnicianSeeder`/`SalesSeeder`, reuse keduanya — TIDAK bikin user Sales/Teknisi baru). Isi: user `busdev@whusnet.com`/`password` (role `business_development`, scope `all_pop`); 8 paket contoh persis dari permintaan awal user (Net138/150/165/198 + varian Khusus, kode sudah ada di `InternetPackageSeeder`) masuk `restricted_packages`; 2 Master Agent contoh; 3 pelanggan demo (`sales_user_id` merujuk user Sales & Teknisi demo yang SUDAH ADA) lengkap dengan `CustomerService` (biaya langganan) + `CustomerAcquisition` (periode berjalan) — dibuat LANGSUNG lewat seeder (bypass `CustomerObserver`, pola sama `CustomerAcquisitionFactory` di test), bukan simulasi submit form, supaya Dashboard Omset Sales & filter "Diinput Oleh" (`customer-acquisitions.index`) langsung ada isinya tanpa registrasi manual — termasuk baris demo Teknisi walau Teknisi belum punya akses registrasi pelanggan sungguhan di UI (disengaja, biar filter Role "Teknisi" sudah bisa dicoba). Fallback POP demo (`Pop::factory()`) kalau DB benar-benar kosong POP (`MasterPopSeeder` sengaja tidak dipanggil `DatabaseSeeder` — POP diisi manual admin di data produksi). Idempotent penuh (`updateOrCreate`/`firstOrCreate`/cek-manual-lalu-`factory()->create()` di semua titik — `Customer::firstOrCreate` TIDAK dipakai langsung karena beberapa kolom NOT NULL cuma terisi otomatis lewat `CustomerFactory`). Test: `BusinessDevelopmentSeederTest` (6 kasus — user Busdev demo, 8 paket restriksi persis, 2 Agent, pelanggan demo ke-link Sales & Teknisi existing + ada CustomerService/CustomerAcquisition, idempoten dijalankan 2x, Dashboard Omset menampilkan data hasil seeder). Regresi disapu (termasuk reseed dev DB manual) — 31 test hijau.
**Sidebar dirapikan (permintaan user)** — 4 link lepas (Busdev/Omset Sales/Master Agent/Restriksi Paket) yang sebelumnya berdiri sendiri di sidebar digabung jadi SATU submenu collapsible **"Business Development"** (`resources/views/layouts/app.blade.php`, pola `toggleSubmenu()` sama persis submenu lain — "Pembayaran", "Pengguna & RBAC", dst). Label sub-item "Busdev" diganti **"Pelanggan Aktif < 30 Hari"** biar gak redundan sama judul group (permintaan eksplisit user "kalau bisa ini diganti"). Gerbang group: gabungan OR keempat permission (`customer_acquisitions.view`/`sales_omset_dashboard.view`/`agents.view`/`package_restrictions.view`) — role yang cuma punya SEBAGIAN (mis. Sales cuma `customer_acquisitions.view`) tetap lihat group-nya tapi cuma sub-item yang dia punya izinnya. **Insiden pas ngerjain**: file `app.blade.php` sempat kesenggol proses LAIN yang aktif nulis ke repo yang sama di sesi ini (bukan cuma sekali — beberapa file lain juga kena, lihat catatan `CustomerRegistrationRequest`/`CustomerReferralFkTest`/`PackageRestrictionTest` sebelumnya) — label "Business Development" yang baru ditulis ke-timpa jadi teks acak ("Tiara Macan") tepat setelah disimpan, ketahuan lewat test gagal (`assertSee` tidak ketemu), dibenerin lagi begitu ketauan. Test baru: `SidebarBusinessDevelopmentGroupTest` (2 kasus — Busdev lihat group lengkap 4 sub-item, Sales cuma lihat sub-item yang dia punya izinnya). Regresi disapu — 31 test hijau.
**Koreksi formula Omset Sales (laporan user, sesi sama)** — user klarifikasi ulang: Omset Sales BUKAN "biaya dikurangi PPN" (Rp133.500 untuk Rp150.000) seperti spek awal, tapi **NILAI PPN 11% itu sendiri** (Rp16.500). Dikonfirmasi eksplisit lewat AskUserQuestion (dua angka itu kontradiktif, gak bisa ditebak) sebelum diubah — CLAUDE.md § Berhenti & Tanya Kalau soal perhitungan uang. Ternyata ini metrik BEDA dari kolom "Harga Dikurangi PPN" di `/customer-acquisitions` (yang TETAP Rp133.500, tidak diubah — dipakai Busdev pantau biaya net, bukan komisi Sales). Fix: `CustomerAcquisition` dapat accessor BARU `omset_sales` (`biaya_langganan × 11%`, reuse konstanta private `BUSDEV_PPN_PERCENT` yang sama — DUA accessor, SATU sumber persentase), `harga_dikurangi_ppn` yang lama TIDAK disentuh. `SalesOmsetDashboardController` diganti pakai `omset_sales` (dulu keliru reuse `harga_dikurangi_ppn` — pelanggaran niat awal "jangan bikin formula baru" yang sekarang ternyata memang harus dipisah, dikoreksi eksplisit oleh user, bukan inisiatif sendiri). Test: `SalesOmsetDashboardTest::test_omset_matches_users_exact_example` (150.000 → 16.500, cocok persis contoh user) + `CustomerAcquisitionModuleTest::test_omset_sales_stays_a_separate_metric_from_harga_dikurangi_ppn` (guard regresi — dua metrik gak boleh disatukan lagi). Regresi disapu — 20 test hijau.
**Kolom Dashboard Omset Sales dirombak (permintaan user, sesi sama)** — tabel utama: **Sales | Jumlah Pelanggan | Total Biaya Langganan | Total Harga Dikurangi PPN | Total Omset** (dulu cuma Sales/Jumlah/Total Omset). Modal Rincian Omset: **Pelanggan | Tanggal Aktivasi | POP | Biaya Langganan | Harga Dikurangi PPN | Omset** (dua kolom baru: Tanggal Aktivasi, Harga Dikurangi PPN — dikonfirmasi ke user dulu lewat AskUserQuestion soal "Harga Dikurangi PPN" vs "Biaya di kurangi PPN" yang disebut dobel di permintaan, ternyata cuma kepencet dobel/typo, bukan 2 kolom beda). `SalesOmsetDashboardController` nambah agregasi `total_biaya_langganan`/`total_harga_dikurangi_ppn` per Sales + field `tanggal_aktivasi`/`harga_dikurangi_ppn` di breakdown. Test baru: `SalesOmsetDashboardTest::test_new_columns_show_correct_totals_and_breakdown_fields`. Regresi disapu — 20 test hijau.
**Filter Role & Nama ditambah ke Dashboard Omset Sales (permintaan user, sesi sama)** — pola SAMA persis `/customer-acquisitions` (dropdown Role → Nama menyempit, khusus role ber-`is_package_restricted`, reuse `customers.sales_user_id`). Kolom **"Role"** juga ditambah ke tabel utama (nama role Sales-nya, dari `salesUser.role`). `SalesOmsetDashboardController` dapat 2 query param (`role_id`, `sales_user_id`) di-`whereHas` ke `customer`/`customer.salesUser`, tetap di dalam POP scope existing — dropdown filter sendiri (daftar nama) SENGAJA tidak di-scope POP (sama keputusan `/customer-acquisitions`: yang wajib scope itu data omsetnya, bukan daftar nama di filter). Test baru: `SalesOmsetDashboardTest::test_filter_by_role_and_name_narrows_dashboard_and_shows_role_column`; test lama `test_pop_scope_restricts_dashboard_visibility` disesuaikan (nama Sales di luar scope tetap boleh nongol di dropdown, yang gak boleh itu baris tabelnya). Regresi disapu — 24 test hijau.
**Pesan penjelas ID Sales (permintaan user, 2026-09-14)** — form Registrasi/Edit Pelanggan (`customers/create.blade.php`, `customers/edit.blade.php`): actor ber-role restricted (Sales, atau role lain yang ditandai lewat toggle Role Management) tetap dapat autofill terkunci seperti sebelumnya. Actor DI LUAR role itu (Admin/Busdev/dst, jalur dropdown manual) sekarang lihat pesan penjelas di atas dropdown — "Auto-deteksi ID Sales cuma berlaku untuk role: {daftar nama role restricted}. Role Anda ({role dia}) di luar itu — pilih manual di bawah." Daftar nama role diambil live dari `Role::where('is_package_restricted', true)` (`$restrictedRoleNames`, dilewatkan `CustomerController::create()`/`edit()`) — bukan teks hardcode "Sales, Teknisi", biar ikut berubah kalau role restricted ditambah/dicabut lewat toggle. Test baru: `CustomerReferralFkTest::test_non_restricted_actor_sees_explanatory_message_restricted_actor_does_not`. Regresi disapu — 21 test hijau. | Done — 2026-09-14 |
| ADHOC-66 | **SN Laporan Pemasangan: fallback teks manual DICABUT** — koreksi lanjutan ADHOC-54, permintaan eksplisit user, dikonfirmasi via AskUserQuestion (3 opsi: block total / block-kalau-item-terdaftar / batalkan) — user pilih **block total**. Sebelumnya teknisi TANPA SN di custody Gudang tetap bisa ngetik SN manual (fallback buat device belum ke-track Inventory); sekarang SN **cuma boleh dari dropdown custody**, gak ada jalan lain. `CustomerInstallationController`: helper baru `eligibleSerialsForTeam(?Task)` (dipakai `report()` & `storePemasangan()`, satu query dua pemakai), validasi `storePemasangan()` diganti — `serial_number` jadi `prohibited` (klien gak boleh kirim sama sekali), `selected_inventory_serial_id` jadi `required` + `Rule::in($eligibleSerialIds)` (dibatasi ke custody tim task ini, bukan cuma "ada di tabel"), pesan custom kalau custody kosong ("ambil barang dari Gudang dulu"). View `installations/report.blade.php`: opsi "Tidak tercatat Gudang / isi manual" & `<input>` teks SN dicabut total; custody kosong → dropdown `disabled` + pesan merah instruksi ambil barang. JS: `toggleSerialManual()` dihapus (gak perlu sinkron manual↔dropdown lagi), `aktivasiRequiredFields`/`formFields.pemasangan.required` ganti acuan dari `serial_number` ke `selected_inventory_serial_id`. Test: `InstallationSerialDropdownAuthorityTest` dirombak (fallback test lama JADI test negatif "ditolak", + test baru SN dari custody teknisi lain ikut ditolak). **Blast radius luas** — 3 file test lain yang submit `serial_number` manual tanpa custody ikut pecah, diperbaiki dengan helper `issueActiveSerialTo()` (Receive Pusat→Transfer Cabang→Issue Teknisi) per file: `InstallationActivatedWebhookTest` (cache per-SN karena ada test yang submit SN sama 2x), `InstallationSpeedtestActivationGateTest` (nambah Pop `pusat` yang sebelumnya gak ada), `InstallationMaterialCustodyReconcileTest`. Regresi disapu: 22 test / 97 assertion di 4 file terkait, hijau. Full suite: 1951 passed, 13 skipped, 12 gagal — **nol overlap** sama file yang disentuh task ini (`CustomerHubModalFooterResponsiveTest`, `ImportLegacySqlPusatParentTest` x2, `InternetPackageSeederTest` x3, `NominalRupiahBertitikDiterimaTest` — route `customers.invoices.manual` belum ada, `PaymentReceiptPrintTest` x2, `PostTargetRenderedServerSideTest`, `QrStaffPageSmokeTest` — semua pre-existing, sudah dicatat dirty di entri ADHOC sebelumnya). | Done — 2026-09-12 |
| ADHOC-67 | **Validasi "Biaya Instalasi" Bisnis vs CS — dinamis per kategori paket** — di luar sprint aktif, permintaan eksplisit user, disaring lewat beberapa ronde AskUserQuestion (posisi di alur, apa yang terjadi kalau ditolak, sumber aturan). Klarifikasi final: alur registrasi TETAP SAMA PERSIS buat semua kategori (tidak nyentuh `WorkflowTransition` sama sekali — bukan step baru di state machine); yang beda cuma paket **Bisnis** ATAU jalur normal (survey→ACC/CS→verifikasi, prorate dkk) SELESAI seperti biasa, lalu Busdev **tambahan** mengisi nominal "Biaya Instalasi" lewat modul Busdev yang sudah ada (`/customer-acquisitions`, ADHOC-64) — bukan approve/reject, murni isi data. Mekanisme "tau paket mana punya siapa" dibuat generik: kolom baru `package_categories.installation_fee_approval_permission` (nullable string, migration `add_installation_fee_approval_permission_to_package_categories_table`) menyimpan SATU KODE PERMISSION BEBAS (bukan enum "Busdev"/"CS" tetap) — kategori manapun (termasuk yang ditambah belakangan) tinggal diisi kode permission siapa pun yang berhak, TANPA ubah kode sama sekali kalau kebijakan berubah. `CustomerAcquisition` (ADHOC-64) dapat kolom `installation_fee` (nullable decimal, migration terpisah) + 3 method resolusi: `packageCategory()` (lookup by name — `internet_packages.category` bukan FK, cache statis per-request biar list gak N+1), `needsInstallationFeeValidation()`, `installationFeeApprovalPermission()`. `CustomerAcquisitionController::updateInstallationFee()` — SATU-SATUNYA aksi tulis di modul ini, gerbangnya DINAMIS (`abort_unless($request->user()->hasPermission($record->installationFeeApprovalPermission()))`), bukan middleware permission statis; kategori yang `needsInstallationFeeValidation()` false ditolak 404 (field itu gak relevan buat baris itu). View kolom baru "Biaya Instalasi": form input HANYA muncul kalau user login punya permission yang dikonfigurasi baris itu, selain itu tampil read-only/"Menunggu Busdev". Permission baru `customer_acquisitions.installation_fee.update` (sub-feature terpisah, `CustomerAcquisitionFeatureSeeder` diperluas) — default digrant ke role `business_development` (role asli dari ADHOC-65, dipakai sebagai approver Busdev sungguhan, BUKAN 'sales' seperti asumsi awal sebelum ADHOC-65 ditemukan sudah ada) + wildcard `admin`/`owner`; seeder yang sama otomatis memetakan 3 kategori "%Bisnis%" existing ke permission ini sebagai nilai awal (bisa diubah kapan saja). **Master Kategori Paket dapat halaman kelola BARU** (`master/package_categories/index.blade.php` + `edit.blade.php`, `PackageCategoryController::index/edit/update()`, route `/master/package-categories`, permission `packages.view`/`packages.update` — sebelumnya cuma modal quick-create ADHOC-63) — satu-satunya tempat admin memilih permission per kategori, dropdown-nya SEMUA kode permission di tabel `permissions` (sengaja tidak dibatasi, sesuai permintaan "field pilih permission spesifik"). `business_development` role ditambah `packages.update` biar bisa akses halaman ini sendiri. Test: `InstallationFeeDynamicApprovalTest` (6 — Home Broadband gak butuh validasi, Bisnis butuh & permission-nya kebaca benar, user tanpa permission ditolak 403, `business_development` berhasil isi, kategori yang gak butuh ditolak 404, **inti "dinamis"**: pindah permission kategori ke kode lain di tengah jalan langsung mencabut akses user lama tanpa ubah kode), `PackageCategoryManagementTest` (4 — admin set/kosongkan permission, kode karangan ditolak validasi, role tanpa `packages.update` ditolak 403). **Insiden pas ngerjain**: `layouts/app.blade.php` kesenggol proses lain yang aktif nulis ke repo yang sama (persis pola yang dicatat ADHOC-65) — judul submenu "Business Development" ke-timpa jadi "Tiara Macan", ketauan dari `SidebarBusinessDevelopmentGroupTest` merah, dibenerin balik. Regresi disapu: `CustomerAcquisitionModuleTest` (9), `SidebarBusinessDevelopmentGroupTest` (2), + sapuan gabungan Customer Acquisition/Business Development/Sales Omset/Permission Generator/Role Matrix/Sidebar — 52 test hijau.

**Koreksi (laporan user, sesi sama)** — dua keluhan: (1) Master Kategori Paket cuma bisa edit, user minta CRUD penuh; (2) field "pilih permission spesifik" ternyata dalam praktiknya "terlalu teknis, cuma developer yang paham" (dropdown isinya ratusan kode kayak `warehouse_transfer.receive` — gak bisa dibaca admin biasa). Fix (2) lebih besar: kolom `package_categories.installation_fee_approval_permission` (string bebas) **DIGANTI** `installation_fee_approval_role_id` (FK `roles`, migration ditulis ulang di file yang sama — masih uncommitted, aman diedit langsung tanpa migration tambahan) — dropdown-nya sekarang nama ROLE ("Business Development", "Admin", dst), bukan kode permission. Gerbang tulis (`CustomerAcquisition::canBeValidatedBy()`) jadi DUA LAPIS: (a) permission `customer_acquisitions.installation_fee.update` via Role Matrix — jalur teknis, tetap ada buat admin/owner override — LEBIH DULU dicek; (b) `role_id` user PERSIS sama dengan role yang dipilih admin di kategori — jalur utama, gampang dipahami. Permission itu SENGAJA **dicabut** dari grant default `business_development` di `RolePermissionSeeder` (sebelumnya ikut digrant di draf awal) — kalau tidak, admin gak akan pernah bisa "mencabut" akses business_development cuma dengan mindah role di Master Kategori Paket, karena permission overridenya bikin dia selalu lolos apa pun konfigurasinya (ketahuan dari test dinamis yang awalnya gagal). Fix (1): `PackageCategoryController` dapat `create()`/`store()`/`destroy()` (sebelumnya cuma `index()`/`edit()`/`update()`) — `destroy()` hard-delete HANYA kalau kategori belum dipakai paket manapun (`internet_packages.category` masih string, dicek `exists()`), kalau sudah dipakai ditolak & diarahkan nonaktifkan lewat Edit (pola sama master lain di repo — kategori terpakai tidak boleh hilang). Metode JSON lama buat modal quick-create ADHOC-63 di-rename `store()`→`quickStore()` (biar gak tabrakan nama sama `store()` baru yang full-page), route `master.paket.categories.store` disesuaikan. View baru: `master/package_categories/create.blade.php` (form tambah, dropdown role), tombol Hapus di `index.blade.php`, dropdown role (bukan permission) di `edit.blade.php`. Test disesuaikan total: `InstallationFeeDynamicApprovalTest` (7 — ganti semua assersi dari permission ke role, tambah test override-permission-selalu-lolos + reassign-role-mencabut-akses), `PackageCategoryManagementTest` (8 — tambah create/duplicate-name-ditolak/destroy-berhasil/destroy-ditolak-karena-dipakai). Regresi disapu: 57 test hijau (Customer Acquisition/Installation Fee/Package Category/Business Development/Sales Omset/Permission Generator/Role Matrix/Sidebar).

**Koreksi kedua (laporan user, 2026-09-14)** — user awalnya kira `installation_fee` field baru krn `extra_installation_fee` (CS, `finalVerify()`) dikira gak ada; setelah dijelasin dua-duanya sudah ada & beda, user MINTA `installation_fee` juga ikut TERBIT jadi tagihan sungguhan (bukan cuma catatan Busdev spt sebelumnya), dan permission validasi keduanya harus beda entry di Role Matrix (bagian ini ternyata SUDAH terpenuhi sejak awal: `customers.detail.installation.validate` vs `customer_acquisitions.installation_fee.update` sudah dua permission terpisah). Root konteks: Invoice Awal (`extra_installation_fee`) LAZIMNYA sudah dibayar di tempat saat aktivasi (`finalVerify()` comment sendiri) — REKOMENDASI eksplisit ke user (disetujui): JANGAN gabung ke satu field/mutasi invoice yang sama (resiko ubah nominal invoice yang sudah lunas, ngerusak rekonsiliasi pembayaran), TETAP dua field terpisah, tapi `installation_fee` sekarang benar-benar menerbitkan invoice — via `Invoice` BARU & TERPISAH, bukan menimpa Invoice Awal. `customer_acquisitions` dapat kolom `installation_fee_invoice_id` (nullable FK `invoices`, `nullOnDelete`). `CustomerAcquisitionController::updateInstallationFee()` reuse `ManualInvoiceService::create()` (service yang sama dipakai halaman Buat Tagihan Manual, BUKAN nulis ulang logika hitung tagihan) — invoice tipe INSIDENTAL (auto-resolve dari baris non-langganan), kategori pendapatan "Jasa Instalasi" / sub "Biaya Aktivasi" (`RevenueCategory::CODE_JASA_INSTALASI`/`RevenueSubcategory::CODE_BIAYA_AKTIVASI`, sudah ada sejak ADHOC-60, tinggal dipakai). Begitu invoice terbit, baris TERKUNCI — `installation_fee_invoice_id` sudah terisi menolak update lanjutan (pesan arahkan koreksi lewat menu Tagihan biasa: batalkan lalu terbitkan ulang) — proteksi yang sama alasannya dengan kenapa dua field ini dipisah sejak awal, sekarang berlaku juga buat invoice `installation_fee` sendiri begitu ada uang yang mungkin sudah bergerak. View: baris yang sudah py invoice nampilin nominal + link nomor invoice + badge status (Lunas/Belum Dibayar, dari `Invoice::invoice_status` — SUDAH di-cast enum `InvoiceStatus` di model, bukan string) menggantikan form input; ditemukan & dibenerin SAAT VERIFIKASI manual (bukan dari test) bug penulisan Blade yang salah wrap ulang enum yang sudah ter-cast (`InvoiceStatus::from($enumInstance)` — meledak) dan salah bandingin (`$enum === 'lunas'` — selalu false) — ditambah test baru khusus (`test_index_page_renders_after_installation_fee_invoice_is_issued`) yang RENDER halaman sungguhan lewat HTTP test client biar kelas bug ini gak lolos lagi ke depannya (assertion lama cuma cek DB, gak pernah nge-render view-nya). Test: `InstallationFeeDynamicApprovalTest` nambah 3 (total 9) — invoice sungguhan terbentuk dgn tipe/nominal/customer benar, baris terkunci setelah invoice terbit (percobaan ubah nominal lagi diabaikan, invoice count tetap 1), halaman render OK pasca-terbit. Regresi disapu: 18 test hijau (Customer Acquisition + Installation Fee digabung), pint bersih.

**Modul BARU — Antrean "Menunggu Verifikasi Busdev" (permintaan user, sesi sama, 2026-09-14)** — reversal SADAR dari keputusan awal ADHOC-67 ("alur tetap sama, Busdev cuma nyusul isi data, BUKAN gate"): sekarang beneran jadi gate. Disaring dulu lewat AskUserQuestion (apa yang ketunda, isi aksi Busdev, alur tolak) sebelum nyentuh `WorkflowTransition` — area paling rawan di luar Ticket/FopTask. Hasil konfirmasi: (a) yang ketunda `customers.status` (BUKAN cuma tampilan), (b) aksi Busdev = isi nominal + tombol approve eksplisit jadi SATU submit ("Verifikasi & Aktifkan"), (c) **TANPA jalur tolak** sama sekali. Enum baru `WorkflowTransition::WAITING_BUSDEV_VERIFICATION` disisipkan `VERIFICATION_ADMIN` → **[WAITING_BUSDEV_VERIFICATION | ACTIVE]** → `[ACTIVE]` (satu-satunya next state, gak ada balik) — dicek dulu blast radius-nya (`grep` exhaustive match): CUMA SATU match-exhaustive di seluruh app (`allowedNextTransitions()` sendiri), `finalVerify()` sendiri ternyata BYPASS `CustomerWorkflowService`/enum transition sama sekali (langsung `$customer->update(['status'=>...])`) jadi resiko `UnhandledMatchError` di tempat lain nol. `CustomerVerificationController::finalVerify()`: SEMUA proses CS (Invoice Awal, prorate, cid, `service_status`/`billing_status`/`activation_date`, approve Task Pemasangan) **TIDAK BERUBAH SAMA SEKALI** buat kedua kategori — SATU baris beda: `customers.status` diisi `WAITING_BUSDEV_VERIFICATION` (bukan `ACTIVE`) kalau `Customer::needsBusdevInstallationFeeVerification()` true (resolve kategori paket → `PackageCategory::installationFeeApprovalRole()`, method baru di model `Customer` — pola sama persis `CustomerAcquisition::packageCategory()`, duplikasi kecil disengaja karena `CustomerAcquisition` belum tentu ada di titik ini). Notifikasi (telegram, notify creator, redirect message, audit log action) dibuat kondisional biar gak bilang "resmi aktif" ke pelanggan yang masih nunggu. Home Broadband dkk (`installation_fee_approval_role_id` NULL) sama sekali tidak kena — tetap lompat langsung ke ACTIVE seperti sebelum ADHOC-67 ada. Logika terbit-invoice `installation_fee` DIEKSTRAK dari `CustomerAcquisitionController` ke service baru `InstallationFeeInvoiceService` (satu tempat, dipakai DUA pemanggil — controller lama jadi *fallback* buat kasus pinggir, modul baru jadi jalur utama) — cegah rumus/kategori tagihan menyimpang antar pemanggil. Modul baru: `BusdevVerificationController` (`index()` antrean status=WAITING_BUSDEV_VERIFICATION wajib POP scope, `show()`+`verify()` halaman detail — pola aksi #2 CLAUDE.md, bukan modal, karena ada input+validasi), route `/busdev-verifications`, view `busdev-verifications/index.blade.php` + `show.blade.php`. `verify()`: SATU transaksi DB — terbitkan invoice (`InstallationFeeInvoiceService`), `customers.status`→ACTIVE (memicu `CustomerObserver` bikin baris `CustomerAcquisition` seperti biasa), baris itu LANGSUNG diisi `installation_fee`/`installation_fee_invoice_id` di request yang sama (gak pernah nongol "Menunggu Validasi" di `/customer-acquisitions` buat pelanggan yang lewat gate ini), AuditLog `activate_from_busdev_verification`, notif ke pendaftar. Gerbang tulis DINAMIS sama seperti ADHOC-67 (`Customer::canInstallationFeeBeValidatedBy()`, method baru identik `CustomerAcquisition::canBeValidatedBy()` — override permission via Role Matrix ATAU role user cocok config Master Kategori Paket). Permission baru `busdev_verification.view` (feature root, CUMA view — aksi tulis dinamis) via `BusdevVerificationFeatureSeeder`, digrant ke `business_development`+`atasan` (monitoring)+`admin` wildcard. Sidebar: sub-item baru "Menunggu Verifikasi Busdev" masuk submenu Business Development yang sudah ada (bukan group baru). Test: `BusdevVerificationGateTest` (6 test baru — Home tetap langsung aktif, Bisnis nyangkut WAITING_BUSDEV_VERIFICATION + Invoice Awal tetap terbit + baris CustomerAcquisition BELUM ada, pelanggan pending muncul di antrean, role tanpa konfigurasi ditolak 403, role terkonfigurasi berhasil verifikasi+aktivasi+baris CustomerAcquisition ke-prefill lengkap, verifikasi kedua ke pelanggan yang sudah ACTIVE ditolak 404). Regresi disapu: `CustomerFinalVerificationTest` (2, dicek eksplisit `InternetPackageSeeder` ikut nanam kategori Bisnis TAPI role-nya NULL karena seeder itu gak dipanggil bareng `CustomerAcquisitionFeatureSeeder` — aman, gate gak pernah nyala), `InitialInvoiceProrateAdminOverrideTest`/`CustomerLifecycleNotificationTest`/`InitialInvoiceProrateIgnoresClientAmountTest`/`AktivasiTertagihDobelKarenaActivationDateStaleTest`/`TagihanAwalPeriodeIkutTanggalAktivasiTest` (21 gabungan) + sapuan gabungan Customer Acquisition/Installation Fee/Package Category/Business Development/Sales Omset/Permission Generator/Role Matrix/Sidebar (103 total) — semua hijau, pint bersih.

**Koreksi penamaan (permintaan user, sesi sama)** — "Busdev" dinilai gak konsisten dipakai di kode (nama role sungguhannya `business_development`). Rename PENUH, TANPA sisa istilah lama di kode: `WorkflowTransition::WAITING_BUSDEV_VERIFICATION`→`WAITING_BUSINESS_DEVELOPMENT_VERIFICATION` (value `waiting_business_development_verification` — aman diganti, 0 baris data dev pakai value lama, dicek eksplisit sebelum rename), `BusdevVerificationController`→`BusinessDevelopmentVerificationController`, `BusdevVerificationFeatureSeeder`→`BusinessDevelopmentVerificationFeatureSeeder`, permission+feature `busdev_verification.view`→`business_development_verification.view`, route+view folder `/busdev-verifications`→`/business-development-verifications`, test `BusdevVerificationGateTest`→`BusinessDevelopmentVerificationGateTest`. **UI tetap pakai singkatan "BD"** (bukan nama lengkap, sesuai permintaan eksplisit) — label "Menunggu Verifikasi BD", judul halaman "Verifikasi BD", teks "BD isi Biaya Instalasi". Regresi disapu ulang.

**Bug ketemu user (CID C00RQ002023, sesi sama)** — pelanggan kategori Bisnis, di halaman `verifications/{customer}/admin` kolom "BIAYA PEMASANGAN" (`extra_installation_fee`) ke-prefill OTOMATIS dari `internet_packages.installation_fee` (fitur lama, sudah ada sebelum ADHOC-67 — prefill base biaya instalasi paket). Ini kebentur sama modul baru: kalau CS asal terima prefill itu, biaya instalasi ke-charge DOBEL — sekali di Invoice Awal (CS), sekali lagi di tagihan Insidental (BD). Fix: `verifications/admin.blade.php` — kalau `$customer->needsBusdevInstallationFeeVerification()` true, input `extra_installation_fee` diganti readonly bertuliskan "0 — ditagih terpisah oleh BD" + `<input type="hidden" value="0">`, dengan catatan penjelasan. `CustomerVerificationController::finalVerify()` MENIMPA `$validated['extra_installation_fee']` jadi 0 di server buat kategori ini (bukan cuma disable di form — POST manual/replay tetap bisa kirim nilai apa pun), butuh eager-load `customerService.internetPackage` ditambah ke `showAdmin()` (sebelumnya cuma `customerService` datar + `internetPackage` legacy langsung di `customers`, bukan lewat `customerService`, jadi `Customer::packageCategory()` gak nemu apa-apa tanpa ini). **Nilai bawaan paket TETAP dipakai** — cuma dipindah ke tempat yang benar: `business-development-verifications/show.blade.php` sekarang prefill "Biaya Instalasi" dari `internet_packages.installation_fee` yang sama (bisa diubah BD kalau nego harga), jadi BD gak perlu lihat Master Paket buat tau angka bawaannya. Test baru: `BusinessDevelopmentVerificationGateTest::test_extra_installation_fee_is_forced_to_zero_for_bisnis_category_even_if_client_sends_nonzero` (POST manual kirim 250rb, invoice awal tetap tersimpan 0) + `test_business_development_show_page_prefills_installation_fee_from_package_base_value` (kebalikannya — nilai 250rb yang sama muncul di form BD). Regresi disapu: 72 test hijau (finalVerify/BD verification/prorate/notification/queue scope), pint bersih. Data pelanggan Testing 1 (CID C00RQ002023) sendiri belum tersentuh apa pun — statusnya masih `verification_admin`, belum sempat di-final-verify, jadi gak ada invoice/data yang perlu dikoreksi.

**Susulan (pertanyaan user, sesi sama)** — tombol submit `verifications/admin.blade.php` (id `btn-activate`) selalu bertuliskan "Aktivasi & Terbitkan Tagihan", padahal buat kategori Bisnis aksi itu TIDAK mengaktifkan pelanggan (nyangkut di antrean BD dulu) — label lama jadi bohong. Fix: label tombol kondisional — Home tetap "Aktivasi & Terbitkan Tagihan", Bisnis jadi **"Verifikasi & Terbitkan Tagihan"**. Ikut disamakan tempat lain yang sama-sama salah klaim "mengaktifkan": banner info "Langkah Terakhir" di atas form, dan dialog konfirmasi JS sebelum submit (judul + isi + teks tombol) — kedua dicabang lewat `data-needs-bd` di `<form id="verifyForm">` (dibaca JS) supaya gak nembak `@if` Blade di dalam template literal JS. Test baru: `BusinessDevelopmentVerificationGateTest::test_verification_admin_button_label_reflects_category` (Home lihat label lama, Bisnis lihat label baru & TIDAK lihat label lama). Regresi disapu: 83 test hijau, pint bersih.

**Susulan besar — halaman BD di-reuse dari `/verifications/{id}/admin` (permintaan user, sesi sama)** — sebelumnya `/business-development-verifications/{id}` (`show.blade.php`) cuma form minimal (satu input Biaya Instalasi), user eksplisit minta tampilannya SAMA PERSIS dengan halaman "Verifikasi & Pemasangan" CS (tab Registrasi/Survey/Pemasangan/Pengujian lengkap, badge status "Verifikasi BD"), cuma tab "Verifikasi"-nya yang beda isi (form BD, bukan form CS). Solusinya BUKAN duplikat halaman 1450 baris, tapi REUSE view `verifications/admin.blade.php` yang sama persis buat kedua controller — satu sumber kebenaran, gak ada dua page yang bisa saling menyimpang. Eager-load + kalkulasi material/alat kerja (~60 baris, sebelumnya cuma di `CustomerVerificationController::showAdmin()`) DIEKSTRAK ke service baru `CustomerVerificationDetailService::load()` — dua controller (CS & BD) manggil service yang sama, otorisasi TETAP di masing-masing controller (beda gerbang: CS pakai permission `customers.detail.installation.*` dkk, BD pakai `Customer::canInstallationFeeBeValidatedBy()` dinamis). `verifications/admin.blade.php` sendiri: (1) `$isVerifAdminStage` ditambah status `WAITING_BUSINESS_DEVELOPMENT_VERIFICATION` (tab Pengujian/Verifikasi tetap kebuka buat pelanggan tahap ini — datanya kan sudah final dari CS); (2) `$stageBadgeLabel`/breadcrumb dicabang match — badge "Verifikasi BD", breadcrumb ke `business-development-verifications.index`; (3) tab "Verifikasi" dibelah `@if($isWaitingBdStage) [cabang BD: ringkasan layanan + form installation_fee + tombol "Verifikasi & Aktifkan", GATED `canInstallationFeeBeValidatedBy()` — kalau gak berhak, tampil pesan read-only, TANPA tombol Tolak/Revisi sama sekali sesuai keputusan "tanpa jalur tolak"] @else [cabang CS existing, TIDAK diubah sama sekali]`. JS `calculateFees()` (dipanggil unconditional di `DOMContentLoaded`, sebelumnya gak ada null-check) ditambah early-return kalau `#billing_params` gak ada di DOM — SATU titik guard, bukan nyisir tiap pemanggil, karena elemen itu emang gak pernah dirender di cabang BD. `BusinessDevelopmentVerificationController::show()`: permission gate (`canInstallationFeeBeValidatedBy`, 404 kalau status bukan WAITING_BD) TIDAK DILONGGARKAN — dicoba dilonggarkan sebentar (supaya viewer non-BD bisa "liat aja") lalu DIBATALKAN karena bakal ngerusak test 403 yang udah dikunci sebelumnya; POP scope check ditambahkan (`show()` sebelumnya GAK ada, cuma `verify()` yang py — celah kecil ditutup sekalian). View lama `business-development-verifications/show.blade.php` DIHAPUS (dead code, gak ada pemanggil lain). Test baru: `test_business_development_show_page_reuses_verification_admin_view` (badge + semua tab-btn ada, form CS gak ikut render), `test_verification_admin_route_shows_bd_branch_for_waiting_bd_customer` (kebalikannya — akses lewat route CS pun tetap nunjukin cabang BD, murni berdasar status, bukan route). Diverifikasi juga end-to-end lewat curl ke pelanggan real (id 2005, CID C00RQ002023 — user sendiri berhasil `finalVerify()` set finish setelah fix kolom sebelumnya, invoice awal Rp373.333 terbit, status `waiting_business_development_verification`): halaman BD render 200, prefill Biaya Instalasi 250.000 dari harga bawaan paket, semua tab ada. Regresi disapu: 116 test hijau (targeted) + 591 test hijau (sapuan luas Customer/Verification/Survey/Installation, 4 gagal pra-existing tak tersentuh), pint bersih.

**Susulan (pertanyaan user, sesi sama)** — "kenapa gak ada data yang sudah diverifikasi admin?" — benar, cabang BD di atas cuma nampilin "Ringkasan Layanan" STATIS dari master paket (harga bulanan, diskon, PPN default), bukan hasil ACTUAL yang CS sudah proses & terbitkan (`finalVerify()`). BD gak bisa lihat tanggal aktivasi yang dipakai CS, prorata sesungguhnya, atau bukti "Biaya Pemasangan" di tagihan CS memang 0 (bukan kecatet dobel). Fix: `CustomerVerificationDetailService::load()` nambah query `$initialInvoice` (Invoice tipe AWAL/REAKTIVASI milik pelanggan itu, terbaru) ke hasil load — otomatis ikut ke DUA pemanggil (CS & BD, gak perlu ubah controller). Tab Verifikasi cabang BD (`verifications/admin.blade.php`) dapat blok baru "Hasil Verifikasi CS — {nomor invoice}" (badge status Lunas/Belum Dibayar, Tanggal Aktivasi, Prorata, Biaya Pemasangan CS, Total Tagihan Awal, link "Lihat Detail Tagihan Awal" ke `invoices.show`) — ditaruh SETELAH "Ringkasan Layanan" (paket) dan SEBELUM form isi Biaya Instalasi, pola tabel mengikuti blok serupa yang sudah ada di `customers/show.blade.php` ("RINCIAN TAGIHAN AWAL / REGISTRASI"). Test baru: `test_business_development_show_page_displays_the_already_issued_initial_invoice` (nomor invoice & link detail tagihan muncul di halaman BD). Diverifikasi live ke pelanggan real (id 2005): "Hasil Verifikasi CS — INV-20260914-6AA76D3A1DAA7" + "Total Tagihan Awal" muncul di halaman. Regresi disapu: 117 test hijau, pint bersih.

**Susulan besar — Invoice Awal kategori Bisnis DITUNDA sampai BD verifikasi (koreksi arsitektural, permintaan user, sesi sama)** — user menandai celah desain sejak ADHOC-67 awal: Invoice AWAL (tagihan registrasi) selama ini SELALU terbit di `finalVerify()` (CS), buat SEMUA kategori termasuk Bisnis — padahal niat aslinya begitu ada gate BD, tagihan pertama pelanggan Bisnis baru boleh sah terbit SETELAH BD juga menyetujui (ada Biaya Instalasi yang keputusannya mestinya disatukan). Sebelum fix ini, urutan sungguhannya: CS verifikasi → Invoice Awal LANGSUNG terbit → (kalau Bisnis) nyangkut nunggu BD cuma buat status+Biaya Instalasi. Fix membalik urutan itu murni buat kategori Bisnis: CS verifikasi → **hitungannya di-snapshot, invoice BELUM terbit** → BD verifikasi → Invoice Awal BARU terbit (bareng invoice Biaya Instalasi). Non-Bisnis TIDAK BERUBAH (tetap terbit langsung di CS). Mekanisme: migration `add_pending_initial_invoice_to_customers_table` — kolom JSON nullable `customers.pending_initial_invoice` (ditambahkan ke `#[Fillable]` model — Laravel 13 attribute, BUKAN `$fillable` klasik, ketauan dari test gagal "trying to access array offset on null" sebelum kolomnya ternyata gak pernah kesimpan) menyimpan `{billing: <hasil InitialInvoiceService::calculate()>, issue_date}` persis yang sudah dihitung & dikonfirmasi CS ke pelanggan — BUKAN dihitung ulang saat BD verifikasi (harga paket/diskon bisa berubah di antara dua titik waktu). Logika terbit-invoice DIEKSTRAK dari `finalVerify()` ke method baru `InitialInvoiceService::issue()` (satu rumus nomor/baris invoice, dipakai DUA titik tanpa duplikasi): `CustomerVerificationController::finalVerify()` panggil langsung buat non-Bisnis, kategori Bisnis cuma nyimpen snapshot ke kolom baru (invoice-nya SAMA SEKALI gak dibuat di titik ini — beda dari sebelumnya yang tetap bikin invoice cuma nunda status pelanggannya doang). `BusinessDevelopmentVerificationController::verify()` baca snapshot, panggil `InitialInvoiceService::issue()` dengan angka yang sama, TIGA efek dalam satu transaksi (invoice AWAL terbit, invoice Biaya Instalasi terbit — tetap terpisah, status→ACTIVE), snapshot ditimpa null setelah dipakai. Guard baru: `verify()` nolak 422 kalau `pending_initial_invoice` kosong (data cacat/dimanipulasi luar alur). Tab Verifikasi cabang BD (`verifications/admin.blade.php`) dapat cabang tampilan kedua — `@if($initialInvoice)` [invoice sungguhan, sudah ada sejak susulan sebelumnya] `@elseif($pendingInitialInvoice)` [BARU: kartu sama tapi badge "Belum Terbit" + keterangan "tagihan awal baru terbit setelah Anda verifikasi", angka dari snapshot bukan invoice DB] — dibedakan `CustomerVerificationDetailService::load()` yang sekarang expose kedua-duanya (`pendingInitialInvoice` null kalau `$initialInvoice` sudah ada, biar gak dobel tampil). Pesan Telegram/redirect/AuditLog kategori Bisnis di `finalVerify()` disamakan biar gak lagi ngaku "Tagihan pertama dibuat" — sekarang eksplisit "tagihan pertama BELUM terbit, menunggu BD". Test disesuaikan/ditambah di `BusinessDevelopmentVerificationGateTest`: assersi invoice-langsung-terbit-buat-Bisnis DIBALIK jadi assersi-tidak-ada (2 test), test baru `test_business_development_show_page_displays_pending_cs_verification_result` (badge "Belum Terbit" sebelum BD verifikasi) + `test_business_development_verify_issues_the_pending_initial_invoice` (invoice AWAL beneran kebentuk PERSIS setelah `verify()`, nominalnya sama dengan snapshot, snapshot ke-null-kan, invoice Biaya Instalasi tetap kena PPN layanan terpisah). Regresi disapu: 13 test file ini + 118 test sapuan luas (Customer/Verification/PackageCategory/InstallationFee/ManualInvoice/Prorate/Notification) hijau, pint bersih.

**Koreksi ketiga — 2 invoice DIGABUNG jadi SATU (bug ditemukan user, sesi sama, 2026-09-16)** — user menegaskan ulang requirement awal yang sempat ke-implementasi keliru: "tagihan itu nanti mencatat biaya yang di verifikasi admin **dan** biaya yang di Validasi Bussiness Development" — SATU tagihan, bukan dua. Susulan sebelumnya (2026-09-14) menunda invoice sampai BD verifikasi (BENAR), tapi begitu BD verifikasi malah menerbitkan **DUA invoice terpisah** (Invoice Awal via `InitialInvoiceService::issue()` + Invoice Biaya Instalasi via `InstallationFeeInvoiceService::issue()`, tipe INSIDENTAL) — user menunjukkan bukti nyata: `INV-20260914-6AA7A5612D38A` "Tagihan Awal (PSB)" dan `INV-202609-0002` "Tagihan Lain-lain" nyangkut di SATU pelanggan yang sama. Fix: `InitialInvoiceService` dapat method baru `withInstallationFee(array $billing, float $installationFee): array` — menyuntikkan nominal Biaya Instalasi BD ke `extra_installation_fee` snapshot CS (yang di `finalVerify()` dipaksa 0), lalu menghitung ULANG `subtotal`/`ppn_amount`/`total_amount` dengan rumus yang SAMA PERSIS dengan `calculate()` (bukan cuma menjumlah mentah dua angka — PPN harus ikut dihitung ulang di atas subtotal barunya). `BusinessDevelopmentVerificationController::verify()` dirombak: panggil `withInstallationFee()` dulu baru `issue()` SEKALI — `InstallationFeeInvoiceService` DICABUT total dari controller ini (import & dependency injection dihapus). `customer_acquisitions.installation_fee_invoice_id` sekarang diisi ID invoice AWAL yang sama (bukan invoice kedua). **`InstallationFeeInvoiceService` TIDAK dihapus** — tetap dipakai `CustomerAcquisitionController::updateInstallationFee()` (fallback pelanggan lama yang Invoice Awal-nya SUDAH terbit duluan/mungkin sudah lunas sebelum modul gate ini ada) — di jalur itu TETAP wajib invoice terpisah, menggabungkannya berarti menimpa tagihan yang mungkin sudah direkonsiliasi. Klarifikasi due date ("jatuh tempo salah, harusnya maks tanggal 10") ditanyakan balik lewat AskUserQuestion — user menjelaskan itu memang aturan Tagihan Bulanan yang SUDAH ADA (mulai tgl 1, jatuh tempo tgl 10, `GenerateMonthlyInvoicesCommand`), bukan aturan baru buat Invoice Awal — jadi TIDAK ada perubahan kode soal due date; masalah "tagihan nyangkut 2" itu sendiri yang menimbulkan kesan tanggalnya salah, dan sudah selesai dengan penggabungan invoice. View `verifications/admin.blade.php` kartu "Hasil Verifikasi CS" (cabang belum terbit) dapat catatan tambahan "digabung SATU invoice dengan Biaya Instalasi", label kolom total diperjelas jadi "Subtotal CS (Estimasi, Belum + Biaya Instalasi)". Test: `BusinessDevelopmentVerificationGateTest::test_business_development_verify_issues_one_merged_initial_invoice` (ganti nama dari versi lama, assert cuma 1 invoice per customer, `extra_installation_fee` = nominal BD, `total_amount` dihitung ulang benar, `installation_fee_invoice_id` = ID invoice yang sama) + `test_configured_role_can_verify_and_activate_with_prefilled_installation_fee` diupdate (assert `invoice_type` = `awal`, bukan `insidental`). Regresi disapu: 13 test file ini + 118 test sapuan luas (Customer/Verification/PackageCategory/InstallationFee/ManualInvoice/Prorate/Notification) hijau, pint bersih. Dokumentasi `docs/business-development/` (baru dibuat sehari sebelumnya) dikoreksi ulang di 4 file (README/business-logic/flowchart/database-schema) supaya gak mendokumentasikan desain 2-invoice yang sudah dibatalkan. | Done — 2026-09-16 |

#### ADHOC-54 — Modul Gudang/Inventory & Asset Traceability (2026-09-02)

**Kenapa di luar sprint:** Sprint 8.10 aktif ngerjain Audit Trail + Notification System (`S8.10-T003`, FOP Notification Dashboard) — gak ada irisan scope sama modul gudang. User eksplisit minta dikerjakan paralel, bukan gantiin sprint aktif.

**Dokumen rancangan (baca urut, tiap dokumen nambah lapisan ke yang sebelumnya):**
1. [`docs/plan/warehouse/warehouse_inventory_asset_traceability_analysis.md`](plan/warehouse/warehouse_inventory_asset_traceability_analysis.md) — struktur besar + koreksi ISP lokal (§29): hierarki gudang 2-level (Pusat/Cabang, bukan 3 termasuk mini_pop), RBAC lewat POP scope bukan role baru, larangan duplikasi tabel (`task_materials`/`customer_technical_details` udah ada, jangan bikin `installation_items`/`customer_assets`), 3 pertanyaan Batch C terjawab, tabel minimum Fase 1 (5 tabel: `warehouses` *(reuse `pops.id`, gak perlu tabel fisik terpisah)*, `inventory_balances`, `inventory_transactions`, `inventory_serials`, `technician_custody`).
2. [`_advanced.md`](plan/warehouse/warehouse_inventory_asset_traceability_analysis_advanced.md) — klasifikasi `tracking_type` (§16): 3 nilai bukan 7, boundary Asset vs Installable Device via `ownership_mode`, BATCH sebagai QUANTITY+`lot_no` (bukan genealogy penuh), `SOLD_TO_CUSTOMER` didrop (CPE company-owned dikonfirmasi user).
3. [`rancangan-ui.md`](plan/warehouse/rancangan-ui.md) — RBAC/permission konkret (`config/rbac.php` entries, `ActionCode::RECEIVE` baru), 9+ layar (reuse pola existing: stat cards ADHOC-14, tabel padat ADHOC-08/09, PRG redirect, `<x-material-rows>`), split form Aktif/Pasif (§3), `equipment_class` dua-level dgn accessor (§3.1), snapshot harga di ISSUE bukan submit (§3.5), reassign custody teknisi resign/cuti (§3.6), dua neraca custody-vs-gudang (§3.7), alur `lot_no`/`TechnicianCustody`/FIFO consumption (§3.8), matrix akses `fop` read-only (§1.3).
4. [`kontrol-anti-manipulasi.md`](plan/warehouse/kontrol-anti-manipulasi.md) — 7 kontrol anti-fraud dgn preseden kode existing (`CashDepositService` buat segregation of duty, `FileUploadService` buat bukti fisik), termasuk structural constraint + return reconciliation buat verifikasi material non-serial (§7) — TANPA threshold nominal/jadwal opname tetap, sesuai keputusan user (monitoring berbasis status barang, badge durasi, opname event-driven).

**Urutan implementasi Fase 1** (disepakati, belum dieksekusi): Enum → Migration (7 langkah) → RBAC seeder → Model → Observer (ledger append-only) → Service (Receive→Transfer→Issue→consumeFromCustody→Reassign→Adjustment, test tiap service) → integrasi ke form laporan existing + halaman Verifikasi FOP (ADHOC-28) → Controller/route/view → test regresi penuh.

**Catatan teknis krusial buat implementasi (jangan lewat pas coding):**
- DB default SQLite — `lockForUpdate()` gak ngasih row-lock beneran (grammar SQLite Laravel gak nerbitin `FOR UPDATE`), aman di skala sekarang karena SQLite ngunci whole-file per write-transaction, tapi jangan diasumsikan portable ke MySQL/Postgres tanpa recheck.
- Penomoran `TRF-`/`ISS-`/`REQ-` global (MAX+1, pola sama `TKT-`/`TFOP-`/`TASK-`) — **bukan** didaftarkan ke `docs/ID_NUMBERING_RULES.md` (itu khusus customer ID per-POP via `PopSequence`, beda mekanisme total). Daftarkan ke CLAUDE.md § Penomoran & ID pas implementasi nyata.
- `unit_price_snapshot` di `task_materials` disalin dari `inventory_transactions` (ISSUE), bukan diquery ulang ke harga master saat submit laporan — dan buat item BATCH, itu harga PER-LOT (bukan flat per-item).
- Ledger append-only ditegakkan di Observer (`InventoryTransaction::update()`/`delete()` ditolak), bukan cuma konvensi.

**Status implementasi (2026-09-02) — SELESAI, Fase 0-8 + ronde penutup:**
- **Fase 1 Enum** — `TrackingType`, `EquipmentClass`, `OwnershipMode`, `InventoryTransactionType` (+`INSTALL`), `SerialStatus` (12 case), `CustodyStatus`, `TransferStatus`; `ActionCode::RECEIVE`.
- **Fase 2 Migration** — 11 migration: kolom baru di `items`/`item_categories`, tabel `inventory_balances`/`inventory_serials`/`inventory_transfers`/`inventory_transactions`/`technician_custody`, `lot_no`+`unit_price_snapshot` di `task_materials`/`technician_custody`, `selected_inventory_serial_id` di `customer_installations`, `issued_from_pop_id` di `technician_custody`+`inventory_serials` (buat POP-scoping halaman Custody/Traceability).
- **Fase 3 RBAC** — `WarehouseFeatureSeeder` (7 feature: dashboard/transfer/issue/custody/traceability/adjustment/reassign), permission `warehouse*.*` di-assign ke `admin`+`pop_admin` lewat `RolePermissionSeeder`.
- **Fase 4 Model** — 5 model baru (`InventoryBalance`, `InventorySerial`, `InventoryTransfer`, `InventoryTransaction`, `TechnicianCustody`) + relasi tambahan di `Item`/`ItemCategory`/`Pop`/`FopTask`/`Customer`/`User`.
- **Fase 5 Observer** — `InventoryTransactionObserver` menolak `update()`/`delete()` (ledger append-only beneran, bukan konvensi).
- **Fase 6 Service** — `InventoryReceiveService`, `InventoryTransferService`, `InventoryIssueService`, `InventoryService` (`consumeFromCustody()` FIFO pooled per-tim, `installSerial()`, `reconcileMaterialsAgainstCustody()`), `InventoryReassignService`, `InventoryAdjustmentService`, `InsufficientCustodyException`.
- **Fase 7 Integrasi** — form Laporan Pemasangan/Maintenance dipecah Perangkat Aktif (dropdown SN) vs Material Pasif; `storeSpeedtest()`/`TaskMaintenanceController::store()` panggil `reconcileMaterialsAgainstCustody()` + `installSerial()`; Halaman Verifikasi Admin FOP menampilkan Perangkat Aktif terpasang. INFR/O-REQ dicek — sudah tercakup jalur generik `TaskMaintenanceController`, tidak perlu perubahan terpisah.
- **Fase 8 UI Owner/Admin** — `WarehouseController` (dashboard + low-stock alert), `WarehouseTransferController`, `WarehouseIssueController`, `WarehouseCustodyController`, `WarehouseTraceabilityController`, sidebar "Gudang & Inventory".
- **Ronde penutup ("Selesaikan")** — gap dari self-review ditutup: `WarehouseAdjustmentController`+`WarehouseReassignController` (lapor rusak/hilang/opname, reassign custody teknisi resign/rotasi — service-nya sudah ada sejak Fase 6, cuma belum ada UI-nya), akses FOP ke data Perangkat Aktif dipindah ke `fop_tasks/history_detail.blade.php` (halaman Verifikasi Admin ternyata digerbangi `customers.detail.installation.validate` yang FOP tidak punya — ketimbang melebarkan permission approval ke FOP, dipakai halaman FOP yang sudah ada), widget "Stok Saya" (custody+SN aktif) di `/tasks-saya`.
- **Bug ditemukan+diperbaiki saat verifikasi ronde penutup** (di luar scope asli, tapi blocking & production-nyata): `RolePermissionSeeder` dipanggil dua kali di `DatabaseSeeder` (sebelum & sesudah `TaskFeatureSeeder`) dan `sync()`-nya full-replace permission tiap role — akibatnya grant `task.view.own`/`task.execute` ke Teknisi dari `TaskFeatureSeeder` selalu ke-wipe oleh pemanggilan kedua, bikin Teknisi historically tidak pernah benar-benar bisa akses `/tasks-saya` dari fresh seed. Ditambahkan eksplisit ke array `teknisi` di `RolePermissionSeeder`.
- **Verifikasi**: `php artisan test`/`vendor/bin/phpunit` tidak bisa dieksekusi di shell sesi ini (PHP native Windows gagal resolve `vendor/autoload.php` lewat UNC path `\\wsl.localhost\...`) — file test PHPUnit tetap ditulis (`InstallationMaterialCustodyReconcileTest`, `MaintenanceMaterialCustodyReconcileTest`, `InventoryServiceLifecycleTest`, `InventoryTransactionAppendOnlyTest`, `WarehouseTransferAndIssueTest`, `WarehouseCustodyAndTraceabilityTest`), tapi verifikasi aktual tiap fase dilakukan lewat skrip tinker end-to-end (render view + assert state DB) terhadap sqlite `:memory:`. **Belum pernah dijalankan lewat PHPUnit runner beneran** — jalankan `php artisan test --compact` begitu lingkungan memungkinkan, sebelum anggap modul ini production-ready.
- **Belum diverifikasi visual di browser** — Alpine/Tailwind hasil `npm run build` belum dicek langsung; sebelum pakai di production jalankan `npm run build` dan cek tampilan Dashboard/Transfer/Issue/Custody/Traceability/Adjustment/Reassign/Receive.
- **Gap Receive ditutup (2026-09-02, ketauan pas user tanya cara input 100 SN modem ZTE)** — `InventoryReceiveService` (Fase 6) gak pernah dipanggil controller manapun sampai sekarang, jadi barang gak bisa masuk ke Gudang Pusat lewat UI sama sekali (cuma bisa Transfer/Issue barang yang udah ada). Ditambah `WarehouseReceiveController` + `/warehouse/receive/create` (`GET`)+`/warehouse/receive` (`POST`, permission REUSE `warehouse_transfer.create`), view `warehouse/receive/create.blade.php`, dan `<x-inventory-line-rows>` diperluas prop `:with-price="true"` (opsional, cuma dipakai Receive — Transfer/Issue gak berubah).
- **Gap kedua ditutup (masih 2026-09-02, user nanya "UI-nya udah bener?")** — Receive awalnya cuma flash-redirect ke Dashboard, TANPA reference_number & TANPA halaman detail — beda dari Transfer (`InventoryTransfer` header + `show()`) dan Issue (`ISS-...` + `show()`), gak bisa direview lagi barang apa aja yang masuk di satu faktur. Ditambah `InventoryReceiveService::receiveBatch()` (generate `RCV-YYYY-NNNN`, bungkus banyak item dalam satu faktur jadi satu reference — `receiveQuantity()`/`receiveSerialized()` primitif lama TETAP dipertahankan gak berubah, dipakai test existing), `WarehouseReceiveController::show()`, view `warehouse/receive/show.blade.php` (pola sama `warehouse/issues/show.blade.php`, tombol Cetak Bon). Tinker-verified: 100 SN + qty item dalam satu submit → satu reference number → halaman show nampilin semua baris + harga.
- **`WarehouseExampleSeeder`** (2026-09-02) — data contoh manual (`php artisan db:seed --class=WarehouseExampleSeeder`), demo alur penuh Receive→Transfer→Issue, idempotent buat item/POP/teknisi (skip re-seed ledger otomatis kalau SN demo udah ada, biar gak nabrak unique constraint).
- **3 koreksi UX ronde ketiga (2026-09-02, user: "Overall Gudang masih berantakan")** — ketauan 2 dari 3 keluhan itu BUG nyata, bukan cuma kurang rapi:
  1. **Master Barang gak bisa daftarin barang serialized sama sekali** — `ItemController`/form create-edit gak pernah punya field `tracking_type`/`ownership_mode`, jadi SEMUA barang lewat UI otomatis `quantity` (default migration) selama-lamanya; satu-satunya jalan bikin barang SN sebelumnya cuma tinker/seeder manual. Ditambah section "Cara Lacak Stok" (radio Quantity/Serial Number/Batch + select Kepemilikan buat Serialized) di `master/items/{create,edit}.blade.php` + validasi di `ItemController::validateItem()` (qty/batch dipaksa `ownership_mode=installable` — kombinasi ganjil "kabel jadi aset perusahaan" gak masuk akal). `tracking_type` DIKUNCI (readonly badge, bukan radio) begitu barang udah punya `inventory_transactions` — ganti cara hitung stok barang yang udah jalan bikin data historis gak konsisten, dicek via `$item->inventoryTransactions()->exists()`, ditegakkan server-side (bukan cuma disable UI).
  2. **`<x-inventory-line-rows>` grid overflow** — versi lama `grid-cols-12` kebagi item(4)+SN(6)+harga(3)+hapus(1) = 14 kolom pas `:with-price` aktif, numpuk berantakan. Dirombak jadi kartu 2-baris (barang di atas, detail SN/qty/lot/harga di bawah) — rapi berapa pun kombinasi field yang tampil. Ditambah counter live "N SN terdeteksi" (Alpine, hitung ulang tiap ketik/paste) — sebelumnya gak ada cara verifikasi "beneran ke-paste 100 SN apa nggak" sebelum submit.
  3. **Issue ribet** — admin harus hafal/nebak SN & qty yang ada di cabang sebelum ngetik manual, baru ketauan salah pas submit (`InvalidArgumentException`). Opsi yang dipilih user: panel referensi read-only (bukan picker penuh) — `WarehouseIssueController::availableStock()` (endpoint JSON baru, permission sama `warehouse_issue.create`) + panel "Stok Tersedia di Cabang Ini" di `warehouse/issues/create.blade.php` yang auto-fetch pas Cabang dipilih; klik chip SN langsung isi ke baris form lewat window event (`pick-serial` → `onPickSerial()` di `inventoryLineRows()`), qty/batch cukup ditampilkan sebagai referensi angka (gak bisa diklik, karena jumlah yang diambil tetap keputusan admin). Dropdown Teknisi TETAP lintas-cabang (keterbatasan struktural terdokumentasi — belum ada data "staf ditempatkan di cabang mana", bukan bug baru).
  Semua tinker-verified (render + assert JSON payload + assert lock behavior), gak ada regresi di alur Receive/Transfer/Issue existing.
- **Audit mendalam + perbaikan P0 (2026-09-02, user: "siapa tau dari dalam lebih kacau lagi")** — 3 agent audit paralel (views/UX, service logic, RBAC/scope) ketemu 40+ temuan. **P0 (keamanan/data-integrity kritis, dikerjakan) — 4 hal:**
  1. **POP-scope bypass sistemik** — controller LIST udah bener scope-nya, tapi HAMPIR SEMUA controller detail/mutasi (`WarehouseTransferController::show/receive`, `WarehouseIssueController::show/availableStock/store`, `WarehouseAdjustmentController` semua method, `WarehouseReassignController` semua method) gak re-check scope — dropdown POP di halaman create cuma penyaring tampilan, `pop_admin` scoped bisa lihat/tulis cabang lain langsung lewat id/pop_id. Ditambah trait `App\Http\Controllers\Warehouse\Concerns\AuthorizesWarehousePop` (`assertPopInScope()`/`assertPopIdInScope()`), dipakai di titik yang bolong.
  2. **Race condition `InventorySerial`** — qty path (`InventoryBalance`) selalu `lockForUpdate()`, serial path gak pernah. Ditambah di `InventoryIssueService::issueSerialized()`, `InventoryTransferService::dispatchSerialized()`, `InventoryService::installSerial()`, `InventoryReassignService::returnSerialToWarehouse()`/`transferSerialToTechnician()` — pola re-fetch+lock DI DALAM transaction yang udah ada.
  3. **`InventoryTransferService::receiveTransfer()` dobel-konfirmasi** — cek status dipindah ke dalam transaction + `lockForUpdate()` re-fetch, dari sebelumnya dicek di luar transaction pakai object yang bisa stale.
  4. **Guard `mini_pop`** — `InventoryAdjustmentService::adjustPopBalance()` dan `InventoryReassignService::returnToWarehouse()` sebelumnya gak pernah cek `Pop::isWarehouse()` (beda dari RECEIVE/TRANSFER/ISSUE yang udah nolak), bisa nulis balance ke `mini_pop`. Ditambah guard eksplisit.
  Ketemu juga 1 bug lazy-loading gak terkait (view `warehouse/issues/show.blade.php` butuh `createdBy` yang gak pernah di-eager-load) — sekalian dibenerin karena ketauan pas verifikasi.
  Tinker-verified: `pop_admin` scoped Cabang A dapet 403 di 10 titik (Transfer/Issue/Adjustment/Reassign lintas-cabang), owner (all-POP) & pop_admin ke cabang sendiri TETAP jalan normal (regresi negatif+positif), lock gak ganggu jalur sukses, mini_pop ditolak. Semua script tinker lama (`check_selesaikan.php`, `check_receive.php`, `check_ux_fixes.php`, `check_demo_seeder.php`) di-re-run, gak ada regresi.
  **P1 (10 bug fungsional) & P2 (9 UX/navigasi) — DITUNDA, belum dikerjakan sesi ini** (keputusan eksplisit user, scope P0 doang dulu):
  - P1: SN kosong disalahartikan qty=0 di `<x-inventory-line-rows>`; `adjustCustody()` bisa `status=RETURNED` tapi `qty_remaining>0`; `adjustSerialStatus()` gak cek status asal (SN `INSTALLED` bisa langsung di-LOST tanpa clear customer_id); `reconcileMaterialsAgainstCustody()` gak self-transaction; `reference_number` kosong buat REASSIGN/ADJUSTMENT; literal `'pusat'`/`'cabang'` di-hardcode alih-alih `Pop::isWarehouse()`; `resolveLastCost()` duplikat + gak filter by type; lock-ordering deadlock risk multi-line batch; `STOCK_OPNAME` enum dead code; `confirmedQty` negatif diserap diam-diam di `receiveTransfer()`. Race kondisi nomor referensi (`MAX+1` gak locked) SENGAJA masuk P1 bukan P0 — pola sama dipakai `TicketService`/`TaskService`/`FopTaskProvisioningService` di seluruh repo, bukan risiko baru Gudang perkenalkan sendiri.
  - P2: sidebar gak ke-cover role `warehouse_adjustment`/`warehouse_reassign` doang; dot timeline Traceability gak `position:relative` (numpuk, bukan sejajar); Transfer create gak py panel stok kayak Issue; qty/lot di panel Issue gak bisa diklik (beda dari SN chip); SN di Custody/show pages gak link ke Traceability; placeholder `reason` gak konsisten 5 form; form Adjustment Balance gak nunjukin stok saat ini; field wajib Reassign gak `required`; tabel "Sudah Diterima" Transfer-show gak py `<thead>`.
- **P1 selesai (2026-09-02) — 10 bug fungsional:**
  1. **SN kosong disalahartikan qty=0** — `normalizeLines()` di 3 controller (Transfer/Issue/Receive) sekarang query `tracking_type` BARANG dari DB dulu (bukan nebak dari ada/enggaknya key request), tolak jelas "SN wajib diisi, gak boleh kosong" kalau serialized tapi kosong, dan tolak "bukan tipe Serial Number" kalau qty item dikirimin serial_numbers. **Bug tersembunyi ketauan pas verifikasi**: `pluck()` di Eloquent Builder ikut nerapin cast model — perbandingan awal `=== TrackingType::SERIALIZED->value` (enum vs string) gak akan pernah match, harus `=== TrackingType::SERIALIZED` (enum vs enum).
  2. **`adjustCustody()` status ganjil** — custody yang qty_remaining-nya balik positif ("ditemukan lagi") padahal status sebelumnya RETURNED/CONSUMED (terminal), sekarang ditarik balik ke PARTIALLY_USED.
  3. **`adjustSerialStatus()` gak cek status asal** — SCRAPPED sekarang jadi TERMINAL (gak bisa diubah lagi), status-sama-ke-status-sama ditolak, dan transisi dari INSTALLED ngosongin `customer_id`/`fop_task_id`/`installed_at` (biar gak nyangkut "masih keliatan terpasang" padahal statusnya udah LOST/DAMAGED/dst) — `current_pop_id`/`current_technician_id` TETAP dibiarkan (forensik "terakhir di mana").
  4. **`reconcileMaterialsAgainstCustody()` dibungkus `DB::transaction()` sendiri** — gak gantung disiplin caller lagi.
  5. **`reference_number` ditambah buat REASSIGN (`RSG-YYYY-NNNN`) & ADJUSTMENT (`ADJ-YYYY-NNNN`)** — pola `MAX+1` sama persis RECEIVE/TRANSFER/ISSUE (race generate-nya sengaja gak dibenahi, sama seperti keputusan P0/P1).
  6. **`Pop::isPusat()`/`isCabang()` baru** — 3 Service ganti literal `$pop->type !== 'pusat'/'cabang'` jadi manggil method ini; `InventoryTransferService::assertType()` generik dihapus, diganti 2 pengecekan eksplisit.
  7. **`resolveLastCost()` difilter `type=RECEIVE`** — sebelumnya `latest('id')` tanpa filter type bisa kebaca snapshot dari RETURN/TRANSFER_CUSTODY yang baris ledger-nya kebetulan lebih baru tapi harganya lebih LAMA dari RECEIVE terakhir.
  8. **Lock-ordering deadlock risk** — `receiveBatch()`/`createTransfer()`/`issue()` sekarang `usort()` baris per `item_id` sebelum lock diambil (dulu ikut urutan form apa adanya).
  9. **`STOCK_OPNAME` didokumentasikan ulang sbg RESERVED** (bukan dihapus, bukan diwire) — jujur diakui belum reachable dari Service manapun, ditandai eksplisit "jangan dipakai sampai Fase 2 opname beneran dibangun" ketimbang dibiarkan mendokumentasikan perilaku yang gak ada.
  10. **`confirmedQty` negatif di `receiveTransfer()` ditolak eksplisit** (`InvalidArgumentException`), bukan diserap diam-diam jadi skip.
  Tinker-verified semua 10 poin + regresi 4 script lama (`check_p0_security.php`, `check_ux_fixes.php`, `check_receive.php`, `check_selesaikan.php`, `check_demo_seeder.php`) tetap lolos.
- **P2 selesai (2026-09-02) — 9 poin UX/navigasi, audit mendalam Gudang tuntas semua tier (P0+P1+P2):**
  1. **Sidebar Gudang gak ke-cover role `warehouse_adjustment`/`warehouse_reassign`** — gate `@if` di `layouts/app.blade.php` ditambah 2 permission itu, plus link baru "Lapor Rusak/Hilang/Opname" (submenu Gudang) — sebelumnya cuma reachable lewat Dashboard/Custody.
  2. **Dot timeline Traceability numpuk** — `<li class="ml-4">` jadi `<li class="relative ml-4">` (dot child `absolute` butuh ancestor `relative`, sebelumnya ke-`relative`-in ke `<ol>` alih-alih tiap `<li>`).
  3. **Transfer create sekarang py panel "Stok Tersedia di Pusat Ini"** — endpoint baru `WarehouseTransferController::availableStock()` (pola sama `WarehouseIssueController`) + route `warehouse.transfers.available-stock`, panel React ke `from_pop_id` (bukan cabang).
  4. **Qty/lot di panel Stok Tersedia sekarang bisa diklik** (dulu cuma teks referensi, beda perlakuan dari SN chip) — event `pick-qty` baru di `inventoryLineRows()` (shared component), cocokkan `item_id`+`lot_no` (bukan `item_id` doang, biar BATCH multi-lot gak ketuker), qty diisi PENUH sebesar yang tersedia (bisa dikurangi manual).
  5. **SN di halaman Custody/Transfer-show/Issue-show/Receive-show sekarang link ke Traceability** (`?sn=`), gated `warehouse_traceability.view`.
  6. **Reason field 5 form (Adjustment×3, Reassign×2) sekarang py `<datalist>` saran konsisten** (snake_case: `rusak_terjatuh`/`hilang_transit`/`selisih_opname`/`rusak_kelembaban` buat kerugian fisik; `resign`/`cuti`/`rotasi`/`mutasi_cabang` buat reassign) — tetap free-text (gak dipaksa), placeholder & suggestion konsisten lintas form.
  7. **Form Adjustment Balance sekarang nunjukin stok saat ini** kalau dibuka lewat link "Sesuaikan" Dashboard (bawa `pop_id`+`item_id` di query) — pola sama Custody/Serial adjustment yang udah nunjukin duluan. Gak live-update kalau dropdown diganti manual (bukan AJAX, disclose eksplisit di komentar controller).
  8. **Field wajib Reassign (`cabang_pop_id`/`new_technician_id`) sekarang `required`**, di-toggle JS ngikutin field yang lagi tampil (radio return/transfer) — sebelumnya dua-duanya gak required sama sekali, validasi HTML5 gak pernah kena.
  9. **Tabel "Sudah Diterima" Transfer-show sekarang py `<thead>`** — konsisten sama tabel "Barang Dikirim" di atasnya.
  Tinker-verified semua 9 poin + regresi 5 script lama (P0/P1/UX/Receive/demo seeder) tetap lolos.
- **Tata ulang layout Gudang + audit logic tombol konfirmasi (2026-09-03, laporan user "list/detail tersembunyi")** — investigasi nemu IA Gudang sebelumnya (Dashboard + Management Stock + header tab `x-warehouse.header`, hasil kerja sesi ini yang gak sempat ke-summary) udah bagus, TAPI satu gap sistemik: **gak ada satupun cara balik ke halaman detail Transfer/Issue/Receive** sesudah redirect awal pas create/konfirmasi — Dashboard cuma nampilin 25 baris ledger TANPA link ke dokumen sumbernya (cuma sub-link SN ke Traceability), jadi begitu tergeser dari 25 baris itu, dokumennya ilang gak ke-reach lagi kecuali nebak URL. Sekalian ketauan bug: `$typeBadge` di Dashboard nyocokin ke string sintetis (`dispatch_transfer`/`issue_technician` dst) yang gak pernah match `App\Enums\InventoryTransactionType` asli, badge SELALU jatuh ke default abu-abu.
  - **`WarehouseHistoryController` (baru)** + halaman **Riwayat Mutasi** (`/warehouse/history`, tab baru di `x-warehouse.header`) — paginasi 30/halaman, filter tipe+gudang, tiap baris klik langsung ke `warehouse.transfers.show`/`warehouse.issues.show`/`warehouse.receive.show` sesuai tipe transaksinya. ADHOC ini yang jadi jawaban utuh buat "list/detail tersembunyi", bukan cuma nge-tambal Dashboard.
  - **Dashboard `$typeBadge` dibenerin** (cocok ke value enum asli) + baris ledger-nya sekarang KLIK-ABLE ke dokumen sumber (sama logic-nya kayak History) + link "Lihat Semua Riwayat →" ganti dari nyasar ke Management Stok jadi ke halaman History yang bener.
  - **Logic tombol "Konfirmasi Penerimaan Transfer" diperiksa ulang** — backend (P0 lockForUpdate + scope assert di `receive()`) udah bener & masih utuh. Ketauan 1 inkonsistensi UI: `transfers/show.blade.php` nampilin tombol Konfirmasi cuma berdasar PERMISSION (`warehouse_transfer.receive`), TANPA cek SCOPE — seorang admin yang bisa buka `show()` dari sisi Pusat (Pusat ada di scope-nya) tetap ngeliat tombol Konfirmasi buat transfer ke Cabang yang sebenernya DI LUAR scope-nya, walau `receive()` bakal nolak 403 begitu beneran diklik. Ditambah `$canReceive` (dihitung `WarehouseTransferController::show()`, pola sama `assertPopInScope`) buat nyembunyiin tombol kalau bakal ditolak — tombol sekarang cuma nongol kalau aksinya beneran bakal berhasil.
  Tinker-verified: Dashboard+History badge & link bener, filter History jalan, tombol Konfirmasi nongol/kesembunyi sesuai scope, gak ada regresi di P0/receive/demo-seeder.
- **Format nomor referensi Gudang diganti (2026-09-03, keputusan eksplisit user)** — `RCV-`/`TRF-`/`ISS-`/`ADJ-`/`RSG-` sebelumnya `{PREFIX}-{tahun}-{4 digit}` (reset tahunan), diganti `{PREFIX}-{YYYYMMDD}-{6 digit}` — stempel TANGGAL PENUH (gampang dibaca "kapan persisnya"), tapi lingkup reset counter-nya per BULAN (bukan per hari/tahun): angka terus nambah tiap hari dalam bulan yang sama, baru balik ke `000001` pas bulan berganti. Diterapkan konsisten di 5 `generateReferenceNumber()` (`InventoryReceiveService`/`InventoryTransferService`/`InventoryIssueService`/`InventoryAdjustmentService`/`InventoryReassignService`) — query `LIKE '{PREFIX}-{yearMonth}%'` (6 digit YYYYMM, bukan 8 digit YYYYMMDD) buat nyakup semua tanggal dalam bulan yang sama tanpa date-range query. Kolom `reference_number` (varchar 30) masih cukup buat format baru (20 karakter). Tinker-verified: format bener, counter lanjut lintas hari dalam bulan sama, reference bulan lalu (angka gede) TERISOLASI gak ke-drag ke bulan ini, reset ke 1 begitu bulan ganti (diverifikasi lewat isolasi query, bukan fake system clock — `date()` native gak respect `Carbon::setTestNow()`), gak ada regresi P0/P1/demo-seeder.
- **`ItemSeeder`/`ItemCategorySeeder` diganti barang riil ISP (2026-09-02)** — versi lama cuma 11 barang generik tanpa `tracking_type`/`ownership_mode` sama sekali (semua diam-diam `quantity`/`installable`), gak ada satupun contoh barang aktif ke-seed. Diganti 34 barang nyata: modem ONT ZTE/Huawei/Fiberhome, radio wireless Ubiquiti (NanoStation/LiteBeam/PowerBeam), router Mikrotik, media converter, splitter/ODP/ODC, kabel dropcore+feeder, patch cord, aksesoris pasang — masing-masing `tracking_type`/`ownership_mode` sesuai jenisnya (modem/radio/router = `serialized`; kabel feeder = `batch`; sisanya `quantity`), `equipment_class_override` dicontohkan di Antena Grid (kategori `antena_radio` defaultnya aktif, tapi antena grid piringan itu sendiri pasif — gak ada elektronik). **`ItemCategorySeeder` (baru)** nambah 2 kategori NON-SYSTEM (`modem_ont`, `router_gateway`, keputusan eksplisit user) — dipisah dari `media_converter` bawaan biar laporan bisa bedain ONT pelanggan vs media converter murni vs router infra; 7 kategori bawaan (migration-baked, `is_system=true`) TIDAK disentuh. Tinker-verified: 9 kategori total, klasifikasi tiap barang bener, idempotent, `CustomerSurveyController`'s hardcode `kabel_dropcore` gak kesentuh, gak ada regresi di `WarehouseExampleSeeder`.
- **Tutup gap siklus Install → Terminate → Retrieve (2026-09-03, ketauan dari pertanyaan user "trackingnya sampai mana")** — audit `SerialStatus` nemuin siklus `InventorySerial` gak pernah nutup: begitu SN `INSTALLED` di pelanggan, satu-satunya jalur keluar sebelumnya cuma "Lapor Rusak/Hilang" (`adjustSerialStatus`). Task **Ambil Modem (DEAC)** yang selesai cuma nandain `customer_devices.device_retrieved_at` (tabel legacy) — `InventorySerial`-nya sendiri permanen macet di `INSTALLED` walau device fisik udah beneran balik ke gudang, gak pernah muncul lagi di dropdown SN Laporan Pemasangan pelanggan lain.
  - `InventoryReassignService::returnInstalledSerialFromCustomer()` (baru) — terima SN asal `INSTALLED` (beda dari `returnSerialToWarehouse()` yang asalnya `ISSUED`/custody teknisi), tujuan gudang diambil dari `issued_from_pop_id` SN itu sendiri (gudang cabang asal ISSUE terakhir — bukan `pop_id` task retrieval, yang bisa jadi mini_pop/kecamatan), clear `customer_id`/`fop_task_id`/`installed_at`/`current_technician_id`, tulis ledger `RETURN` bertaut `fop_task_id` ke FopTask instalasi lama biar Traceability tetap runut.
  - `TaskService::complete()` — begitu task `AMBIL_MODEM` selesai, selain update `device_retrieved_at` (legacy, tetap dipertahankan), sekarang juga panggil method di atas buat tiap `InventorySerial` yang `customer_id`-nya cocok & masih `INSTALLED`. SN yang gak pernah lewat modul Inventory (data lama/manual) otomatis diabaikan — query kosong.
  Tinker-verified: rangkaian penuh Receive→Transfer→Issue→Install→(task Ambil Modem selesai) — SN balik `AVAILABLE` di Cabang asal, semua kolom pelanggan ke-clear, ledger RETURN tercatat+bertaut FopTask lama, SN bisa di-issue lagi, guard nolak SN yang bukan INSTALLED, gak ada regresi P0/format nomor referensi.
  **4 status `SerialStatus` (`RECEIVED`/`RESERVED`/`IN_USE`/`RETURNED`) tetap dead code** — gak ada Service yang pernah nulis status itu (di luar scope pertanyaan user, dicatat aja biar gak dikira kelupaan).

**Yang sengaja ditunda ke Fase 2** (jangan dibangun sekarang): approval berlapis Stock Request, Quarantine/Repair terstruktur, reorder alert otomatis, Inventory Control Tower dashboard enterprise.
...
#### ADHOC-55 — Fase 2 Gudang, Prioritas 1: Bukti Fisik & Stock Opname (2026-09-03)

**Kenapa:** rancangan Fase 2 (`docs/plan/warehouse/fase-2-adaptasi-wms.md`) nemuin gap resiko fraud nyata di `InventoryAdjustmentService` — klaim LOST/DAMAGED/SCRAPPED cuma modal `reason` teks bebas, gak ada guard bukti fisik. Dieksekusi atas permintaan eksplisit user ("kerjakan P1 dengan benar"), P2-P5 tetap ditunda (nilai/urgensi lebih rendah, dikonfirmasi ke user).

**Perubahan:**
- Migration `evidence_file_path` (nullable) di `inventory_transactions`.
- `InventoryAdjustmentService::adjustSerialStatus()` — guard baru: transisi ke LOST/DAMAGED/SCRAPPED wajib `evidence_file_path` terisi, QUARANTINE tetap boleh tanpa foto.
- `InventoryAdjustmentService::adjustCustody()` — `reason` sekarang wajib salah satu `REASON_CATEGORIES` (`lost`/`damaged`/`quarantine`/`shrinkage_on_return`/`stock_opname_diff`, bukan teks bebas lagi), evidence wajib buat kategori `lost`/`damaged`.
- `InventoryAdjustmentService::recordStockOpname()` — method baru, dipisah total dari `adjustPopBalance()`: input JUMLAH FISIK hasil hitung (bukan delta), selisih dihitung sistem, hasil PAS (selisih nol) TETAP tercatat sebagai baris ledger `STOCK_OPNAME` (satu-satunya jalur yang boleh qty nol) — mengaktifkan `InventoryTransactionType::STOCK_OPNAME` yang sejak Fase 1 didefinisikan tapi sengaja belum dipakai.
- `FileUploadService::uploadWarehouseEvidence()` — method baru (folder `warehouse/evidence/{type}`), dipakai upload foto kondisi/BAP dari `WarehouseAdjustmentController::storeSerial()`/`storeCustody()`.
- Controller baru: `WarehouseAdjustmentController::createOpname()`/`storeOpname()` + view `warehouse.adjustments.opname` + route `warehouse.adjustments.opname.{create,store}` (permission reuse `warehouse_adjustment.create`).
- `WarehouseStockController`/`warehouse/stock/index.blade.php` (Management Stock) — kolom baru "Opname terakhir" per baris (query `MAX(created_at)` ter-scope ke item/pop yang tampil di halaman, bukan full-table scan); item yang belum pernah diopname tampil beda ("Belum pernah", bukan disamakan dengan yang baru opname).
- `<x-warehouse.header>` — menu "Aksi Lainnya" nambah entri "Stock Opname" terpisah dari "Penyesuaian Stok" (dua tujuan beda: opname vs koreksi manual).

**Test:** `InventoryAdjustmentEvidenceRequiredTest` (10 test, Service-level — guard evidence per kombinasi status/reason), `WarehouseAdjustmentEvidenceUploadTest` (4 test, HTTP-level — validasi `required_if` + file beneran tersimpan), `WarehouseStockOpnameTest` (7 test — `recordStockOpname()` selisih nol/kurang/lebih/negatif, flow HTTP penuh, tampilan "opname terakhir"/"Belum pernah" di Management Stock, permission gate).

#### ADHOC-55 — Fase 2 Gudang, Prioritas 2: Laporan Agregat Periodik (2026-09-03)

**Kenapa:** data pergerakan & kerugian gudang udah lengkap tercatat di ledger sejak Fase 1, tapi Dashboard cuma nampilin snapshot titik-waktu (25 baris ledger terakhir campur semua tipe) — gak ada cara lihat tren per gudang per bulan tanpa scroll manual. Dieksekusi lanjutan langsung setelah P1 atas permintaan user.

**Perubahan:**
- `WarehouseReportController::index()` — SATU halaman dua tab (Pergerakan Barang / Kerugian), filter `period` (bulan) + `pop_id`.
- Tab **Pergerakan Barang**: agregat qty RECEIVE/TRANSFER-masuk/TRANSFER-keluar/ISSUE per gudang per bulan. Atribusi POP akurat penuh (`from_pop_id`/`to_pop_id` selalu keisi buat 3 tipe ini). Gudang tanpa pergerakan di periode itu gak tampil (bukan baris nol).
- Tab **Kerugian**: rekap `ADJUSTMENT` per kategori `reason` (`InventoryAdjustmentService::REASON_CATEGORIES`) per bulan. **Keterbatasan didokumentasikan eksplisit** (bukan disembunyikan): klaim lewat `adjustCustody()`/`adjustSerialStatus()` pas barang di custody teknisi gak punya `pop_id` sama sekali di ledger-nya (`current_pop_id` null by design pas di tangan teknisi) — baris begini tampil "— (Custody Teknisi)", BUKAN dipaksa nebak cabang asalnya. Filter `pop_id` eksplisit sengaja gak ikutin baris tanpa atribusi ini.
- Permission baru `warehouse_report.view` (`config/rbac.php`, `WarehouseFeatureSeeder` — 8 feature root gudang sekarang) — digrant ke `owner` (wildcard), `atasan`, `admin`, `pop_admin` (scope cabangnya sendiri via `EffectiveAccessService`). **Sengaja TIDAK** ke `fop`/`noc`/`teknisi` (laporan agregat gudang di luar kebutuhan role-role itu, konsisten pembatasan Fase 1 §1.3 rancangan-ui.md).
- Nav: tab "Laporan" di `<x-warehouse.header>` + entri "Laporan Gudang" di sidebar Gudang.

**Realtime tetap SKIP** — keputusan sadar Fase 1 (rancangan-ui.md §2.1), volume transaksi gudang ISP lokal gak sebanding kompleksitas broadcast channel tambahan.

**Test:** `WarehouseReportMovementSummaryTest` (7 test — agregat benar per gudang, transaksi luar periode gak ikut, gudang tanpa pergerakan gak muncul, filter pop_id, POP scope pop_admin, permission gate), `WarehouseReportAdjustmentSummaryTest` (5 test — rekap custody tanpa atribusi pop, rekap pop-balance teratribusi benar, transaksi luar periode gak ikut, filter pop_id eksplisit exclude baris custody, permission gate).

#### ADHOC-57 — Ambang Stok Rendah + Permintaan Stok Cabang→Pusat (2026-09-03)

**Kenapa:** user tanya eksplisit — "kalau POP Jetis (cabang) discope ke satu admin, dia cuma bisa kelola Jetis doang kan? Terus kalau stok Jetis habis dan admin Pusat gak sadar, gimana?" Audit jawabannya nemuin DUA lubang nyata: (1) `InventoryBalance::minimum_stock` — kolomnya ADA & dipakai `isLowStock()`/badge "Stok Rendah", tapi **gak ada form manapun buat ngisinya** di seluruh codebase (grep bersih, cuma dibaca gak pernah ditulis) — badge itu praktis MATI di semua gudang; (2) walau (1) dibenerin, tetap PASIF (pull) — Pusat harus buka halaman & notice sendiri, gak ada sinyal aktif. Dikerjakan dua-duanya atas permintaan eksplisit user ("kerjakan keduanya").

**Perubahan 1 — Ambang Stok Rendah (`WarehouseStockController`):**
- `createThreshold()`/`storeThreshold()` — form isi `minimum_stock`/`maximum_stock` per gudang+barang(+lot). **SENGAJA bukan lewat `InventoryAdjustmentService`/ledger** — ini konfigurasi ambang, bukan pergerakan barang; `qty` gak pernah disentuh method ini, cuma dua kolom threshold. Route baru reuse permission `warehouse_adjustment.create` (bukan permission baru).
- Tombol "Ambang" baru di baris tabel Management Stock, sebelah "Sesuaikan".

**Perubahan 2 — Permintaan Stok (modul baru):**
- Migration `stock_requests` (header: `reference_number` REQ-YYYYMMDD-NNNNNN, `cabang_pop_id`, `status`, `requested_by`, `decided_by`/`decided_at`/`decision_notes`) + `stock_request_items` (barang+qty per baris).
- `StockRequestStatus` enum (pending/fulfilled/rejected/cancelled). `StockRequestService` (create/fulfill/reject/cancel) — **SENGAJA cuma tiket komunikasi**, gak ada method yang nyentuh `InventoryBalance`/`inventory_transactions` sama sekali (prinsip "jangan bikin sumber kebenaran kedua"). Fulfilled cuma nandain status; Transfer sungguhan tetap dibikin terpisah lewat `WarehouseTransferController` existing — halaman detail kasih link langsung ke situ.
- `WarehouseStockRequestController` — index (Pusat lihat SEMUA pending lintas cabang dalam scope-nya, pop_admin cuma lihat cabangnya sendiri), create/store (cabang ajukan), show, fulfill/reject (Pusat), cancel (**cuma pengaju sendiri** — dicek eksplisit di controller, `auth()->id() === requested_by`, BUKAN cuma permission, karena permission `.cancel` dipegang seluruh role `pop_admin` secara umum).
- Permission baru `warehouse_stock_request.{view,create,approve,reject,cancel}` (`config/rbac.php`, `WarehouseFeatureSeeder` — 9 feature root gudang sekarang). Grant: `owner` wildcard; `atasan` view doang (monitor); `admin` view+create+approve+reject (operasional Pusat penuh); `pop_admin` view+create+cancel (ajuin & batalin punya sendiri, TANPA approve/reject — itu keputusan Pusat).
- Nav: tab "Permintaan Stok" di `<x-warehouse.header>` + sidebar.

**Test:** `WarehouseStockThresholdTest` (5 test — set minimum bikin `isLowStock()` jadi true, muncul di filter low_stock_only, POP scope pop_admin, permission gate), `StockRequestFlowTest` (10 test — cabang ajukan, Pusat lihat lintas cabang tanpa diberitahu manual, POP scope pop_admin gak lihat cabang lain, fulfill, reject wajib alasan, pengaju batalin punya sendiri, BUKAN pengaju gak bisa batalin punya orang lain, request yang udah diputuskan gak bisa diproses ulang, permission gate).
...
| `InvoiceController@store` (ADHOC-70, Tagihan Manual) | `amount` |
...
#### ADHOC-25 — OCR kwitansi: hasil tebakan tak boleh jadi keputusan (DITUTUP)

> **Ditutup 2026-09-26 (ADHOC-99):** OCR Gemini dihapus dari kode. Catatan di bawah dipertahankan
> sebagai arsip keputusan — kalau OCR mau dihidupkan lagi, lubang ini wajib ditutup dulu.
...
> ⚠️ Lubangnya dorman selama OCR mati, dan hidup tepat
...
> OCR Gemini dihapus 2026-09-26 (ADHOC-99) — driver Gemini di daftar bawah sudah tidak ada.
