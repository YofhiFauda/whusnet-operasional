...
| ADHOC-40 | **Rancangan API Eksternal** — (1) **webhook pemasangan** outbound: satu event `installation.activated`, dipicu **tombol Aktivasi** di Laporan Pemasangan (`storePemasangan()`, step 5→6) karena di titik itulah Nama/POP/Desa/Paket/SN/ODP lengkap sekaligus; HMAC, outbox+retry, `idempotency_key` = id baris `customer_installations` (Aktivasi bisa ditekan berulang untuk koreksi & revisi). Konsumen pertama: gateway AI agent (OpenClaw) → Telegram — Whusnet berhenti di POST, nol kode Telegram. (2) **portal pelanggan** — merinci kontrak QR §6.6 yang sudah dikonfirmasi pemilik produk (`login_id` `{prefix_pop}-{customer_code}`, `customer_portal_accounts` + `customer_portal_tokens` terpisah, klaim via PIN, kwitansi lewat `ReceiptPresenter` dipangkas, status tiket lewat `Ticket::resolveStatus()`, `invoice.updated` dari `Invoice::recalculateFromPayments()`). Repo belum punya API layer sama sekali. **Nol dependensi baru.** Dokumen: [`docs/api/`](api/README.md). Rev. 2 (15 temuan review ditambal, 2026-08-18) · Rev. 3 (titik pemicu dikoreksi ke tombol Aktivasi, 2026-08-19) | **Rancangan** — 2026-08-19 |
