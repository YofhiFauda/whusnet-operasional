...
- `InstallationWebhookPresenter` + listener → satu baris outbox **per endpoint** di
  dalam transaksi; job pengirim `afterCommit`.
- **Dua transport**: `http_json` (Website B — HMAC, JSON) dan `telegram`
  (Telegram Eksternal — bot token & chat id dari `webhook_endpoints.config`, **bukan**
  dari `config('services.telegram.*')`).
...
- Job pengirim: backoff 1m/5m/30m/2j/6j maks 8x, penonaktifan endpoint setelah gagal
  beruntun. Sukses `http_json` = 2xx; sukses `telegram` = `ok: true` di body.
...
| `WebhookTelegramUsesEndpointCredentialsTest` | Transport `telegram` memakai `bot_token`/`chat_id` dari `webhook_endpoints.config`, **bukan** `config('services.telegram.*')` — penjaga langsung atas pemisahan internal/eksternal |
