...
| `terminated` | `active` (khusus "Langganan Lagi", ADHOC-85 — sebelumnya final; lihat §8 & [`../plan/billing/rancangan-terminate-reactivate-state-machine.md`](../plan/billing/rancangan-terminate-reactivate-state-machine.md)) |
...
- **Penulis kedua sejak 2026-09-14: `CustomerController::update()` (Edit Pelanggan).** Step "Parameter Teknis" di `/customers/{id}/edit` sekarang juga nulis ke `customer_devices`/`customer_technical_details` (subset field yang sama: device_type, brand, model, serial_number, mac_address, wifi_ssid/password, pppoe_username/password, odp_number/port, olt_number/slot/port, vlan, router_number, initial_attenuation) — jalur admin **tanpa** gerbang workflow (gak perlu status `installation_in_progress`, gak perlu jadi anggota tim task, semua field nullable). Keputusan sadar: field ini kadang perlu dikoreksi admin di luar siklus laporan teknisi. **Risikonya nyata** — dua controller independen bisa saling timpa kalau admin & teknisi edit bersamaan; kalau ke depan risiko ini gak bisa diterima, jalur write dari Edit Pelanggan yang harus dicabut (jadi read-only + link ke Laporan Pemasangan), bukan sebaliknya.
...
- **Bayar di muka (rancangan ADHOC-92, belum diimplementasi):** pelanggan boleh membayar Invoice AWAL + beberapa bulan sekaligus. Tagihan AWAL tetap sebesar prorate; kelebihan bayar masuk Saldo Pelanggan (mekanisme overpay ADHOC-38 sudah menjalankan ini) dan nantinya dipakai otomatis untuk tagihan BULANAN saat terbit. Lihat [analisa-rancangan-saldo-pelanggan.md](../plan/billing/analisa-rancangan-saldo-pelanggan.md).
...
## 8. Terminasi Layanan (`CustomerTerminationController` → `CustomerTerminationService`, denda ADHOC-69)

- Controller cuma validasi + guard state machine; semua business logic ada di `CustomerTerminationService::terminate()` (CLAUDE.md "semua business logic di Service") — set `customer.status=terminated`, `customer.termination_reason_id`/`termination_note`, `customer_service.service_status=berhenti`, dan (kondisional) terbitkan invoice denda.
- **Lewat `CustomerWorkflowService::transition()` sejak 2026-09-19 (ADHOC-85)** — hanya `active`/`suspended` yang bisa diputus (pre-check `canTransitionTo()` di controller → redirect back + flash `error` kalau tidak), `terminated_at` diisi `transition()`, dan tercatat di `customer_status_logs`. Rancangan lengkap: [`../plan/billing/rancangan-terminate-reactivate-state-machine.md`](../plan/billing/rancangan-terminate-reactivate-state-machine.md).
- **Alasan sejak ADHOC-69 (2026-09-23): dropdown master, bukan lagi teks bebas.** Form kirim `termination_reason_id` (FK ke `customer_termination_reasons`, wajib) + `termination_note` (teks bebas, opsional). Disimpan di kolom `customers.termination_reason_id`/`termination_note` — List Putus baca dari relasi ini, BUKAN lagi `AuditLog.new_values.reason` di memori. Baris `AuditLog` module `customers` action `terminate` **tetap ditulis** (dibaca import legacy & jejak historis), tapi bukan lagi sumber utama.
- **Denda Putus Langganan (ADHOC-69):** masa langganan pelanggan ≤1 tahun (`customer_services.activation_date`) → `penalty_amount` wajib diisi form (0 sah), invoice `manual`/`lainnya`/sub "Denda Putus Langganan" terbit kalau >0. Masa >1 tahun → field denda tidak tampil, `penalty_amount` diabaikan server. Detail lengkap (aturan denda, guard anti tamper, bentuk invoice): [`../billing-pembayaran/README.md` § Denda Putus Langganan](../billing-pembayaran/README.md#denda-putus-langganan-adhoc-69).
- **Notifikasi in-app (2026-08-07):** pendaftar asli pelanggan (`customers.created_by`, relasi `Customer::creator()`) dapet notif ERROR (sekarang menyebut nominal denda kalau terbit, atau "tidak ada denda" kalau tidak), di-skip kalau yang eksekusi terminasi = pendaftar itu sendiri. **`suspended` SENGAJA gak dapet notif serupa** — beda dari `active`/`terminated` yang masing-masing punya SATU action controller khusus, `suspended` cuma keset lewat form edit generik (`CustomerController::update()`), risiko false-positive kalau dipaksa. Detail: `docs/plan/analisa-status-implementasi-notifikasi.md` §8.5.

### List Putus Langganan + Ambil Alat + Langganan Lagi (2026-07-20, alasan & sort diperbarui ADHOC-69 2026-09-23)

- List **Putus Langganan** (`/customers/terminated`) tabel: ID, Nama, Kontrak (Sewa/Beli — dari `customer_service.contract_type`), Alasan Putus (relasi `customer->terminationReason->name`, fallback `AuditLog` untuk data lama pra-ADHOC-69), **Sales** (`customer->salesUser->name`) & **Teknisi Survei** (`customer->latestSurvey->technician->name`, ADHOC-69 — buat kebutuhan komisi), Tanggal Pemutusan, Status Alat, Action. Filter dropdown Alasan + sort kolom Alasan Putus bekerja di level query (`RendersCustomerList`), bukan di memori.
- **Urut DESC berdasarkan Tanggal Pemutusan** — kolom `terminated_at` asli (diisi `transition()`), bukan lagi subquery `AuditLog`.
...
- **Ambil Alat** (`CustomerController::retrieveDevice`) — **tidak langsung menandai alat diambil** (deskripsi lama "set `device_retrieved_at = now()`" di sini sudah usang: tombol ini sudah lama hanya membuat task; ADHOC-86 menambah form laporan khusus dan alur gudang). Tombol membuat **FopTask kategori Ambil Modem (DEAC, `DRAFT`)**; FOP menunjuk teknisi; teknisi mengisi form laporan pengambilan alat; `device_retrieved_at` baru terisi kalau hasilnya **"diambil"** (alur lengkap: [docs/warehouse/business-logic.md §12a](../warehouse/business-logic.md)). Guard: `customer.status === 'terminated'`, POP & desa terisi, alat belum tercatat diambil, dan belum ada task Ambil Alat yang berjalan. **Pelanggan tanpa baris `customer_devices` tidak lagi ditolak (2026-09-21, temuan uji manual):** dibuatkan baris placeholder `device_type = 'Data Migrasi Legacy'` (`CustomerDevice::LEGACY_DEVICE_TYPE`, pola sama import & backfill) — di data dev hanya 229 dari 1.957 pelanggan punya baris itu (dibuat backfill hanya untuk yang sudah putus), jadi tanpa ini ~1.700 pelanggan legacy mentok saat diputus. Permission `customers.detail.devices.retrieve` *(dipisah dari `customers.update` 2026-07-20, lihat [docs/rbac/business-logic.md § 3.1](../rbac/business-logic.md#31-langkah-nambah-permission-baru-fitur-existing--contoh-nyata-customersdetaildevicesretrieve))*.
- **Langganan Lagi** (`CustomerController::reactivate`) — muncul di SEMUA baris terminated (gak digate status alat). Set `customer.status = 'active'` LANGSUNG (bukan lewat alur survey/verifikasi ulang, keputusan bisnis eksplisit: infrastruktur dianggap masih terpasang) + `customer_service.service_status = 'aktif'`. **Sejak 2026-09-19 (ADHOC-85) lewat `CustomerWorkflowService::transition(ACTIVE, 'Langganan Lagi')`** — enum `terminated` sekarang `[ACTIVE]` (bukan lagi `[]`), jejaknya masuk `customer_status_logs` + audit `Customer Workflow`; audit khusus `reactivate` dihapus (baris lama tetap utuh). Efek samping: `terminated_at` **sengaja tidak dikosongkan** (dipakai churn `DashboardController::growthStats`), pelanggan menerima notifikasi aktivasi lagi, dan `CustomerObserver` memulihkan akses portal — akun `disabled` → `pending_claim` (password lama mati), QR + PIN baru diterbitkan, jadi **kartu QR baru perlu dicetak**. Permission tetap `customers.detail.installation.validate`.
- **Status Alat = 3 keadaan (ADHOC-88, 2026-09-21):** **Sudah Diambil** (hijau, `device_retrieved_at` terisi) > **Sedang Diproses** (biru, ada FopTask Ambil Alat yang belum selesai/dibatalkan — definisi sama dengan penjaga tombol, satu kueri per halaman) > **Belum Diambil** (kuning). Tombol **Ambil Alat hanya tampil di "Belum Diambil"**; hilang saat Sedang Diproses (supaya tidak dijadwalkan dua kali) dan Sudah Diambil. Task dibatalkan, atau teknisi melapor "alat tidak ditemukan/pelanggan menolak", membuat status kembali Belum Diambil dan tombol muncul lagi.
- **Langganan Lagi mengosongkan `device_retrieved_at` (ADHOC-88):** pelanggan yang kembali akan dipasangi modem baru, jadi flag masa langganan lama dicabut (kalau tidak, badge menyesatkan dan pemutusan berikutnya ditolak "alat sudah diambil"). **Riwayat pengambilan yang sudah terjadi tidak hilang** — tersimpan di `device_retrieval_logs` dan tampil di tab Perangkat Detail Pelanggan.
- **Implikasi:** `terminated` punya jalur resmi kembali ke `active` lewat state machine. `rejected` **tetap** bypass (Kembalikan, §7) — belum diresmikan.
