...
- **Lewat `CustomerWorkflowService::transition()` sejak 2026-09-19 (ADHOC-85)** — sebelumnya update status langsung tanpa validasi. Sekarang hanya `active`/`suspended` yang bisa diputus (pre-check `canTransitionTo()` di controller → redirect back + flash `error` kalau tidak), `terminated_at` diisi `transition()`, dan tercatat di `customer_status_logs`. Rancangan lengkap: [`../plan/billing/rancangan-terminate-reactivate-state-machine.md`](../plan/billing/rancangan-terminate-reactivate-state-machine.md).
- Alasan `required` di form — disimpan di `AuditLog.new_values.reason` (module `customers`, action `terminate`), gak ada kolom dedicated. Baris audit ini **tetap ditulis** walau `transition()` sudah menulis audit `Customer Workflow` sendiri, karena List Putus Langganan & import legacy membacanya. `old_values.status` sekarang berisi status asli (dulu di-hardcode `active`).
