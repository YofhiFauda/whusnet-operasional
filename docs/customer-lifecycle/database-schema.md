...
| `sales_code`, `agent_code`, `referral_customer_code` | string | Atribusi sales (teks bebas, data lama) |
| `sales_user_id` | FK → `users.id`, nullable | Sales/Agent/Referral naik level dari varchar bebas jadi FK asli (migrasi `add_sales_agent_referral_fk_to_customers_table`) — dipakai hitung komisi & agregasi omset Busdev. Ditampilkan di List Putus (kolom Sales, ADHOC-69) |
| `rejected_at` | timestamp, nullable | Fase 5.1 (2026-08) — diisi `CustomerWorkflowService::transition()` saat transisi ke `rejected`. Kolom nyata pengganti subquery JSON `AuditLog` di ORDER BY List Pelanggan Gagal |
| `terminated_at` | timestamp, nullable | Fase 5.1 — diisi `transition()` saat transisi ke `terminated`. **Sengaja tidak dikosongkan** saat Langganan Lagi (dipakai churn `DashboardController::growthStats`) |
| `termination_reason_id` | FK → `customer_termination_reasons.id`, nullable, `restrictOnDelete()` | **ADHOC-69 (2026-09-23)** — sumber utama Alasan Putus di List Putus, menggantikan `AuditLog.new_values.reason` di memori. Data lama (putus sebelum fitur ini) NULL, tidak di-backfill. Detail: [docs/billing-pembayaran/database-schema.md § customer_termination_reasons](../billing-pembayaran/database-schema.md#tabel-customer_termination_reasons-adhoc-69) |
| `termination_note` | text, nullable | ADHOC-69 — catatan tambahan bebas, pelengkap `termination_reason_id` |
...
- `device_retrieved_at` (nullable timestamp, migrasi `2026_07_18_163955_add_device_retrieved_at_to_customer_devices_table`) — **baru**. Dicek buat kolom "Status Alat" di List Putus Langganan (null = "Belum di Ambil", terisi = "Sudah di Ambil"). **Diisi** `TaskService::complete()` saat task Ambil Modem (DEAC) selesai dengan hasil **"diambil"** (bukan lagi langsung oleh tombol "Ambil Alat" — tombol itu sekarang hanya membuat task), atau `InventoryReassignService::receiveSerialFromCustomerAtWarehouse()` untuk modem yang diantar pelanggan. **Dikosongkan** saat FOP me-reject task DEAC dan saat Langganan Lagi (ADHOC-88); riwayat pengambilan yang sudah terjadi tetap ada di `device_retrieval_logs` ([warehouse/database-schema.md](../warehouse/database-schema.md)). Sebelum kolom ini ada, gak ada cara sistem tahu status pengambilan alat pelanggan yang putus langganan — lihat [business-logic.md §8](business-logic.md#8-terminasi-layanan-customerterminationcontroller--customerterminationservice-denda-adhoc-69).

- **Baris placeholder legacy (2026-09-21):** `device_type` wajib diisi, sehingga pelanggan hasil migrasi yang tak pernah lewat alur instalasi diberi `device_type = 'Data Migrasi Legacy'` (konstanta `CustomerDevice::LEGACY_DEVICE_TYPE`) — dibuat oleh import pelanggan, `BackfillDeviceRetrievedStatusCommand`, dan **sekarang juga oleh tombol Ambil Alat** kalau barisnya belum ada. Baris ini hanya tempat `device_retrieved_at`; SN/merek/model tidak diisi.
...
**Catatan:** tabel ini cuma keisi lewat `CustomerWorkflowService::transition()`. **Sejak ADHOC-85 (2026-09-19), terminasi (`CustomerTerminationService::terminate()`) JUGA menulis ke sini** — sebelum itu, terminasi update status langsung tanpa lewat `transition()`, jadi baris `terminated` gak pernah ada di tabel ini. Reaktivasi ("Langganan Lagi") juga lewat `transition()` sejak ADHOC-85 (lihat [business-logic.md §8](business-logic.md#8-terminasi-layanan-customerterminationcontroller--customerterminationservice-denda-adhoc-69)).
...
- **List Pelanggan Gagal & List Putus Langganan (2026-07-20) awalnya baca langsung dari `AuditLog`, bukan kolom dedicated** — desain asli: `CustomerController::index()` query `AuditLog` per customer buat ambil alasan+tanggal: module `Customer Workflow` action `status_transition` dengan `new_values->status='rejected'` (Pelanggan Gagal), atau module `customers` action `terminate` (Putus Langganan).
  - **Fase 5.1 (2026-08) menambah kolom nyata `rejected_at`/`terminated_at`** — ORDER BY sekarang pakai kolom itu langsung, bukan subquery `AuditLog::selectRaw('MAX(created_at)')` lagi.
  - **ADHOC-69 (2026-09-23) menambah `termination_reason_id`** — Alasan Putus di List Putus sekarang baca dari relasi `terminationReason` (master `customer_termination_reasons`), **bukan lagi** `AuditLog.new_values.reason`. AuditLog cuma fallback buat data lama yang putus sebelum fitur ini live (`termination_reason_id` NULL). List Pelanggan Gagal **belum** dapat perlakuan sama — alasan reject masih baca `AuditLog.new_values.note` (di luar scope ADHOC-69).
  - Dua action lain di module yang sama: `status_restore` (Kembalikan dari Gagal, `CustomerController::restoreFromFailed()`) dan `reactivate` (Langganan Lagi, `CustomerController::reactivate()`) — dua-duanya nyimpen `old_values`/`new_values.status` dan **sejak ADHOC-85** juga tercatat di `customer_status_logs` lewat `transition()` (lihat [business-logic.md §7](business-logic.md#reject-reject) & [§8](business-logic.md#8-terminasi-layanan-customerterminationcontroller--customerterminationservice-denda-adhoc-69)).
