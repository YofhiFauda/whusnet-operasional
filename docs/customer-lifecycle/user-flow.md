...
3. **Ambil Alat** — cuma muncul kalau status alat masih "Belum Diambil" (hilang saat "Sedang Diproses" dan "Sudah Diambil"). Klik → konfirmasi → sistem **membuat task Ambil Modem (DEAC)** untuk FOP — alat BELUM ditandai diambil; badge berubah jadi **"Sedang Diproses"**. FOP menunjuk teknisi → teknisi mengisi Laporan Ambil Alat (SN dari stiker, foto kondisi, kelengkapan) → `device_retrieved_at` terisi → badge **"Sudah Diambil"** → gudang cabang menerima modemnya di Terima Retur ([alur lengkap](../warehouse/user-flow.md)). Kalau alat tidak ditemukan/pelanggan menolak: task selesai, badge kembali "Belum Diambil" dan tombol muncul lagi. Modem yang diantar pelanggan sendiri (tanpa task) dicatat gudang lewat "Terima modem dari pelanggan". Permission `customers.detail.devices.retrieve` *(dipisah dari `customers.update` 2026-07-20 — biar granular, gak numpang di permission edit-data-pelanggan generik; lihat [docs/rbac/business-logic.md § 3.1](../rbac/business-logic.md#31-langkah-nambah-permission-baru-fitur-existing--contoh-nyata-customersdetaildevicesretrieve))*.
4. **Langganan Lagi** — selalu muncul (gak peduli status alat). Klik → konfirmasi → pelanggan **langsung aktif lagi** (`status=active`), TANPA lewat survey/verifikasi ulang (asumsi infrastruktur masih terpasang). `device_retrieved_at` dikosongkan (ADHOC-88), tetapi **riwayat pengambilan alat tetap tampil** di tab Perangkat Detail Pelanggan. Permission `customers.detail.installation.validate`.
