...
| `notes` | text | ✔ | Catatan bebas (ticket-origin: pointer sistem pendek dari `composeFopNotes()`, mis. `"Ticket TKT-xxx — dikirim oleh yyy."`; manual dari `/fop-tasks`: bebas isi FOP). **JANGAN pernah digabung ke `task->description`** saat `Task` eksekusi dibuat — kolom ini ditampilkan sendiri di box "Catatan FOP" di `tasks/show.blade.php` (lihat [task-teknisi/business-logic.md § 9](../task-teknisi/business-logic.md#9-pemisahan-catatan--issue-teknis-catatan-fop-catatan-teknis-noc)) |
...
| `sla_breach_notified_at` | timestamp | ✔ | **Baru (2026-08-07, migrasi `2026_08_07_102131_add_sla_breach_notified_at_to_fop_tasks_table`)**. Dedup flag buat command scheduled `fop-tasks:check-sla-breach` (`app/Console/Commands/CheckFopTaskSlaBreach.php`, jadwal `everyThirtyMinutes()` — lihat `docs/plan/analisa-status-implementasi-notifikasi.md` §8.4). Di-set `now()` begitu `slaDeadline()` kelewat dan notif in-app udah dikirim ke role `fop` di POP task itu — command berikutnya skip task yang kolom ini udah keisi (gak notif ulang tiap 30 menit). **Reset otomatis ke `null`** lewat `FopTask::booted()` → `updating` begitu `task_date` berubah (reschedule) — deadline baru, layak breach-check ulang dari nol. |
