...
3. Klik tiket manapun → buka **Detail Riwayat** (`/fop-tasks/history/{id}`, Task 10): Info Task (termasuk Alasan Cancel kalau `dibatalkan`), Durasi & SLA Pengerjaan (dual-cycle: mulai/pending/resume/selesai, status SLA on-time/over), Laporan (baca dari Survey/Instalasi/Maintenance sesuai kategori), dan **Histori Status** — log lengkap tiap perubahan status + siapa + kapan, audit trail utama.
