...
Satu baris item, **tanpa baris kosong filler** — sempat dibolak-balik: dibuang (2026-09-22, dianggap peninggalan struk manual) → dikembalikan (2026-09-23, biar free space selaras dgn tinggi tabel Rincian Total yang saat itu masih tabel terpisah) → **dibuang lagi permanen (2026-09-26)** begitu Rincian Pembayaran & Rincian Total DISATUKAN jadi satu `<table>` — alasan "selaras tinggi" jadi tidak berlaku lagi (satu tabel yang sama, tidak ada lagi dua tabel bersebelahan yang perlu diselaraskan), dan 2 baris kosong itu makan ruang vertikal yang berharga di kertas 5,5in yang tingginya pas-pasan untuk kwitansi berbaris banyak (Sub Total/DP/Lebih Bayar/Titip Saldo/Total).
...
- CSS `@page` yang ada sekarang di `payments/receipt.blade.php` (`size: auto; margin: 0;`) itu warisan struk thermal (printer thermal otomatis pas ke lebar rol-nya) — **harus diganti** untuk jalur print fisik ke NCR: `@page { size: 9.5in 5.5in; margin: 0; }` — persegi panjang landscape ~16:9 (lebar 9,5in > tinggi 5,5in), sama seperti bentuk fisik continuous form 1/2 part-nya apa adanya.
- **Revisi 2026-09-26 — PDF Portal ikut ukuran fisik yang sama juga (bukan A4):** keputusan awal "PDF Portal tetap A4" dibalik user — PDF Portal cuma pernah dilihat/diunduh, tak pernah dicetak ke printer kantor, jadi ukurannya bebas ditentukan; dipilih SAMA PERSIS dengan lembar fisik staf (9,5×5,5in) daripada A4 yang menyisakan banyak spasi kosong di bawah kontennya — sekaligus biar satu ukuran dipelihara di satu tempat, bukan dua definisi berbeda. Angkanya disatukan di `App\Services\Receipts\ReceiptPaperSize` (dipakai `payments/receipt.blade.php` buat `@page` DAN `PortalPaymentController::receiptPdf()` buat `->setPaper([0, 0, ReceiptPaperSize::widthPt(), ReceiptPaperSize::heightPt()])`, bukan `->setPaper('a4')` lagi). Tampilan iframe modal Portal (`receiptView()`, layar bukan kertas) tetap tidak kena ukuran cetak sama sekali — cuma soal render layar.
- Ini **bukan pengecualian dari prinsip "satu template identik"** — isi/layout HTML-nya tetap sama persis di semua tempat (§Layout & Pemetaan Field tidak berubah). Yang beda cuma **toolbar** (`$isPdf` menyembunyikannya), bukan ukuran kertas lagi — sejak revisi 2026-09-26 ukuran kertas PDF Portal dan fisik staf sama persis.
- **Bug PDF Portal abu-abu + spasi kosong (koreksi 2026-09-26):** dompdf merender statis dan membaca stylesheet sebagai media `screen` (bukan `print`) secara default — gaya "bersih" (background putih, lembar full-width) yang tadinya cuma dipasang di dalam `@media print` diam-diam TIDAK PERNAH kepakai di PDF Portal, sehingga PDF ikut gaya interaktif "kartu 700px di atas backdrop abu-abu" yang harusnya cuma buat preview staf di browser. Diperbaiki: gaya bersih ditulis LANGSUNG (dikondisikan `$isPdf`, bukan di dalam `@media print`) supaya berlaku juga saat dompdf merender.
- **Kolom tabel dirapatkan (koreksi 2026-09-26):** kolom Harga/Total sempat dilebarkan (15%→20%) + `white-space: nowrap` ditambah di sel nominal/label ringkasan supaya "Rp 4.900.000"/"Lebih Bayar"/"Titip Saldo" gak patah jadi 2 baris — percobaan sebelumnya (ukuran potret sempit 5,5×9,5in) bikin kolom kepepet; sekarang kertas balik ke lebar 9,5in jadi longgar, tapi perubahan lebar kolom & nowrap dipertahankan (gak masalah, cuma nambah napas).
