...
> **Deviasi 2026-09-24 (keputusan user, menggantikan §2 tabel & §4.5 poin 1 di bawah):** K2 ("satu kemampuan, dua pintu", waive dibundel SATU TRANSAKSI dengan terminate) **dibatalkan** setelah diimplementasi & direview user. Alasan: kompleksitas dropdown periode+catatan di tengah form Putus Langganan tidak sepadan — "kenapa harus ada 2 tempat buat hal yang sama". Sekarang **SATU PINTU SAJA**: aksi "Cuti Berlangganan / Bebaskan Tagihan Periode" di Detail Pelanggan (`CustomerBillingWaiverController`), independen dari terminasi. Form Request Putus Langganan (`CustomerTerminationController`/`CustomerTerminationService`) kembali murni alasan+denda+catatan, TIDAK menyentuh `BillingPeriodWaiverService` sama sekali. Konsekuensi: G6 (§4.3) dilonggarkan — pembebasan tagihan sekarang boleh dipanggil untuk pelanggan `active`/`suspended`/`terminated` (bukan cuma active/suspended), supaya admin masih bisa bersihin tagihan yang kelupaan dibebaskan sebelum putus, sesudahnya. Konsekuensi UX: admin yang mau putus+bebasin tagihan sekaligus harus 2 langkah manual (bebasin tagihan dulu via Cuti, baru putus) — bukan lagi 1 submit atomik. `BillingWaiverSource::TERMINATION` dipertahankan di enum (data model & kasus uji G1-G3) tapi TIDAK ADA LAGI pemanggil yang memproduksinya — data nyata cuma akan berisi `source=leave`.
...
- **Detail Pelanggan** — form Putus Langganan berlabel **"Request Putus Langganan"** (label saja, §2). **Deviasi 2026-09-24: TIDAK lagi membawa dropdown/waive tagihan** — murni alasan+denda+catatan (lihat catatan deviasi di atas). Kalau ada tagihan yang mau dibebaskan bersamaan dengan putus, admin pakai aksi "Cuti Berlangganan"/"Bebaskan Tagihan" di bawah SEBELUM atau SESUDAH klik Putus — dua langkah manual, bukan satu transaksi atomik.
- **Detail Pelanggan** — aksi **"Cuti Berlangganan" / "Bebaskan Tagihan Periode"** (inline toggle, pola CLAUDE.md #3) — SATU-SATUNYA pintu ke `BillingPeriodWaiverService` sekarang: pilih satu atau beberapa periode (yang sudah terbit + ke depan), catatan wajib, simpan. **Ditampilkan juga untuk pelanggan yang sudah `terminated`** (G6 dilonggarkan, deviasi 2026-09-24) — bukan cuma active/suspended — supaya admin masih bisa bersihin tagihan yang kelupaan dibebaskan sebelum putus resmi. Dropdown menampilkan riwayat tagihan langganan terbaru pelanggan itu + periode ke depan yang belum terbit; yang **tidak eligible tetap tampil tetapi nonaktif dengan alasannya** ("Sudah dibayar", "Di luar jendela 1 bulan", "Bukan tagihan langganan", "Sudah dibebaskan sebelumnya"), supaya admin paham kenapa Agustus tidak bisa dipilih alih-alih mengira daftarnya salah. Daftar waiver aktif tampil di tab Tagihan (periode, alasan, siapa, kapan, tombol cabut bila punya permission).
...
| K2 | ~~Satu kemampuan, dua pintu (Deaktivasi & Cuti)~~ **DIBATALKAN 2026-09-24** — satu kemampuan, **SATU pintu** (Cuti Berlangganan/Bebaskan Tagihan saja, independen dari terminate) | ~~Dua fitur terpisah dengan tabel masing-masing~~ — sudah dieksekusi lalu direvisi user, lihat catatan deviasi §1 |
