...
@section('content')
    @include('partials.collector-realtime', ['channels' => ['App.Models.User.' . auth()->id()], 'audiens' => 'kolektor'])
...
class="space-y-6" id="live-content">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                Daftar pelanggan yang perlu didatangi dalam jendela tagih ({{ $dueWindowDays }} hari sebelum jatuh tempo).
            </p>
        </div>
    ...
    @if ($errors->any())
        <x-ui.alert variant="error" title="Setoran Gagal" class="rounded-2xl shadow-xs">
    ...
    @if (session('success'))
        <x-ui.alert variant="success" class="rounded-2xl shadow-xs">{{ session('success') }}</x-ui.alert>
    @endif
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div class="bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl p-4 shadow-xs relative overflow-hidden transition-all hover:border-violet-300 dark:hover:border-violet-600/50">
            ...
            <div class="mt-2 flex items-baseline gap-2 min-w-0">
                <span class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 font-mono truncate">{{ number_format($jumlahPelanggan, 0, ',', '.') }}</span>
        ...
        <div class="bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl p-4 shadow-xs relative overflow-hidden transition-all hover:border-sky-300 dark:hover:border-sky-600/50">
            ...
            <div class="mt-2 flex items-baseline gap-2 min-w-0">
                <span class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 font-mono truncate">{{ number_format($invoices->total(), 0, ',', '.') }}</span>
        ...
        <div class="bg-white dark:bg-slate-800/90 border border-amber-200/80 dark:border-amber-500/30 rounded-2xl p-4 shadow-xs relative overflow-hidden transition-all hover:border-amber-400 dark:hover:border-amber-500/60">
            ...
            <div class="mt-2 min-w-0">
                <span class="text-lg sm:text-2xl font-bold text-amber-700 dark:text-amber-400 font-mono truncate block">Rp {{ number_format((float) $totalTunggakan, 0, ',', '.') }}</span>
            </div>
            <div class="mt-1 text-[11px] text-slate-400 dark:text-slate-500 font-medium">
    ...
    {{-- Financial Status & Setoran (Kas di Tangan) --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div class="min-w-0">
                    ...
                    <div class="text-xs text-slate-300 mt-1 flex flex-wrap items-center gap-2">
                        <span class="px-2 py-0.5 rounded-full bg-slate-700/80 text-emerald-300 text-[11px] font-semibold border border-slate-600">
                            {{ $unsettledCount }} transaksi
                        </span>
                        <span>belum diserahkan ke admin</span>
                    </div>
                </div>

                @if ($canDeposit && $balance > 0)
                    <form action="{{ route('collector-worklist.deposit') }}" method="POST" class="shrink-0 w-full sm:w-auto"
                          data-confirm="Setorkan seluruh saldo Rp{{ number_format($balance, 0, ',', '.') }} ({{ $unsettledCount }} pembayaran) ke admin? Saldo Anda jadi nol dan menunggu verifikasi.">
                        @csrf
                        <input type="hidden" name="idempotency_key" value="{{ 'deposit-'.auth()->id().'-'.now()->timestamp.'-'.\Illuminate\Support\Str::random(8) }}">
                    ...
                    @foreach ($pendingDeposits as $deposit)
                        <div class="flex flex-wrap items-center justify-between gap-2 text-xs bg-slate-800/80 p-2.5 rounded-xl border border-slate-700/60">
                            <div class="flex items-center gap-2 min-w-0">
                                <span class="font-mono text-slate-300 font-semibold truncate">{{ $deposit->deposit_number }}</span>
                            </div>
                            @if ($deposit->status === \App\Enums\DepositStatus::MENUNGGU_VERIFIKASI)
                            ...
                            @else
                                <span class="px-2.5 py-1 rounded-lg bg-rose-500/20 text-rose-300 border border-rose-500/30 text-[11px] font-semibold shrink-0">
                                    Kurang setor Rp {{ number_format($deposit->outstandingShortfall(), 0, ',', '.') }}
                                </span>
                            @endif
                        </div>
                    @endforeach
        ...
        {{-- Card Status Kurang Setor --}}
        <div class="rounded-2xl p-4 sm:p-5 border shadow-xs flex flex-col justify-between transition-all {{ $outstandingShortfall > 0 ? 'bg-rose-50/80 dark:bg-rose-500/10 border-rose-200 dark:border-rose-500/30' : 'bg-white dark:bg-slate-800/90 border-slate-200/80 dark:border-slate-700/80' }}">
            <div class="min-w-0">
                <div class="flex items-center justify-between">
                    <span class="text-[11px] font-bold uppercase tracking-wider {{ $outstandingShortfall > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-400 dark:text-slate-500' }}">Kurang Setor</span>
                    <div class="w-8 h-8 rounded-xl {{ $outstandingShortfall > 0 ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400' : 'bg-slate-100 dark:bg-slate-700 text-slate-400' }} flex items-center justify-center shrink-0">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                    </div>
                </div>
                <div class="font-bold text-xl sm:text-3xl mt-2 font-mono truncate {{ $outstandingShortfall > 0 ? 'text-rose-700 dark:text-rose-400' : 'text-slate-900 dark:text-slate-100' }}">
                    Rp {{ number_format($outstandingShortfall, 0, ',', '.') }}
                </div>
            </div>
            <p class="text-xs {{ $outstandingShortfall > 0 ? 'text-rose-600 dark:text-rose-400/90' : 'text-slate-400 dark:text-slate-500' }} mt-3">
                Nominal ini tidak otomatis nol saat Anda menyetor. Lunasi lewat setoran berikutnya.
            </p>
        </div>
    </div>

    {{-- Kunjungan Hari Ini (Summary Pill List - Collapsible & Compact) --}}
        ...
        <div x-data="{ expanded: false }" class="bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl p-3.5 shadow-xs transition-all">
            <div class="flex items-center justify-between gap-2">
                <div class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 flex items-center gap-2">
                    ...
                    <span>Kunjungan Hari Ini ({{ $todayVisits->count() }})</span>
                </div>
                
                @if ($todayVisits->count() > 3)
                    <button type="button" @click="expanded = !expanded" class="text-xs font-semibold text-sky-600 dark:text-sky-400 hover:underline flex items-center gap-1 cursor-pointer">
                        <span x-text="expanded ? 'Sembunyikan' : 'Lihat Semua ({{ $todayVisits->count() }})'"></span>
            ...
            {{-- Mode Ringkas (Default): 1 Baris Horizontal Swipe --}}
            <div x-show="!expanded" class="mt-2 flex items-center gap-2 overflow-x-auto pb-1 custom-scrollbar">
                @foreach ($todayVisits as $visit)
                    <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/80 text-xs shrink-0 shadow-2xs">
                        <span class="font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap">{{ $visit->customer->full_name ?? '-' }}</span>
                        <span class="px-1.5 py-0.5 rounded-lg text-[10px] font-bold shrink-0 {{ $visit->result === \App\Enums\VisitResult::BAYAR ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-slate-200/70 text-slate-700 dark:bg-slate-700 dark:text-slate-300' }}">
                            {{ $visit->result->label() }}@if ($visit->promised_date) ({{ $visit->promised_date->format('d/m') }})@endif
                        </span>
                    </div>
                @endforeach
            </div>

            {{-- Mode Expanded: Grid / Flex Wrap --}}
            <div x-show="expanded" x-cloak class="mt-2.5 flex flex-wrap gap-2 pt-2 border-t border-slate-100 dark:border-slate-700/60">
                @foreach ($todayVisits as $visit)
                    <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/80 text-xs shadow-2xs">
                        <span class="font-semibold text-slate-800 dark:text-slate-200">{{ $visit->customer->full_name ?? '-' }}</span>
                        <span class="px-1.5 py-0.5 rounded-lg text-[10px] font-bold {{ $visit->result === \App\Enums\VisitResult::BAYAR ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-slate-200/70 text-slate-700 dark:bg-slate-700 dark:text-slate-300' }}">
                            {{ $visit->result->label() }}@if ($visit->promised_date) (janji {{ $visit->promised_date->format('d/m/Y') }})@endif
                        </span>
                    </div>
                @endforeach
            </div>
    ...
    {{-- Navigation Tabs: Tagihan (siapa yang perlu didatangi) vs Sudah Bayar
         (siapa yang sudah bayar & uangnya sedang menunggu disetor). Tab
         kedua ada supaya kolektor bisa cross check sendiri sebelum menekan
         "Setor ke Admin" — bukan cuma percaya angka total saldo. --}}
    <div class="border-b border-slate-200 dark:border-slate-700/80 overflow-x-auto custom-scrollbar pb-1">
        <div class="flex gap-1.5 sm:gap-2 min-w-max">
            <a href="{{ route('collector-worklist.index', ['tab' => 'tagihan']) }}"
               class="px-3 sm:px-4 py-2.5 sm:py-3 text-xs font-semibold border-b-2 -mb-px transition-all inline-flex items-center gap-2 {{ $tab === 'tagihan' ? 'border-sky-600 text-sky-600 dark:text-sky-400 dark:border-sky-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200' }}">
                <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                <span>Tagihan</span>
            </a>
            <a href="{{ route('collector-worklist.index', ['tab' => 'bayar']) }}"
               class="px-3 sm:px-4 py-2.5 sm:py-3 text-xs font-semibold border-b-2 -mb-px transition-all inline-flex items-center gap-2 {{ $tab === 'bayar' ? 'border-sky-600 text-sky-600 dark:text-sky-400 dark:border-sky-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200' }}">
    ...
    <div class="bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl p-4 shadow-xs">
        <form action="{{ route('collector-worklist.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
            <input type="hidden" name="tab" value="tagihan">
            <div class="relative flex-1">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari Nama, CID, atau No. Invoice..."
            ...
            <div class="flex items-center gap-2">
                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-500 text-white font-semibold text-sm px-5 py-2.5 rounded-xl transition-all shadow-xs active:scale-95 cursor-pointer">
                    Cari
                </button>
                @if($search !== '')
                    <a href="{{ route('collector-worklist.index', ['tab' => 'tagihan']) }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700/80 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold text-sm px-4 py-2.5 rounded-xl transition-all cursor-pointer">
                        Reset
                    </a>
                @endif
            </div>
    ...
    {{-- Main Worklist Collector Table / Cards --}}
    @if ($canPay)
        ...
        @endpush
    @else
        {{-- Mode Read-Only tanpa permission kolektor.pay --}}
                        ...
                        @forelse ($invoices as $invoice)
                            <tr class="block sm:table-row bg-white dark:bg-slate-800/90 sm:bg-transparent rounded-2xl sm:rounded-none border border-slate-200/80 dark:border-slate-700/80 sm:border-x-0 sm:border-t-0 sm:border-b p-4 sm:p-0 shadow-xs sm:shadow-none hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                                <td class="block sm:table-cell px-0 sm:px-6 py-1 sm:py-4">
                                    <div class="font-bold text-slate-900 dark:text-slate-100">{{ $invoice->customer->full_name ?? '-' }}</div>
    ...
    @else
    {{-- Tab Sudah Bayar — view-only, daftar PAYMENT (bukan invoice) yang
         belum ikut setoran mana pun. Persis isi setoran yang akan terbentuk
         kalau "Setor ke Admin" ditekan sekarang. --}}
    <div class="bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-2xl overflow-hidden shadow-xs">
        <div class="overflow-x-auto">
            <table class="w-full border-collapse text-left text-sm text-slate-700 dark:text-slate-200 min-w-0 sm:min-w-full">
                <thead class="hidden sm:table-header-group">
    ...
    {{-- Responsive Dialog: Mobile Bottom Sheet Drawer / Desktop Centered Modal --}}
    @if ($canLogVisit)
        <div x-show="visitModalOpen" 
            ...
            <div class="flex min-h-full items-end sm:items-center justify-center p-0 sm:p-4 text-left">
                <div x-show="visitModalOpen"
                    ...
                    <!-- Form -->
                    <form action="{{ route('collector-worklist.visits.store') }}" method="POST" class="space-y-4" data-confirm="Simpan catatan kunjungan ini?">
                        @csrf
                        ...
                        <div class="space-y-1">
                            ...
                            <select name="result" required class="w-full text-xs sm:text-sm px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500">
                                @foreach (\App\Enums\VisitResult::manualValues() as $value)
                                    <option value="{{ $value }}">{{ \App\Enums\VisitResult::from($value)->label() }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="date" name="promised_date" min="{{ now()->format('Y-m-d') }}" class="w-full text-xs sm:text-sm px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500">
                        </div>
