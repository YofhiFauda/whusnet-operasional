{{--
    Wilayah Filter (Fase 5.4)
    -------------------------
    Dropdown pill multi-select untuk filter Kecamatan + Desa di daftar pelanggan.
    - Kecamatan: search dinamis ke /api/wilayah/districts, multi-select.
    - Desa: CASCADE dari kecamatan terpilih (hanya desa di kecamatan yang dipilih),
      search ke /api/wilayah/villages?district_id[]=... — mencegah desa senama
      lintas kecamatan muncul semua (duplikasi). Tanpa kecamatan → diminta pilih dulu.
    Pilihan dikirim district_id[]/village_id[] → controller whereIn.

    Props:
      - selectedDistricts : Collection {id,name}
      - selectedVillages  : Collection {id,name,district_id} (district_id untuk prune)
--}}
@props([
...
<div
    x-data="wilayahFilter({
        d: @js($selectedDistricts->map(fn ($x) => ['id' => (int) $x->id, 'name' => $x->name])->values()),
        v: @js($selectedVillages->map(fn ($x) => ['id' => (int) $x->id, 'name' => $x->name, 'district_id' => (int) ($x->district_id ?? 0), 'district' => optional($x->district)->name])->values()),
        formId: @js($formId)
    })"
    ...
    class="relative"
>
    {{-- Hidden inputs yang ikut ke-submit sebagai filter --}}
    ...
    <div x-show="open" x-cloak
        ...
        {{-- ── Seksi Desa (cascade dari kecamatan) ── --}}
        <div x-show="tab === 'desa'">
            {{-- Belum pilih kecamatan → minta pilih dulu --}}
