        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            <div class="space-y-3">
                <div class="flex justify-between border-b border-border/50 py-1">
                    ...
                    <span class="font-semibold text-text-main">{{ strtoupper($device->device_type) }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Merk / Tipe</span>
                    <span class="font-semibold text-text-main">{{ trim(($device->brand ?? '-') . ' ' . ($device->model ?? '')) }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Serial Number</span>
                    <span class="font-semibold text-text-main font-mono">{{ $device->serial_number ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">MAC Address</span>
                    <span class="font-semibold text-text-main font-mono">{{ $device->mac_address ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">IP Address</span>
                    <span class="font-semibold text-text-main font-mono">{{ $maskSensitive($device->ip_address) }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">VLAN ID</span>
                    <span class="font-semibold text-text-main font-mono">{{ $maskSensitive($device->vlan_id) }}</span>
                </div>
            </div>
            <div class="space-y-3">
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Username PPPoE</span>
                    <span class="font-semibold text-text-main font-mono">{{ $device->pppoe_username ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Password PPPoE</span>
                    <span class="font-semibold text-text-main font-mono">{{ $maskSensitive($device->pppoe_password) }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">SSID WiFi</span>
                    <span class="font-semibold text-text-main">{{ $device->wifi_ssid ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Password WiFi</span>
                    <span class="font-semibold text-text-main font-mono">{{ $maskSensitive($device->wifi_password) }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">ODP / Port</span>
                    <span class="font-semibold text-text-main">{{ $device->odp ?: '-' }} / {{ $device->odp_port ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Redaman / Mode</span>
                    <span class="font-semibold text-text-main">{{ $device->signal_rx_power !== null ? $device->signal_rx_power . ' dBm' : '-' }} / {{ $device->connection_mode ?: '-' }}</span>
                </div>
            </div>
            <div class="md:col-span-2 pt-2">
                <span class="block text-text-muted mb-1">Catatan Teknis:</span>
                <p class="p-3 bg-surface-muted rounded border border-border italic text-text-secondary">{{ $device->technical_note ?: 'Tidak ada catatan' }}</p>
            </div>
        </div>
    </div>
@else
        ...
        <div class="border border-border rounded-lg overflow-hidden">
            ...
            <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                <div class="space-y-3">
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Jenis Perangkat</span>
                        <span class="font-semibold text-text-main">{{ $fallbackDeviceType }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Merk / Tipe</span>
                        <span class="font-semibold text-text-main">{{ $technicalDetail?->passive_device ?: '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Jenis Perangkat Pasif</span>
                        <span class="font-semibold text-text-main">{{ $passiveDeviceTypeLabels[$technicalDetail?->passive_device_type] ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Jumlah / Panjang</span>
                        <span class="font-semibold text-text-main">{{ $technicalDetail?->passive_device_qty ?: '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Catatan Perangkat Pasif</span>
                        <span class="font-semibold text-text-main">{{ $technicalDetail?->passive_device_note ?: '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Serial Number</span>
                        <span class="font-semibold text-text-main font-mono">{{ $technicalDetail?->router_or_ont_serial ?? $customer->ont_sn ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">MAC Address</span>
                        <span class="font-semibold text-text-main font-mono">{{ $technicalDetail?->router_mac ?? $technicalDetail?->antenna_mac ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">IP Address</span>
                        <span class="font-semibold text-text-main font-mono">{{ $maskSensitive($technicalDetail?->ip_address ?? $customer->ip_address) }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">VLAN ID</span>
                        <span class="font-semibold text-text-main font-mono">{{ $maskSensitive($customer->vlan_id) }}</span>
                    </div>
                </div>
                <div class="space-y-3">
                    <div class="flex justify-between border-b border-border/50 py-1">
                    ...
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">SSID WiFi</span>
                        <span class="font-semibold text-text-main">{{ $technicalDetail?->ssid ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Password WiFi</span>
                        <span class="font-semibold text-text-main font-mono">-</span>
                    ...
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">ODP / Port</span>
                        <span class="font-semibold text-text-main">{{ $technicalDetail?->odp_number ?? $customer->odp_code ?? '-' }} / {{ $technicalDetail?->odp_port ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between border-b border-border/50 py-1">
                        <span class="text-text-muted">Redaman / Mode</span>
                        <span class="font-semibold text-text-main">{{ ($technicalDetail?->fiber_signal ?? $technicalDetail?->wireless_signal) ?: '-' }} / {{ $technicalDetail?->connection_type ?: '-' }}</span>
                    </div>
                </div>
                <div class="md:col-span-2 pt-2">
                    <span class="block text-text-muted mb-1">Catatan Teknis:</span>
                    <p class="p-3 bg-surface-muted rounded border border-border italic text-text-secondary">{{ $technicalDetail?->note ?? 'Tidak ada catatan' }}</p>
                </div>
            </div>
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            <div class="space-y-3">
                <div class="flex justify-between border-b border-border/50 py-1">
                    ...
                    <span class="font-semibold text-text-main">{{ $tech->passive_device ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Jenis Perangkat Pasif</span>
                    <span class="font-semibold text-text-main">{{ $passiveDeviceTypeLabels[$tech->passive_device_type] ?? '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Jumlah / Panjang</span>
                    <span class="font-semibold text-text-main">{{ $tech->passive_device_qty ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Catatan Perangkat Pasif</span>
                    <span class="font-semibold text-text-main">{{ $tech->passive_device_note ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Nomor Cabang (IDCABANG)</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->branch_number ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Nomor POP (IDWILAYAH)</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->pop_number ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Nomor OLT</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->olt_number ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Slot OLT</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->olt_slot ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Nomor Port OLT</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->olt_port ?: '-' }}</span>
                </div>
            </div>
            <div class="space-y-3">
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Nomor Router / MAC</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->router_number ?: '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Redaman Awal</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->initial_attenuation !== null ? $tech->initial_attenuation . ' dB' : '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">Redaman Terkini (Actual)</span>
                    <span class="font-semibold text-text-main font-mono">{{ $tech->actual_attenuation !== null ? $tech->actual_attenuation . ' dB' : '-' }}</span>
                </div>
                <div class="flex justify-between border-b border-border/50 py-1">
                    <span class="text-text-muted">VLAN (Teknis Jaringan)</span>
                    <span class="font-semibold text-text-main font-mono">{{ $maskSensitive($tech->vlan) }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('customers.device.store', $customer->id) }}" method="POST">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @endphp
                    <input type="text" name="brand" id="brand" value="{{ old('brand', $defaultBrand) }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs text-text-main">
                </div>
                <div>
                    ...
                    @endphp
                    <input type="text" name="model" id="model" value="{{ old('model', $defaultModel) }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $device?->serial_number ?? $technicalDetail?->router_or_ont_serial ?? $customer->ont_sn) }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    <label for="mac_address" class="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">MAC Address</label>
                    <input type="text" name="mac_address" id="mac_address" value="{{ old('mac_address', $device?->mac_address ?? $technicalDetail?->router_mac ?? $technicalDetail?->antenna_mac) }}" placeholder="AA:BB:CC:DD:EE:FF" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="pppoe_username" id="pppoe_username" value="{{ old('pppoe_username', $device?->pppoe_username) }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="pppoe_password" id="pppoe_password" value="{{ $canViewSensitiveDeviceFields ? old('pppoe_password', $device?->pppoe_password) : '' }}" {{ !$canViewSensitiveDeviceFields ? 'placeholder=********' : '' }} class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    <label for="wifi_ssid" class="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">SSID WiFi</label>
                    <input type="text" name="wifi_ssid" id="wifi_ssid" value="{{ old('wifi_ssid', $device?->wifi_ssid ?? $technicalDetail?->ssid) }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="wifi_password" id="wifi_password" value="{{ $canViewSensitiveDeviceFields ? old('wifi_password', $device?->wifi_password) : '' }}" {{ !$canViewSensitiveDeviceFields ? 'placeholder=********' : '' }} class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    <label for="ip_address" class="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">IP Address</label>
                    <input type="text" name="ip_address" id="ip_address" value="{{ $canViewSensitiveDeviceFields ? old('ip_address', $device?->ip_address ?? $technicalDetail?->ip_address ?? $customer->ip_address) : '' }}" {{ !$canViewSensitiveDeviceFields ? 'placeholder=********' : 'placeholder=192.168.1.1' }} class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="odp" id="odp" value="{{ old('odp', $device?->odp ?? $technicalDetail?->odp_number ?? $customer->odp_code) }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    <label for="odp_port" class="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">Port ODP</label>
                    <input type="text" name="odp_port" id="odp_port" value="{{ old('odp_port', $device?->odp_port ?? $technicalDetail?->odp_port) }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    <label for="olt_number" class="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">Nomor OLT</label>
                    <input type="text" name="olt_number" id="olt_number" value="{{ old('olt_number', $technicalDetail?->olt_number) }}" placeholder="Contoh: OLT-01" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    <label for="olt_slot" class="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">Slot OLT</label>
                    <input type="text" name="olt_slot" id="olt_slot" value="{{ old('olt_slot', $technicalDetail?->olt_slot) }}" placeholder="Contoh: 0/1" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="olt_port" id="olt_port_tech" value="{{ old('olt_port', $technicalDetail?->olt_port) }}" placeholder="Contoh: 1" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    <label for="tech_vlan" class="block text-[10px] font-bold text-text-muted uppercase tracking-wider mb-1">VLAN (Jaringan)</label>
                    <input type="text" name="tech_vlan" id="tech_vlan" value="{{ $canViewSensitiveDeviceFields ? old('tech_vlan', $technicalDetail?->vlan) : '' }}" {{ !$canViewSensitiveDeviceFields ? 'placeholder=********' : 'placeholder=Contoh:\ 100' }} class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="passive_device" id="passive_device" value="{{ old('passive_device', $technicalDetail?->passive_device) }}" placeholder="Contoh: Antena Grid 25dBi" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs text-text-main">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="passive_device_qty" id="passive_device_qty" value="{{ old('passive_device_qty', $technicalDetail?->passive_device_qty) }}" placeholder="Contoh: 150 meter / 1 pasang" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs text-text-main">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="router_number" id="router_number" value="{{ old('router_number', $technicalDetail?->router_number) }}" placeholder="Contoh: RTR-001" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="branch_number" id="branch_number" value="{{ old('branch_number', $technicalDetail?->branch_number) }}" placeholder="Contoh: CB001" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="pop_number" id="pop_number" value="{{ old('pop_number', $technicalDetail?->pop_number) }}" placeholder="Contoh: WL0001" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="initial_attenuation" id="initial_attenuation_dev" value="{{ old('initial_attenuation', $technicalDetail?->initial_attenuation) }}" placeholder="-19.50" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div>
                    ...
                    <input type="text" name="actual_attenuation" id="actual_attenuation_dev" value="{{ old('actual_attenuation', $technicalDetail?->actual_attenuation) }}" placeholder="-21.20" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                ...
                <div>
                    ...
                    @endphp
                    <input type="number" step="0.01" name="signal_rx_power" id="signal_rx_power" value="{{ $sigPower }}" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs font-mono text-text-main">
                </div>
                <div class="md:col-span-2">
                    ...
                    <textarea name="technical_note" id="technical_note" rows="3" class="w-full px-3 py-2 bg-surface border border-border rounded-md shadow-sm focus:ring-primary focus:border-primary text-xs text-text-main">{{ old('technical_note', $device?->technical_note ?? $technicalDetail?->note) }}</textarea>
                </div>
            </div>
