{{--
    Tab Pemasangan — mengikuti redesign_detail_pelanggan_whusnet.html (TAB 6):
    ringkasan waktu/teknisi → tabel material terpakai → tabel alat kerja →
    grid 3 foto (pemasangan / TTD berita acara / speedtest) → card laporan uji.

    Palet slate/sky eksplisit + dark:, sejalan dengan customers/show.blade.php.
--}}
        ...
        @foreach($customer->installations()->latest()->get() as $installation)
            ...
            <div class="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                ...
                <div class="p-5 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                        <div class="space-y-2.5">
                            ...
                            <div class="flex justify-between gap-3 py-1 border-b border-slate-100 dark:border-slate-700/50">
                                ...
                                <span class="font-semibold text-slate-900 dark:text-slate-100 searchable-text">
                                    {{ $tglSelesai }} — {{ $installation->end_time ? substr($installation->end_time, 0, 5) . ' WIB' : '-' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2.5">
                            ...
                            <div class="flex justify-between gap-3 py-1 border-b border-slate-100 dark:border-slate-700/50">
                                ...
                                <span class="font-bold searchable-text {{ $hasTestReport ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400' }}">
                                    {{ $hasTestReport ? 'Lolos Pengujian Uji Coba' : 'Belum Ada Laporan Uji' }}
                                </span>
                            </div>
                            <div class="pt-1">
                                ...
                                <p class="p-3 bg-slate-50 dark:bg-slate-900/50 rounded border border-slate-200 dark:border-slate-700 italic text-slate-600 dark:text-slate-300 searchable-text">
                                    {{ $installation->installation_note ?: 'Tidak ada catatan' }}
                                </p>
                            </div>
                        </div>
                    </div>

                    {{-- 1. TABEL PERANGKAT PASIF & MATERIAL TERPAKAI AKTUAL --}}
                    <div>
                            ...
                            <div class="overflow-x-auto border border-slate-200 dark:border-slate-700 rounded-lg">
                                <table class="w-full text-left border-collapse text-xs">
                                    ...
                                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                                        @foreach($alatPemasangan as $alat)
                                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                    ...
                    {{-- FOTO DOKUMENTASI PEMASANGAN, SPEED TEST & BERITA ACARA --}}
                    <div>
...
{{-- CARD LAPORAN UJI HASIL SPEEDTEST (QUALITY REPORT) --}}
@if($hasTestReport)
    @php
    ...
    <div class="p-4 bg-slate-50/70 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700 rounded-lg space-y-3">
        <div class="flex flex-wrap gap-2 items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-700">
            <div>
                ...
                <span class="text-xs font-bold text-slate-900 dark:text-slate-100">
                    Waktu Uji: {{ $tech->test_date ? \App\Support\IndonesianDate::date($tech->test_date) : '-' }}{{ $tech->test_time ? ' — ' . substr($tech->test_time, 0, 5) . ' WIB' : '' }}
                </span>
            </div>
            <span class="text-[10px] font-mono font-bold px-2.5 py-1 rounded border {{ $conformityBadge }}">
                @if($conformity !== null)
                    {{ number_format($conformity, 1) }}% (Target Paket {{ $paketTarget > 0 ? rtrim(rtrim(number_format($paketTarget, 2, '.', ''), '0'), '.') . ' Mbps' : '-' }} — Hasil {{ $tech->test_download !== null ? number_format($tech->test_download, 2) . ' Mbps' : '-' }})
                @else
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono text-xs">
            <div class="p-3 bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
                ...
                <span class="text-base font-bold text-slate-900 dark:text-slate-100 searchable-text">{{ $tech->test_download !== null ? number_format($tech->test_download, 2) . ' Mbps' : '-' }}</span>
            </div>
            <div class="p-3 bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
                <span class="text-[10px] text-slate-400 font-sans block">Speed Upload</span>
                <span class="text-base font-bold text-slate-900 dark:text-slate-100 searchable-text">{{ $tech->test_upload !== null ? number_format($tech->test_upload, 2) . ' Mbps' : '-' }}</span>
            </div>
            <div class="p-3 bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
                ...
                <span class="text-base font-bold text-slate-900 dark:text-slate-100 searchable-text">
                    {{ $tech->latency_ms !== null ? number_format($tech->latency_ms, 1) . 'ms' : '-' }} / {{ $tech->jitter_ms !== null ? number_format($tech->jitter_ms, 1) . 'ms' : '-' }}
                </span>
            </div>
            <div class="p-3 bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
                ...
                <span class="text-base font-bold text-emerald-600 dark:text-emerald-400 searchable-text">
                    {{ $tech->packet_loss_percent !== null ? number_format($tech->packet_loss_percent, 2) . '%' : '0.00%' }} ({{ $tech->quality_score ? $tech->quality_score . '/5 ★' : '—' }})
                </span>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-x-6 gap-y-1 text-xs text-slate-600 dark:text-slate-300 pt-2 border-t border-slate-200 dark:border-slate-700">
            <span>Sinyal Redaman Awal Pemasangan: <strong class="font-mono text-slate-900 dark:text-slate-100">{{ $tech->initial_attenuation ?: '-' }} dBm</strong></span>
            <span>Redaman Terkini (Aktual): <strong class="font-mono text-emerald-600 dark:text-emerald-400">{{ $tech->actual_attenuation ?: '-' }} dBm</strong></span>
            <span>Skor Kualitas Uji: <strong class="font-bold text-emerald-600 dark:text-emerald-400">{{ $tech->quality_score ? $tech->quality_score . '/5' : '-' }}</strong></span>
        </div>
    </div>
@endif
...
<div id="installation-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <form action="{{ route('customers.installation.store', $customer->id) }}" method="POST" enctype="multipart/form-data" class="overflow-y-auto">
            @csrf
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div class="md:col-span-2">
                    ...
                    <div class="flex flex-wrap gap-4">
                        @foreach(['scheduled' => 'Terjadwal', 'in_progress' => 'Proses', 'completed' => 'Selesai', 'failed' => 'Gagal'] as $value => $label)
                            <label class="inline-flex items-center">
                                <input type="radio" name="installation_status" value="{{ $value }}" class="accent-sky-600"
                                       {{ ($latestInst && $latestInst->installation_status === $value) || (! $latestInst && $value === 'scheduled') ? 'checked' : '' }}>
                                <span class="ml-2 text-xs text-slate-800 dark:text-slate-200">{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="fop_id" id="fop_id_inst" placeholder="TFOP-2026-...." value="{{ $latestInst ? $latestInst->fop_id : '' }}"
                           class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono text-slate-800 dark:text-slate-200">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="installation_note" id="installation_note" rows="3" placeholder="Tuliskan catatan pemasangan pelanggan..."
                              class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-800 dark:text-slate-200">{{ $latestInst ? $latestInst->installation_note : '' }}</textarea>
                </div>
...
<div id="test-report-modal" class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <form action="{{ route('customers.test-report.store', $customer->id) }}" method="POST" enctype="multipart/form-data" class="overflow-y-auto">
            @csrf
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div>
