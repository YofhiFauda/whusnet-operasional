    ...
    <div class="border border-border rounded-xl p-5 bg-gradient-to-r from-surface-muted/30 via-surface to-surface-muted/30 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-3 shrink-0">
            <div class="bg-surface border border-border rounded-lg px-3.5 py-2 text-center">
                ...
                <span class="text-base font-extrabold text-text-main">{{ $totalTickets }}</span>
            </div>
        ...
        <div class="space-y-4">
            ...
            @foreach($customerTasks as $task)
                ...
                <div class="border border-border rounded-xl bg-surface shadow-sm overflow-hidden transition-all hover:border-border/80">
                    ...
                    <div onclick="toggleTicketHistory('{{ $uniqueId }}')" class="p-4 bg-surface hover:bg-surface-muted/50 transition-colors cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 select-none">
                        <div class="flex items-start md:items-center gap-3.5">
                            ...
                            <div>
                                ...
                                <h4 class="text-xs font-bold text-text-main mt-1">{{ $task->title }}</h4>
                                @if($task->description)
                                    <p class="text-[11px] text-text-muted mt-0.5 line-clamp-1">{{ $task->description }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 pt-3 md:pt-0 border-border text-xs">
                            <div class="text-right">
                                ...
                                <span class="font-semibold text-text-secondary">{{ $assignedTeam }}</span>
                    ...
                    <div id="{{ $uniqueId }}" class="hidden border-t border-border bg-surface-muted/20 p-5 space-y-6">
                        <!-- Task Metadata Panel -->
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-surface p-4 rounded-xl border border-border text-xs">
                            <div>
                                ...
                                <span class="font-bold text-text-main">{{ $task->creator ? $task->creator->name : 'System' }}</span>
                                <span class="block text-[10px] text-text-muted">{{ $task->created_at ? \Carbon\Carbon::parse($task->created_at)->translatedFormat('d M Y, H:i') : '-' }} WIB</span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-text-muted uppercase">Jadwal / Terjadwal</span>
                                <span class="font-bold text-text-main">{{ $task->scheduled_at ? \Carbon\Carbon::parse($task->scheduled_at)->translatedFormat('d M Y, H:i') : 'Belum Dijadwalkan' }}</span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-text-muted uppercase">FOP Reviewer</span>
                                <span class="font-bold text-text-main">{{ $task->fop ? $task->fop->name : '-' }}</span>
                                <span class="block text-[10px] text-text-muted">Status: {{ strtoupper($task->fop_review_status ?? 'PENDING') }}</span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-text-muted uppercase">Target SLA</span>
                                <span class="font-bold text-text-main">{{ $task->sla_minutes ? $task->sla_minutes . ' Menit' : '-' }}</span>
                            </div>
                        </div>
                                    ...
                                    <div class="bg-surface p-4 rounded-xl border border-border">
                                        ...
                                        <div class="grid grid-cols-3 gap-2">
                                            @foreach($task->evidences as $ev)
                                                <a href="{{ Storage::url($ev->file_path) }}" target="_blank" class="block aspect-square rounded-lg overflow-hidden border border-border bg-surface-muted hover:opacity-90">
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="text-xs">
                                                ...
                                                <div class="text-text-muted mt-0.5 flex items-center gap-1.5">
                                                    <span>Oleh:</span>
                                                    <span class="font-semibold text-text-secondary">{{ $alog->user ? $alog->user->name : 'System' }}</span>
                                                </div>
                                                @if(!empty($alog->new_values))
                                                    <div class="mt-1.5 p-2 rounded bg-surface-muted border border-border font-mono text-[11px] text-text-secondary">
                                                        @foreach($alog->new_values as $k => $v)
                                                            @if(is_scalar($v))
                                                                <div><strong class="text-text-secondary">{{ ucwords(str_replace('_', ' ', $k)) }}:</strong> {{ $v }}</div>
                                                            @endif
                                                        @endforeach
                                                    </div>
                                                @endif
                                ...
                                <div class="relative pl-5 border-l-2 border-border space-y-4 ml-1">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-text-muted">Dibuat oleh {{ $task->creator ? $task->creator->name : 'System' }} pada {{ \Carbon\Carbon::parse($task->created_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-text-muted">Jadwal: {{ \Carbon\Carbon::parse($task->scheduled_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-text-muted">Mulai dikerjakan pada {{ \Carbon\Carbon::parse($task->started_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-text-muted">Selesai pada {{ \Carbon\Carbon::parse($task->completed_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-text-muted">Dibatalkan pada {{ \Carbon\Carbon::parse($task->cancelled_at)->translatedFormat('d M Y, H:i') }} WIB. Alasan: {{ $task->cancel_reason ?? '-' }}</p>
                                        </div>
                                    </div>
                                    @endif
                                </div>
            ...
            @foreach($customerFopTasks as $ftask)
                ...
                <div class="border border-border rounded-xl bg-surface shadow-sm overflow-hidden transition-all hover:border-border/80">
                    <div onclick="toggleTicketHistory('{{ $uniqueId }}')" class="p-4 bg-surface hover:bg-surface-muted/50 transition-colors cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 select-none">
                        <div class="flex items-start md:items-center gap-3.5">
                            ...
                            <div>
                                ...
                                <h4 class="text-xs font-bold text-text-main mt-1">{{ $ftask->issue ?? $ftask->tugas ?? 'Tugas Lapangan' }}</h4>
                                @if($ftask->notes)
                                    <p class="text-[11px] text-text-muted mt-0.5 line-clamp-1">{{ $ftask->notes }}</p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div id="{{ $uniqueId }}" class="hidden border-t border-border bg-surface-muted/20 p-5 space-y-4">
                        <div class="bg-surface p-5 rounded-xl border border-border">
                            <h5 class="text-xs font-bold text-text-main uppercase tracking-wider mb-4 pb-2 border-b border-border flex items-center gap-2">
                                <svg class="w-4 h-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            ...
                            <div class="relative pl-5 border-l-2 border-primary/20 space-y-4 ml-1">
                                <div class="relative">
                                    ...
                                    <div class="text-xs">
                                        ...
                                        <p class="text-text-muted">Tanggal Tugas: {{ $ftask->task_date ? \Carbon\Carbon::parse($ftask->task_date)->translatedFormat('d M Y, H:i') : '-' }} WIB</p>
                                        @if($ftask->notes)
                                            <p class="mt-1.5 p-2 rounded bg-surface-muted border border-border font-mono text-[11px] text-text-secondary">{{ $ftask->notes }}</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
