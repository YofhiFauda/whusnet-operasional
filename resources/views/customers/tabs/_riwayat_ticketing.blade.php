    ...
    <div class="border border-slate-200 dark:border-slate-700 rounded-lg p-5 bg-slate-50/70 dark:bg-slate-900/40 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-3 shrink-0">
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3.5 py-2 text-center">
                ...
                <span class="text-base font-extrabold text-slate-900 dark:text-slate-100">{{ $totalTickets }}</span>
            </div>
            <div class="bg-emerald-50/10 border border-emerald-500/30 rounded-lg px-3.5 py-2 text-center">
                ...
                <span class="text-base font-extrabold text-emerald-500">{{ $completedTickets }}</span>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-4">
            ...
            @foreach($customerTasks as $task)
                ...
                <div class="border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 shadow-sm overflow-hidden transition-all hover:border-slate-300 dark:border-slate-600">
                    ...
                    <div onclick="toggleTicketHistory('{{ $uniqueId }}')" class="p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:bg-slate-900/50 transition-colors cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 select-none">
                        <div class="flex items-start md:items-center gap-3.5">
                            ...
                            <div>
                                ...
                                <h4 class="text-xs font-bold text-slate-900 dark:text-slate-100 mt-1">{{ $task->title }}</h4>
                                @if($task->description)
                                    <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{{ $task->description }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 pt-3 md:pt-0 border-slate-200 dark:border-slate-700 text-xs">
                            <div class="text-right">
                                ...
                                <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $assignedTeam }}</span>
                    ...
                    <div id="{{ $uniqueId }}" class="hidden border-t border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 p-5 space-y-6">
                        <!-- Task Metadata Panel -->
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-700 text-xs">
                            <div>
                                ...
                                <span class="font-bold text-slate-900 dark:text-slate-100">{{ $task->creator ? $task->creator->name : 'System' }}</span>
                                <span class="block text-[10px] text-slate-500 dark:text-slate-400">{{ $task->created_at ? \Carbon\Carbon::parse($task->created_at)->translatedFormat('d M Y, H:i') : '-' }} WIB</span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Jadwal / Terjadwal</span>
                                <span class="font-bold text-slate-900 dark:text-slate-100">{{ $task->scheduled_at ? \Carbon\Carbon::parse($task->scheduled_at)->translatedFormat('d M Y, H:i') : 'Belum Dijadwalkan' }}</span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">FOP Reviewer</span>
                                <span class="font-bold text-slate-900 dark:text-slate-100">{{ $task->fop ? $task->fop->name : '-' }}</span>
                                <span class="block text-[10px] text-slate-500 dark:text-slate-400">Status: {{ strtoupper($task->fop_review_status ?? 'PENDING') }}</span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase">Target SLA</span>
                                <span class="font-bold text-slate-900 dark:text-slate-100">{{ $task->sla_minutes ? $task->sla_minutes . ' Menit' : '-' }}</span>
                            </div>
                        </div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="text-xs">
                                                ...
                                                <div class="text-slate-500 dark:text-slate-400 mt-0.5 flex items-center gap-1.5">
                                                    <span>Oleh:</span>
                                                    <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $alog->user ? $alog->user->name : 'System' }}</span>
                                                </div>
                                                @if(!empty($alog->new_values))
                                                    <div class="mt-1.5 p-2 rounded bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 font-mono text-[11px] text-slate-700 dark:text-slate-300">
                                                        @foreach($alog->new_values as $k => $v)
                                                            @if(is_scalar($v))
                                                                <div><strong class="text-slate-700 dark:text-slate-300">{{ ucwords(str_replace('_', ' ', $k)) }}:</strong> {{ $v }}</div>
                                                            @endif
                                                        @endforeach
                                                    </div>
                                                @endif
                                ...
                                <div class="relative pl-5 border-l-2 border-slate-200 dark:border-slate-700 space-y-4 ml-1">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400">Dibuat oleh {{ $task->creator ? $task->creator->name : 'System' }} pada {{ \Carbon\Carbon::parse($task->created_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400">Jadwal: {{ \Carbon\Carbon::parse($task->scheduled_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400">Mulai dikerjakan pada {{ \Carbon\Carbon::parse($task->started_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400">Selesai pada {{ \Carbon\Carbon::parse($task->completed_at)->translatedFormat('d M Y, H:i') }} WIB</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400">Dibatalkan pada {{ \Carbon\Carbon::parse($task->cancelled_at)->translatedFormat('d M Y, H:i') }} WIB. Alasan: {{ $task->cancel_reason ?? '-' }}</p>
                                        </div>
                                    </div>
                                    @endif
                                </div>
            ...
            @foreach($customerTickets as $ticket)
                ...
                <div class="border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 shadow-sm overflow-hidden transition-all hover:border-slate-300 dark:border-slate-600">
                    <div onclick="toggleTicketHistory('{{ $uniqueId }}')" class="p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:bg-slate-900/50 transition-colors cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 select-none">
                        <div class="flex items-start md:items-center gap-3.5">
                            <div class="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center shrink-0 {{ $isFop ? 'text-teal-600' : 'text-amber-600' }} font-mono text-[11px] font-bold">
                            ...
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    @if($isFop)
                                        <span class="font-mono font-bold text-xs text-teal-600">{{ $ticket->fopTask?->task_number ?? $ticket->ticket_number }}</span>
                                        <span class="px-2 py-0.5 rounded text-[10px] font-bold border bg-teal-50/10 text-teal-500 border-teal-500/20">TICKET FOP</span>
                                    @else
                                        <span class="font-mono font-bold text-xs text-amber-600">{{ $ticket->ticket_number }}</span>
                                        <span class="px-2 py-0.5 rounded text-[10px] font-bold border bg-amber-50/10 text-amber-600 border-amber-500/20">TICKET {{ strtoupper($ticket->handler->label()) }}</span>
                                    @endif
                                    <span class="px-2 py-0.5 rounded text-[10px] font-semibold border {{ $ticket->statusBadgeClasses() }}">{{ strtoupper($ticket->statusLabel()) }}</span>
                                </div>
                                <h4 class="text-xs font-bold text-slate-900 dark:text-slate-100 mt-1">{{ $typeLabel }} — {{ $ticket->detail_keluhan ?? 'Tiket Pelanggan' }}</h4>
                                @if(!$isFop && $ticket->catatan_teknis)
                                    <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{{ $ticket->catatan_teknis }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 pt-3 md:pt-0 border-slate-200 dark:border-slate-700 text-xs">
                            <div class="text-right">
                                <span class="block text-[10px] text-slate-500 dark:text-slate-400 font-medium">{{ $isFop ? 'Teknisi Ditugaskan' : 'Ditangani Oleh' }}</span>
                                <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $isFop ? $assignedTeam : $ticket->handler->label() }}</span>
                                <span class="block text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                                    {{ $ticket->created_at ? \Carbon\Carbon::parse($ticket->created_at)->translatedFormat('d M Y') : '-' }}
                                </span>
                            </div>
                    ...
                    <div id="{{ $uniqueId }}" class="hidden border-t border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 p-5 space-y-4">
                        <div class="bg-white dark:bg-slate-800 p-5 rounded-lg border border-slate-200 dark:border-slate-700">
                            <h5 class="text-xs font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider mb-4 pb-2 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between gap-2">
                                <span class="flex items-center gap-2">
                                    <svg class="w-4 h-4 text-sky-600 dark:text-sky-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    {{ $isFop ? 'Riwayat Penanganan Ticket FOP' : 'Riwayat Penanganan Ticket ' . $ticket->handler->label() }}
                                </span>
                                @if($isFop && $ticket->fop_task_id)
                                    <a href="{{ route('fop-tasks.history.show', $ticket->fop_task_id) }}" class="text-[11px] font-semibold text-sky-600 hover:underline normal-case">Lihat Detail FOP &rarr;</a>
                                @else
                                    <a href="{{ route('tickets.show', $ticket->id) }}" class="text-[11px] font-semibold text-sky-600 hover:underline normal-case">Lihat Detail Tiket &rarr;</a>
                                @endif
                            </h5>
                            ...
                            <div class="relative pl-5 border-l-2 border-sky-200 dark:border-sky-800 space-y-4 ml-1">
                                <div class="relative">
                                    ...
                                    <div class="text-xs">
                                        ...
                                        <p class="text-slate-500 dark:text-slate-400">Oleh {{ $ticket->creator?->name ?? 'System' }} pada {{ $ticket->created_at ? \Carbon\Carbon::parse($ticket->created_at)->translatedFormat('d M Y, H:i') : '-' }} WIB</p>
                                    </div>
                                </div>
                                ...
                                @if($isFop)
                                    <div class="relative">
                                        ...
                                        <div class="text-xs">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400">Status pengerjaan lapangan saat ini: {{ $ticket->statusLabel() }}</p>
                                        </div>
                                    </div>
                                @elseif($ticket->status === \App\Enums\TicketHandlingStatus::CLOSED)
                                    <div class="relative">
                                        <div class="absolute -left-[25px] top-1 w-4 h-4 rounded-full bg-emerald-600 border-2 border-white dark:border-slate-800"></div>
                                        <div class="text-xs">
                                            <span class="font-bold text-emerald-500">Selesai di {{ $ticket->handler->label() }}</span>
                                            <p class="text-slate-500 dark:text-slate-400">Diselesaikan pada {{ $ticket->resolved_at ? \Carbon\Carbon::parse($ticket->resolved_at)->translatedFormat('d M Y, H:i') : '-' }} WIB</p>
                                        </div>
                                    </div>
                                @elseif($ticket->status === \App\Enums\TicketHandlingStatus::CANCELLED)
                                    <div class="relative">
                                        <div class="absolute -left-[25px] top-1 w-4 h-4 rounded-full bg-rose-600 border-2 border-white dark:border-slate-800"></div>
                                        <div class="text-xs">
                                            <span class="font-bold text-rose-500">Dibatalkan di {{ $ticket->handler->label() }}</span>
                                        </div>
                                    </div>
                                @else
                                    <div class="relative">
                                        <div class="absolute -left-[25px] top-1 w-4 h-4 rounded-full bg-amber-500 border-2 border-white dark:border-slate-800"></div>
                                        <div class="text-xs">
                                            <span class="font-bold text-slate-900 dark:text-slate-100">Sedang ditangani {{ $ticket->handler->label() }}</span>
                                        </div>
                                    </div>
                                @endif
                            </div>
                ...
                <div class="border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 shadow-sm overflow-hidden transition-all hover:border-slate-300 dark:border-slate-600">
                    <div onclick="toggleTicketHistory('{{ $uniqueId }}')" class="p-4 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:bg-slate-900/50 transition-colors cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 select-none">
                        <div class="flex items-start md:items-center gap-3.5">
                            ...
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span class="font-mono font-bold text-xs text-teal-600">{{ $ftask->task_number ?? ('FOP-TSK-' . $ftask->id) }}</span>
                                ...
                                <h4 class="text-xs font-bold text-slate-900 dark:text-slate-100 mt-1">{{ $ftask->issue ?? $ftask->tugas ?? 'Tugas Lapangan' }}</h4>
                                @if($ftask->notes)
                                    <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{{ $ftask->notes }}</p>
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="relative pl-5 border-l-2 border-sky-200 dark:border-sky-800 space-y-4 ml-1">
                                <div class="relative">
                                    ...
                                    <div class="text-xs">
                                        ...
                                        <p class="text-slate-500 dark:text-slate-400">Tanggal Tugas: {{ $ftask->task_date ? \Carbon\Carbon::parse($ftask->task_date)->translatedFormat('d M Y, H:i') : '-' }} WIB</p>
                                        @if($ftask->notes)
                                            <p class="mt-1.5 p-2 rounded bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 font-mono text-[11px] text-slate-700 dark:text-slate-300">{{ $ftask->notes }}</p>
                                        @endif
                                    </div>
                                </div>
                            </div>
