                    ...
                    <tr class="hover:bg-slate-50/45 dark:hover:bg-slate-700/25 transition-colors" id="invoice-row-{{ $invoice->id }}">
                        ...
                        <td class="px-6 py-3.5 whitespace-nowrap">
                            <div class="flex items-center gap-1.5">
                                ...
                                <a href="{{ route('invoices.show', $invoice->id) }}"
                                   class="font-mono font-bold {{ $invoiceNumberColor }}"
                                   @if($hasPiutang) title="Pelanggan memiliki piutang" @endif>{{ $invoice->invoice_number }}</a>
                            </div>
                        ...
                        <td class="px-6 py-3.5 text-right whitespace-nowrap">
                            <div class="inline-flex items-center gap-1.5">
                                @can('payments.create')
                                    <span id="invoice-pay-btn-{{ $invoice->id }}" class="{{ in_array($invoice->invoice_status->value, ['lunas', 'batal'], true) ? 'hidden' : '' }}">
                                        <button type="button"
                                ...
                                @endcan
                                @if($invoice->invoice_status->value === 'lunas' && auth()->user()->hasPermission('payments.view') && $latestInvoicePayment)
                                    <a href="{{ route('payments.receipt', $latestInvoicePayment->id) }}" target="_blank" class="inline-flex items-center gap-1 px-2.5 py-1.5 border border-sky-200 dark:border-sky-800 bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 hover:bg-sky-100 dark:hover:bg-sky-900/60 rounded-md transition-colors text-xs font-semibold" title="Cetak Struk Pembayaran Terakhir">
...
{{-- Realtime tanpa reload: patch baris invoice di tempat begitu payment
     tercatat, baik dari aksi sendiri (event `payment-recorded` dari modal)
     maupun dari user lain di POP yang sama (broadcast Reverb
     App\Events\InvoiceStatusUpdated). Baris cicilan baru tetap butuh buka
     ulang/expand — di luar cakupan Fase 1 (docs/plan/analisa-realtime-spa-
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const popIds = @json($invoices->pluck('pop_id')->unique()->values());
