...
@section('breadcrumb_parent', 'Daftar Tagihan')
...
<div class="space-y-6 screen-only max-w-full pb-20 md:pb-8">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-surface p-5 sm:p-6 rounded-2xl border border-border shadow-2xs">
        <div class="space-y-1.5">
            <div class="flex items-center gap-2.5 flex-wrap">
                <a href="{{ route('invoices.index') }}" class="p-1.5 text-text-muted hover:text-text-main rounded-lg hover:bg-surface-muted transition-colors" title="Kembali">
                ...
                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-surface-muted border border-border text-xs font-mono font-bold text-text-main">
                    ...
                    <button onclick="copyToClipboard('{{ $invoice->invoice_number }}', 'No. Invoice')" class="text-text-muted hover:text-primary transition-colors cursor-pointer" title="Salin No. Invoice">
                ...
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border {{ $badgeClass }}">
                    <span class="w-1.5 h-1.5 rounded-full bg-current {{ $invoice->invoice_status->value === 'sebagian' ? 'animate-pulse' : '' }}"></span>
            ...
            <p class="text-xs text-text-muted flex items-center gap-2">
                <span>Periode: <strong class="text-text-main font-mono">{{ $invoice->billing_period }}</strong></span>
                <span>&bull;</span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 xl:gap-5">
        ...
        <div class="bg-surface border border-border rounded-2xl p-5 shadow-2xs transition-all hover:border-primary/40 hover:shadow-md duration-200">
            ...
            <div class="text-xl sm:text-2xl font-bold font-mono text-text-main">Rp {{ number_format($totalAmount, 0, ',', '.') }}</div>
            <div class="mt-2 text-[11px] text-text-muted flex justify-between">
                <span>Harga Paket Subtotal:</span>
                <span class="font-mono font-medium text-text-main">Rp {{ number_format((float) $invoice->subtotal, 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <div class="bg-surface border border-border rounded-2xl p-5 shadow-2xs transition-all hover:border-amber-500/40 hover:shadow-md duration-200">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="p-2 rounded-xl {{ $remainingAmount > 0 ? 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400' : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' }}">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </div>
            </div>
            <div class="text-xl sm:text-2xl font-bold font-mono {{ $remainingAmount > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                Rp {{ number_format($remainingAmount, 0, ',', '.') }}
            </div>
            <div class="mt-2 text-[11px] text-text-muted flex justify-between">
                <span>Sudah Terbayar:</span>
                <span class="font-mono font-medium text-emerald-600 dark:text-emerald-400">Rp {{ number_format($paidAmount, 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <div class="bg-surface border border-border rounded-2xl p-5 shadow-2xs transition-all hover:border-purple-500/40 hover:shadow-md duration-200">
            ...
            <div class="text-xl sm:text-2xl font-bold font-mono text-text-main">{{ optional($invoice->due_date)->format('d/m/Y') ?? '-' }}</div>
            <div class="mt-2 text-[11px] text-text-muted flex justify-between">
                <span>Tanggal Terbit:</span>
                <span class="font-mono font-medium text-text-main">{{ optional($invoice->issue_date)->format('d/m/Y') ?? '-' }}</span>
            </div>
        </div>
        ...
        <div class="bg-surface border border-border rounded-2xl p-5 shadow-2xs transition-all hover:border-emerald-500/40 hover:shadow-md duration-200">
            ...
            <div class="text-lg sm:text-xl font-bold text-text-main truncate">{{ $invoice->pop->name ?? '-' }}</div>
            <div class="mt-2 text-[11px] text-text-muted truncate">
                Oleh: {{ $invoice->creator->name ?? 'System' }}
            </div>
        </div>
    </div>
            ...
            <div class="bg-surface border border-border rounded-2xl overflow-hidden shadow-2xs">
                ...
                <div id="pane-items" class="p-6 space-y-6">
                    <div class="flex items-center justify-between border-b border-border pb-3">
                        ...
                        <span class="text-xs font-mono text-text-muted">ID Invoice: {{ $invoice->id }}</span>
                    </div>
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="flex justify-between items-center py-2 border-b border-border">
                            <div class="space-y-0.5">
                                ...
                                <p class="text-[11px] text-text-muted">{{ $invoice->customerService->package_name_snapshot ?? $invoice->internetPackage->name ?? 'Paket Internet' }} &bull; Periode {{ $invoice->billing_period }}</p>
                            </div>
                            <span class="font-mono font-bold text-text-main text-sm">Rp {{ number_format((float) $invoice->subtotal, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-border text-emerald-600 dark:text-emerald-400">
                            ...
                            <span class="font-mono font-semibold text-sm">- Rp {{ number_format((float) $invoice->discount, 0, ',', '.') }}</span>
                        </div>
                        @endif
                        ...
                        @if($ppnRate > 0)
                        <div class="flex justify-between items-center py-2 border-b border-border">
                            <span class="text-text-secondary font-medium">PPN ({{ number_format($ppnRate, 0) }}%)</span>
                            <span class="font-mono font-semibold text-text-main text-sm">Rp {{ number_format($ppnAmount, 0, ',', '.') }}</span>
                        </div>
                        @else
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-border">
                            ...
                            <span class="font-mono font-semibold text-text-main text-sm">Rp {{ number_format((float) $invoice->prorate_amount, 0, ',', '.') }}</span>
                        </div>
                        @endif
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-border">
                            ...
                            <span class="font-mono font-semibold text-text-main text-sm">Rp {{ number_format((float) $invoice->extra_cable_fee, 0, ',', '.') }}</span>
                        </div>
                        @endif
                        ...
                        <div class="pt-4 space-y-2.5 border-t-2 border-border">
                            <div class="flex justify-between items-center text-sm font-bold">
                                ...
                                <span class="font-mono text-lg text-text-main">Rp {{ number_format($totalAmount, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-xs text-emerald-600 dark:text-emerald-400 font-semibold">
                                <span>Sudah Terbayar</span>
                                <span class="font-mono text-sm">- Rp {{ number_format($paidAmount, 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm font-extrabold pt-2.5 border-t border-border">
                                <span class="{{ $remainingAmount > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                                    {{ $remainingAmount > 0 ? 'SISA YANG HARUS DIBAYAR' : 'STATUS TAGIHAN LUNAS' }}
                                </span>
                                <span class="font-mono text-xl {{ $remainingAmount > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                                    Rp {{ number_format($remainingAmount, 0, ',', '.') }}
                                </span>
                            </div>
                        </div>
                                    ...
                                    @foreach($invoice->payments as $payment)
                                        ...
                                        <tr class="hover:bg-surface-muted/50 transition-colors">
                                            <td class="px-5 py-3.5 font-semibold text-text-main">
                                                @if($meta)
                                                    Cicilan Ke-{{ $meta['number'] }}
                                                @else
                                                    &mdash;
                                                @endif
                                            </td>
                                            <td class="px-4 py-3.5 font-mono font-bold text-primary">
                                                <a href="{{ route('payments.show', $payment->id) }}" class="hover:underline">
                                            ...
                                            <td class="px-4 py-3.5 font-medium text-text-main whitespace-nowrap">{{ $payment->receiver->name ?? '-' }}</td>
                ...
                <div id="pane-audit" class="hidden p-6 space-y-4">
                    ...
                    <div class="p-4 bg-surface-muted/50 rounded-xl border border-border space-y-2.5 text-xs max-w-md">
                        ...
                        <div class="flex justify-between items-center pb-2 border-b border-border">
                            ...
                            <span class="font-mono font-bold text-text-main">{{ $invoice->old_invoice_id }}</span>
                        </div>
                        @endif
                        @if($invoice->old_cost_id)
                        <div class="flex justify-between items-center pb-2 border-b border-border">
                            <span class="text-text-muted">ID Biaya Lama:</span>
                            <span class="font-mono font-bold text-text-main">{{ $invoice->old_cost_id }}</span>
                        </div>
                        @endif
                        @if($invoice->old_request_id)
                        <div class="flex justify-between items-center">
                            <span class="text-text-muted">ID Permintaan Lama:</span>
                            <span class="font-mono font-bold text-text-main">{{ $invoice->old_request_id }}</span>
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="lg:col-span-5 xl:col-span-4 lg:sticky lg:top-6 space-y-6 self-start">
            ...
            <div class="bg-surface border border-border rounded-2xl p-5 shadow-2xs space-y-4">
                <div class="flex items-center justify-between pb-3 border-b border-border">
                    ...
                    <a href="{{ route('customers.show', $invoice->customer_id) }}" class="text-xs font-semibold text-primary hover:underline flex items-center gap-1">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="space-y-1 min-w-0">
                        <a href="{{ route('customers.show', $invoice->customer_id) }}" class="font-bold text-text-main text-sm hover:text-primary transition-colors block truncate">
                            {{ $invoice->customer->full_name ?? '-' }}
                        </a>
                        <div class="flex items-center gap-1 text-xs text-text-muted font-mono">
                            <span>CID: {{ $invoice->customer->cid ?? $invoice->customer->customer_code ?? '-' }}</span>
                            @if($invoice->customer->cid || $invoice->customer->customer_code)
                                <button onclick="copyToClipboard('{{ $invoice->customer->cid ?? $invoice->customer->customer_code }}', 'CID')" class="hover:text-primary transition-colors cursor-pointer" title="Salin CID">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                </button>
                            @endif
                        </div>
                ...
                <div class="space-y-2.5 text-xs pt-3 border-t border-dashed border-border text-text-secondary">
                    <div class="flex justify-between gap-2">
                        ...
                        <span class="font-mono font-semibold text-text-main">{{ $invoice->customer->primary_phone ?? $invoice->customer->phone ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between gap-2">
                        <span class="text-text-muted">POP / Cabang:</span>
                        <span class="font-semibold text-text-main">{{ $invoice->pop->name ?? '-' }}</span>
                    </div>
                    <div>
                ...
                <div class="space-y-3 text-xs">
                    <div>
                        ...
                        <p class="font-bold text-text-main text-sm mt-0.5">
                            {{ $invoice->customerService->package_name_snapshot ?? $invoice->internetPackage->name ?? 'Paket Internet ISP' }}
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 pt-1">
                        <div class="p-2.5 rounded-xl bg-surface-muted/60 border border-border">
                            ...
                            <span class="font-mono font-bold text-text-main text-xs">
                                {{ $invoice->customerService->download_speed_snapshot ?? '-' }} / {{ $invoice->customerService->upload_speed_snapshot ?? '-' }}
                            </span>
                        </div>
                        <div class="p-2.5 rounded-xl bg-surface-muted/60 border border-border">
                            <span class="text-[10px] text-text-muted block">Harga Langganan</span>
                            <span class="font-mono font-bold text-text-main text-xs">
                                Rp {{ number_format((float) ($invoice->customerService->monthly_price ?? $invoice->subtotal), 0, ',', '.') }}
                            </span>
                        </div>
                    </div>
                </div>
...
<div class="fixed bottom-0 inset-x-0 z-30 bg-surface/90 backdrop-blur-md border-t border-border p-3 flex items-center gap-2 sm:hidden no-print mobile-action-bar">
    @if($customerPhone)
        <a href="{{ $waUrl }}" target="_blank" class="p-3 bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 rounded-xl border border-emerald-200 dark:border-emerald-800 flex items-center justify-center shrink-0" title="Kirim WA">
