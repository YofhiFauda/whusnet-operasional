...
<form action="{{ route('master.distribusi.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl mx-auto">
    ...
    <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm col-span-1 lg:col-span-2 space-y-5">
        <div class="border-b border-slate-100 pb-3">
            ...
            <p class="text-xs text-slate-400 mt-0.5">Lengkapi form berikut untuk menambahkan master data distribusi baru.</p>
        </div>
        ...
        <div class="space-y-4 pt-2">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" required placeholder="Contoh: Distribusi Area Sleman Timur..."
                          class="w-full px-3 py-2 border @error('description') border-rose-500 focus:ring-rose-500 @else border-slate-300 focus:ring-sky-500 focus:border-sky-500 @enderror rounded-md text-sm text-slate-800 focus:outline-none focus:ring-1">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-[10px] text-rose-500 mt-1 font-semibold">{{ $message }}</p>
                @enderror
            </div>
