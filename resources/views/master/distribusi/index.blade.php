...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 mb-6 shadow-sm">
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200">Manajemen Distribusi / Wilayah Cabang</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Kelola data distribusi area yang berada di bawah naungan POP / Cabang.</p>
        </div>
        <div class="flex flex-wrap items-center gap-4 md:gap-6">
            <div class="text-center">
                <span class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">TOTAL DISTRIBUSI</span>
                <span class="text-lg font-bold text-slate-800 dark:text-slate-200 data-text">{{ \App\Models\Distribution::count() }}</span>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 mb-6 shadow-sm">
    <form action="{{ route('master.distribusi.index') }}" method="GET" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full lg:max-w-2xl">
            ...
            <!-- Filter POP -->
            <div>
                <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5">Cabang / POP</label>
                <select name="pop_id" class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500 bg-white dark:bg-slate-800">
        ...
        <div class="flex items-center gap-2 w-full lg:w-auto shrink-0 justify-end">
            ...
            @if($search || $pop_id)
            <a href="{{ route('master.distribusi.index') }}" class="inline-flex items-center justify-center p-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm text-sm font-medium text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-800/50 transition-colors focus:outline-none cursor-pointer" title="Reset Filters">
            ...
            <a href="{{ route('master.distribusi.create') }}" class="flex-1 lg:flex-none inline-flex items-center justify-center gap-2 px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-semibold text-white bg-sky-600 dark:bg-sky-500 hover:bg-sky-700 transition-colors focus:outline-none cursor-pointer">
...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden shadow-sm">
    ...
    <div class="p-16 text-center">
        ...
        <h4 class="text-sm font-bold text-slate-800 dark:text-slate-200">Tidak ada Distribusi ditemukan</h4>
        ...
        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            <thead class="bg-slate-50 dark:bg-slate-800/50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nama Distribusi</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Deskripsi</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Cabang</th>
                    @if(auth()->user()->hasPermission('manage_pop'))
            ...
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700/50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ $distribusi->name }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-slate-600 dark:text-slate-400">{{ $distribusi->description }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($distribusi->pop)
                            <div class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ $distribusi->pop->name }}</div>
                            <span class="block text-[10px] text-slate-400 dark:text-slate-500 font-mono">{{ $distribusi->pop->code }}</span>
                        @else
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div class="flex items-center justify-center gap-2">
                            <!-- Edit -->
                            <a href="{{ route('master.distribusi.edit', $distribusi) }}" class="p-1 text-slate-400 dark:text-slate-500 hover:text-sky-600 hover:bg-sky-50 dark:hover:bg-sky-900/20 rounded-md transition-colors" title="Ubah Distribusi">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
