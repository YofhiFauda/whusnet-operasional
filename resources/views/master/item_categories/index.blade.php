...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 mb-6 shadow-sm">
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200">Master Kategori Barang</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Kategori dipakai di Master Barang, Estimasi Kebutuhan Alat (survey), Perangkat Pasif Terpakai (pemasangan), dan Jenis Perangkat Pasif di data teknis pelanggan. Kategori tidak bisa dihapus — laporan lama menyimpan kodenya; yang sudah tidak dipakai cukup dinonaktifkan.</p>
        </div>
        <div class="flex flex-wrap items-center gap-4 md:gap-6">
            <div class="text-center">
                ...
                <span class="text-lg font-bold text-slate-800 dark:text-slate-200 data-text">{{ \App\Models\ItemCategory::count() }}</span>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 mb-6 shadow-sm">
    <form action="{{ route('master.item-categories.index') }}" method="GET" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full lg:max-w-2xl">
            <div>
                ...
                <div class="relative">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Kode atau nama kategori..."
        ...
        <div class="flex items-center gap-2 w-full lg:w-auto shrink-0 justify-end">
            ...
            @if($search || $status)
            <a href="{{ route('master.item-categories.index') }}" class="inline-flex items-center justify-center p-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm text-sm font-medium text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors focus:outline-none cursor-pointer" title="Reset Filters">
            ...
            <a href="{{ route('master.item-categories.create') }}" class="flex-1 lg:flex-none inline-flex items-center justify-center gap-2 px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-semibold text-white bg-sky-600 dark:bg-sky-500 hover:bg-sky-700 transition-colors focus:outline-none cursor-pointer">
...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden shadow-sm">
    ...
    <div class="p-16 text-center">
        ...
        <p class="text-xs text-slate-400 dark:text-slate-500 max-w-sm mx-auto mt-1">
            ...
            @else
            Mulai dengan menambahkan kategori baru menggunakan tombol Tambah Kategori.
            @endif
        </p>
    </div>
    @else
        ...
        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            <thead class="bg-slate-50 dark:bg-slate-800/50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Satuan Default</th>
                    <th scope="col" class="px-6 py-3 text-right text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Jumlah Barang</th>
            ...
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700/50">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="text-sm font-mono text-slate-600 dark:text-slate-400">{{ $category->code }}</span>
                        @if($category->is_system)
                        <span class="ml-1.5 inline-flex px-1.5 py-0.5 text-[10px] font-semibold rounded bg-slate-100 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700" title="Kategori bawaan — kode terkunci karena dipakai data lama">bawaan</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="text-sm font-mono text-slate-600 dark:text-slate-400">{{ $category->default_unit }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right">
                        <span class="text-sm font-mono text-slate-600 dark:text-slate-400">{{ $category->items_count }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('master.item-categories.edit', $category) }}" class="p-1 text-slate-400 dark:text-slate-500 hover:text-sky-600 hover:bg-sky-50 dark:hover:bg-sky-900/20 rounded-md transition-colors" title="Ubah Kategori">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
