...
@section('title', 'Ubah Kategori Paket - Whusnet Operasional')
@section('page_title', 'Ubah Kategori Paket')

@section('content')
<div class="max-w-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6">
    <form action="{{ route('master.package-categories.update', $packageCategory) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $packageCategory->name) }}" required maxlength="100"
            ...
            <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-1">
                Nama dipakai apa adanya sebagai string di paket yang sudah ada — ganti nama di sini TIDAK otomatis mengubah paket lama, paket itu bisa "kehilangan" kategorinya di filter/laporan.
            </p>
        </div>

        <div class="flex items-center gap-2">
            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $packageCategory->is_active) ? 'checked' : '' }}
        ...
        <div>
            ...
            <select name="installation_fee_approval_role_id" class="w-full px-3 py-2 text-xs font-medium border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all">
                ...
                @foreach($roles as $role)
                    <option value="{{ $role->id }}" {{ (string) old('installation_fee_approval_role_id', $packageCategory->installation_fee_approval_role_id) === (string) $role->id ? 'selected' : '' }}>
                        {{ $role->name }}
                    </option>
                @endforeach
            </select>
