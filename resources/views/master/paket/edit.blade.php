...
<div class="max-w-4xl">
    <div class="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/70 flex items-center justify-between">
            <div>
                <h2 class="text-base font-bold text-slate-800">Edit Paket Internet</h2>
                <p class="text-xs text-slate-500 mt-0.5">Ubah data paket dari master internet_packages.</p>
            </div>
            <span class="{{ $paket->is_active ? 'bg-green-50 text-green-700 border-green-100' : 'bg-rose-50 text-rose-700 border-rose-100' }} inline-flex items-center px-2.5 py-0.5 rounded text-[11px] font-bold border">
        ...
        <form action="{{ route('master.paket.update', $paket) }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div>
                    ...
                    <input type="text" id="modem" name="modem" value="{{ old('modem', $paket->modem) }}" class="w-full px-3 py-2 border border-slate-300 rounded-md text-sm">
                </div>
                <div>
                    <label for="max_users" class="block text-xs font-semibold text-slate-600 mb-1.5">Maks User</label>
                    <input type="number" min="1" id="max_users" name="max_users" value="{{ old('max_users', $paket->max_users) }}" class="w-full px-3 py-2 border border-slate-300 rounded-md text-sm">
                </div>
                <div>
                    <label for="ip_address_type" class="block text-xs font-semibold text-slate-600 mb-1.5">Jenis IP</label>
                    <input type="text" id="ip_address_type" name="ip_address_type" value="{{ old('ip_address_type', $paket->ip_address_type) }}" class="w-full px-3 py-2 border border-slate-300 rounded-md text-sm">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" id="profile" name="profile" value="{{ old('profile', $paket->profile) }}" class="w-full px-3 py-2 border border-slate-300 rounded-md text-sm">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="terms" name="terms" rows="3" class="w-full px-3 py-2 border border-slate-300 rounded-md text-sm resize-none">{{ old('terms', $paket->terms) }}</textarea>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-sky-600 rounded-md hover:bg-sky-700">Perbarui Paket</button>
            </div>
        </form>
    </div>
</div>
