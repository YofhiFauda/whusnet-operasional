    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-4 md:gap-6">
            <div class="text-center">
                ...
                <span class="text-lg font-bold text-slate-800 dark:text-slate-200 data-text">{{ \App\Models\Pop::count() }}</span>
            </div>
            ...
            <div class="text-center">
                ...
                <span class="text-lg font-bold text-slate-800 dark:text-slate-200 data-text">{{ \App\Models\Pop::where('type', 'pusat')->count() }}</span>
            </div>
            <div class="h-8 w-px bg-slate-200 dark:bg-slate-700 hidden sm:block"></div>
            <div class="text-center">
                <span class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">CABANG</span>
                <span class="text-lg font-bold text-slate-800 dark:text-slate-200 data-text">{{ \App\Models\Pop::where('type', 'cabang')->count() }}</span>
            </div>
            <div class="h-8 w-px bg-slate-200 dark:bg-slate-700 hidden sm:block"></div>
            <div class="text-center">
                <span class="block text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">MINI POP</span>
                <span class="text-lg font-bold text-slate-800 dark:text-slate-200 data-text">{{ \App\Models\Pop::where('type', 'mini_pop')->count() }}</span>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 mb-6 shadow-sm">
    <form action="{{ route('master.pop.index') }}" method="GET" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 w-full lg:w-auto shrink-0 justify-end">
            ...
            <a href="{{ route('master.pop.index') }}" class="inline-flex items-center justify-center p-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm text-sm font-medium text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-800/50 transition-colors focus:outline-none cursor-pointer" title="Reset Filters">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            ...
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-100 dark:divide-slate-700/50">
                @foreach($pops as $pop)
                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 dark:bg-slate-800/50 transition-colors pop-row" data-id="{{ $pop->id }}" data-parent-id="{{ $pop->parent_id ?? '' }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center" style="padding-left: {{ ($pop->depth ?? 0) * 1.5 }}rem">
                            ...
                            @else
                                ...
                                @endif
                            @endif
                            <div class="text-sm font-semibold text-slate-800 dark:text-slate-200">{{ $pop->name }}</div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-xs text-slate-500 dark:text-slate-400">
                        <span class="font-mono font-bold text-slate-700 dark:text-slate-300">{{ $pop->pop_code ?? '-' }}</span>
                        <span class="block text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">ID: {{ $pop->registration_prefix ?? '-' }} / CID: {{ $pop->cid_prefix ?? '-' }}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($pop->type === 'pusat')
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">
                        @if($pop->parent)
                        <span class="font-medium text-slate-700 dark:text-slate-300">{{ $pop->parent->name }}</span>
                        <span class="block text-[10px] text-slate-400 dark:text-slate-500 font-mono">({{ $pop->parent->code }})</span>
                        @else
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-xs text-slate-500 dark:text-slate-400">
                        @if($pop->city)
                        <span class="font-medium text-slate-700 dark:text-slate-300">{{ $pop->city }}</span>
                        <span class="block text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{{ $pop->district ?? '-' }}, {{ $pop->village ?? '-' }}</span>
                        @else
                        <span class="text-slate-300">-</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-xs text-slate-500 dark:text-slate-400">
                        @if($pop->pic_name)
                        <span class="font-medium text-slate-700 dark:text-slate-300 block">{{ $pop->pic_name }}</span>
                        <span class="text-slate-400 dark:text-slate-500 font-mono block mt-0.5">{{ $pop->pic_phone ?? '-' }}</span>
                        @else
                        <span class="text-slate-300">-</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            @if(auth()->user()->hasPermission('manage_pop'))
                            <!-- Edit -->
                            <a href="{{ route('master.pop.edit', $pop) }}" class="p-1 text-slate-400 dark:text-slate-500 hover:text-sky-600 hover:bg-sky-50 dark:hover:bg-sky-900/20 rounded-md transition-colors" title="Ubah POP">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <form action="{{ route('master.pop.toggle', $pop) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" 
                                        class="p-1 rounded-md transition-colors cursor-pointer {{ $pop->status === 'active' ? 'text-green-500 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20' : 'text-slate-400 dark:text-slate-500 hover:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20' }}"
