...
@section('title', 'Ubah Rekening Bank - Whusnet Operasional')
@section('page_title', 'Ubah Rekening Bank')
...
<form action="{{ route('master.rekening.update', $account) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl mx-auto">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 shadow-sm col-span-1 lg:col-span-2 space-y-5">
        <div class="border-b border-slate-100 dark:border-slate-700/50 pb-3">
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Pembayaran yang sudah tercatat menyimpan nama bank &amp; nomor rekening apa adanya — mengubah data di sini hanya berlaku untuk pembayaran baru.</p>
        </div>

        @include('master.rekening._form', ['account' => $account])
