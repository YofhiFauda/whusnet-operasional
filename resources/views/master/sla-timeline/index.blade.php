...
<div x-data="slaTimelineMatrix()">
    <div class="bg-white border border-slate-200 rounded-lg p-5 mb-6">
        <h3 class="text-sm font-semibold text-slate-800">Master Timeline SLA</h3>
    ...
    <div class="bg-white border border-slate-200 rounded-lg overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="bg-slate-50 text-slate-500">
                <tr>
                    <th class="text-left px-4 py-3 font-medium sticky left-0 bg-slate-50">Paket Internet</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @foreach ($packages as $package)
                    <tr>
                        <td class="px-4 py-3 font-medium text-slate-800 sticky left-0 bg-white whitespace-nowrap">
                            {{ $package->name }}
                            <div class="text-xs text-slate-400">{{ $package->package_code }}</div>
                        </td>
                        @foreach ($taskTypes as $type)
                            @php
                            ...
                            <td class="px-4 py-2">
                                <div class="flex items-center gap-1">
                                    ...
                                    >
                                    <select
                                        class="rounded-md border border-slate-300 px-1 py-1 text-sm"
    ...
    <p class="text-xs text-slate-500 mt-3" x-show="savedMessage" x-text="savedMessage"></p>
</div>
