...
@section('title', 'Tambah Kategori Issue - Whusnet Operasional')
@section('page_title', 'Tambah Kategori Issue')
...
<!-- Form Container -->
<form action="{{ route('master.ticket-issue-categories.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl mx-auto">
    @csrf
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 shadow-sm col-span-1 lg:col-span-2 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Lemot"
            ...
            <div>
                ...
                <select name="default_priority" id="default_priority" required
                    ...
                    @foreach(\App\Enums\FopTaskPriority::cases() as $priority)
                        <option value="{{ $priority->value }}" {{ old('default_priority') === $priority->value ? 'selected' : '' }}>{{ $priority->value }}</option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            ...
            <div>
                ...
                <select name="sla_source" id="sla_source" required
                    ...
                    <option value="prioritas" {{ old('sla_source', 'prioritas') === 'prioritas' ? 'selected' : '' }}>Prioritas</option>
                    <option value="paket" {{ old('sla_source') === 'paket' ? 'selected' : '' }}>Paket</option>
                </select>
        ...
        <div class="pt-2">
            <label class="flex items-start gap-2.5 cursor-pointer">
                <input type="checkbox" name="is_batch" value="1" {{ old('is_batch') ? 'checked' : '' }}
