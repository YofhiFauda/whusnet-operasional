...
@section('title', 'Tambah Alat Kerja - Whusnet Operasional')
@section('page_title', 'Tambah Alat Kerja')
...
<form action="{{ route('master.work-tools.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl mx-auto">
    @csrf
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 shadow-sm col-span-1 lg:col-span-2 space-y-5">
        <div class="border-b border-slate-100 dark:border-slate-700/50 pb-3">
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Alat kerja dibawa ke lokasi lalu dibawa pulang — tidak habis dipakai, tidak punya jumlah. Kalau barangnya justru ditinggal di pelanggan, tempatnya di Master Barang.</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="code" id="code" value="{{ old('code') }}" required placeholder="Contoh: SPLICER"
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Fusion Splicer"
        ...
        <div>
            ...
            <input type="text" name="note" id="note" value="{{ old('note') }}" placeholder="Keterangan singkat yang muncul di bawah nama alat pada checklist"
