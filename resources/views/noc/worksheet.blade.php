                ...
                <tbody class="divide-y divide-border">
                    @forelse($tickets as $ticket)
                        ...
                        <tr data-ticket-row="{{ $ticket->id }}"
                            ...
                            {{-- Dropdown Aksi Langsung per Baris — eksekusi cepat tanpa perlu buka drawer --}}
                                        ...
                                        <div x-show="open"
                                            ...
                                            <div class="py-1">
                                                @if($actions['can_close'])
                                                    <button type="button"
                                                            @click.stop="open = false; $dispatch('ticket-drawer-action', { id: {{ $ticket->id }}, action: 'close' })"
                                                ...
                                                @if($actions['can_escalate_fop'])
                                                    <button type="button"
                                                            @click.stop="open = false; $dispatch('ticket-drawer-action', { id: {{ $ticket->id }}, action: 'fop' })"
                                            ...
                                            @if($actions['can_return_to_helpdesk'] || $actions['can_cancel'])
                                                <div class="py-1">
                                                    @if($actions['can_return_to_helpdesk'])
                                                        <button type="button"
                                                                @click.stop="open = false; $dispatch('ticket-drawer-action', { id: {{ $ticket->id }}, action: 'return' })"
                                                    ...
                                                    @if($actions['can_cancel'])
                                                        <button type="button"
                                                                @click.stop="open = false; $dispatch('ticket-drawer-action', { id: {{ $ticket->id }}, action: 'cancel' })"
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $tab === 'assign_fop' ? 13 : 11 }}" class="px-3 py-10 text-center text-text-muted">
                                @if($tab === 'assign_fop')
