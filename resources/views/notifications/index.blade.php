        ...
        @forelse($notifications as $notif)
            ...
            <div id="notif-row-{{ $notif->id }}" 
                ...
                <div class="mt-3 sm:mt-0 sm:ml-4 shrink-0 flex gap-2">
                    {{-- Kedua target POST dirender server-side (route()), bukan dirakit
                         di JS dari id notifikasi. ADHOC-20 langkah 3. --}}
                    <button type="button"
                            onclick="toggleRead(this)"
                            data-read="{{ $isRead ? 'true' : 'false' }}"
                            data-row-id="{{ $notif->id }}"
                            data-url-read="{{ route('notifications.markRead', $notif->id) }}"
                            data-url-unread="{{ route('notifications.markUnread', $notif->id) }}"
