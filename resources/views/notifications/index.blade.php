...
<div class="space-y-6">
    ...
    <x-ui.card class="p-4">
        <form method="GET" action="{{ route('notifications.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div>
                <label for="type" class="block text-xs font-semibold text-slate-600 mb-1.5">Tipe Notifikasi</label>
            ...
            <div>
                <label for="user_id" class="block text-xs font-semibold text-slate-600 mb-1.5">Penerima</label>
    ...
    <div class="space-y-4">
        @forelse($notifications as $notif)
            @php
                ...
                $borderClass = $isRead ? 'border-slate-200' : 'border-sky-200';
                $bgClass = $isRead ? 'bg-white opacity-75' : 'bg-sky-50/20';
            ...
            <div id="notif-row-{{ $notif->id }}" class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 border rounded-xl shadow-sm transition-all duration-200 {{ $borderClass }} {{ $bgClass }} hover:shadow-md">
                <div class="flex items-start gap-4 flex-1 min-w-0">
                    <div class="mt-1 shrink-0">
                        <span class="inline-flex items-center justify-center h-10 w-10 rounded-full @if($type === 'success') bg-emerald-100 text-emerald-600 @elseif($type === 'error') bg-rose-100 text-rose-600 @elseif($type === 'warning') bg-amber-100 text-amber-600 @else bg-sky-100 text-sky-600 @endif">
                            @if($type === 'success')
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <h3 class="text-sm font-semibold text-slate-900 truncate">{{ $data['title'] ?? 'No Title' }}</h3>
                            <x-ui.badge variant="{{ $badgeVariant }}">{{ strtoupper($type) }}</x-ui.badge>
                            @if(auth()->user()->hasPermission('task.view.all') && $notif->notifiable)
                                <span class="text-xs text-slate-500 font-medium">untuk {{ $notif->notifiable->name }}</span>
                            @endif
                        </div>
                        <p class="text-sm text-slate-700 mt-1 leading-relaxed">{{ $data['message'] ?? 'No Message' }}</p>
                        <div class="flex flex-wrap items-center gap-3 mt-2 text-xs text-slate-500">
                            <span class="font-mono">{{ \App\Support\IndonesianDate::dateTime($notif->created_at) }}</span>
                            @if(!empty($data['action_url']))
                                <div class="h-3 w-px bg-slate-300"></div>
                                <a href="{{ $data['action_url'] }}" class="text-sky-600 hover:text-sky-800 hover:underline inline-flex items-center gap-1 font-medium transition-colors">
                                    Lihat Detail
                ...
                <div class="mt-4 sm:mt-0 sm:ml-6 shrink-0 flex gap-2">
                    <button type="button" 
                            onclick="toggleRead('{{ $notif->id }}', this)" 
                            data-read="{{ $isRead ? 'true' : 'false' }}"
                            class="px-3 py-1.5 border rounded-lg text-xs font-semibold transition-all duration-150 cursor-pointer @if($isRead) bg-slate-50 text-slate-700 border-slate-200 hover:bg-slate-100 @else bg-sky-50 text-sky-700 border-sky-200 hover:bg-sky-100 @endif">
        ...
        @empty
            <x-ui.card class="p-8 text-center">
                ...
                <h3 class="mt-4 text-sm font-semibold text-slate-900">Tidak Ada Notifikasi</h3>
                <p class="mt-1 text-sm text-slate-500">Tidak ada notifikasi yang ditemukan pada filter ini.</p>
            </x-ui.card>
        @endforelse
    </div>
...
async function toggleRead(id, button) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (newReadState) {
                card.classList.remove('bg-sky-50/20', 'border-sky-200');
                card.classList.add('bg-white', 'border-slate-200', 'opacity-75');
                button.innerHTML = 'Tandai Belum Dibaca';
                button.classList.remove('bg-sky-50', 'text-sky-700', 'border-sky-200');
                button.classList.add('bg-slate-50', 'text-slate-700', 'border-slate-200');
            } else {
                card.classList.remove('bg-white', 'border-slate-200', 'opacity-75');
                card.classList.add('bg-sky-50/20', 'border-sky-200');
                button.innerHTML = 'Tandai Dibaca';
                button.classList.remove('bg-slate-50', 'text-slate-700', 'border-slate-200');
                button.classList.add('bg-sky-50', 'text-sky-700', 'border-sky-200');
            }
