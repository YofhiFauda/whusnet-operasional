...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <nav class="flex text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider gap-2 mb-2">
            <a href="{{ route('invoices.index') }}" class="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">Daftar Tagihan</a>
            <span>/</span>
            <a href="{{ route('invoices.show', $invoice->id) }}" class="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">{{ $invoice->invoice_number }}</a>
            <span>/</span>
            <span class="text-slate-600 dark:text-slate-400">Input Pembayaran</span>
        </nav>
        <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Input Pembayaran {{ $invoice->invoice_number }}</h1>
    </div>
    <a href="{{ route('invoices.show', $invoice->id) }}" class="inline-flex items-center gap-1.5 px-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-md transition-colors text-xs font-semibold shadow-sm focus:outline-none">
        Kembali
    </a>
</div>
        ...
        <form action="{{ route('invoices.payments.store', $invoice->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            ...
            <div>
                ...
                <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                    Sisa tagihan: Rp {{ number_format((float) $invoice->remaining_amount, 2, ',', '.') }}. Boleh diisi lebih besar — kelebihannya otomatis tercatat sebagai lebih bayar, tidak perlu dihitung manual.
                </p>

                {{-- Konsekuensi cicil/lunas/lebih bayar ditampilkan SEBELUM
                     submit: admin sering baru sadar setelah tersimpan. --}}
            ...
            <div>
                ...
                <textarea name="note" id="note" rows="3" class="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 text-xs bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100">{{ old('note') }}</textarea>
            </div>

            <div class="flex justify-end gap-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                <a href="{{ route('invoices.show', $invoice->id) }}" class="px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 font-semibold rounded-md shadow-sm transition-colors text-xs">
                    Batal
                </a>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6 h-fit">
        ...
        <div class="space-y-3 text-sm">
            <div>
                ...
                <p class="font-semibold text-slate-900 dark:text-slate-100 mt-1">{{ $invoice->customer->full_name ?? '-' }}</p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="font-mono text-slate-900 dark:text-slate-200 mt-1">{{ $invoice->invoice_number }}</p>
            </div>
            <div>
            ...
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-2 text-xs">
                ...
                <div class="flex justify-between gap-2 font-bold {{ (float)$invoice->remaining_amount > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                    <span>Sisa</span>
