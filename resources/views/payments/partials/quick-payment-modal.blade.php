        ...
        <form id="quick-payment-form" class="bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200/80 dark:border-slate-800 w-full max-w-3xl overflow-hidden transform transition-all flex flex-col md:grid md:grid-cols-2">
            ...
            <div class="flex flex-col justify-between h-full bg-white dark:bg-slate-900">
                <div class="p-6 space-y-4">
                    ...
                    {{-- Transfer: tampil hanya saat metode = transfer. Field
                    ...
                    {{-- Kolektor: tampil hanya saat metode = kolektor. Daftar
                         diisi dari `available_collectors` hasil fetch
                         invoice — satu sumber, bukan hardcode di blade. --}}
                    ...
                    {{-- Bukti Pembayaran DIHAPUS dari modal ini atas permintaan
                         user (2026-08-18) — diganti checklist Saldo Pelanggan
                         di bawah. Field `proof_file` di backend tetap nullable
                         (dipakai form penuh payments/create.blade.php), jadi
                         tak ada perubahan validasi server yang dibutuhkan.

                         Saldo Pelanggan: tampil hanya kalau pelanggan punya
                         saldo aktif (dari SISA BAYAR/lebih bayar sebelumnya —
                         CustomerBalanceService::credit() dipanggil otomatis
                         saat overpay terjadi). Nominal yang dipakai dikirim
                         TERPISAH (`use_balance_amount`) dari nominal tunai/
                         transfer/kolektor — server yang menggabungnya, supaya
                         audit tetap jelas asal tiap rupiah
                         (lihat PaymentService::record()). --}}
