                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap">
                                ...
                                @if($customer->old_customer_id)
                                    <div class="text-[10px] font-normal text-slate-400 dark:text-slate-500">Lama: {{ $customer->old_customer_id }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-bold text-slate-900 dark:text-slate-100">{{ $customer->full_name }}</div>
                                <div class="text-xs text-slate-500 dark:text-slate-400">{{ $customer->primary_phone ?? $customer->phone }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($customer->internetPackage)
                                    <div class="font-medium text-slate-800 dark:text-slate-200">{{ $customer->internetPackage->name }}</div>
                                    <div class="text-xs text-slate-500 dark:text-slate-400">{{ $customer->internetPackage->download_speed_mbps }} Mbps</div>
                                @else
                                    <span class="text-slate-400 dark:text-slate-500">-</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold {{ $completenessBadges[$customer->data_completeness_status] ?? 'bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700' }}">
                                    {{ $completenessLabels[$customer->data_completeness_status] ?? $customer->data_completeness_status }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold {{ $customer->subscriptionStatus ? $customer->subscriptionStatus->badgeClasses() : 'bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 border-slate-100 dark:border-slate-700/50' }}">
