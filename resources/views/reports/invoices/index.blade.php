    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-slate-900 dark:text-slate-100 mt-0.5">Rp {{ number_format($totalAmountSum, 2, ',', '.') }}</h3>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-slate-900 dark:text-slate-100 mt-0.5">Rp {{ number_format($totalPaidSum, 2, ',', '.') }}</h3>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-slate-900 dark:text-slate-100 mt-0.5">Rp {{ number_format($totalTunggakanSum, 2, ',', '.') }}</h3>
            </div>
        </div>
    </div>
        ...
        <form method="GET" action="{{ route('reports.invoices.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <input id="billing_period" type="month" name="billing_period" value="{{ $billingPeriod }}" class="w-full rounded-md border-slate-300 dark:border-slate-600 text-sm focus:border-sky-500 focus:ring-sky-500">
                </div>
                    ...
                    @forelse($invoices as $invoice)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap">
                                <a href="{{ route('invoices.show', $invoice->id) }}" class="text-sky-600 hover:text-sky-800 dark:hover:text-sky-300 hover:underline">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($invoice->customer)
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400">{{ $invoice->customer->customer_code }}</div>
                                @else
                            ...
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-400 whitespace-nowrap text-center text-xs">
                                ...
                                <div class="text-slate-400 dark:text-slate-500 mt-0.5">Tempo: {{ $invoice->due_date ? $invoice->due_date->format('d/m/Y') : '-' }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right font-bold {{ $invoice->remaining_amount > 0 && $invoice->invoice_status->value !== 'batal' ? 'text-rose-600 dark:text-rose-400' : 'text-slate-500 dark:text-slate-400' }} whitespace-nowrap">
