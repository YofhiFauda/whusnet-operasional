...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-slate-900 dark:text-slate-100 mt-0.5">Rp {{ number_format($totalValidSum, 2, ',', '.') }}</h3>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 shadow-sm">
        <form method="GET" action="{{ route('reports.payments.index') }}" id="report-filter-form" class="space-y-4">
            <!-- Preset Periode -->
            <div>
                ...
                <div>
                    ...
                    <select id="collector_id" name="collector_id" class="w-full rounded-md border-slate-300 dark:border-slate-600 text-sm focus:border-sky-500 focus:ring-sky-500">
                        ...
                        @foreach($collectors as $collector)
                            <option value="{{ $collector->id }}" @selected((string)$collectorId === (string)$collector->id)>
                                {{ $collector->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    @forelse($payments as $payment)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-6 py-4 font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap">
                                <a href="{{ route('payments.show', $payment->id) }}" class="text-sky-600 hover:text-sky-800 dark:hover:text-sky-300 hover:underline">
                                    {{ $payment->payment_number }}
                                </a>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($payment->invoice)
                                    <a href="{{ route('invoices.show', $payment->invoice_id) }}" class="text-slate-700 dark:text-slate-300 hover:text-sky-600 hover:underline font-medium">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($payment->customer)
                                    ...
                                    <div class="text-xs text-slate-500 dark:text-slate-400">{{ $payment->customer->customer_code }}</div>
                                @else
                                    <span class="text-slate-400 dark:text-slate-500">-</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold {{ $statusBadges[$payment->payment_status->value] ?? 'bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700' }}">
