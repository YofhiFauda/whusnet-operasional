                    ...
                    <div id="step-panel-1" class="step-panel space-y-6">
                        ...
                        <div class="bg-slate-50/70 dark:bg-slate-900/50 rounded-xl p-5 border border-slate-200/60 dark:border-slate-700/60 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 text-xs">
                            ...
                            <div>
                                ...
                                <span class="block text-xs font-semibold text-slate-800 dark:text-slate-200">{{ $customer->gender?->label() ?? '-' }}</span>
                            </div>
                            ...
                            <div>
                                ...
                                <span class="block text-xs {{ $customer->alternative_phone ? 'data-text font-semibold text-slate-800 dark:text-slate-200' : 'text-slate-400 dark:text-slate-500' }}">{{ $customer->alternative_phone ?? '-' }}</span>
                            </div>

                            <div>
                                <span class="block text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-1">Alamat Email</span>
                                <span class="block text-xs {{ $customer->email ? 'font-semibold text-slate-800 dark:text-slate-200' : 'text-slate-400 dark:text-slate-500' }}">{{ $customer->email ?? '-' }}</span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 md:col-span-1 pt-2">
                                <div>
                                    ...
                                    <span class="block text-xs data-text font-semibold text-slate-800 dark:text-slate-200">{{ $customer->latitude ?? '-' }}</span>
                                </div>
                                <div>
                                    <span class="block text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-1">Longitude</span>
                                    <span class="block text-xs data-text font-semibold text-slate-800 dark:text-slate-200">{{ $customer->longitude ?? '-' }}</span>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 text-xs">
                            ...
                            <div class="md:col-span-2 pt-3 border-t border-slate-100 dark:border-slate-700/60">
                                <x-work-tool-checklist
                                    ...
                                    :tools="$workTools"
                                    :rows="$workToolRows"
                                    label="Alat Kerja Yang Perlu Dibawa Teknisi"
                                    hint="Centang alat yang harus dibawa tim pemasangan nanti. Ini hasil pengamatan medan Anda — teknisi pemasangan membacanya sebelum berangkat. Material habis pakai dicatat di daftar di atas, bukan di sini."
                                />
                            </div>
