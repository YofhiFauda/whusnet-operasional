{{--
    Drawer detail tiket — SATU partial dipakai Worksheet Helpdesk
    (tickets/create.blade.php) DAN Worksheet NOC (noc/worksheet.blade.php).
    Acuan layout: worksheet_helpdesk_noc_v2.html (#ticketDrawer).

    Dua halaman kerja itu SENGAJA gak melempar user ke /tickets/{id} cuma buat
    baca detail — halaman penuh disisakan buat arsip (Ticket Selesai, Ticket
    Dibatalkan, History Ticketing) yang memang berangkat dari daftar statis.

    Kontrak dengan halaman pemanggil (biar partial ini gak tahu-menahu soal
    struktur data masing-masing halaman):

      buka          : window.dispatchEvent(new CustomEvent('open-ticket-drawer', { detail: { id } }))
      tutup         : window.dispatchEvent(new CustomEvent('close-ticket-drawer'))
      tombol aksi   : partial ini men-dispatch 'ticket-drawer-action'
                      { detail: { id, action: 'close'|'fop'|'noc'|'return'|'cancel' } }
                      → halaman pemanggil yang nyambungin ke dialog konfirmasi
                        + endpoint-nya sendiri. Konfirmasi & POST TIDAK diduplikasi
                        di sini; satu sumber per halaman.

    Isi drawer diambil dari endpoint `tickets.detail-json` (bercek `tickets.view`
    + POP scope), bukan dari payload daftar — riwayat & lampiran cuma dibutuhin
    buat satu tiket yang dibuka.
--}}
{{--
...
<div x-data="ticketDetailDrawer()" x-on:open-ticket-drawer.window="open($event.detail.id)"
    ...
    {{--
        ...
        navbar tetap utuh & bisa diklik, bukan ketiban panel setengah transparan.

        z-[60] LITERAL, bukan `z-drawer`: token `--z-drawer` cuma dideklarasikan
        di :root, DI LUAR blok @theme — dan z-index bukan namespace yang
        di-generate Tailwind v4 dari @theme. Jadi class `z-drawer` gak pernah ada
        di CSS hasil build (cek: public/build/assets/*.css cuma punya .z-10..z-50)
        dan elemennya jatuh ke z-index:auto — kalah sama sidebar mobile (z-40).
        Pola yang beneran jalan di repo ini: literal (lihat components/dialog z-[100]).
    --}}
    ...
    <div x-show="shown" x-cloak
         ...
         {{-- dvh (bukan vh): di mobile, address bar yang muncul-hilang bikin 100vh
              lebih tinggi dari area kelihatan, footer drawer kepotong. Halaman
              Worksheet Helpdesk sudah pakai 100dvh, samakan. --}}
                ...
                <div class="space-y-4">
                    ...
                    <div class="rounded-xl border border-border bg-surface-muted p-4 space-y-3">
                        ...
                        <dl class="grid grid-cols-2 gap-2">
                            ...
                            <div>
                                {{--
                                    Buat tiket jalur FOP, ini "kapan diserahkan ke FOP" &
                                    "berapa lama di meja Ticketing" — BUKAN waktu pekerjaan
                    ...
                    {{--
                        Tiket "Terputus" — pernah dieskalasi ke FOP tapi FopTask-nya
                        udah dihapus (fop_task_id nullOnDelete). Pakai flag
                        `fop_task_orphan` dari server, JANGAN dicek lewat
                        `!fop_task && fop_task_number`: dua-duanya turunan relasi yang
                        sama, jadi begitu task-nya hilang dua-duanya null dan kondisi
                        itu gak akan pernah nyala. Tanpa cabang ini, box FOP hilang
                    ...
                    {{-- Aksi — flag dari Ticket::actionFlagsFor(), gerbang tampilan doang --}}
                    ...
                    {{-- Lampiran — disk privat, link lewat endpoint bercek permission --}}
    ...
    function ticketDetailDrawer() {
        return {
            ...
            async open(id) {
                ...
                try {
                    const res = await fetch(`{{ url('/api/tickets') }}/${id}/detail`, {
