...
<div class="mb-6 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <a href="{{ route('users.create') }}" class="inline-flex items-center justify-center rounded-md bg-sky-600 dark:bg-sky-500 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-700 dark:hover:bg-sky-600">
            Tambah User
        </a>
        @endif
    </div>
    ...
    <div class="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div class="rounded-md border border-slate-200 dark:border-slate-700 px-3 py-2 text-center">
            ...
            <div class="text-lg font-bold text-slate-800 dark:text-slate-200">{{ $totalUsers }}</div>
        </div>
        <div class="rounded-md border border-slate-200 dark:border-slate-700 px-3 py-2 text-center">
            <div class="text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500">Owner/Admin</div>
            <div class="text-lg font-bold text-slate-800 dark:text-slate-200">{{ $fullAccessUsers }}</div>
        </div>
        <div class="rounded-md border border-slate-200 dark:border-slate-700 px-3 py-2 text-center">
            <div class="text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500">Dengan POP</div>
            <div class="text-lg font-bold text-slate-800 dark:text-slate-200">{{ $withPopUsers }}</div>
        </div>
        <div class="rounded-md border border-slate-200 dark:border-slate-700 px-3 py-2 text-center">
            <div class="text-[10px] font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500">Tanpa POP</div>
            <div class="text-lg font-bold text-slate-800 dark:text-slate-200">{{ $withoutPopUsers }}</div>
        </div>
    </div>
</div>
...
<div class="mb-6 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
    <form action="{{ route('users.index') }}" method="GET" class="grid grid-cols-1 gap-4 lg:grid-cols-5 lg:items-end">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('users.index') }}" class="inline-flex items-center justify-center rounded-md border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50 dark:bg-slate-700/50 dark:hover:bg-slate-700 dark:bg-slate-700/50">
                Reset
            </a>
        </div>
    </form>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50 bg-white dark:bg-slate-800">
                ...
                <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-700/60">
                    <td class="px-6 py-4">
                        ...
                        <div class="text-xs text-slate-400 dark:text-slate-500 font-mono">{{ $user->phone ?? '-' }}</div>
                    </td>
                            ...
                            <div class="space-y-1">
                                @foreach($user->pops->take(3) as $pop)
                                    <div class="text-xs text-slate-700 dark:text-slate-300">
                                        ...
                                        <span class="text-slate-400 dark:text-slate-500">({{ $pop->code }})</span>
                                    </div>
                                @endforeach
                                @if($user->pops->count() > 3)
                                    <div class="text-xs text-slate-400 dark:text-slate-500">+{{ $user->pops->count() - 3 }} POP lainnya</div>
                                @endif
                            </div>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-center">
                        <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-semibold {{ $user->status?->value === 'active' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50' : 'bg-rose-50 text-rose-700 border border-rose-200' }}">
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('users.edit', $user) }}" class="inline-flex items-center rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-800/50">
                                Edit
                            </a>
                            <a href="{{ route('users.pops.edit', $user) }}" class="inline-flex items-center rounded-md border border-sky-200 dark:border-sky-800/50 bg-sky-50 dark:bg-sky-900/20 px-3 py-1.5 text-xs font-semibold text-sky-700 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-900/40 dark:bg-sky-900/40">
                                Atur Cabang
                            </a>
                        </div>
                        @else
