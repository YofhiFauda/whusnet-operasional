...
@section('breadcrumb_parent', $breadcrumbQueueName)
@section('breadcrumb_parent_url', $breadcrumbQueueRoute)
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <div class="flex items-center gap-3 mt-1 flex-wrap">
                    <span class="text-xs font-mono text-text-muted">{{ $customer->display_id }}</span>
            ...
            <div class="bg-surface-muted dark:bg-transparent border border-border rounded-xl p-5 mb-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <span class="block text-sm font-mono font-bold text-text-main">{{ $survey->nearest_odp ?? '-' }}</span>
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-3 pt-4 border-t border-border mt-2">
                        ...
                        <p class="text-sm text-text-secondary whitespace-pre-wrap">{{ $survey->survey_note }}</p>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div id="tab-pemasangan" class="tab-panel p-6 md:p-8 hidden">
            ...
            <div class="mb-6 bg-primary-soft border border-primary-border rounded-xl p-5">
                ...
                <div class="mt-4 pt-4 border-t border-primary-border">
                    ...
                    <p class="text-sm text-text-secondary whitespace-pre-wrap">{{ $installation->installation_note }}</p>
                </div>
                @endif
            </div>
            @else
            ...
            <div class="mb-6">
                ...
                <div class="bg-surface-muted dark:bg-transparent border border-border rounded-xl p-5">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-5">
                        ...
                        <div>
                            <span class="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1">{{ $field['label'] }}</span>
                            <span class="block text-sm {{ ($field['mono'] ?? false) ? 'font-mono' : '' }} text-text-main font-medium">
                                {{ $field['value'] }}
                            </span>
                        </div>
                        @endforeach
        ...
        <div id="tab-pengujian" class="tab-panel p-6 md:p-8 hidden">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-primary-soft border border-primary-border rounded-xl p-4 text-center">
                    ...
                    <span class="block text-2xl font-extrabold font-mono text-primary">{{ $downloadSpeed ?? '-' }}</span>
                ...
                <div class="bg-primary-soft border border-primary-border rounded-xl p-4 text-center">
                    ...
                    <span class="block text-2xl font-extrabold font-mono text-primary">{{ $uploadSpeed ?? '-' }}</span>
                    <span class="block text-xs text-text-muted mt-1">Mbps</span>
                </div>
                ...
                <div class="bg-warning-bg border border-warning-border rounded-xl p-4 text-center">
                    ...
                    <span class="block text-2xl font-extrabold font-mono text-warning">{{ $techDetail->latency_ms ?? '-' }}</span>
                ...
                <div class="bg-info-bg border border-info-border rounded-xl p-4 text-center">
                    ...
                    <span class="block text-2xl font-extrabold font-mono text-info">{{ $techDetail->jitter_ms ?? '-' }}</span>
                    <span class="block text-xs text-text-muted mt-1">ms</span>
                </div>
            </div>
            ...
            <div class="bg-surface-muted dark:bg-transparent border border-border rounded-xl p-5 mb-6">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <span class="block text-sm font-mono font-bold {{ ($techDetail->packet_loss_percent ?? 0) <= 1 ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400' }}">
                    ...
                    <div>
                        ...
                        <span class="block text-sm font-mono font-bold text-text-main">{{ $techDetail->actual_attenuation ?? '-' }}</span>
                    </div>
                    @if($package)
                    <div>
                        <span class="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1">Paket (Target)</span>
                        <span class="block text-sm font-bold text-text-main">{{ $package->name }} ({{ $package->download_speed_mbps ?? '-' }} Mbps)</span>
                    </div>
                    @endif
                    @if($conformity !== null)
                    <div>
                        <span class="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1">% Sesuai Paket</span>
                        <span class="block text-xl font-extrabold font-mono {{ $downloadColor === 'emerald' ? 'text-emerald-700 dark:text-emerald-400' : ($downloadColor === 'amber' ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400') }}">
                ...
                <div class="mt-4 pt-4 border-t border-border">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-xs font-bold font-mono {{ $downloadColor === 'emerald' ? 'text-emerald-600 dark:text-emerald-400' : ($downloadColor === 'amber' ? 'text-amber-650 dark:text-amber-400' : 'text-red-600 dark:text-red-400') }}">{{ number_format($conformity, 1) }}%</span>
                    </div>
                    ...
                    <div class="bg-surface border border-border rounded-xl p-6 space-y-5 shadow-sm">
                        ...
                        <div class="mb-5">
                            ...
                            <input type="date" name="issue_date" id="issue_date"
                                ...
                                required value="{{ old('issue_date', date('Y-m-d')) }}" onchange="resetProrateOverride(); calculateFees();">
                        ...
                        <div class="border-t border-border pt-5">
                            {{-- Nominal turunan (subtotal, diskon, PPN, total) TIDAK jadi input.
                                 Server menghitung ulang semuanya di InitialInvoiceService dan
                                 mengabaikan kiriman klien, jadi input readonly cuma bikin admin
                                 ...
                                 SATU pengecualian: biaya berlangganan (prorata) di bawah — server
                                 tetap menghitung ulang otomatis, tapi admin boleh menimpanya untuk
                                 kasus nego harga / koreksi pembulatan (lihat
                            ...
                            <div class="mb-5">
                                ...
                                <div class="relative">
                                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-text-disabled text-sm font-medium">Rp</span>
                                    {{-- Terisi otomatis dari hitungan prorata (hari aktif / hari
                                         sebulan × harga paket) tiap kali TANGGAL AKTIVASI berubah.
                                         Admin bisa mengedit manual untuk berjaga-jaga (nego harga,
                                         koreksi pembulatan) — sekali diedit manual, nilainya tidak
                                         ditimpa ulang oleh perhitungan otomatis kecuali tanggal
                            ...
                            {{-- KWITANSI. Bahasa manusia, bukan kalkulator: "ditagih 30 hari
                                 (2–31 Agu 2026)" menjelaskan prorata pakai rentang tanggal, bukan
                                 cuma "30 dari 31 hari" yang ambigu (kenapa bukan 31?). Baris
                                 terakhir menjawab pertanyaan pelanggan yang paling sering
                                 ("bulan depan bayar berapa?") tanpa admin perlu menghitung
                                 sendiri. --}}
                            <div class="mt-6 rounded-xl border border-border bg-surface-muted dark:bg-transparent p-5">
                                ...
                                <dl class="space-y-2 text-sm">
                                    ...
                                    {{-- Diskon & PPN cuma muncul kalau memang ada. Untuk semua paket
                                         ...
                                         tidak pernah tampil — field-nya tetap dipertahaman di
                                    ...
                                    <div class="flex justify-between gap-4">
                                        <dt class="text-text-secondary">PPN {{ rtrim(rtrim(number_format($service->ppn, 2, ',', '.'), '0'), ',') }}%</dt>
