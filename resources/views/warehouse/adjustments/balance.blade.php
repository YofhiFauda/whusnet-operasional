...
<x-warehouse.header active="stock" title="Penyesuaian Stok (Stock Adjustment / Opname)" subtitle="Koreksi stok fisik gudang akibat selisih opname, kerusakan material, atau penyusutan." backUrl="{{ route('warehouse.stock.index') }}" />
    ...
    <form action="{{ route('warehouse.adjustments.balance.store') }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select name="pop_id" required class="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all">
            ...
            <div>
                ...
                <select name="item_id" required class="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="lot_no" value="{{ old('lot_no', request('lot_no')) }}" placeholder="mis. LOT-2026-001"
                       class="w-full text-xs font-mono px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all">
            </div>
            <div>
