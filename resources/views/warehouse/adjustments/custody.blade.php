...
@section('title', 'Penyesuaian Barang di Tangan Teknisi - Whusnet Operasional')
@section('page_title', 'Penyesuaian Barang Teknisi')

@section('content')

<x-warehouse.header active="custody" title="Penyesuaian Material di Tangan Teknisi" subtitle="Koreksi material hilang, susut, atau rusak dari yang dipegang oleh teknisi." backUrl="{{ route('warehouse.custody.index') }}" />
...
<div class="max-w-2xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-6 sm:p-8 shadow-xs">
    ...
    <div class="mb-5 p-3.5 rounded-lg bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between text-xs">
        <span class="text-slate-600 dark:text-slate-400 font-medium">Sisa custody aktif saat ini:</span>
    ...
    <form action="{{ route('warehouse.adjustments.custody.store', $custody) }}" method="POST" enctype="multipart/form-data" class="space-y-4" x-data="{ reason: '{{ old('reason', '') }}', evidenceRequired: false }" x-init="evidenceRequired = ['lost', 'damaged'].includes(reason)">
        @csrf
        <div>
            <label class="block mb-1.5 text-xs font-bold text-slate-700 dark:text-slate-200">Jumlah Penyesuaian (+/-) <span class="text-rose-500">*</span></label>
            <input type="number" step="0.01" name="qty_delta" required placeholder="mis. -10 untuk susut / hilang"
                   class="w-full text-xs font-mono font-bold px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all">
        </div>

        <div>
            <label class="block mb-1.5 text-xs font-bold text-slate-700 dark:text-slate-200">Kategori Alasan <span class="text-rose-500">*</span></label>
            <select name="reason" required x-model="reason" @change="evidenceRequired = ['lost', 'damaged'].includes(reason)"
                ...
                @foreach($reasonCategories as $code => $label)
                <option value="{{ $code }}" {{ old('reason') === $code ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div x-show="evidenceRequired" x-cloak>
            ...
            <p class="text-[11px] text-slate-400 mt-1">Wajib buat kategori Hilang/Rusak — kontrol-anti-manipulasi.md §2. Tanpa foto, klaim gak bisa disimpan.</p>
        </div>
