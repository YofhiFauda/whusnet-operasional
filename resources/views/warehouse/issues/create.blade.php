...
<div x-data="warehouseIssueManager(
        @js($itemOptions),
        @js($categoryOptions),
        @js(old('lines', [])),
        @js(route('warehouse.issues.available-stock')),
        '{{ old('cabang_pop_id', request()->query('pop_id', '')) }}',
        '{{ old('technician_id', request()->query('technician_id', '')) }}'
     )"
    ...
    <x-warehouse.header
        active="stock"
        title="Serah ke Teknisi"
        subtitle="Penyerahan material / ONT Cabang $\rightarrow$ Teknisi Lapangan"
        backUrl="{{ route('warehouse.stock.index') }}"
    />

    <form id="issue-form" action="{{ route('warehouse.issues.store') }}" method="POST" class="space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            {{-- ========================================================
                 ...
                 Gudang & Teknisi, Asisten Dispatch, & Daftar Rincian Barang
                 ======================================================== --}}
            <div class="lg:col-span-8 space-y-4">
                
                {{-- 1. KARTU GUDANG CABANG & TEKNISI PENERIMA --}}
                <div class="bg-white dark:bg-slate-800/95 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-4 sm:p-5 shadow-xs">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                        {{-- Gudang Cabang Asal --}}
                        <div>
                            <label for="cabang_pop_id" class="block mb-1.5 text-xs font-bold text-slate-700 dark:text-slate-200 flex items-center justify-between">
                                <span>Gudang Cabang Asal <span class="text-rose-500">*</span></span>
                                <span class="text-[10px] font-bold px-1.5 py-0.2 rounded bg-indigo-100 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300">CABANG</span>
                            </label>
                            <div class="relative">
                                <select name="cabang_pop_id" id="cabang_pop_id" x-model="cabangPopId" @change="onCabangChange()" required
                        ...
                        {{-- Teknisi Lapangan Penerima --}}
                        <div>
                            <label for="technician_id" class="block mb-1.5 text-xs font-bold text-slate-700 dark:text-slate-200 flex items-center justify-between">
                                <span>Teknisi Lapangan Penerima <span class="text-rose-500">*</span></span>
                                <span class="text-[10px] font-bold px-1.5 py-0.2 rounded bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300">TEKNISI / FOP</span>
                            </label>
                            <div class="relative">
                                <select name="technician_id" id="technician_id" x-model="technicianId" required
                                        class="w-full min-h-[44px] text-xs font-semibold pl-9 pr-8 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/60 dark:bg-slate-900/60 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all cursor-pointer">
                                    <option value="">— Pilih Teknisi Penerima —</option>
                                    @foreach($technicians as $technician)
                                    <option value="{{ $technician->id }}">
                                        {{ $technician->name }}{{ $technician->relationLoaded('role') && $technician->role ? ' ('.strtoupper($technician->role->name).')' : '' }}
                                    </option>
                                    @endforeach
                                </select>
                                <svg class="w-4 h-4 text-purple-600 dark:text-purple-400 absolute left-3 top-3 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="bg-white dark:bg-slate-800/95 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-4 shadow-xs"
                     x-data="{
                        ...
                        submitBulk() {
                            if (! cabangPopId) {
                                window.Toast?.warning('Pilih Gudang Cabang', 'Pilih Gudang Cabang terlebih dahulu sebelum memproses nomor seri.');
                                return;
                            }
                    ...
                    <div class="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <div class="inline-flex p-1 bg-slate-100 dark:bg-slate-900/80 rounded-lg border border-slate-200/60 dark:border-slate-700/60">
                            <button type="button" @click="assistantMode = assistantMode === 'camera' ? '' : 'camera'"
                                    :class="assistantMode === 'camera' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs font-bold' : 'text-slate-600 dark:text-slate-400 font-semibold hover:text-slate-900 dark:hover:text-slate-200'"
                            ...
                            <button type="button" @click="assistantMode = assistantMode === 'bulk' ? '' : 'bulk'"
                                    :class="assistantMode === 'bulk' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs font-bold' : 'text-slate-600 dark:text-slate-400 font-semibold hover:text-slate-900 dark:hover:text-slate-200'"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg transition-colors duration-150 cursor-pointer">
                    ...
                    {{-- Warning jika belum pilih Gudang Cabang --}}
                    <div x-show="!cabangPopId" class="mt-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-xs text-amber-800 dark:text-amber-300 flex items-center gap-2">
                        ...
                        <span>Pilih <strong>Gudang Cabang</strong> di atas untuk memuat data stok fisik dan mengaktifkan scanner.</span>
                    </div>

                    {{-- Collapsible Assistant Drawer Container --}}
                    <div x-show="cabangPopId && assistantMode !== ''" x-cloak x-collapse.duration.200ms class="mt-3.5 pt-3.5 border-t border-slate-100 dark:border-slate-700/60">
                        ...
                        <div x-show="assistantMode === 'camera'"
                            ...
                            <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                                <span>Arahkan kamera ke barcode SN modem / ONT yang siap diserahkan ke teknisi.</span>
                                <span class="text-[11px] font-mono text-indigo-600 dark:text-indigo-400 font-bold" x-text="availableStockItems.length + ' Jenis Item Tersedia'"></span>
                            </div>
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1.5">
                                    ...
                                    <span class="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full"
                                          :class="bulkParsedCount > 0 ? 'bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'"
                                ...
                                <textarea x-model="bulkInput" @input="updateBulkCount()" rows="3"
                                          ...
                                          class="w-full font-mono text-xs px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"></textarea>
                                
                                <div class="mt-2 flex items-center justify-between gap-3">
                                    <p class="text-[10px] text-slate-400">Sistem otomatis mencocokkan SN dengan stok fisik di Gudang Cabang ini.</p>
                                    <button type="button" @click="submitBulk()" :disabled="bulkParsedCount === 0"
                                            class="inline-flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 text-white rounded-lg text-xs font-bold transition-all shadow-xs shrink-0 cursor-pointer">
                    ...
                    <div x-show="mismatches.length > 0" x-cloak class="mt-3 bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-800 rounded-lg p-3">
                        <div class="flex items-center justify-between mb-1">
                            <div class="flex items-center gap-1.5">
                                ...
                                <p class="text-xs font-bold text-rose-800 dark:text-rose-300">SN Tidak Ditemukan di Stok Cabang Ini</p>
                            </div>
                        ...
                        <p class="text-[11px] text-rose-700/90 dark:text-rose-400 mb-2">
                            Nomor seri berikut tidak berstatus <em>AVAILABLE</em> di Gudang Cabang yang dipilih:
                        </p>
                ...
                {{-- 3. DAFTAR BARIS BARANG YANG DISERAHKAN --}}
                <div class="bg-white dark:bg-slate-800/95 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-4 sm:p-5 shadow-xs space-y-4">
                    ...
                    <template x-if="rows.length === 0">
                        <div class="py-10 px-4 text-center border-2 border-dashed border-slate-200 dark:border-slate-700/80 rounded-lg bg-slate-50/50 dark:bg-slate-900/30">
                            <h4 class="text-xs font-bold text-slate-700 dark:text-slate-200">Belum ada barang yang diserahkan</h4>
                            <p class="text-[11px] text-slate-400 mt-0.5">Gunakan asisten di atas, klik chip stok di cabang ini, atau klik tambah item.</p>
                            <button type="button" @click="addRow()" class="mt-3 inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition-all shadow-xs cursor-pointer">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>
                    ...
                    <div class="space-y-3.5">
                        <template x-for="(row, index) in rows" :key="index">
                            <div class="bg-slate-50/80 dark:bg-slate-900/60 border border-slate-200/90 dark:border-slate-700/80 rounded-lg p-3.5 sm:p-4 space-y-3 transition-all">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                                    <div>
                                        ...
                                        <select x-model="row.category_id" @change="row.item_id = ''; row.tracking_type = ''; row.unit = '';"
                                                class="w-full min-h-[42px] text-xs font-semibold px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                                ...
                                <template x-if="row.tracking_type === 'serialized'">
                                    <div class="bg-indigo-50/40 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/40 rounded-lg p-3 space-y-2.5">
                                        <div class="flex items-center justify-between">
                                            <label class="text-[10px] font-bold uppercase tracking-wider text-indigo-900 dark:text-indigo-300">
                                                Daftar Nomor Seri yang Diserahkan (1 Baris per Unit)
                                            </label>
                                            <span class="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full"
                                                  :class="serialCount(row) > 0 ? 'bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300' : 'bg-slate-200 dark:bg-slate-700 text-slate-500'"
                                        ...
                                        <textarea :name="`lines[${index}][serial_numbers]`" x-model="row.serial_numbers" rows="2" required
                                                  ...
                                                  class="w-full text-xs font-mono px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500"></textarea>
                                        <p class="text-[10px] text-slate-400">Tips: Klik chip nomor seri di panel Stok di Cabang Ini untuk memasukkan SN secara instan.</p>
                                    </div>
                                </template>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 items-end">
                                            <div>
                                                <label class="block mb-1 text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">
                                                    <span>Jumlah (</span><span class="text-indigo-600 dark:text-indigo-400" x-text="row.unit || 'Unit'"></span><span>)</span> <span class="text-rose-500">*</span>
                                                </label>
            ...
            <div class="hidden lg:block lg:col-span-4 space-y-4 sticky top-20">
                ...
                {{-- 1. Ringkasan Serah Terima Desktop --}}
                <div class="bg-white dark:bg-slate-800/95 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-5 shadow-xs space-y-4">
                    <div class="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-700/60">
                        <h4 class="text-xs font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider flex items-center gap-2">
                            <span class="w-2 h-2 rounded-full bg-indigo-500"></span>
                            <span>Ringkasan Serah Terima</span>
                        </h4>
                        <span class="text-[10px] font-bold font-mono px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400">
                            LIVE
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 text-center">
                        ...
                        <div class="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-700/60">
                            ...
                            <span class="text-sm font-bold font-mono text-indigo-600 dark:text-indigo-400" x-text="totalSnCount + ' Unit'">0 Unit</span>
                        </div>
                    ...
                    <div class="pt-2 border-t border-slate-100 dark:border-slate-700/60 space-y-1.5 text-xs">
                        <div class="flex items-center gap-2">
                            <span :class="cabangPopId ? 'text-emerald-500 font-bold' : 'text-slate-300'">✓</span>
                            <span :class="cabangPopId ? 'text-slate-700 dark:text-slate-200' : 'text-slate-400'">Gudang Cabang Terpilih</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span :class="technicianId ? 'text-emerald-500 font-bold' : 'text-slate-300'">✓</span>
                            <span :class="technicianId ? 'text-slate-700 dark:text-slate-200' : 'text-slate-400'">Teknisi Lapangan Terpilih</span>
                        </div>
                        <div class="flex items-center gap-2">
                    ...
                    <div class="pt-2 border-t border-slate-100 dark:border-slate-700/60 space-y-2">
                        <button type="submit" form="issue-form"
                                ...
                                class="w-full min-h-[44px] inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 dark:disabled:bg-slate-700 disabled:text-slate-400 text-white rounded-lg text-xs font-bold shadow-xs transition-all cursor-pointer">
                ...
                {{-- 2. PANEL STOK DI CABANG INI (QUICK PICKER DARI GUDANG CABANG) --}}
                <div class="bg-white dark:bg-slate-800/95 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-4 shadow-xs space-y-3">
                    <div class="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-700/60">
                        ...
                        <span class="text-[10px] text-slate-400" x-show="cabangPopId && !loadingStock" x-text="filteredAvailableStock.length + ' item'"></span>
                    </div>

                    {{-- Search Filter Input for Available Stock --}}
                    <div x-show="cabangPopId && availableStockItems.length > 0" class="relative">
                        <input type="text" x-model="stockSearchQuery" placeholder="Cari nama barang atau SN..."
                               class="w-full text-xs font-medium pl-8 pr-3 py-1.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                    ...
                    <template x-if="!cabangPopId">
                        <p class="py-4 text-center text-[11px] text-slate-400">Pilih Gudang Cabang untuk melihat stok siap serah.</p>
                    </template>

                    {{-- Loading State --}}
                    <template x-if="cabangPopId && loadingStock">
                    ...
                    <div class="space-y-2.5 max-h-72 overflow-y-auto scroll-smooth pr-1" x-show="cabangPopId && !loadingStock && availableStockItems.length > 0">
                                        ...
                                        <div class="flex flex-wrap gap-1">
                                            <template x-for="sn in item.serials" :key="sn">
                                                <button type="button" @click="$dispatch('pick-serial', { itemId: item.item_id, serialNumber: sn })"
                                                    class="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded-md bg-white dark:bg-slate-800 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-600 hover:text-white transition-colors cursor-pointer"
                ...
                <div class="bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-4 shadow-xs space-y-2 text-[11px] text-slate-500 dark:text-slate-400">
                    <h5 class="text-xs font-bold text-slate-800 dark:text-slate-200">📌 Panduan Serah Terima</h5>
                    <p>&bull; <strong>Custody Teknisi:</strong> Barang berpindah tanggung jawab ke teknisi untuk instalasi pelanggan.</p>
                    <p>&bull; <strong>Pengurangan Stok:</strong> Stok fisik di cabang asal langsung terpotong begitu form disimpan.</p>
                </div>
            </div>
        </div>
            ...
            <div x-show="openMobileSheet" x-cloak
                ...
                <div class="grid grid-cols-2 gap-2 text-center text-xs">
                    ...
                    <div class="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60">
                        ...
                        <span class="font-bold font-mono text-indigo-600 dark:text-indigo-400" x-text="totalSnCount + ' Unit'"></span>
                    </div>
                </div>
                ...
                <div class="space-y-2 text-xs pt-1">
                    ...
                    <div class="flex items-center gap-2">
                        <span :class="cabangPopId ? 'text-emerald-500 font-bold' : 'text-slate-300'">✓</span>
                        <span :class="cabangPopId ? 'text-slate-700 dark:text-slate-200 font-semibold' : 'text-slate-400'">Gudang Cabang Terpilih</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span :class="technicianId ? 'text-emerald-500 font-bold' : 'text-slate-300'">✓</span>
                        <span :class="technicianId ? 'text-slate-700 dark:text-slate-200 font-semibold' : 'text-slate-400'">Teknisi Penerima Terpilih</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span :class="validItemCount > 0 ? 'text-emerald-500 font-bold' : 'text-slate-300'">✓</span>
                ...
                <div x-show="cabangPopId && availableStockItems.length > 0" class="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-2">
                    <div class="flex items-center justify-between">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Pilih Cepat dari Stok Cabang:</span>
                    </div>
                                ...
                                <div class="flex flex-wrap gap-1">
                                    <template x-for="sn in (item.serials || [])" :key="sn">
                                        <button type="button" @click="$dispatch('pick-serial', { itemId: item.item_id, serialNumber: sn }); openMobileSheet = false;"
                                                class="text-[10px] font-mono font-bold px-2 py-0.5 rounded-md bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300"
                                                x-text="sn"></button>
                                    </template>
                                </div>
                            </div>
                ...
                <div class="pt-3 border-t border-slate-100 dark:border-slate-800">
                    <button type="submit" form="issue-form"
                            :disabled="!isFormValid"
                            @click="openMobileSheet = false"
                            class="w-full min-h-[44px] inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 text-white rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                        <span>Simpan Serah Terima Sekarang</span>
                    </button>
                </div>
            </div>
            ...
            <div class="fixed bottom-3 inset-x-3 md:left-64 md:right-0 md:px-6 z-40 pointer-events-none">
                <div class="bg-slate-900/95 dark:bg-slate-900/95 backdrop-blur-xl border border-white/10 text-white rounded-lg shadow-2xl p-2.5 flex items-center justify-between gap-3 max-w-lg mx-auto pointer-events-auto">
                    ...
                    <div class="flex-1 min-w-0 pl-2 cursor-pointer select-none" @click="openMobileSheet = true">
                        <div class="flex items-center gap-1.5">
                            <span class="font-mono text-sm font-extrabold text-indigo-400 truncate" x-text="totalSnCount + ' SN • ' + validItemCount + ' Item'">0 Item</span>
                        ...
                        <p class="text-[10px] text-slate-400 truncate">
                            <span x-text="cabangPopId ? 'Cabang siap' : 'Pilih Cabang'"></span> &bull; <span x-text="technicianId ? 'Teknisi siap' : 'Pilih Teknisi'"></span>
                        </p>
                    </div>
                    ...
                    <button type="submit" form="issue-form"
                            :disabled="!isFormValid"
                            class="min-h-[42px] px-4 py-2 bg-indigo-500 hover:bg-indigo-600 disabled:bg-slate-800 disabled:text-slate-500 text-white font-extrabold rounded-lg text-xs flex items-center justify-center gap-1.5 transition-all shadow-md active:scale-95 shrink-0 cursor-pointer">
...
@push('scripts')
<script>
function warehouseIssueManager(itemOptions, categoryOptions, oldRows, stockEndpoint, initialCabangPopId, initialTechnicianId) {
    return {
        itemOptions: itemOptions,
        categoryOptions: categoryOptions,
        stockEndpoint: stockEndpoint,
        cabangPopId: initialCabangPopId || '',
        ...
        init() {
            ...
            // 3. Load stok awal jika cabang sudah terisi
            if (this.cabangPopId) {
                this.loadAvailableStock(this.cabangPopId);
            }
        },
        ...
        get isFormValid() {
            if (!this.cabangPopId) return false;
        ...
        // transfers/create.blade.php (2026-09-08, laporan user).
        findEmptyRow() {
        ...
        onScan(code) {
            ...
            window.Toast?.warning('SN Tidak Ditemukan', `"${code}" bukan stok Gudang Cabang yang dipilih.`);
        },
        ...
        processBulk(input) {
            ...
            if (matched > 0 || mismatched > 0) {
                window.Toast?.success('Batch Diproses', `${matched} SN masuk daftar serah terima` + (mismatched > 0 ? `, ${mismatched} tidak ditemukan di stok.` : '.'));
            }
        }
    };
}
