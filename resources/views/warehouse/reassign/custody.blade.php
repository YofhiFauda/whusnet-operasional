...
@section('title', 'Alihkan Barang di Tangan Teknisi - Whusnet Operasional')
@section('page_title', 'Alihkan Barang')

@section('content')

<x-warehouse.header active="custody" title="Alihkan Material Teknisi" subtitle="Pengalihan seluruh sisa material teknisi yang cuti/resign kembali ke gudang cabang atau dialihkan ke teknisi lain." backUrl="{{ route('warehouse.custody.index') }}" />
...
<div class="max-w-2xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-6 sm:p-8 shadow-xs">
    <div class="mb-5 pb-4 border-b border-slate-100 dark:border-slate-700/60 flex items-center justify-between">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-sky-50 dark:bg-sky-950/50 text-sky-600 dark:text-sky-400 flex items-center justify-center border border-sky-100 dark:border-sky-800/60">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5"/>
                </svg>
            </div>
            <div>
                <h3 class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ $custody->technician->name }}</h3>
                <p class="text-xs text-slate-400">{{ $custody->item->name }}{{ $custody->lot_no ? ' • Lot '.$custody->lot_no : '' }}</p>
            </div>
        </div>
    ...
    <div class="mb-5 p-3.5 rounded-lg bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between text-xs">
        <span class="text-slate-600 dark:text-slate-400 font-medium">Sisa barang yang dialihkan:</span>
        <span class="font-mono font-bold text-slate-900 dark:text-slate-100 text-sm">
            {{ rtrim(rtrim(number_format((float) $custody->qty_remaining, 2, ',', '.'), '0'), ',') }} {{ $custody->item->unit }}
        </span>
    </div>

    <form action="{{ route('warehouse.reassign.custody.store', $custody) }}" method="POST" class="space-y-4">
        @csrf
        ...
        <div class="flex items-center gap-4 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700/60">
            <label class="inline-flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-200 cursor-pointer">
                <input type="radio" name="action" value="return" onchange="toggleReassignFields()" checked class="text-sky-600 focus:ring-sky-500">
                <span>Kembalikan ke Gudang Cabang</span>
            </label>
            <label class="inline-flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-200 cursor-pointer">
                <input type="radio" name="action" value="transfer" onchange="toggleReassignFields()" class="text-sky-600 focus:ring-sky-500">
        ...
        <div id="field-transfer" style="display:none">
            ...
            <select name="new_technician_id" class="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all">
        ...
        <div>
            ...
            <input type="text" name="reason" required maxlength="255" list="reason-suggestions" placeholder="mis. resign, cuti, rotasi, mutasi_cabang"
                   class="w-full text-xs font-medium px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all">
        ...
        <div>
            ...
            <textarea name="notes" rows="2" placeholder="Keterangan serah terima..."
                      class="w-full text-xs px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all"></textarea>
        </div>
...
function toggleReassignFields() {
    ...
    document.getElementById('field-return').style.display = isReturn ? '' : 'none';
    document.getElementById('field-transfer').style.display = isReturn ? 'none' : '';
