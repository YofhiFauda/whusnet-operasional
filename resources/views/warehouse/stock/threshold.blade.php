...
<x-warehouse.header active="stock" title="Atur Ambang Stok Rendah" subtitle="Isi angka minimum — begitu stok gudang turun di bawahnya, badge 'Stok Rendah' otomatis nyala di Kelola Stok & Dashboard." backUrl="{{ route('warehouse.stock.index') }}" />
...
<div class="max-w-2xl bg-white dark:bg-slate-800/90 border border-slate-200/80 dark:border-slate-700/80 rounded-lg p-6 sm:p-8 shadow-xs">
    <div class="mb-6 pb-4 border-b border-slate-100 dark:border-slate-700/60 flex items-center justify-between">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 flex items-center justify-center border border-rose-100 dark:border-rose-800/60">
    ...
    <form action="{{ route('warehouse.stock.threshold.store') }}" method="POST" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <select name="pop_id" required class="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all">
            ...
            <div>
                ...
                <select name="item_id" required class="w-full text-xs font-semibold px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" name="lot_no" value="{{ old('lot_no', request('lot_no')) }}" placeholder="mis. LOT-2026-001"
                       class="w-full text-xs font-mono px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50/50 dark:bg-slate-900/60 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all">
            </div>
            <div>
        ...
        <div class="pt-4 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold shadow-xs shadow-rose-600/20 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer">
