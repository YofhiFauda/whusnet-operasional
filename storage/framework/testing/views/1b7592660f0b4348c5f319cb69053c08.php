...
<div class="space-y-6" x-data="{ activeCategory: 'all', permSearch: '', filterMatchCount: 0 }">
        ...
        <div>
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
                    Matrix Hak Akses: <?php echo e($role->name); ?>
            ...
            <p class="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
                Atur kewenangan aksi operasional untuk jabatan <strong class="text-slate-700 dark:text-slate-200"><?php echo e($role->name); ?></strong> berdasarkan urutan fungsi kerja.
            </p>
        </div>
    ...
    <div class="space-y-3">
        ...
        <div class="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-thin">
            ...
            <?php $__currentLoopData = $functionalCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $catKey => $catMeta): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button type="button"
                    @click="activeCategory = '<?php echo e($catKey); ?>'"
                    :class="activeCategory === '<?php echo e($catKey); ?>' ? 'bg-sky-600 text-white font-semibold shadow-xs' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/60 border border-slate-200 dark:border-slate-700/80'"
                    class="px-3 py-1.5 rounded-full text-xs whitespace-nowrap transition-all">
                <?php echo e($catMeta['title']); ?>
    ...
    <form action="<?php echo e(route('roles.update', $role)); ?>" method="POST" id="matrixForm" onsubmit="return handleFormSubmit(this);">
        ...
        <div class="bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/80 rounded-lg shadow-xs overflow-hidden">
            ...
            <div id="featureTree" class="divide-y divide-slate-200 dark:divide-slate-700/80">
                ...
                <?php $__currentLoopData = $groupedFeatures; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $catKey => $catData): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if(count($catData['items']) > 0): ?>
                <div class="category-block border-b border-slate-200 dark:border-slate-700/80 last:border-b-0"
                     x-show="activeCategory === 'all' || activeCategory === '<?php echo e($catKey); ?>'"
                     data-category-key="<?php echo e($catKey); ?>">
                    ...
                    <div class="px-4 sm:px-6 py-3 bg-slate-100/70 dark:bg-slate-900/60 border-b border-slate-200/80 dark:border-slate-700/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div class="flex items-center gap-2.5">
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold border <?php echo e($catData['meta']['badge']); ?>">
                                <?php echo e($catData['meta']['title']); ?>

                            </span>
                            <span class="text-xs text-slate-500 dark:text-slate-400 hidden md:inline">
                                — <?php echo e($catData['meta']['subtitle']); ?>

                            </span>
                        </div>
                        <div class="flex items-center gap-2">
                            <button type="button"
                                    onclick="toggleCategoryGroup('<?php echo e($catKey); ?>', true)"
                                    class="text-[11px] font-semibold text-sky-600 dark:text-sky-400 hover:underline">
                                Pilih Semua Kategori Ini
                            </button>
                            <span class="text-slate-300 dark:text-slate-700">|</span>
                            <button type="button"
                                    onclick="toggleCategoryGroup('<?php echo e($catKey); ?>', false)"
                    ...
                    <div class="divide-y divide-slate-100 dark:divide-slate-700/50">
                        <?php $__currentLoopData = $catData['items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feature): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>

                        
                        <div class="feature-group-node"
                             data-feature-code="<?php echo e($feature->code); ?>"
                             data-feature-name="<?php echo e(strtolower($featureMeta[$feature->code]['name'] ?? $feature->name)); ?>"
                                    ...
                                    <div class="min-w-0">
                                        <div class="flex items-center gap-2 flex-wrap">
                                            <span class="text-sm font-bold text-slate-900 dark:text-slate-100">
                                                <?php echo e($featureMeta[$feature->code]['name'] ?? $feature->name); ?>

                                            </span>

                                        </div>
                                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                            <?php echo e($featureMeta[$feature->code]['desc'] ?? $descriptions[$feature->code] ?? 'Pengaturan hak akses modul ' . $feature->name); ?>
                            ...
                            <div x-show="expanded" x-collapse class="bg-white dark:bg-slate-800/80">
                                ...
                                <div class="px-6 sm:px-10 py-3 flex flex-wrap gap-2 border-b border-slate-100 dark:border-slate-700/50"
                                    ...
                                    <label class="perm-label flex items-start gap-2.5 cursor-pointer p-2.5 rounded-lg border border-slate-200 dark:border-slate-700/80 bg-slate-50/50 dark:bg-slate-800/50 hover:bg-slate-100/80 dark:hover:bg-slate-700/60 transition-all group select-none flex-1 min-w-[210px] max-w-xs"
                                           data-perm-code="<?php echo e($perm->code); ?>"
                                           data-perm-name="<?php echo e(strtolower($info['label'] . ' ' . $info['desc'])); ?>"
                                           title="<?php echo e($info['desc']); ?>">
                                        <input type="checkbox"
                                               name="permissions[]"
                                        ...
                                        <div class="flex flex-col gap-1 min-w-0">
                                            <div class="flex items-center gap-1.5 flex-wrap">
                                                <span class="px-2 py-0.5 rounded-md text-[11px] font-semibold border <?php echo e($info['badge']); ?>">
                                ...
                                <div class="divide-y divide-slate-100 dark:divide-slate-700/50">
                                    ...
                                    <div class="pl-4 sm:pl-8"
                                         data-feature-code="<?php echo e($child->code); ?>"
                                         data-feature-name="<?php echo e(strtolower($featureMeta[$child->code]['name'] ?? $child->name)); ?>"
                                                ...
                                                <div class="min-w-0">
                                                    <div class="flex items-center gap-2 flex-wrap">
                                                        <span class="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200">
                                                            <?php echo e($featureMeta[$child->code]['name'] ?? $child->name); ?>

                                                        </span>
                                                    </div>
                                                    <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                                                        <?php echo e($featureMeta[$child->code]['desc'] ?? $descriptions[$child->code] ?? 'Akses sub-fitur ' . $child->name); ?>
                                        ...
                                        <div x-show="childExpanded" x-collapse class="pb-2">
                                            ...
                                            <div class="mx-4 sm:mx-6 mt-1 border-l-2 border-slate-200 dark:border-slate-700/80 pl-4 space-y-2.5">
                                                ...
                                                <div class="py-2"
                                                     data-feature-code="<?php echo e($grandchild->code); ?>"
                                                     data-feature-name="<?php echo e(strtolower($featureMeta[$grandchild->code]['name'] ?? $grandchild->name)); ?>">
                                                    ...
                                                    <div class="flex flex-col gap-0.5 mb-2">
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            <span class="text-xs font-bold text-slate-800 dark:text-slate-200">
                                                                <?php echo e($featureMeta[$grandchild->code]['name'] ?? $grandchild->name); ?>

                                                            </span>
                                                            <span class="font-mono text-[10px] text-slate-400 dark:text-slate-400">
                                                                (<?php echo e($grandchild->code); ?>)
                                                            </span>
                                                        </div>
                                                        <p class="text-[11px] text-slate-500 dark:text-slate-400 pl-3.5">
                                                            <?php echo e($featureMeta[$grandchild->code]['desc'] ?? $descriptions[$grandchild->code] ?? ''); ?>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        if (window.Toast) {
            Toast.warning(
                'Role Standar Sistem',
                'Perubahan permission berlaku untuk semua pengguna dengan role <?php echo e($role->name); ?>.',
                7000
            );
        }
    });
