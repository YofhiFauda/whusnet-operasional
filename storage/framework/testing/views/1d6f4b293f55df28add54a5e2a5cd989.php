...
<div x-data="taskWorksheet()" class="max-w-4xl mx-auto space-y-5">
    ...
    <div class="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 rounded-3xl p-5 sm:p-6 text-white shadow-xl shadow-slate-950/20 relative overflow-hidden border border-slate-800 select-none">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
                ...
                <h1 class="text-lg sm:text-xl font-bold tracking-tight text-white font-ui">Halo, <?php echo e(auth()->user()->name); ?> 👋</h1>
        ...
        <div class="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-300">
            ...
            <a href="<?php echo e(route('tasks.own.history')); ?>" class="font-bold text-sky-400 hover:text-sky-300 flex items-center gap-1 transition-colors font-ui">
    ...
    <?php if($myCustodies->isNotEmpty() || $mySerials->isNotEmpty()): ?>
    <div class="bg-surface border border-border rounded-2xl p-4 shadow-xs">
        ...
        <div class="flex flex-wrap gap-2">
            <?php $__currentLoopData = $mySerials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $serial): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <span class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-sky-50 dark:bg-sky-950/30 border border-sky-200 dark:border-sky-800/60 text-[11px] font-ui">
                <span class="font-semibold text-sky-700 dark:text-sky-300"><?php echo e($serial->item->name); ?></span>
                <span class="font-mono text-sky-600 dark:text-sky-400">SN <?php echo e($serial->serial_number); ?></span>
            </span>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php $__currentLoopData = $myCustodies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $custody): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <span class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-surface-muted border border-border text-[11px] font-ui">
                <span class="font-semibold text-text-main"><?php echo e($custody->item->name); ?></span>
                <span class="font-mono text-text-secondary"><?php echo e(rtrim(rtrim(number_format((float) $custody->qty_remaining, 2, ',', '.'), '0'), ',')); ?> <?php echo e($custody->item->unit); ?></span>
                <span class="text-text-muted">· <?php echo e($custody->ageLabel()); ?></span>
            </span>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    </div>
    ...
    <div x-data="technicianNotifier()" class="space-y-3.5">
        <div class="space-y-3.5" id="today-task-list">
            <?php $__currentLoopData = $tasks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php echo $__env->make('tasks.partials.own-card', ['task' => $task], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        ...
        <div id="empty-state-card"
             ...
             style="<?php echo e($tasks->count() > 0 ? 'display: none;' : ''); ?>">
            ...
            <a href="<?php echo e(route('tasks.show', $task)); ?>"
               class="relative flex items-center justify-between bg-surface border border-border rounded-xl pl-5 pr-4 py-3.5 hover:border-slate-300 dark:hover:border-slate-700 hover:shadow-sm transition-all duration-200 group">
                ...
                <div class="flex items-center gap-3">
                    <span class="text-[9px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border <?php echo e($task->task_type->cardClasses()); ?> shrink-0">
                        <?php echo e($task->task_type->label()); ?>

                    </span>
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-text-main truncate group-hover:text-primary transition-colors font-ui"><?php echo e($task->customer?->full_name ?? $task->title); ?></p>
                        <p class="text-[11px] text-text-muted mt-0.5 font-ui">
                            <?php echo e($task->scheduled_at?->translatedFormat('l, d M')); ?> &middot;
                            <span class="font-mono"><?php echo e($task->scheduled_at?->format('H:i')); ?></span>
                        </p>
                    </div>
                </div>
