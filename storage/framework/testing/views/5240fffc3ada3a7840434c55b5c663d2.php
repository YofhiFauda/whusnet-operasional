<?php $__env->startSection('title', 'Kolektor: ' . $collector->name . ' - Whusnet Operasional'); ?>
<?php $__env->startSection('page_title', $collector->name); ?>

<?php $__env->startSection('content'); ?>
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h3 class="text-slate-800 dark:text-slate-200 text-sm font-semibold uppercase tracking-wider">Kolektor — <?php echo e($collector->name); ?></h3>
...
<div class="flex gap-1 border-b border-slate-200 dark:border-slate-700 mb-6">
    <a href="<?php echo e(route('collectors.show', ['collector' => $collector->id, 'tab' => 'worklist'])); ?>"
       class="px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors <?php echo e($tab === 'worklist' ? 'border-sky-600 text-sky-700 dark:text-sky-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'); ?>">
        Worklist &amp; Bayar
    </a>
    <a href="<?php echo e(route('collectors.show', ['collector' => $collector->id, 'tab' => 'assign'])); ?>"
       class="px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors <?php echo e($tab === 'assign' ? 'border-sky-600 text-sky-700 dark:text-sky-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'); ?>">
        Atur Pelanggan
    </a>
</div>
...
<?php if($tab === 'worklist'): ?>
            ...
            <table class="w-full border-collapse text-left text-sm text-slate-700 dark:text-slate-200">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    <?php $__empty_1 = true; $__currentLoopData = $invoices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/45 dark:hover:bg-slate-700/25 transition-colors" data-invoice-row="<?php echo e($invoice->id); ?>">
                            <td class="px-4 py-3.5 text-center">
                                <input type="checkbox" class="cb-row-checkbox rounded border-slate-300 dark:border-slate-600 dark:bg-slate-900 cursor-pointer" value="<?php echo e($invoice->id); ?>">
                            </td>
                            ...
                            <td class="px-6 py-3.5 font-mono whitespace-nowrap"><?php echo e($invoice->invoice_number); ?></td>
                            <td class="px-6 py-3.5 text-right font-mono cb-sisa">Rp <?php echo e(number_format((float) $invoice->remaining_amount, 0, ',', '.')); ?></td>
                            <td class="px-6 py-3.5">
                                <input type="number" step="0.01" min="1" max="<?php echo e((float) $invoice->remaining_amount); ?>" value="<?php echo e((float) $invoice->remaining_amount); ?>" class="cb-amount w-32 font-sans text-sm px-2 py-1.5 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-200 focus:outline-none focus:border-sky-500">
                            </td>
                            <td class="px-6 py-3.5">
                            ...
                            <td class="px-6 py-3.5">
                                <input type="date" class="cb-collected-date text-sm px-2 py-1.5 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-200 focus:outline-none focus:border-sky-500" value="<?php echo e(now()->format('Y-m-d')); ?>">
                            </td>
                            <td class="px-6 py-3.5 text-right whitespace-nowrap">
                                <button type="button" onclick="cbSubmitSingle(<?php echo e($invoice->id); ?>)" class="inline-flex items-center px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md transition-colors text-xs font-semibold cursor-pointer">
                                    Bayar
                                </button>
                            </td>
                        </tr>
    ...
    <script>
        const CB_COLLECTOR_ID = <?php echo e($collector->id); ?>;
        const CB_STORE_URL = '<?php echo e(route('collector-batch.store', $collector->id)); ?>';
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
            <div class="px-6 py-3.5 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/50">
                <h4 class="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Pelanggan Saat Ini (<?php echo e($assignedCustomers->total()); ?>)</h4>
            </div>
            <div class="divide-y divide-slate-100 dark:divide-slate-700/50 max-h-[32rem] overflow-y-auto">
                <?php $__empty_1 = true; $__currentLoopData = $assignedCustomers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $customer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="px-6 py-3 flex items-center justify-between gap-3">
                        <div>
                            <div class="font-semibold text-sm text-slate-900 dark:text-slate-200"><?php echo e($customer->full_name); ?></div>
                            <div class="text-[10px] text-slate-400 dark:text-slate-500 font-mono"><?php echo e($customer->cid ?? $customer->customer_code); ?> &bull; <?php echo e($customer->pop->name ?? '-'); ?></div>
                        </div>
                        <form action="<?php echo e(route('collectors.release', ['collector' => $collector->id, 'customer' => $customer->id])); ?>" method="POST" onsubmit="return confirm('Lepas <?php echo e($customer->full_name); ?> dari kolektor ini?');">
                            <?php echo csrf_field(); ?>
            ...
            <div class="px-6 py-3 border-t border-slate-100 dark:border-slate-700">
                <?php echo e($assignedCustomers->links()); ?>
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
            ...
            <div class="p-6">
                <form action="<?php echo e(route('collectors.show', ['collector' => $collector->id, 'tab' => 'assign'])); ?>" method="GET" class="flex gap-2 mb-4">
                    <input type="hidden" name="tab" value="assign">
                    <input type="text" name="search" value="<?php echo e($search); ?>" placeholder="Cari nama, kode, atau CID..." class="flex-1 font-sans text-sm px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-200 focus:outline-none focus:border-sky-500">
                ...
                <?php if($searchResults !== null): ?>
                    <form action="<?php echo e(route('collectors.assign', $collector->id)); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                        <div class="divide-y divide-slate-100 dark:divide-slate-700/50 max-h-80 overflow-y-auto border border-slate-100 dark:border-slate-700 rounded-md mb-3">
                            <?php $__empty_1 = true; $__currentLoopData = $searchResults; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $customer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <label class="px-4 py-2.5 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-700/30 cursor-pointer">
                                    <input type="checkbox" name="customer_ids[]" value="<?php echo e($customer->id); ?>" class="rounded border-slate-300 dark:border-slate-600 dark:bg-slate-900">
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-medium text-slate-800 dark:text-slate-200 truncate"><?php echo e($customer->full_name); ?></div>
                                        <div class="text-[10px] text-slate-400 dark:text-slate-500">
                                            <?php echo e($customer->pop->name ?? '-'); ?>

                                            <?php if($customer->collector): ?>
                                                &bull; Sekarang: <?php echo e($customer->collector->name); ?>
                        ...
                        <?php if($searchResults->isNotEmpty()): ?>
                            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold py-2.5 rounded-md transition-colors cursor-pointer">
                                Assign Terpilih ke <?php echo e($collector->name); ?>

                            </button>
                        <?php endif; ?>
                        <div class="mt-2"><?php echo e($searchResults->links()); ?></div>
                    </form>
                <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/yopi/whusnet/whusnet-operasional/resources/views/collectors/show.blade.php ENDPATH**/ ?>