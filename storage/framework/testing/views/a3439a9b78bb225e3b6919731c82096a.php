...
<div x-data="fopTaskPageHandler()" x-init="initTeamConflicts(); initFopTaskEchoListeners()" x-effect="document.body.classList.toggle('overflow-hidden', modal.open || teamConflictModal.open || teamSelectionModal.open || switchTechModal.open || cancelModal.open || filterDrawerOpen || bulkTeamModal.open)" class="px-3 sm:px-4 py-4 sm:py-6 max-w-12xl mx-auto space-y-4 sm:space-y-5 pb-20 md:pb-6">
    ...
    <?php if(isset($todayTeams) && $todayTeams->isNotEmpty()): ?>
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 sm:p-4 shadow-2xs font-ui">
            <div class="flex items-center justify-between mb-2.5">
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">Tim Kerja Hari Ini (<?php echo e(now()->translatedFormat('l, d M Y')); ?>)</h2>
                </div>
                <div class="text-[11px] text-slate-500 dark:text-slate-400">
                    <span class="font-bold text-slate-700 dark:text-slate-200"><?php echo e($todayTeams->count()); ?></span> Tim Aktif
                    <?php
                        $standbyCount = $technicians->filter(fn($t) => ($t->today_task_count ?? 0) === 0)->count();
                    ?>
                    <?php if($standbyCount > 0): ?>
                        &middot; <span class="text-emerald-600 dark:text-emerald-400 font-semibold"><?php echo e($standbyCount); ?> Teknisi Standby</span>
                    <?php endif; ?>
                </div>
            </div>
            <div class="flex items-center gap-2.5 overflow-x-auto pb-1 custom-scrollbar">
                <?php $__currentLoopData = $todayTeams; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $teamItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                        $isTeamSelected = request('team_id') == $teamItem['id'];
                    ?>
                    <a href="<?php echo e($isTeamSelected ? route('fop-tasks.index', request()->except(['team_id', 'page'])) : route('fop-tasks.index', array_merge(request()->except(['page']), ['team_id' => $teamItem['id']]))); ?>"
                       class="group shrink-0 p-2.5 rounded-lg border transition-all cursor-pointer text-left min-w-[170px] sm:min-w-[200px] <?php echo e($isTeamSelected ? 'bg-sky-50 dark:bg-sky-950/40 border-sky-400 dark:border-sky-600 ring-2 ring-sky-500/20 shadow-xs' : 'bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700 hover:border-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50'); ?>">
                        <div class="flex items-center justify-between gap-1.5 mb-1.5">
                            <span class="text-xs font-bold text-slate-900 dark:text-slate-100 group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors"><?php echo e($teamItem['name']); ?></span>
                            <span class="px-1.5 py-0.5 rounded text-[10px] font-mono font-bold <?php echo e($teamItem['task_count'] > 0 ? 'bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300' : 'bg-slate-200 dark:bg-slate-700 text-slate-600'); ?>"><?php echo e($teamItem['task_count']); ?> Task</span>
                        </div>
                        <div class="flex flex-wrap gap-1">
                            <?php $__currentLoopData = $teamItem['members']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <span class="inline-flex items-center px-1.5 py-0.2 rounded text-[10px] bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600 font-medium">
                                    <?php echo e(explode(' ', trim($m['name']))[0]); ?>
                            ...
                            <td class="px-3 py-2 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button type="button" @click="copyWhatsAppFormat({
                                        task_number: '<?php echo e($task->task_number); ?>',
                                        category: '<?php echo e($task->category instanceof \App\Enums\TaskType ? $task->category->value : $task->category); ?>',
                                        tugas: <?php echo \Illuminate\Support\Js::from($task->tugas)->toHtml() ?>,
                                        customer_name: <?php echo \Illuminate\Support\Js::from($task->customer?->full_name ?? $task->ticket?->customer_name ?? $task->tugas)->toHtml() ?>,
                                        cid: '<?php echo e($task->customer?->cid ?? $task->ticket?->customer?->cid ?? '—'); ?>',
                                        phone: '<?php echo e($task->customer?->primary_phone ?? $task->ticket?->customer_phone ?? '—'); ?>',
                                        address: <?php echo \Illuminate\Support\Js::from($task->customer?->address ?? $task->ticket?->customer_address ?? '—')->toHtml() ?>,
                                        village: <?php echo \Illuminate\Support\Js::from($task->village?->name ?? '—')->toHtml() ?>,
                                        odp: '<?php echo e($task->ticket?->customer_odp ?? '—'); ?>',
                                        issue: <?php echo \Illuminate\Support\Js::from($task->issue ?? $task->ticket?->detail_keluhan ?? '—')->toHtml() ?>,
...
<script>
    function fopTaskPageHandler() {
        return {
            ...
            selectedTaskIds: [],
            pageTaskIds: <?php echo json_encode($fopTasks->pluck('id'), 15, 512) ?>,
            ...
            submitBulkAssignTeam() {
                ...
                fetch('<?php echo e(route("fop-tasks.bulk-assign-team")); ?>', {
                    method: 'POST',
                    headers: {
            ...
            async fetchTechAvailability(dateStr, excludeTaskId = null) {
                ...
                try {
                    const url = `<?php echo e(route('fop-tasks.technicians-availability')); ?>?date=${encodeURIComponent(dateStr)}&exclude_task_id=${excludeTaskId || ''}`;
