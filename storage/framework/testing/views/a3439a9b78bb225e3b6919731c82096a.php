...
<div x-data="fopTaskPageHandler()" x-init="initTeamConflicts(); initFopTaskEchoListeners()" x-effect="document.body.classList.toggle('overflow-hidden', modal.open || teamConflictModal.open || teamSelectionModal.open || switchTechModal.open || cancelModal.open)" class="px-4 py-6 max-w-12xl mx-auto space-y-5">
    ...
    <form method="GET" action="<?php echo e(route('fop-tasks.index')); ?>" class="flex flex-col gap-3 pb-3">
        ...
        <div class="flex items-center justify-between mt-1">
            ...
            <div class="flex items-center gap-3">
                <?php if(request()->anyFilled(['search', 'category', 'status', 'priority', 'village_id', 'team_id'])): ?>
                    <a href="<?php echo e(route('fop-tasks.index')); ?>" class="text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors font-ui">Reset</a>
                <?php endif; ?>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50 text-[11px] text-slate-700 dark:text-slate-300">
                    <?php $__empty_1 = true; $__currentLoopData = $fopTasks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-800/50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-800/50 transition-colors align-top" id="fop-task-row-<?php echo e($task->id); ?>" data-pop-id="<?php echo e($task->pop_id); ?>">
...
<script>
    function fopTaskPageHandler() {
        return {
            ...
            teamConflictModal: { open: false, conflicts: <?php echo json_encode($teamConflicts ?? [], 15, 512) ?> },
            ...
            techniciansData: <?php echo json_encode($technicians->map(fn($t) => ['id' => $t->id, 'name' => $t->name])->toArray()); ?>,
            teamsData: <?php echo json_encode($teams, 15, 512) ?>,
            ...
            sendUpdateRequest(taskId, data, refreshRow = true) {
                const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                return fetch(`<?php echo e(url('/fop-tasks')); ?>/${taskId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
