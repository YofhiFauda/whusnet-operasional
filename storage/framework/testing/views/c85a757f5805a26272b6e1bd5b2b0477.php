    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <nav aria-label="Breadcrumb" class="flex items-center gap-2 text-xs text-text-muted mb-1">
                <a href="<?php echo e(route('invoices.index')); ?>" class="hover:text-text-main transition-colors">Daftar Tagihan</a>
                ...
                <a href="<?php echo e(route('invoices.show', $invoice->id)); ?>" class="font-mono hover:text-text-main transition-colors"><?php echo e($invoice->invoice_number); ?></a>
                <svg class="h-3 w-3 text-text-muted/60" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="font-semibold text-text-main">Input Pembayaran</span>
            </nav>
            <h1 class="text-xl sm:text-2xl font-bold text-text-main tracking-tight">Input Pembayaran <?php echo e($invoice->invoice_number); ?></h1>
        </div>
        <div>
            <a href="<?php echo e(route('invoices.show', $invoice->id)); ?>" class="inline-flex items-center gap-1.5 px-3.5 py-2 border border-border bg-surface hover:bg-surface-muted text-text-secondary rounded-lg transition-colors text-xs font-semibold shadow-2xs focus:outline-none">
            ...
            <form action="<?php echo e(route('invoices.payments.store', $invoice->id)); ?>" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-text-muted mt-1.5">
                        Sisa tagihan: <span class="font-mono font-bold text-text-main">Rp <?php echo e(number_format((float) $invoice->remaining_amount, 2, ',', '.')); ?></span>. Boleh diisi lebih besar — kelebihannya otomatis tercatat sebagai lebih bayar.
                    </p>
                ...
                <div class="flex items-center justify-end gap-2 pt-4 border-t border-border">
                    <a href="<?php echo e(route('invoices.show', $invoice->id)); ?>" class="px-4 py-2 border border-border text-text-secondary bg-surface hover:bg-surface-muted font-semibold rounded-lg shadow-2xs transition-colors text-xs">
                        Batal
                    </a>
        ...
        <div class="bg-surface border border-border rounded-xl p-6 shadow-2xs h-fit space-y-4">
            ...
            <div class="space-y-3 text-xs">
                <div>
                    ...
                    <p class="font-bold text-text-main text-sm mt-0.5"><?php echo e($invoice->customer->full_name ?? '-'); ?></p>
                    <p class="text-[10px] text-text-muted font-mono">CID: <?php echo e($invoice->customer->cid ?? $invoice->customer->customer_code ?? '-'); ?></p>
                </div>

                <div>
                    <p class="text-[10px] font-semibold text-text-muted uppercase tracking-wider">POP / Cabang</p>
                    <p class="font-medium text-text-main mt-0.5"><?php echo e($invoice->pop->name ?? '-'); ?></p>
                </div>

                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <p class="text-[10px] font-semibold text-text-muted uppercase tracking-wider">No. Tagihan</p>
                        <p class="font-mono font-semibold text-primary mt-0.5"><?php echo e($invoice->invoice_number); ?></p>
                    </div>
                    <div>
                        <p class="text-[10px] font-semibold text-text-muted uppercase tracking-wider">Periode</p>
                        <p class="font-mono text-text-main mt-0.5"><?php echo e($invoice->billing_period); ?></p>
                    </div>
                </div>
                ...
                <div class="pt-3 border-t border-dashed border-border space-y-2 text-xs">
                    ...
                    <div class="flex justify-between gap-2 pt-2 border-t border-border font-bold text-sm <?php echo e((float)$invoice->remaining_amount > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'); ?>">
