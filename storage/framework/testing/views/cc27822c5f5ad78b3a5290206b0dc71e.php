...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <nav aria-label="Breadcrumb" class="flex items-center gap-2 text-xs text-text-muted mb-1">
                <a href="<?php echo e(route('payments.index')); ?>" class="hover:text-text-main transition-colors">Pembayaran</a>
        ...
        <div>
            <a href="<?php echo e(route('payments.index')); ?>" class="inline-flex items-center gap-1.5 px-3.5 py-2 border border-border bg-surface hover:bg-surface-muted text-text-secondary rounded-lg transition-colors text-xs font-semibold shadow-2xs">
    ...
    <div class="bg-amber-50/80 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl p-5 shadow-2xs flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="space-y-1">
            ...
            <div class="text-2xl sm:text-3xl font-bold text-amber-800 dark:text-amber-300 font-mono">
                Rp <?php echo e(number_format($totalOverpay, 0, ',', '.')); ?>
    ...
    <div class="bg-surface border border-border rounded-xl p-5 shadow-2xs">
        <form action="<?php echo e(route('payments.overpay')); ?>" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
            <!-- Search Pelanggan -->
            <div class="sm:col-span-2">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" id="search" value="<?php echo e($search); ?>" placeholder="Cari Nama, CID, atau Kode Pelanggan..." 
            ...
            <div class="flex items-center gap-2">
                ...
                <?php if($search !== '' || $popId !== ''): ?>
                    <a href="<?php echo e(route('payments.overpay')); ?>" class="px-3.5 py-2 border border-border bg-surface hover:bg-surface-muted text-text-secondary rounded-lg transition-colors text-xs font-semibold text-center">
                        Reset
                    </a>
                <?php endif; ?>
            </div>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-surface-muted/50 transition-colors">
                            ...
                            <td class="px-4 py-3.5 whitespace-nowrap">
                                ...
                                <div class="text-[10px] text-text-muted font-mono">CID: <?php echo e($payment->customer->cid ?? $payment->customer->customer_code ?? '-'); ?></div>
                            </td>
                            ...
                            <td class="px-4 py-3.5 text-right font-mono font-bold text-amber-600 dark:text-amber-400">
                                Rp <?php echo e(number_format((float) $payment->overpay_amount, 2, ',', '.')); ?>
