<?php $__env->startSection('title', 'Detail Pembayaran - Whusnet Operasional'); ?>
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h1 class="text-xl font-bold text-text-main tracking-tight">Detail Pembayaran <?php echo e($payment->payment_number); ?></h1>
    </div>
    <div class="flex gap-2">
        <?php if($payment->invoice): ?>
            <a href="<?php echo e(route('invoices.show', $payment->invoice_id)); ?>" class="inline-flex items-center gap-1.5 px-4 py-2 bg-primary hover:bg-primary-focus text-white rounded-md transition-colors text-xs font-semibold shadow-sm focus:outline-none">
                Detail Tagihan
            </a>
        <?php endif; ?>
    ...
    <div class="lg:col-span-2 bg-surface border border-border rounded-lg overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5 text-sm">
            <div>
                ...
                <p class="font-mono font-bold text-text-main mt-1"><?php echo e($payment->payment_number); ?></p>
            </div>
            <div>
                <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">Tanggal Bayar</p>
                <p class="text-text-main mt-1"><?php echo e(optional($payment->payment_date)->format('d/m/Y')); ?></p>
            </div>
            <div>
                <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">Metode Bayar</p>
                <p class="font-semibold text-text-main mt-1"><?php echo e(strtoupper($payment->payment_method)); ?></p>
            </div>
            <div>
                <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">Nominal Bayar</p>
                <p class="font-mono font-bold text-text-main mt-1">Rp <?php echo e(number_format((float) $payment->amount, 2, ',', '.')); ?></p>
            </div>
            <div>
                <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">Diterima Oleh</p>
                <p class="text-text-main mt-1"><?php echo e($payment->receiver->name ?? '-'); ?></p>
            </div>
            <div>
                <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">POP / Cabang</p>
                <p class="text-text-main mt-1"><?php echo e($payment->pop->name ?? '-'); ?></p>
            </div>
        </div>
        ...
        <div class="border-t border-border bg-primary/5 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                ...
                <div>
                    ...
                    <span class="font-mono font-bold text-text-main"><?php echo e($payment->old_payment_id); ?></span>
                </div>
                <?php endif; ?>
                <?php if($payment->old_transaction_id): ?>
                <div>
                    <span class="text-text-muted block">ID Transaksi Lama:</span>
                    <span class="font-mono font-bold text-text-main"><?php echo e($payment->old_transaction_id); ?></span>
                </div>
                <?php endif; ?>
                <?php if($payment->old_request_id): ?>
                <div>
                    <span class="text-text-muted block">ID Permintaan Lama:</span>
                    <span class="font-mono font-bold text-text-main"><?php echo e($payment->old_request_id); ?></span>
                </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="border-t border-border p-6">
            ...
            <p class="text-sm text-text-secondary"><?php echo e($payment->note ?: 'Tidak ada catatan.'); ?></p>
        </div>

        <div class="border-t border-border p-6">
            <h3 class="text-xs font-bold text-text-main uppercase tracking-wider mb-3">Bukti Pembayaran</h3>
            <?php if($payment->proof_file): ?>
                <a href="<?php echo e(asset('storage/' . $payment->proof_file)); ?>" target="_blank" class="inline-flex items-center px-4 py-2 bg-primary hover:bg-primary-focus text-white rounded-md transition-colors text-xs font-semibold">
                    Lihat Bukti Pembayaran
                </a>
                <p class="text-[10px] text-text-muted mt-2 font-mono"><?php echo e($payment->proof_file); ?></p>
            <?php else: ?>
                                ...
                                <?php $__currentLoopData = $payment->auditLogs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $auditLog): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 align-top">
                                            <pre class="max-w-xs whitespace-pre-wrap break-words text-[10px] text-text-muted bg-surface border border-border rounded p-2"><?php echo e($auditLog->old_values ? json_encode($auditLog->old_values, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) : '-'); ?></pre>
                                        </td>
                                        <td class="px-4 py-3 align-top">
                                            <pre class="max-w-xs whitespace-pre-wrap break-words text-[10px] text-text-muted bg-surface border border-border rounded p-2"><?php echo e($auditLog->new_values ? json_encode($auditLog->new_values, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) : '-'); ?></pre>
                                        </td>
                                    </tr>
        ...
        <div class="bg-surface border border-border rounded-lg p-6">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <?php if($payment->invoice): ?>
                        <a href="<?php echo e(route('invoices.show', $payment->invoice_id)); ?>" class="font-mono font-semibold text-primary hover:text-primary-focus mt-1 inline-block"><?php echo e($payment->invoice->invoice_number); ?></a>
                    <?php else: ?>
                ...
                <div>
                    ...
                    <p class="font-mono text-text-main mt-1"><?php echo e($payment->invoice->billing_period ?? '-'); ?></p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">Total Tagihan</p>
                    <p class="font-mono text-text-main mt-1">Rp <?php echo e(number_format((float) ($payment->invoice->total_amount ?? 0), 2, ',', '.')); ?></p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">Sisa Tagihan</p>
                    <p class="font-mono text-text-main mt-1">Rp <?php echo e(number_format((float) ($payment->invoice->remaining_amount ?? 0), 2, ',', '.')); ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-surface border border-border rounded-lg p-6">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <p class="font-semibold text-text-main mt-1"><?php echo e($payment->customer->full_name ?? '-'); ?></p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">ID Pelanggan</p>
                    <p class="font-mono text-text-main mt-1"><?php echo e($payment->customer->cid ?? $payment->customer->customer_code ?? '-'); ?></p>
                </div>
                <div>
                    <p class="text-xs font-semibold text-text-muted uppercase tracking-wider">No. HP</p>
                    <p class="font-mono text-text-main mt-1"><?php echo e($payment->customer->primary_phone ?? $payment->customer->phone ?? '-'); ?></p>
                </div>
                <?php if($payment->customer): ?>
                    <a href="<?php echo e(route('customers.show', $payment->customer_id)); ?>" class="inline-flex items-center px-3 py-1.5 border border-border bg-surface hover:bg-surface-muted text-text-secondary rounded-md transition-colors text-xs font-semibold mt-2">
                        Detail Pelanggan
                    </a>
                <?php endif; ?>
            </div>
