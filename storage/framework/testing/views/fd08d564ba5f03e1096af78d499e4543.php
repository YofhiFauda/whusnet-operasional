...
<div class="mb-6">
    <nav class="flex text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider gap-2">
        <a href="<?php echo e(route('surveys.queue')); ?>" class="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">Antrean Survey</a>
        <span>/</span>
        <span class="text-slate-600 dark:text-slate-300">Lapor Data Survey</span>
    </nav>
</div>
...
<form action="<?php echo e(route('customers.survey.store', $customer->id)); ?>" method="POST" enctype="multipart/form-data" id="wizard-form" class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 shadow-sm">
        <div class="flex items-center justify-between mb-3">
            <div>
                <h3 class="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">Kelengkapan Laporan Survey</h3>
            ...
            <div class="text-right">
                ...
                <span class="text-xs text-slate-400 dark:text-slate-500 block mt-0.5"><span id="filled-fields-count" class="data-text">0</span> dari <span id="total-fields-count" class="data-text">4</span> field terisi</span>
            </div>
        </div>
        ...
        <div class="lg:col-span-1 flex flex-col gap-4">
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-5 shadow-sm space-y-5">
                ...
                <div class="space-y-4">
                    ...
                    <button type="button" onclick="goToStep(1)" id="step-nav-1" class="w-full text-left p-3.5 rounded-lg border-2 border-sky-600 dark:border-sky-500 bg-sky-50/30 dark:bg-sky-950/30 transition-all group focus:outline-none">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <span class="text-[9px] text-slate-400 dark:text-slate-500 block mt-1">Data Read-only</span>
                            </div>
                        </div>
                    </button>

                    <!-- Step 2 -->
                    <button type="button" onclick="goToStep(2)" id="step-nav-2" class="w-full text-left p-3.5 rounded-lg border border-red-100 dark:border-red-900/30 bg-red-50/10 dark:bg-red-950/10 hover:bg-red-50/20 dark:hover:bg-red-950/20 transition-all group focus:outline-none">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <span id="step-nav-status-2" class="text-[9px] font-bold block mt-1 uppercase tracking-wider text-red-600 dark:text-red-400">Belum Lengkap</span>
                                <span id="step-nav-missing-2" class="text-[9px] text-slate-400 dark:text-slate-500 block mt-1 leading-relaxed whitespace-pre-wrap">Wajib diisi: Foto Rumah dan Foto ODP Terdekat.</span>
                            </div>
                        </div>
                    </button>
                    ...
                    <!-- Step 4 -->
                    <button type="button" onclick="goToStep(4)" id="step-nav-4" class="w-full text-left p-3.5 rounded-lg border border-red-100 dark:border-red-900/30 bg-red-50/10 dark:bg-red-950/10 hover:bg-red-50/20 dark:hover:bg-red-950/20 transition-all group focus:outline-none">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <span id="step-nav-status-4" class="text-[9px] font-bold block mt-1 uppercase tracking-wider text-red-600 dark:text-red-400">Belum Lengkap</span>
                                <span id="step-nav-missing-4" class="text-[9px] text-slate-400 dark:text-slate-500 block mt-1 leading-relaxed whitespace-pre-wrap"></span>
                            </div>
                        </div>
                    </button>
                </div>
        ...
        <div class="lg:col-span-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm overflow-hidden min-h-[480px] flex flex-col justify-between">
            ...
            <div class="p-6 md:p-8 flex-1">
                ...
                <div id="step-panel-1" class="step-panel space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-xs font-semibold text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/50 p-5 rounded-lg border border-slate-100 dark:border-slate-700/50">
                        <div>
                            ...
                            <span class="block text-sm font-bold text-slate-800 dark:text-slate-200"><?php echo e($customer->full_name); ?></span>
                        </div>

                        <div>
                            <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">NOMOR IDENTITAS (NIK)</span>
                            <span class="block text-sm font-mono font-medium text-slate-800 dark:text-slate-200"><?php echo e($customer->identity_number ?? '-'); ?></span>
                        </div>

                        <div>
                            <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">JENIS KELAMIN</span>
                            <span class="block text-sm text-slate-800 dark:text-slate-200"><?php echo e($customer->gender?->label() ?? '-'); ?></span>
                        </div>

                        <div>
                            <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">NOMOR HP UTAMA</span>
                            <span class="block text-sm font-mono text-slate-800 dark:text-slate-200"><?php echo e($customer->primary_phone ?? $customer->phone ?? '-'); ?></span>
                        </div>

                        <div>
                            <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">NOMOR HP ALTERNATIF</span>
                            <span class="block text-sm font-mono text-slate-800 dark:text-slate-200"><?php echo e($customer->alternative_phone ?? '-'); ?></span>
                        </div>

                        <div>
                            <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">ALAMAT EMAIL</span>
                            <span class="block text-sm text-slate-800 dark:text-slate-200"><?php echo e($customer->email ?? '-'); ?></span>
                        </div>
                        ...
                        <div class="md:col-span-2 mt-2 pt-4 border-t border-slate-200 dark:border-slate-700">
                            ...
                            <span class="block text-sm font-bold text-slate-800 dark:text-slate-200"><?php echo e($customer->address); ?></span>
                        ...
                        <div class="grid grid-cols-2 gap-4 md:col-span-1 mt-2">
                            <div>
                                <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">LATITUDE</span>
                                <span class="block text-sm font-mono text-slate-800 dark:text-slate-200"><?php echo e($customer->latitude ?? '-'); ?></span>
                            </div>
                            <div>
                                <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">LONGITUDE</span>
                                <span class="block text-sm font-mono text-slate-800 dark:text-slate-200"><?php echo e($customer->longitude ?? '-'); ?></span>
                            </div>
                        </div>
                        
                        <div class="mt-2 md:col-span-1">
                            <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">POP CABANG</span>
                            <span class="block text-sm text-slate-800 dark:text-slate-200 font-bold"><?php echo e($customer->pop->name ?? '-'); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div id="step-panel-2" class="step-panel space-y-6 hidden">
                    <div class="border-b border-slate-100 dark:border-slate-700/50 pb-3 mb-6">
                        <h4 class="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">2. UPLOAD DOKUMEN LAMPIRAN</h4>
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Lihat dokumen KTP yang terlampir dan tambahkan Foto Rumah serta ODP Terdekat.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div class="border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 rounded-lg p-5 flex flex-col justify-between shadow-sm relative opacity-90">
                            <?php if($customer->foto_ktp): ?>
                                <div class="text-center py-2 flex flex-col items-center justify-center">
                                    <div class="relative inline-block w-full">
                                        <img class="max-h-32 max-w-full rounded-lg object-contain border border-slate-200 dark:border-slate-700 shadow-sm mx-auto cursor-pointer hover:opacity-90"
                            ...
                            <?php else: ?>
                                <div class="text-center py-4">
                                    ...
                                    <span class="block text-[10px] text-amber-600 dark:text-amber-400 mt-1">Pelanggan belum upload KTP saat registrasi</span>
                                </div>
                            <?php endif; ?>
                        </div>

                        <!-- Foto Rumah (Input Baru) -->
                        <div class="border border-slate-200 dark:border-slate-700 rounded-lg p-5 flex flex-col justify-between hover:border-sky-300 dark:hover:border-sky-600 transition-colors shadow-sm relative bg-white dark:bg-slate-800">
                            <div id="default-placeholder-house_photo" class="text-center py-4">
                                <svg class="mx-auto h-10 w-10 text-sky-400 dark:text-sky-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                                </svg>
                                <span class="block text-xs font-bold text-slate-700 dark:text-slate-300 mt-3">FOTO RUMAH PELANGGAN <span class="text-red-500">*</span></span>
                                <span class="block text-[10px] text-slate-400 dark:text-slate-500 mt-1">Wajib Diisi</span>
                            </div>

                            <div id="preview-container-house_photo" class="hidden text-center py-2 flex flex-col items-center justify-center">
                                <div class="relative inline-block w-full">
                                    <img id="preview-img-house_photo" class="max-h-28 max-w-full rounded-lg object-contain border border-slate-200 dark:border-slate-700 shadow-sm mx-auto" src="" alt="Preview Foto Rumah">
                                ...
                                <span class="block text-xs font-bold text-slate-700 dark:text-slate-300 mt-2 uppercase">PREVIEW FOTO RUMAH</span>
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <label for="house_photo" class="block w-full text-center bg-sky-50 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-800 hover:bg-sky-100 dark:hover:bg-sky-900/50 hover:border-sky-300 dark:hover:border-sky-700 text-sky-700 dark:text-sky-300 text-xs font-bold py-2 px-3 rounded cursor-pointer transition-colors">
                                    Pilih Foto Rumah
                                </label>
                                <span id="file-label-house_photo" class="block text-[10px] text-slate-500 dark:text-slate-400 text-center mt-2 font-mono truncate">Belum ada file dipilih</span>
                            </div>
                        </div>
                        ...
                        <div class="border border-slate-200 dark:border-slate-700 rounded-lg p-5 flex flex-col justify-between hover:border-sky-300 dark:hover:border-sky-600 transition-colors shadow-sm relative bg-white dark:bg-slate-800">
                            <div id="default-placeholder-survey_photo" class="text-center py-4">
                                ...
                                <span class="block text-xs font-bold text-slate-700 dark:text-slate-300 mt-3">FOTO ODP / JALUR <span class="text-red-500">*</span></span>
                                <span class="block text-[10px] text-slate-400 dark:text-slate-500 mt-1">Wajib Diisi</span>
                            </div>

                            <div id="preview-container-survey_photo" class="hidden text-center py-2 flex flex-col items-center justify-center">
                                <div class="relative inline-block w-full">
                                    <img id="preview-img-survey_photo" class="max-h-28 max-w-full rounded-lg object-contain border border-slate-200 dark:border-slate-700 shadow-sm mx-auto" src="" alt="Preview Foto ODP">
                                ...
                                <span class="block text-xs font-bold text-slate-700 dark:text-slate-300 mt-2 uppercase">PREVIEW FOTO ODP</span>
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <label for="survey_photo" class="block w-full text-center bg-sky-50 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-800 hover:bg-sky-100 dark:hover:bg-sky-900/50 hover:border-sky-300 dark:hover:border-sky-700 text-sky-700 dark:text-sky-300 text-xs font-bold py-2 px-3 rounded cursor-pointer transition-colors">
                                    Pilih Foto ODP
                                </label>
                                <span id="file-label-survey_photo" class="block text-[10px] text-slate-500 dark:text-slate-400 text-center mt-2 font-mono truncate">Belum ada file dipilih</span>
                            </div>
                        </div>
                ...
                <div id="step-panel-3" class="step-panel space-y-6 hidden">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-xs font-semibold text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/50 p-5 rounded-lg border border-slate-100 dark:border-slate-700/50">
                        <div>
                            ...
                            <span class="block text-sm font-bold text-slate-800 dark:text-slate-200"><?php echo e($customer->internetPackage->package_code ?? '-'); ?> - <?php echo e($customer->internetPackage->name ?? 'Belum Dipilih'); ?></span>
                        </div>

                        <div>
                            <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">BIAYA BULANAN DASAR</span>
                            <span class="block text-sm font-mono font-bold text-slate-800 dark:text-slate-200">Rp <?php echo e(number_format($customer->internetPackage->monthly_price ?? 0, 0, ',', '.')); ?></span>
                        </div>
                        ...
                        <div class="pt-4 border-t border-slate-200 dark:border-slate-700 md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <span class="block text-sm text-slate-800 dark:text-slate-200 uppercase"><?php echo e($customer->customerService->contract_type ?? 'Sewa'); ?></span>
                            </div>

                            <div>
                                <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">MASA KONTRAK</span>
                                <span class="block text-sm text-slate-800 dark:text-slate-200"><?php echo e($customer->customerService->contract_period_months ?? 12); ?> Bulan</span>
                            </div>

                            <div>
                                <span class="block mb-1 uppercase tracking-wide text-[10px] text-slate-400 dark:text-slate-500">DISKON PROMOSI</span>
                                <span class="block text-sm font-mono text-slate-800 dark:text-slate-200">Rp <?php echo e(number_format($customer->discount_amount ?? 0, 0, ',', '.')); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="step-panel-4" class="step-panel space-y-6 hidden">
                    <div class="border-b border-slate-100 dark:border-slate-700/50 pb-3 mb-6">
                        ...
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Masukkan data teknis hasil pantauan di lapangan.</p>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-xs font-semibold text-slate-500 dark:text-slate-400">
                        <!-- ODP Terdekat -->
                        <div>
                            <label for="nearest_odp" class="block mb-2 uppercase tracking-wide text-slate-700 dark:text-slate-300">ODP TERDEKAT <span class="text-red-500">*</span></label>
                            <input type="text" name="nearest_odp" id="nearest_odp" value="<?php echo e(old('nearest_odp')); ?>" class="w-full text-sm font-mono px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-sky-500 dark:focus:border-sky-400 focus:ring-2 focus:ring-sky-500/25" placeholder="Contoh: ODP-BBD-01">
                        </div>
                        
                        <!-- Estimasi Kabel -->
                        <div>
                            <label for="cable_estimation_meter" class="block mb-2 uppercase tracking-wide text-slate-700 dark:text-slate-300">ESTIMASI KABEL (METER) <span class="text-red-500">*</span></label>
                            <input type="number" name="cable_estimation_meter" id="cable_estimation_meter" min="0" value="<?php echo e(old('cable_estimation_meter')); ?>" class="w-full text-sm font-mono px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-sky-500 dark:focus:border-sky-400 focus:ring-2 focus:ring-sky-500/25" placeholder="0">
                        </div>

                        <!-- Tanggal Request Pemasangan (opsional) -->
                        <div>
                            <label for="requested_installation_date" class="block mb-2 uppercase tracking-wide text-slate-700 dark:text-slate-300">TANGGAL REQUEST PEMASANGAN</label>
                            <input type="date" name="requested_installation_date" id="requested_installation_date" min="<?php echo e(now()->toDateString()); ?>" value="<?php echo e(old('requested_installation_date')); ?>" class="w-full text-sm font-mono px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:outline-none focus:border-sky-500 dark:focus:border-sky-400 focus:ring-2 focus:ring-sky-500/25">
                            <span class="block text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 leading-relaxed">Kosongkan jika pelanggan tidak meminta tanggal tertentu. Diisi hanya jika pelanggan minta dipasang di tanggal tertentu — task pemasangan akan menunggu sampai tanggal itu tiba.</span>
                        </div>

                        <!-- Tingkat Kesulitan -->
                        <div>
                            <label for="difficulty_level" class="block mb-2 uppercase tracking-wide text-slate-700 dark:text-slate-300">TINGKAT KESULITAN <span class="text-red-500">*</span></label>
                            <select name="difficulty_level" id="difficulty_level" class="w-full text-sm font-sans px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:outline-none focus:border-sky-500 dark:focus:border-sky-400 focus:ring-2 focus:ring-sky-500/25">
                        ...
                        <div class="md:col-span-2 border-t border-slate-100 dark:border-slate-700/50 pt-5 mt-2">
                            <label class="block mb-1 uppercase tracking-wide text-slate-700 dark:text-slate-300">ESTIMASI KEBUTUHAN ALAT</label>
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 mb-3 leading-relaxed font-normal">Perkiraan material yang akan dipakai saat pemasangan. Teknisi pemasangan nanti mengisi realisasinya, dan selisihnya dipakai admin verifikasi. Estimasi kabel di atas otomatis jadi satu baris.</p>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.material-rows','data' => ['name' => 'materials','items' => $items,'rows' => $materialRows,'emptyLabel' => 'Belum ada material diestimasi. Klik Tambah Barang bila perlu.']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['name' => 'materials','items' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($items),'rows' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($materialRows),'empty-label' => 'Belum ada material diestimasi. Klik Tambah Barang bila perlu.']); ?>
...
<?php endif; ?>
                        ...
                        <!-- Alat Khusus (catatan bebas, BUKAN material) -->
                        <div class="md:col-span-2">
                            <label for="required_tools" class="block mb-2 uppercase tracking-wide text-slate-700 dark:text-slate-300">ALAT KHUSUS / KENDALA PERALATAN</label>
                            <textarea name="required_tools" id="required_tools" rows="2" class="w-full text-sm font-sans px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-sky-500 dark:focus:border-sky-400 focus:ring-2 focus:ring-sky-500/25" placeholder="Cth: Tangga ekstra panjang, Bor tembus dinding tebal, dll"><?php echo e(old('required_tools')); ?></textarea>
                            <span class="block text-[10px] text-slate-400 dark:text-slate-500 mt-1.5 font-normal">Peralatan kerja non-material (tidak habis pakai). Material habis pakai dicatat di daftar di atas.</span>
                        </div>

                        <!-- Catatan Teknis -->
                        <div class="md:col-span-2">
                            <label for="survey_note" class="block mb-2 uppercase tracking-wide text-slate-700 dark:text-slate-300">CATATAN TEKNIS SURVEY</label>
                            <textarea name="survey_note" id="survey_note" rows="3" class="w-full text-sm font-sans px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-sky-500 dark:focus:border-sky-400 focus:ring-2 focus:ring-sky-500/25" placeholder="Tuliskan kendala teknis atau informasi penting untuk teknisi instalasi..."><?php echo e(old('survey_note')); ?></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    <a href="<?php echo e(route('surveys.queue')); ?>" class="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-xs font-semibold cursor-pointer focus:outline-none">
                        Batal
                    </a>
                    ...
                    <button type="submit" id="btn-submit" class="px-4 py-2 bg-emerald-600 dark:bg-emerald-500 hover:bg-emerald-700 dark:hover:bg-emerald-600 text-white rounded-md transition-colors text-xs font-semibold cursor-pointer hidden focus:outline-none">
