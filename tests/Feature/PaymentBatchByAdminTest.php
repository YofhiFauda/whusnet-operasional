 ...
 */
class PaymentBatchByAdminTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_resubmitting_same_idempotency_key_does_not_create_duplicate_payments(): void
    {
        ...
        $first = $this->actingAs($this->admin)->postJson(route('payment-batches.store', $this->kolektor->id), $payload);
        $first->assertOk();

        $second = $this->actingAs($this->admin)->postJson(route('payment-batches.store', $this->kolektor->id), $payload);
        $second->assertOk();
    ...
    public function test_batch_target_must_actually_be_a_kolektor(): void
    {
        ...
        $response = $this->actingAs($this->admin)->get(route('collector-worksheet.show', $notAKolektor->id));
